...
# Инструменты, которые НЕ входят в ядро, но и откладывать их нельзя.
#
# Развилка «ядро против всего остального» знает только две категории, а
# `secret_request` не помещается ни в одну: в ядро он не входит намеренно
# (его схему не должны оплачивать CLI и десктоп, у которых свой
# маскированный ввод), но и за `tool_search` ему нельзя — он РЕАКТИВНЫЙ.
# Модель обязана знать о нём ровно в ту секунду, когда клиент упомянул
# ключ; поиском его находят те, кто уже догадался, что такой инструмент
# бывает, а весь смысл в том, чтобы агент предложил сам.
#
# Второй, менее очевидный повод (найден живым прогоном 2026-09-06): на
# машине клиента откладываемых инструментов не было НИ ОДНОГО, и ярусный
# механизм не включался. Стоило появиться одному — он включился, убрал
# этот один инструмент и добавил три служебных. Размен в минус.
#
# Список закрытый. Каждое имя здесь — инструмент, который платит схемой
# на каждом вызове, и добавляться сюда должно отдельным решением.
_NEVER_DEFER_EXTRA: frozenset[str] = frozenset(
    {
        "secret_request",
        # Спека 20 — ровно тот же размен, что абзацем выше, и та же
        # ошибка, найденная во второй раз: на клиентской машине без
        # единого MCP-сервера mcp_connect оказывается ЕДИНСТВЕННЫМ
        # откладываемым инструментом. Мост включается от него одного,
        # убирает его из схемы и добавляет три служебных — минус один,
        # плюс три, и подключить сервер модель теперь может только
        # догадавшись сперва сходить в tool_search.
        "mcp_connect",
    }
)
