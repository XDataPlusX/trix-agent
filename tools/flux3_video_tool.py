...
def _default_directory():
    ...
    # Under contained isolation this is <root>/home/Downloads, and the last
    # resort is the profile workspace rather than whatever directory the
    # process happens to be standing in.
    try:
        from hermes_constants import get_profile_workspace_dir, is_contained, user_home_path

        downloads = user_home_path("Downloads")
        if downloads.is_dir():
            return downloads
        return get_profile_workspace_dir() if is_contained() else Path.cwd()
    except Exception:
