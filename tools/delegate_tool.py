...
DELEGATE_BLOCKED_TOOLS = frozenset(
    [
        ...
        # Тот же запрет и по той же причине, только строже: subagent
        # наследует ключ сессии родителя, поэтому его запрос всплыл бы
        # в чате клиента как «нужен ключ для X» — из безнадзорного
        # работника, с формулировкой, которую сочинила модель. А
        # фоновый ребёнок вдобавок переживает ход родителя, чей finally
        # уже снял приёмник: клиент получил бы вопрос, ответить на
        # который некуда.
        "secret_request",  # no asking the client for credentials
