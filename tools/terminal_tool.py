...
# Declared once in ``hermes_constants``: the profile-layout resolver asks the
# same question (is this TERMINAL_CWD a host path?) and a second copy here
# would drift away from it silently.
from hermes_constants import HOST_CWD_PREFIXES as _HOST_CWD_PREFIXES  # noqa: E402
...
# Guard for the config-fallback bridge below.  Normally one-shot: after the
# first attempt either TERMINAL_ENV is set (bridge succeeded — merged config
# always carries terminal.backend) or the import failed and retrying every
# call would be wasted work.  The one case that DOES re-run the bridge is a
# TERMINAL_ENV that was bridged once and has since vanished from os.environ:
# that is somebody wiping the environment (``reload_env()`` dropping a known
# key absent from .env, a launcher resetting os.environ), and falling back to
# the historical "local" default there means running the next command on the
# host that config.yaml asked to sandbox.  See _terminal_config_bridge_pinned.
_terminal_config_bridge_attempted = False
# True when the last bridge run actually produced a TERMINAL_ENV — i.e. there
# is a selection to restore if it disappears.  Stays False when the bridge
# never got that far (import failure), so a broken install still costs at most
# one attempt instead of retrying on every terminal call.
_terminal_config_bridge_pinned = False
...
def _ensure_terminal_env_bridged() -> None:
    ...
    Runs once per process, plus once more whenever a TERMINAL_ENV this bridge
    had already established disappears from the environment again — the
    backstop against any code path that clears env vars (``reload_env()``
    dropping known keys absent from ``.env``, a launcher resetting
    ``os.environ``) silently downgrading a configured sandbox to the host.
    """
    global _terminal_config_bridge_attempted, _terminal_config_bridge_pinned
    if _terminal_config_bridge_attempted and not (
        _terminal_config_bridge_pinned and "TERMINAL_ENV" not in os.environ
    ):
        return
    ...
    try:
        ...
        # Remember whether there is a selection worth restoring later.
        _terminal_config_bridge_pinned = "TERMINAL_ENV" in os.environ
