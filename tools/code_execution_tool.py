...
Lets the LLM write a Python script that calls Trix tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `trix_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `trix_tools.py` with file-based RPC stubs
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Trix-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Trix-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    trix_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'trix_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_trix_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the trix_tools.py stub module.
...
def _execute_remote(
    ...
    The script and the generated trix_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/trix_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_trix_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/trix_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with trix_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="trix_sandbox_")
    ...
    _sock_tag = ""
    # Under contained isolation TMPDIR points inside the profile root, and a
    # root like /home/user/.hermes/profiles/<name>/tmp/ eats most of the
    # 108-byte sun_path budget before the filename even starts — bind() then
    # fails for no reason the operator can see. The socket is an OS runtime
    # channel, not profile data, so it stays in XDG_RUNTIME_DIR (or /tmp) and
    # carries a root tag instead, so its owner is still identifiable.
    try:
        from hermes_constants import (
            get_profile_root_tag,
            get_runtime_socket_dir,
            is_contained,
        )

        if is_contained():
            _sock_tmpdir = get_runtime_socket_dir()
            _sock_tag = f"{get_profile_root_tag()}_"
    except Exception:
        pass
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(
            _sock_tmpdir, f"trix_rpc_{_sock_tag}{uuid.uuid4().hex[:12]}.sock"
        )
    ...
    try:
        # Write the auto-generated trix_tools module.
        ...
        tools_src = generate_trix_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "trix_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # ``trix_tools.py`` always lives in the staging directory, so that
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that calls Trix tools programmatically. "
        ...
        f"Available via `from trix_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from trix_tools import {import_str}` "
