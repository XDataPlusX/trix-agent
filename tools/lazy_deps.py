...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ddgs (DuckDuckGo) — no API key, no side service. Tracks the `ddgs`
    # extra in pyproject.toml; bump both together. scripts/install.sh
    # eager-installs this extra for the curated Trix config template
    # (which defaults web.search_backend to ddgs), and `hermes tools`'
    # own "ddgs" post_setup step (hermes_cli/tools_config.py) calls
    # ensure() here directly when a user (re)selects the row — both
    # routes share this one version pin.
    #
    # NOT a manually-edited-config.yaml self-heal, unlike exa/firecrawl/
    # parallel: those providers' is_available() only checks for an API
    # key, so their search() genuinely is the first thing that imports
    # the SDK and can lazy-install it. DDGSWebSearchProvider.is_available()
    # (plugins/web/ddgs/provider.py) checks THIS SAME IMPORT, so if the
    # package were missing, is_available() would already answer False and
    # _get_capability_backend() (tools/web_tools.py) would never select
    # this provider — search() never gets a chance to call ensure(). A
    # config.yaml edit alone does not trigger an install; only the
    # installer or `hermes tools` does.
