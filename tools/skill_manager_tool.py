...
def _external_dir_write_guard(name: str, skill_dir: Path, action: str) -> Optional[Dict[str, Any]]:
    """Refuse ANY agent-invoked mutation of a skill that lives in the Trix
    company's shared skills folder — foreground calls included, not just
    the background curator.

    **Scope: the contour-managed folder, not every external dir.** An
    earlier version of this guard refused ALL of ``skills.external_dirs``,
    which reverses the deliberate convenience fix behind issues #4759 and
    #4381: a single user who points ``external_dirs`` at their own synced
    vault (an Obsidian folder, a personal dotfiles checkout) wants their
    agent to keep editing skills there in place — there is no "other
    people's system prompts" and no human-approval quarantine to defend in
    that case, and blocking it broke every such installation, Trix
    business machines or not. The property that actually needs protecting
    is narrower: "this directory is wired into OTHER PEOPLE's system
    prompts, and only a human publishing through the contour may change
    it" — which is exactly the Trix company skills folder
    (``hermes_cli/trix_contour.py``, spec 21 §9) and nothing else.
    ``is_contour_managed_skill_path`` answers that narrower question by
    checking for a marker file the contour itself plants at the root of
    the folder it owns — see its docstring in ``agent/skill_utils.py``.

    A shared company skills folder is wired into EVERY profile's system
    prompt via ``skills.external_dirs``: whoever can write there dictates
    the behaviour of every agent in the company. ``publish_skill``
    quarantines the ADD path (a human must run ``hermes contour
    approve-skill`` before a new skill goes live) — but until this guard,
    nothing quarantined the MODIFY path: any profile with the ``skills``
    toolset (``system_admin`` itself included — it carries that toolset)
    could resolve an already-approved shared skill through ``_find_skill``
    (which walks ``get_all_skills_dirs()``, external dirs included) and
    then ``patch``/``edit``/``write_file``/``remove_file``/``delete`` it in
    place, with no human anywhere in the loop. That is the more dangerous
    half: a human approves a skill's NAME once and essentially never
    re-reads its full text again, so a silent post-approval rewrite is
    exactly the failure the quarantine exists to prevent. The contour's OWN
    writes (``publish_skill``'s copy into ``.pending/``, ``approve_skill``'s
    promotion to live) go around this guard entirely — they live in
    ``hermes_cli/trix_contour.py`` and never call through ``skill_manage``.

    ``_background_review_write_guard`` already refused writes to
    external dirs (the old, broad rule) for the autonomous curator review
    fork — but only there. This check is unconditional (foreground and
    background alike) and fires before that one has a chance to. Reads
    (``skill_view``) are untouched; a shared skill remains fully visible,
    just not writable from inside a session. A personal (unmarked)
    external skills vault is untouched by this check and keeps working
    exactly as it did before.
    """
    try:
        from agent.skill_utils import is_contour_managed_skill_path

        if is_contour_managed_skill_path(skill_dir):
            return {
                "success": False,
                "error": (
                    f"Cannot {action} '{name}': it lives in the company's shared "
                    "skills folder, managed by `hermes contour` — a skill every "
                    "profile in the company picks up automatically. Shared "
                    "skills are changed by a human at the source (`hermes "
                    "contour approve-skill`), not by an agent from inside a "
                    "session. If you need your own variant, create a local "
                    "skill (action='create') instead of editing this one."
                ),
            }
    except Exception:
        logger.debug("external dir write guard lookup failed for %s", name, exc_info=True)
    return None
...
def _edit_skill(name: str, content: str) -> Dict[str, Any]:
    ...
    if org_guard:
        return org_guard
    ext_guard = _external_dir_write_guard(name, existing["path"], "edit")
    if ext_guard:
        return ext_guard
...
def _patch_skill(
    ...
    if org_guard:
        return org_guard
    ext_guard = _external_dir_write_guard(name, skill_dir, "patch")
    if ext_guard:
        return ext_guard
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if org_guard:
        return org_guard
    ext_guard = _external_dir_write_guard(name, existing["path"], "delete")
    if ext_guard:
        return ext_guard
...
def _write_file(name: str, file_path: str, file_content: str) -> Dict[str, Any]:
    ...
    if org_guard:
        return org_guard
    ext_guard = _external_dir_write_guard(name, existing["path"], "write_file")
    if ext_guard:
        return ext_guard
...
def _remove_file(name: str, file_path: str) -> Dict[str, Any]:
    ...
    skill_dir = existing["path"]
    ext_guard = _external_dir_write_guard(name, skill_dir, "remove_file")
    if ext_guard:
        return ext_guard
...
#: Собственный справочник продукта. Это не навык про задачу — это
#: документ, который объясняет агенту, чем он является и что ему можно
#: говорить клиенту.
PRODUCT_MANUAL_SKILL = "trix-agent"


def _product_manual_write_refusal(action: str, name: str) -> Optional[str]:
    """Запретить агенту переписывать собственный справочник продукта.

    Две вещи по отдельности безобидны, а вместе дают тихую поломку:

    1. Системный промпт прямо поощряет: «нашёл в навыке изъян — сразу
       чини его через `skill_manage(action='patch')`».
    2. `hermes update` не трогает навык, который клиент (а значит и
       агент от его имени) изменил, — он остаётся «user-modified
       (kept)».

    Стоит агенту один раз поправить формулировку в этом файле — и машина
    навсегда перестаёт получать НАШИ правки к нему. Именно к нему: к
    тексту, который держит обещания, данные клиенту (не выдавать команд,
    не рассказывать про поверхности, которых у него нет). Проверено на
    живом стенде: после ручной правки одной строки `hermes update`
    честно отрапортовал «Skills are up to date», оставив старую копию.

    Для остальных навыков правка агентом — заявленная возможность
    продукта, и её не трогаем: цена (навык замирает) там окупается
    пользой. Здесь не окупается: этот файл пишем мы.
    """
    if name != PRODUCT_MANUAL_SKILL:
        return None
    if action not in {"edit", "patch", "delete", "write_file", "remove_file"}:
        return None
    return (
        f"'{name}' describes this product itself and is maintained by "
        "XDataPlus, not from inside a session — editing it here would "
        "freeze this machine on your edited copy and stop it receiving "
        "future corrections. Do not retry and do not work around this. "
        "If something in it is wrong or missing, say so in your reply so "
        "the person can pass it to XDataPlus support."
    )
...
def _apply_skill_write_gate(action, name, **payload_kwargs):
    ...
    if action not in {"create", "edit", "patch", "delete", "write_file", "remove_file"}:
        return None
    # Проверяется ДО обхода на повторе одобренной записи: это не
    # согласование, которое можно выдать, а запрет.
    refusal = _product_manual_write_refusal(action, name)
    if refusal:
