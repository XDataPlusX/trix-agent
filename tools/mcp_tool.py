...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    ``HOME`` / ``TMPDIR`` / ``XDG_*`` are copied from *this* process, and the
    engine's own ``HOME`` deliberately stays outside the profile root.  So the
    allowlist alone hands a ``contained`` MCP server the real OS home, and the
    server lays its state down next to every other profile's.  The subprocess
    contract (``hermes_constants.apply_subprocess_containment_env``) is
    therefore applied to the copied baseline — the same single point the
    browser, shell, code-exec and ACP constructors go through — before the
    operator's explicit ``env:`` block, which keeps winning as it does today.

    The contract is applied unguarded, exactly as those six constructors apply
    it: if it cannot run, the spawn fails.  Swallowing the error would start
    the server with the leaked environment this pin exists to prevent, and a
    server that does not start is the cheaper of the two failures.
    """
    try:
