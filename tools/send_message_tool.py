...
def _handle_send(args):
    ...
    media_files, unreachable_media = BasePlatformAdapter.partition_media_delivery_paths(
        media_files
    )
    ...
    try:
        ...
        # Unlike gateway delivery — which runs after the agent's turn has ended
        # and can only tell the USER — this is a tool call, so the agent is
        # still in its loop and can act on the answer. Report the attachments
        # that never left the sandbox so it can retry with a readable path
        # instead of believing it sent them.
        if unreachable_media and isinstance(result, dict):
            result["attachments_not_sent"] = [path for path, _ in unreachable_media]
            result["attachments_not_sent_reason"] = (
                "Path is not readable from the host process that delivers files. "
                "Write the file inside your working directory (or another mounted "
                "path) and reference it there."
            )
