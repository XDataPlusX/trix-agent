...
def check_dangerous_command(command: str, env_type: str,
    ...
    if _should_skip_container_guards(env_type, has_host_access=has_host_access):
        # Trix: изолированная песочница — не повод пропускать УДАЛЕНИЕ, но
        # только для docker. /workspace и /root там примонтированы с хоста
        # мимо docker_volumes, поэтому rm в них стирает данные клиента, а
        # отката нет. singularity/modal/daytona/vercel_sandbox — эфемерные
        # удалённые песочницы без хостовых маунтов, там пропуск остаётся
        # безусловным.
        if env_type != "docker":
            return {"approved": True, "message": None}
        from hermes_cli.trix_sandbox_guard import is_terminal_delete

        if not is_terminal_delete(command):
            return {"approved": True, "message": None}
...
# Module-level (not a local inside the guard below) so other surfaces can
# reference it BY NAME instead of copying the prose. The Trix client prompt
# translates it (hermes_cli/trix_sandbox_guard.client_reason_ru); binding by
# attribute means a rename here fails loudly instead of silently reverting
# that prompt to English.
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    if env_type == "vercel_sandbox":
        return {"approved": True, "message": None}
    if _should_skip_container_guards(env_type, has_host_access=has_host_access):
        # Trix: narrowed to docker only (I3) — singularity/modal/daytona
        # have no host bind-mount either, so this branch never ran for them
        # anyway once vercel_sandbox's own early return above is accounted
        # for; being explicit here keeps this call site symmetric with
        # check_dangerous_command / check_all_command_guards.
        #
        # Deliberately does NOT hand the whole script to
        # detect_dangerous_command: that scan is superlinear on real script
        # text (seconds at 18KB) and this branch runs on EVERY execute_code
        # call in the client's primary mode. is_python_delete prefilters on
        # literal tokens, then runs the same upstream detector over the
        # extracted shell literals only — tens of bytes each.
        if env_type != "docker":
            return {"approved": True, "message": None}
        from hermes_cli.trix_sandbox_guard import is_python_delete

        if not is_python_delete(code):
            return {"approved": True, "message": None}
