...
def sync_skills(quiet: bool = False) -> dict:
    ...
    if (HERMES_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        ...
        return {
            ...
            "user_modified": [], "cleaned": [], "unbundled_removed": [],
            "total_bundled": 0,
    ...
    if not bundled_dir.exists():
        return {
            ...
            "unbundled_removed": [], "total_bundled": 0,
    ...
    unbundled_removed: List[str] = []
    for name in cleaned:
        old_hash = manifest.pop(name)
        removed_path = _remove_unbundled_copy(name, old_hash)
        if removed_path:
            unbundled_removed.append(name)
            if not quiet:
                print(f"  − {name} (больше не поставляется)")
    ...
    return {
        ...
        "cleaned": cleaned,
        "unbundled_removed": unbundled_removed,
...
def _remove_unbundled_copy(name: str, shipped_hash: str) -> Optional[Path]:
    """Delete an installed skill we shipped and no longer ship — if untouched.

    Dropping the manifest entry alone (the previous behaviour) left the
    directory on disk forever: a machine installed before a skill was
    de-bundled kept it, kept paying for its line in the skills index on
    every single turn, and kept being steered by it. New installs got the
    curated set; existing ones never did.

    The safety rule is the one used for the sandbox mirror: **only remove a
    copy that is byte-identical to what we shipped.** A hash mismatch means
    the user or the agent edited it — that is their work, and it stays. An
    unreadable tree also stays: better to leave one skill than to delete
    someone's writing.

    Returns the removed path, or None when nothing was removed.
    """
    try:
        for skill_md in SKILLS_DIR.rglob("SKILL.md"):
            if _read_skill_name(skill_md, skill_md.parent.name) != name:
                continue
            dest = skill_md.parent
            if _dir_hash(dest) != shipped_hash:
                logger.info(
                    "Skill %s is no longer bundled but was modified locally — kept",
                    name,
                )
                return None
            _rmtree_writable(dest)
        ...
        logger.debug("Could not remove unbundled skill %s", name, exc_info=True)
    return None
