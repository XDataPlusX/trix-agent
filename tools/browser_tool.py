...
def _chromium_search_roots() -> List[str]:
    ...
    4. ``%USERPROFILE%\\AppData\\Local\\ms-playwright`` — Playwright's default
       on Windows.
    5. ``~/.agent-browser/browsers`` — where ``agent-browser install`` puts
       its own download.

    Root 5 exists because the assumption behind roots 1-4 went stale.
    ``_chromium_installed``'s own docstring still says agent-browser
    downloads "Playwright's chromium / headless-shell", i.e. into a
    Playwright cache — that was true of older builds. Confirmed by
    execution on a clean client VM (2026-09-03, agent-browser 0.26):
    ``agent-browser install`` printed "Chrome 152.0.7977.75 installed
    successfully / Location: ~/.agent-browser/browsers/chrome-152.0.7977.75"
    and left ``~/.cache/ms-playwright`` non-existent. So the tool installed
    a working browser and this function reported none — the entire
    ``browser_*`` surface then vanishes from the model's schema with no
    error anywhere, which is exactly the silent failure the installer's own
    preflight warning describes.

    The list itself now lives in ``hermes_constants`` so this lookup and the
    containment pin (``apply_machine_browser_env``) cannot drift apart. In
    ``contained`` mode this process's own HOME is unchanged and still finds
    the browser here, but the browser's **node subprocess** runs with
    ``HOME=<root>/home``; the pin hands it the machine build explicitly, and
    it must point at exactly these directories.
    """
    from hermes_constants import _iter_machine_browser_roots

    return _iter_machine_browser_roots()
...
def _chromium_installed() -> bool:
    ...
    for root in _chromium_search_roots():
        ...
        #
        # ``chrome-<version>`` is the third shape, and it is NOT Playwright's:
        # agent-browser 0.26's own installer downloads Chrome for Testing
        # into ``~/.agent-browser/browsers/chrome-<version>/`` (confirmed by
        # execution on a clean client VM, 2026-09-03). Without this branch
        # the tool installs a working browser and this function still reports
        # none — the whole ``browser_*`` surface then disappears from the
        # model's schema silently.
        #
        # The name alone is trusted for the two Playwright shapes (existing
        # behaviour), but a ``chrome-*`` directory must actually contain an
        # executable ``chrome``: this root is written by a downloader whose
        # partially-finished directory has the final name from the start, and
        # a false "installed" here is worse than a false "missing" — the
        # tools would be offered to the model and then fail on use.
        for entry in entries:
            ...
            if entry.startswith("chrome-"):
                candidate = os.path.join(root, entry, "chrome")
