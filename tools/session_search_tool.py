...
# End reasons that mark an explicit reset boundary: the gateway closed the
# session and started a fresh one (idle/daily expiry, /new, /resume switch,
# suspended, resume-pending expiry). Content in these ancestors is NOT in the
# successor's live context — the successor starts empty and only carries the
# routing identity — so they are fair game for discovery. Delegation parents
# (end_reason NULL) are the opposite case: their content is live to the child.
_ROTATION_END_REASONS = frozenset(
    {
        "session_reset",
        "session_switch",
        "idle",
...
def _is_rotation_ended(db, session_id: str) -> bool:
    """Return True if *session_id* ended at an explicit reset boundary.

    Gateway rotation (72h idle reset, /new, /resume switch) closes the old
    session with one of ``_ROTATION_END_REASONS`` and links the fresh
    successor via ``parent_session_id``. The old content is not summarised
    into the successor — it is simply dropped from live context — so hits in
    such ancestors must stay discoverable from the current session.
    """
    if not session_id:
        return False
    try:
        ...
        if not s:
            return False
        return s.get("end_reason") in _ROTATION_END_REASONS
    except Exception:
        return False


# Valid values for the discovery-shape ``scope`` parameter.
_SCOPE_VALUES = ("auto", "conversation", "global")


def _current_conversation_scope(
    db, current_session_id: Optional[str]
) -> Optional[Dict[str, str]]:
    """Exact-conversation identity of the live session, or None.

    The identity is the session row's ``session_key`` — the same stable
    technical id (platform / account / chat / thread-topic, per the routing
    key contract in ``gateway.session.build_session_key``) that survives
    72-hour rotation. Display names are never identity. Rows without a
    session key (CLI/local sessions, legacy rows) have no conversation tier:
    auto scope degrades to the global tier for them, exactly matching
    pre-scoped behaviour.
    """
    ...
    try:
        ...
        logging.debug(
            "session lookup failed for scope resolution: %s",
            current_session_id,
            exc_info=True,
        )
        return None
...
def session_search_allow_cross_profile() -> bool:
    """Whether ``session_search`` may open ANOTHER profile's ``state.db``.

    Reads ``session_search.allow_cross_profile`` from config.yaml (default
    ``False`` — see ``DEFAULT_CONFIG`` in ``hermes_cli/config_defaults.py``).

    Each profile is meant to be a fully isolated instance — its own memory,
    its own conversation history — so a messaging-gateway agent running as
    one profile must not be able to read another profile's sessions just
    because it was asked to (that breaks the "each department has its own
    base" guarantee). The legitimate use of cross-profile reads is the
    desktop app resolving an ``@session:<profile>/<id>`` link the user
    dragged into the chat; a person opts that in explicitly, per profile.
    """
    try:
        ...
        logging.debug("failed to load config for session_search cross-profile gate", exc_info=True)
        return False
    section = cfg.get("session_search")
    if not isinstance(section, dict):
        return False
    return bool(section.get("allow_cross_profile", False))


def _schema_without_cross_profile(schema: Dict[str, Any]) -> Dict[str, Any]:
    """Copy of *schema* with the ``profile`` parameter — and every mention of
    it in the prose — removed.

    The parameter's mere presence in the schema (or its description) is a
    standing invitation for the model to try a cross-profile read. When the
    gate is off the model must never learn the capability exists at all: a
    strict provider rejects an unknown-arg call outright, and a lenient one
    turns every `@session:` link this tool itself emits into a guaranteed
    ``tool_error`` the first time the model tries to follow one, since the
    description used to tell it to call ``session_search(..., profile=...)``
    even with the parameter gone from the schema.
    """
    parameters = schema.get("parameters") or {}
    properties = parameters.get("properties") or {}
    description = schema.get("description") or ""
    if "profile" not in properties and _SESSION_SEARCH_READ_EXAMPLE_OPEN not in description:
        return schema
    new_properties = {k: v for k, v in properties.items() if k != "profile"}
    new_parameters = {**parameters, "properties": new_properties}
    new_description = description.replace(
        _SESSION_SEARCH_READ_EXAMPLE_OPEN, _SESSION_SEARCH_READ_EXAMPLE_CLOSED
    )
    return {**schema, "parameters": new_parameters, "description": new_description}


def get_session_search_schema() -> Dict[str, Any]:
    """Tool schema for ``session_search``, gated on cross-profile config.

    Called from ``model_tools.get_tool_definitions()`` alongside the
    ``browser_navigate``/``execute_code`` dynamic-schema post-processing —
    that function is memoized on a config-mtime fingerprint, so this does
    not change within a conversation/process unless config.yaml itself
    changes, which keeps prompt caching intact.
    """
    if session_search_allow_cross_profile():
        return SESSION_SEARCH_SCHEMA
    return _schema_without_cross_profile(SESSION_SEARCH_SCHEMA)
...
def _resolve_profile_db(profile: str):
    ...
    profile is given (use the caller's default db), and also when *profile*
    names the CURRENTLY ACTIVE profile — that is not a cross-profile read at
    all, so it must keep working even when the cross-profile gate is off.

    Raises ``ValueError`` when *profile* names a different profile and
    ``session_search.allow_cross_profile`` is not enabled in config.yaml.
    """
    ...
    try:
        ...
        logging.debug("get_active_profile_name failed while resolving profile db", exc_info=True)
        active = None

    if canon == active:
        # Same profile the agent is already running as — not cross-profile.
        return None

    if not session_search_allow_cross_profile():
        raise ValueError(
            f"cross-profile session reads are disabled — this agent may only read "
            f"its own profile's sessions, not '{canon}'. Each profile is an isolated "
            f"instance with its own history by design. A person can enable "
            f"session_search.allow_cross_profile: true in config.yaml to allow it "
            f"(intended for the desktop app resolving @session:<profile>/<id> links, "
            f"not for messaging-gateway agents)."
        )
...
def _locate_session_db(session_id: str):
    """Scan for a session id, read-only.
    ...
    Scanning EVERY profile's ``state.db`` is itself a cross-profile read, so
    it is gated exactly like an explicit ``profile=`` argument: with
    ``session_search.allow_cross_profile`` off (default), the scan is limited
    to the currently active profile only — a bare session_id must never leak
    another profile's sessions just because the owning profile wasn't named.
    """
    ...
    if session_search_allow_cross_profile():
        ...
        try:
            ...
            # Fail CLOSED, not to a hardcoded profile name: guessing "default"
            # here would scan a stranger's database whenever this call raises
            # (matches the fail-closed pattern in _resolve_profile_db above).
            logging.debug("get_active_profile_name failed during session locate", exc_info=True)
            active = None
        if active and active != "custom":
            home = profiles_mod.get_profile_dir(active)
        else:
            # get_active_profile_name failed, or HERMES_HOME doesn't map to a
            # recognized profile path (custom deployment) — use HERMES_HOME
            # directly rather than guessing a profile.
...
def _discover_tier(
    ...
    current_lineage_root: Optional[str],
    tier_label: str,
    tier_filters: Optional[Dict[str, str]],
    title_result: Optional[Dict[str, Any]],
) -> tuple[List[Dict[str, Any]], set]:
    """Run one discovery tier (FTS + demotion + lineage dedup + assembly).

    ``tier_filters`` carries the exact conversation-identity filters for the
    conversation tier (``session_key``), or None for the profile-wide global
    tier. Returns the assembled result entries (each tagged with
    ``scope=tier_label``) and the set of lineage roots they cover.
    """
    tier_title = None
    if title_result is not None:
        tier_title = dict(title_result)
        if tier_filters is not None:
            # Conversation tier: a title match only belongs here when the
            # titled session is part of THIS conversation.
            try:
                titled_row = db.get_session(tier_title.get("session_id") or "")
                titled_key = (titled_row or {}).get("session_key")
            except Exception:
                titled_key = None
            if titled_key != tier_filters.get("session_key"):
                tier_title = None
    ...
    if tier_filters:
        search_kwargs.update(tier_filters)
    # Propagates to the caller — a genuinely broken search must surface as a
    # tool error, not as a silent "no sessions found".
    ...
    if tier_title:
        title_lineage = tier_title.pop("_lineage_root", None)
        if title_lineage:
            seen_sessions[title_lineage] = {"_title_only": True}
        results.append(tier_title)
    ...
    for r in raw_results:
        ...
        # Skip the current session lineage — UNLESS the content is not in the
        # live context (memory black hole cases). Three sub-cases:
        #
        ...
        #
        # Gateway rotation (idle/daily reset, /new, /resume switch): the
        # ancestor ended at an explicit reset boundary and the successor
        # started empty — only the routing identity (session_key, chat/thread
        # ids, parent link) survived. The ancestor's content is NOT in the
        # live context, so it must stay discoverable — this is exactly the
        # "same topic after the 72-hour reset" recall case.
        is_compacted_hit = _is_compacted_message(db, r.get("id"))
        is_ended_session = _is_compression_ended(db, raw_sid)
        is_rotation_ended = _is_rotation_ended(db, raw_sid)
        if current_lineage_root and resolved_sid == current_lineage_root:
            if not (is_ended_session or is_compacted_hit or is_rotation_ended):
                continue
    ...
    # Cross-tier merge key: entries carry their lineage root via
    # parent_session_id (when different) or session_id itself.
    covered = set()
    for entry in results:
        entry["scope"] = tier_label
        covered.add(entry.get("parent_session_id") or entry["session_id"])
...
def _discover(
    ...
    Scope ladder (owner decision 2026-09-14): by default search the exact
    current conversation first (``session_key`` equality — stable technical
    identity, never display names); widen to the whole profile only when the
    conversation tier comes up empty. A non-empty local tier is trusted — the
    model can widen explicitly with ``scope='global'`` when the local answer
    looks insufficient. There is deliberately NO middle "same chat, other
    topics" tier. The response always says which scope produced the results
    so the model can tell the user honestly when a search was widened.
    """
    role_list = role_filter if role_filter else ["user", "assistant"]
    current_lineage_root = _resolve_lineage(db, current_session_id) if current_session_id else None
    title_result = _title_match_result(db, query, current_lineage_root)

    scope_norm = scope.strip().lower() if isinstance(scope, str) else ""
    if scope_norm not in _SCOPE_VALUES:
        scope_norm = "auto"

    conv_scope = _current_conversation_scope(db, current_session_id)

    def _run_tier(tier_label: str, tier_filters: Optional[Dict[str, str]]):
        try:
            return _discover_tier(
                db, query, role_list, limit, sort, current_session_id,
                current_lineage_root, tier_label, tier_filters, title_result,
            )
        except Exception as e:
            logging.error("FTS5 search failed: %s", e, exc_info=True)
            return None

    if scope_norm == "conversation" and conv_scope is not None:
        tier_out = _run_tier("conversation", conv_scope)
        if tier_out is None:
            return tool_error(f"Search failed for conversation scope", success=False)
        results, covered = tier_out
        scope_used = "conversation"
    elif scope_norm == "auto" and conv_scope is not None:
        conv_out = _run_tier("conversation", conv_scope)
        if conv_out is None:
            return tool_error("Search failed for conversation scope", success=False)
        conv_results, conv_covered = conv_out
        if conv_results:
            # The current conversation has matching content — that is the
            # owner's contract: this topic first, the whole bot only when
            # the topic has nothing. A short-but-nonempty local tier is NOT
            # auto-widened; the model can widen explicitly with
            # scope="global" when the local answer looks insufficient.
            results, covered = conv_results, conv_covered
            scope_used = "conversation"
        else:
            glob_out = _run_tier("global", None)
            if glob_out is None:
                # Empty conversation tier + broken global pass: report the
                # honest empty result rather than an error the user can't act
                # on differently.
                results, covered = conv_results, conv_covered
                scope_used = "conversation"
            else:
                results, covered = glob_out
                scope_used = "global"
    else:
        # Explicit global, or no conversation identity (CLI/local/legacy
        # rows) — profile-wide search, exactly the pre-scoped behaviour.
        tier_out = _run_tier("global", None)
        if tier_out is None:
            return tool_error("Search failed", success=False)
        results, covered = tier_out
        scope_used = "global"
    ...
    _final_payload = {
        ...
        "results": results,
        "count": len(results),
        "sessions_searched": len(covered),
    }
    if scope_used == "global" and conv_scope is not None:
        if scope_norm == "auto":
            # The ladder widened past the current conversation because the
            # conversation tier came up empty or short — say which, so the
            # model can tell the user the search was expanded.
            _final_payload["scope_expanded"] = True
            _final_payload["scope_note"] = (
                "Searched all sessions of this profile (global scope), not just "
                "the current conversation — state this scope in your answer."
            )
        else:
            # Explicit search-everywhere request — still labelled so the model
            # attributes the results to the whole profile.
            _final_payload["scope_note"] = (
                "Searched all sessions of this profile (global scope) — state "
                "this scope in your answer."
            )
...
def session_search(
    ...
    # Discovery shape
    sort: str = None,
    # Discovery scope: "auto" (default) | "conversation" | "global"
    scope: str = None,
    ...
    ``scope`` (discovery only) controls breadth: ``auto`` searches the exact
    current conversation first and widens to the whole profile when that
    comes up short; ``conversation`` stays in this conversation; ``global``
    searches every session of the profile. The response's ``scope`` field
    reports the tier that produced the results.
...
# The READ-shape example paragraph differs between the open- and closed-gate
# schema variants (see `_schema_without_cross_profile`): the open variant
# teaches the model the `profile=` kwarg exists; the closed variant must not,
# since the parameter isn't in the schema at all when the gate is off.
_SESSION_SEARCH_READ_EXAMPLE_OPEN = (
...
_SESSION_SEARCH_READ_EXAMPLE_CLOSED = (
    "  3) READ — pass `session_id` only (no around_message_id):\n"
    "     session_search(session_id=\"...\")\n"
    "     Dumps the whole session by id (first 20 + last 10 messages when "
    "large). This is how you resolve an `@session:<id>` link the user "
    "dropped into the chat: pass just the id segment (the part after the "
    "last `/`, if any) as session_id.\n\n"
)
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "SOURCE-FIRST LIMIT\n\n"
        "  This tool searches Trix conversation history only. It is not evidence "
        ...
        "without paying for the whole transcript. The response's `scope` field "
        "says \"conversation\" or \"global\"; if global (or scope_expanded), tell "
        "the user the results come from all their sessions, not just this "
        "conversation.\n\n"
    ...
    + (
        ...
        "reformat it as a markdown link or wrap it in backticks. Trix renders "
        ...
        "(`deploy*`).\n\n"
        "WHEN TO USE\n\n"
        "  Reach for this on questions about Trix conversation history itself, such "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "scope": {
                "type": "string",
                "enum": ["auto", "conversation", "global"],
                "description": (
                    "Discovery only. Breadth: 'auto' (default) searches the "
                    "current conversation (this chat/topic) first and widens "
                    "to all profile sessions only when it finds nothing there; "
                    "'conversation' never widens; 'global' searches all "
                    "sessions of this profile (explicit search-everywhere "
                    "requests). The response's `scope` field reports the "
                    "outcome."
                ),
                "default": "auto",
            },
            ...
            "profile": {
                "type": "string",
                "description": (
                    "Optional. Read sessions from another Trix profile's database "
