...
def _capture_required_environment_variables(
    ...
    # Most gateway surfaces (messaging platforms) historically couldn't prompt
    # for a secret at all, so they short-circuited to the "unsupported" hint.
    # Two ways around that now (spec 19, Ruling 2):
    #
    #   1. Interactive gateway surfaces — the desktop app / TUI — set
    #      HERMES_INTERACTIVE and register a secret-capture callback that
    #      routes to a secure secret.request overlay. (HERMES_INTERACTIVE is
    #      the same flag tools/approval.py uses to tell an interactive
    #      surface from a messaging one.)
    #   2. A real messaging session (Telegram, ...) whose TurnRunner
    #      registered a per-SESSION capture record via
    #      tools/secret_capture_gateway.register_notify(). Deliberately NOT
    #      gated on HERMES_INTERACTIVE (a process-global env var — see
    #      GHSA-96vc-wcxf-jjff for why mutating it from a concurrent session
    #      is unsafe) and NOT a redefinition of "interactive" (that flag
    #      means "may ask for dangerous-command approval", not "can receive
    #      a secret") — it's a narrow, session-scoped check answering only
    #      "did Ruling 1's machinery register a capture record for the turn
    #      that's asking right now".
    if _is_gateway_surface() and not (
        env_var_enabled("HERMES_INTERACTIVE") or _session_has_secret_capture()
    ):
        return {
...
def _session_has_secret_capture() -> bool:
    """Whether the CURRENT session (Ruling 1) can actually receive a secret.

    ``TurnRunner.run_sync`` registers a per-``session_key`` notify callback
    with ``tools.secret_capture_gateway`` for every gateway turn (mirroring
    ``agent.clarify_callback`` / ``register_gateway_notify``) and tears it
    down in the same ``finally``. A record existing for the CURRENT session
    is therefore proof the gateway is ready to ask this specific chat for a
    secret right now — never a process-global "some session somewhere is
    interactive" signal.
    """
    try:
        from gateway.session_context import get_session_env
        from tools.secret_capture_gateway import get_notify
    except Exception:
        return False
    session_key = get_session_env("HERMES_SESSION_KEY", "")
    if not session_key:
        return False
    return get_notify(session_key) is not None
    ...
    for scan_dir in dirs_to_scan:
        for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
            ...
            try:
                ...
                # Collapse whitespace (newlines included) before truncating --
                # a hostile/untrusted SKILL.md must not be able to turn one
                # skills_list entry into multiple rendered lines. Limits
                # unchanged from before.
                name = collapse_prompt_whitespace(
                ...
                description = collapse_prompt_whitespace(description)
