...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        # Contained isolation wins over an inherited TMPDIR: a value left in
        # the systemd unit or the operator's shell would otherwise drag every
        # temp file this backend produces — tool results, session snapshots,
        # cwd probes — back out of the profile root. An explicit
        # HERMES_TMP_DIR still wins over both (that is `custom` mode), and
        # shared-host keeps the upstream order untouched.
        try:
            from hermes_constants import ensure_profile_tmp_dir, is_contained

            if is_contained(self.env):
                return str(ensure_profile_tmp_dir(self.env)).rstrip("/") or "/"
        except Exception:
            pass
