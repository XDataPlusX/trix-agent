...
def pick_public_address(addresses: list) -> str:
    """Выбрать из адресов интерфейсов тот, по которому машину видно снаружи.

    Берём только публичные: приватные диапазоны (10/8, 172.16/12,
    192.168/16), loopback и link-local снаружи не работают, а назвать их
    клиенту — то же самое, что дать нерабочую ссылку. Если публичного нет
    (машина за NAT), возвращаем пустую строку: пусть агент честно скажет,
    что ссылку дать не может, чем соврёт адресом.

    Спрашивать адрес у интернета нельзя принципиально: песочница ходит
    через прокси, и любой сервис вида «какой у меня IP» отвечает адресом
    ПРОКСИ. Ровно на этом агент и ошибся 2026-09-09, выдав клиенту
    ссылку на выходной адрес прокси вместо адреса его машины.
    """
...
def detect_public_address() -> str:
    """Публичный адрес этой машины, прочитанный локально."""
    try:
        out = subprocess.run(
    ...
    for line in out.splitlines():
        for token in line.split():
            if "/" in token and token.count(".") == 3:
                found.append(token.split("/", 1)[0])
    return pick_public_address(found)


#: Сколько раз сдвигать диапазон в поисках свободного. Десяти хватает
#: на десять одновременных песочниц — больше на клиентской машине не
#: бывает: чат, задание по расписанию и несколько делегированных.
_MAX_PORT_SHIFT_ATTEMPTS = 10


def parse_publish_spec(spec: str) -> tuple | None:
    """Разобрать значение `-p` в ``(начало, конец, длина)`` хостовой части.

    Понимает и одиночный порт (``18000:18000``), и диапазон
    (``18000-18009:18000-18009``). Возвращает None для всего, что мы не
    берёмся сдвигать: привязок к конкретному адресу (``127.0.0.1:80:80``),
    несовпадающих сторон (``8080:80``) и протокольных суффиксов. Такие
    оставляем как есть -- лучше не тронуть, чем сдвинуть неверно.
    """
    parts = spec.split(":")
    if len(parts) != 2:
        return None
    ...
    if "-" in host:
        try:
            start, end = (int(x) for x in host.split("-", 1))
        except ValueError:
            return None
    else:
        try:
...
def shift_publish_args(args: list, offset: int) -> list:
    """Сдвинуть все публикуемые диапазоны на ``offset`` портов.

    Хостовая и контейнерная стороны сдвигаются ВМЕСТЕ, отображение
    остаётся один-в-один. Это осознанно: если сдвигать только хост, порт
    внутри контейнера и порт снаружи разъезжаются, и агент, который
    привязался к 18000, должен откуда-то узнать, что снаружи это 18010.
    При одинаковом сдвиге ему достаточно знать свой диапазон -- он же и
    ...
    while i < len(args):
        ...
        if a in ("-p", "--publish") and i + 1 < len(args):
            parsed = parse_publish_spec(args[i + 1])
            if parsed:
                start, end, _ = parsed
                new = (f"{start + offset}-{end + offset}:{start + offset}-{end + offset}"
                       if start != end else f"{start + offset}:{start + offset}")
                out.extend([a, new])
            else:
...
def published_range_label(args: list) -> str:
    """«18010-18019» для агента -- то, что он увидит в своём окружении."""
    labels = []
    i = 0
    while i < len(args):
        if args[i] in ("-p", "--publish") and i + 1 < len(args):
            labels.append(args[i + 1].split(":")[0])
            i += 2
            continue
        i += 1
    return ",".join(labels)
...
def is_port_allocation_error(stderr: str) -> bool:
    """Докер отказал именно из-за занятого порта, а не из-за чего-то ещё.

    Текст взят дословно с клиентского стенда 2026-09-09:

        Bind for 0.0.0.0:18000 failed: port is already allocated

    Сверяем по сути, а не по всей фразе: формулировка меняется между
    версиями докера, суть — нет.
    """
    low = (stderr or "").lower()
    return "port is already allocated" in low or "address already in use" in low


def strip_publish_args(args: list) -> tuple:
    """Убрать публикацию портов, не тронув ничего другого.

    Возвращает ``(оставленные, убранные)``. Понимает обе формы, в которых
    докер принимает публикацию: раздельную (``-p 1:1``) и склеенную
    (``--publish=1:1``, ``-p1:1``).

    Почему нельзя просто повторить запуск без ВСЕХ дополнительных
    аргументов: там же живут ограничения исходящего трафика песочницы
    (см. :func:`_extra_args_egress_collisions`). Снять их «чтобы
    контейнер хоть как-то поднялся» — значит тихо убрать границу
    безопасности ради удобства.
    """
    kept, removed = [], []
    i = 0
    while i < len(args):
        a = args[i]
        if a in ("-p", "--publish") and i + 1 < len(args):
            removed.extend([a, args[i + 1]])
            i += 2
            continue
        if a.startswith("--publish=") or (a.startswith("-p") and len(a) > 2):
            removed.append(a)
            i += 1
            continue
        kept.append(a)
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if not reused:
            ...
            # Агент должен знать СВОЙ диапазон: он единственный, по которому
            # поднятое им можно открыть снаружи. Без этого он привязывается
            # наугад и отдаёт клиенту адрес, который не отвечает --
            # наблюдалось на стенде 2026-09-09: 39 вызовов модели подряд на
            # перебор портов, каждый из которых «слушал» изнутри и молчал
            # снаружи.
            _ports_label = published_range_label(run_cmd)
            if _ports_label:
                run_cmd = [*run_cmd[:-3], "-e",
                           f"TRIX_PUBLISHED_PORTS={_ports_label}", *run_cmd[-3:]]
            # Адрес машины читается ЛОКАЛЬНО, с интерфейса. Спрашивать его у
            # интернета нельзя: песочница ходит через прокси, и сервисы вида
            # «какой у меня IP» отвечают адресом прокси.
            _public = detect_public_address()
            if _public:
                run_cmd = [*run_cmd[:-3], "-e",
                           f"TRIX_PUBLIC_ADDRESS={_public}", *run_cmd[-3:]]
            ...
            try:
                ...
                # Порт уже занят -- почти всегда это ВТОРАЯ песочница на
                # той же машине: задание по расписанию рядом с чатом,
                # делегированный работник. Диапазон в конфиге клиента
                # зашит один (`-p 18000-18009`), и второму контейнеру его
                # не дадут никогда.
                #
                # Публикация портов -- удобство (показать клиенту
                # поднятый сайт), а терминал -- сама работа. Ронять
                # вторую сессию целиком ради удобства, которым она,
                # скорее всего, не воспользуется, -- неверный размен:
                # клиент видит не «порт занят», а «задание по расписанию
                # молча не сработало».
                stderr = getattr(e, "stderr", "") or ""
                kept, dropped = strip_publish_args(run_cmd)
                if not (dropped and is_port_allocation_error(stderr)):
                    raise

                # Решение владельца 2026-09-09: ссылка, которую агент даёт
                # клиенту, должна работать всегда. Поэтому сначала ИЩЕМ
                # свободный диапазон, сдвигая исходный на его же длину, и
                # только если свободного нет вовсе -- поднимаемся без
                # публикации (лучше рабочий терминал без ссылок, чем
                # неработающая сессия).
                parsed = [parse_publish_spec(dropped[i + 1])
                          for i in range(0, len(dropped), 2)]
                block = max((p[2] for p in parsed if p), default=0)
                result = None
                for attempt in range(1, _MAX_PORT_SHIFT_ATTEMPTS + 1):
                    if not block:
                        break
                    shifted = shift_publish_args(run_cmd, block * attempt)
                    label = published_range_label(shifted)
                    shifted = [a if not a.startswith("TRIX_PUBLISHED_PORTS=")
                               else f"TRIX_PUBLISHED_PORTS={label}"
                               for a in shifted]
                    subprocess.run([self._docker_exe, "rm", "-f", container_name],
                                   capture_output=True, timeout=10,
                                   stdin=subprocess.DEVNULL)
                    try:
                        result = subprocess.run(
                    ...
                    except subprocess.CalledProcessError as shift_err:
                        if is_port_allocation_error(getattr(shift_err, "stderr", "") or ""):
                            continue
                        raise
                    logger.info(
                        "Контейнер %s получил свободный диапазон портов %s "
                        "(исходный занят другой песочницей).",
                ...
                if result is None:
                    logger.warning(
                        "Свободного диапазона портов не нашлось за %s попыток — "
                        "поднимаю контейнер %s без публикации.",
                        _MAX_PORT_SHIFT_ATTEMPTS, container_name,
                    )
                    subprocess.run([self._docker_exe, "rm", "-f", container_name],
                                   capture_output=True, timeout=10,
                                   stdin=subprocess.DEVNULL)
