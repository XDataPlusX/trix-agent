...
# Служебные файлы гейта — ВНЕ TMPDIR сюиты.
#
# Список текущих падений и сохранённый вывод раннера лежали в `mktemp`, то
# есть в том же TMPDIR, куда пишут сами тесты и куда смотрит любая внешняя
# чистка. Сторож ниже превратил потерю файла в отказ вместо ложно-зелёного
# релиза, но причину не убрал: измерение по-прежнему хранилось на чужой
# территории, и вычищенный сюитой TMPDIR ронял гейт на ровном месте.
# Удаления-и-пересоздания файла сторож к тому же не различает.
#
# `.git` выбран не из красоты: он существует всегда (мы уже в репозитории),
# не попадает в рабочее дерево и переживает любую чистку TMPDIR. В
# worktree это его собственный каталог `.git/worktrees/<имя>`.
GATE_STATE_DIR="$(git rev-parse --git-dir 2>/dev/null || echo "$REPO_ROOT")/trix-release-gate.$$"
mkdir -p "$GATE_STATE_DIR"

# Убирается только за успешным прогоном. После отказа каталог остаётся
# намеренно: в нём лежит сохранённый вывод раннера, то есть единственный
# ответ на вопрос "а почему". Имя содержит pid, параллельные прогоны друг
# другу не мешают.
cleanup_gate_state() {
    local status=$?
    [ "$status" -eq 0 ] && rm -rf "$GATE_STATE_DIR"
    return "$status"
}
...
# Сохранить полный вывод раннера рядом с гейтом и назвать путь.
# Отдельной функцией, потому что нужен на КАЖДОМ отказе, а не только на
# новых падениях: причина смерти раннера живёт в его выводе, и умирать
# вместе с процессом гейта она не должна.
save_run_log() {
    local log="$GATE_STATE_DIR/trix-release-run-$(date -u +%Y%m%d-%H%M%S).log"
    printf '%s\n' "$LAST_RUN_OUTPUT" > "$log" 2>/dev/null \
        && printf '\033[0;36m  Полный вывод прогона: %s\033[0m\n' "$log" >&2
}

# Прогон обязан доказать, что он СОСТОЯЛСЯ, а не просто не наговорил
# плохого. Без этой проверки гейт зеленел на непроверенном дереве:
# `collect_failures` глушила код возврата раннера (`set +e`) и нигде его не
# читала, а раннер, умерший до своей сводки (сломанный venv, OOM, полный
# диск, SIGTERM), не даёт ни строк FAILED -- значит список падений пуст, --
# ни заголовка "=== N files where no tests ran", значит сторож несобранных
# файлов тоже молчит. Дальше по цепочке: "✓ Все файлы сюиты отработали" ->
# "✓ No new failures" -> exit 0.
#
# Ненулевой код сам по себе НЕ отказ: раннер выходит с 1 ровно тогда, когда
# тесты упали, а это то, что гейт и приехал измерять. Различают их два
# признака:
#
#   * сводка "=== Summary: ... ===" -- раннер дошёл до конца и посчитал;
#   * код 128+N -- процесс убит сигналом. Тут не доверяем даже сводке:
#     она могла быть напечатана до сигнала, пока часть файлов не отработала.
require_complete_run() {
    if [ "$LAST_RUN_STATUS" -ge 128 ]; then
        save_run_log
        die "Раннер сюиты убит сигналом (код $LAST_RUN_STATUS), измерение неполное.
  Так выглядят OOM-killer (137), внешний SIGTERM (143) и обрыв сессии.
  Список падений в этом случае пуст не потому, что падений нет, а потому
  что считать было некому. Разберитесь с причиной и повторите прогон."
    fi
    # Строка ищется В СТРОКЕ, а не через трубу. `grep -q` выходит на первом
    # совпадении и закрывает трубу; `printf`, которому осталось что отдать,
    # получает SIGPIPE и код 141, а `set -o pipefail` считает кодом всего
    # конвейера самый правый ненулевой -- то есть код мёртвого `printf`, а
    # не нашедшего совпадение `grep`. Успешный поиск читался как «сводки
    # нет», и гейт отказывал прогону, у которого со сводкой всё в порядке.
    # На коротком выводе этого не видно: пока он влезает в буфер трубы,
    # `printf` успевает договорить. Живой прогон -- 607 КиБ -- не влезал.
    if ! grep -q '^=== Summary: ' <<<"$LAST_RUN_OUTPUT"; then
        save_run_log
        die "Раннер сюиты не напечатал свою сводку (код возврата $LAST_RUN_STATUS).
  Прогон оборвался, не досчитав: сломанный venv, нехватка памяти, полный
  диск. Пустой список падений здесь значит 'мерить было нечем', а не
  'падений нет' -- выпускать по нему нельзя."
    fi
}
...
if [ "$RECORD_BASELINE" = true ]; then
    ...
    tmp="$GATE_STATE_DIR/baseline-candidate.txt"
    collect_failures > "$tmp"

    # Прогон, который не состоялся, нельзя записывать в контракт тем более:
    # базовая линия из пустого измерения объявила бы сюиту целиком зелёной.
    require_complete_run
...
# Зеркало обязано отдавать ровно то, что ждёт рецепт установки.
#
# Иначе ломается тихо: подняли версию playwright, пересобрали архив,
# забыли выложить -- и установка у клиента молча уходит на GitHub. Не
# падает, а просто снова становится непредсказуемой по времени (замерено
# 2026-09-08: от 1 мин 56 с до 11 мин 17 с на одной машине). Через месяц
# никто не свяжет причину со следствием.
#
# Проверка дешёвая: у каждого артефакта на зеркале лежит свой `.sha256`,
# его и спрашиваем -- гигабайты на каждый релиз не качаем. Полную сверку
# содержимого делает сама установка при скачивании.
if [ -x "$REPO_ROOT/scripts/trix_mirror_gate.sh" ]; then
    "$REPO_ROOT/scripts/trix_mirror_gate.sh" || die \
"Зеркало разошлось с рецептом -- релиз остановлен.
Пересоберите артефакт, выложите на зеркало и обновите sha256 в
docs/product/cloud_init/hermes_install_v2.sh, затем повторите."
fi
...
# В каталоге гейта, а не в TMPDIR сюиты -- см. GATE_STATE_DIR.
current="$GATE_STATE_DIR/current-failures.txt"
collect_failures > "$current"

# Первым делом -- состоялся ли прогон вообще. Всё, что ниже, читает
# ПУСТОТУ как "падений нет", поэтому спрашивать надо до, а не после.
require_complete_run
...
# Сверка с базовой линией живёт в hermes_cli/release_gate.py, а не здесь.
# Раньше это были `sort -u` + `comm`: базовая линия сортировалась
# СИСТЕМНЫМ sort (порядок зависит от локали машины), а список падений --
# питоновским sorted() (кодовые точки). `comm` требует ОДНОГО порядка на
# обоих входах и на расхождении врёт в обе стороны. Релиз 0.1.38 был
# остановлен вердиктом "новые падения" на двух тестах, которые в базовой
# линии есть: под en_US.UTF-8 sort ставит test_wecom_callback.py перед
# test_wecom.py, а sorted() -- наоборот. Обратная сторона того же
# рассогласования опаснее: настоящее новое падение в том же участке comm
# молча съедает, и гейт выпускает непроверенное дерево.
# Дескриптор на $current открывается ДО прогона сюиты, а сам файл лежит в
# TMPDIR -- там же, куда пишут тесты и куда смотрит любая внешняя чистка.
# Если файл удалят ВО ВРЕМЯ прогона, `cat` ниже провалится внутри подстановки
# команд (его код возврата там маскируется), на сверку придёт пустая строка --
# и гейт объявит "новых падений нет", а всю базовую линию починенной.
# Измерено 2026-09-14 на этой ветке: TMPDIR указывал на scratch-каталог уже
# завершённого прогона, каталог подчистили на середине сюиты, гейт вернул
# exit 0 и назвал "now pass" все 39 записей базовой линии до единой --
# включая два известных падения, которые обязаны были остаться красными.
#
# Пустой файл -- законный результат (падений нет). ИСЧЕЗНУВШИЙ файл -- потеря
# измерения, и это отказ: молчать здесь значит выпустить непроверенное дерево.
if [ ! -f "$current" ]; then
    die "Файл со списком текущих падений исчез во время прогона сюиты:
    $current
  Его удалил кто-то извне гейта (чистка TMPDIR или сама сюита). Сравнивать
  не с чем: пустой ввод неотличим от 'падений нет' и даёт ложно-зелёный
  релиз. Прогоните гейт заново с TMPDIR, который никто кроме гейта не чистит."
fi

comparison="$(printf '%s\n' "$(cat "$current")" \
    | "$RELEASE_PY" -m hermes_cli.release_gate --compare-baseline "$BASELINE_FILE")" \
    || die "Не удалось сверить падения с базовой линией."
new_failures="$(printf '%s\n' "$comparison" | awk -F'\t' '$1=="new"{print $2}')"
fixed="$(printf '%s\n' "$comparison" | awk -F'\t' '$1=="fixed"{print $2}')"

if [ -n "$new_failures" ]; then
    printf '\033[0;31m✗ New test failures since the baseline:\033[0m\n' >&2
    printf '%s\n' "$new_failures" | sed 's/^/    /' >&2
    # Вывод прогона сохраняется на диск. Без этого отказ называл node-id и
    # молчал о ПРИЧИНЕ: вывод раннера жил только в переменной этого
    # процесса и умирал вместе с ним, а чтобы увидеть сообщение теста,
    # приходилось гонять сюиту заново -- ещё полчаса на то, что уже было
    # посчитано. Особенно дорого это для падений, которые в одиночку не
    # воспроизводятся.
    save_run_log
