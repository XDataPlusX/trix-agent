...
# ── Location variables (computed before we drop env) ───────────────────────
...
#
# TMPDIR belongs in the same list, and leaving it out was not neutral on
# POSIX: `tempfile` reads TMPDIR, then TEMP, then TMP. Forwarding TEMP/TMP
# while dropping TMPDIR INVERTS that precedence -- the suite's temp root
# silently became whatever TEMP said, ignoring the TMPDIR the operator
# actually set. Measured 2026-09-14: a release gate was launched with a short
# private TMPDIR precisely to keep its own bookkeeping safe, but the harness
# also exported TEMP=<long scratch path>; the per-file pytest subprocesses
# took TEMP, and four tests that bind an AF_UNIX socket under `tmp_path`
# failed with "AF_UNIX path too long" (the 108-byte sun_path limit) for
# reasons that had nothing to do with the tree under test. TMPDIR is a
# location variable exactly like the rest -- no credential can ride in it.
WIN_ENV=()
for _win_var in USERPROFILE HOMEDRIVE HOMEPATH LOCALAPPDATA APPDATA SYSTEMROOT TMPDIR TEMP TMP; do
  if [ -n "${!_win_var:-}" ]; then
...
#
# CI is forwarded for the same reason. Two tests already branch on it --
# tests/hermes_cli/test_setup_wizard_page_jsdom.py's canary ("skip is right
# locally, a lie in CI") and test_local_shell_init.py's /bin/bash guard --
# and `env -i` was stripping it, so both silently took their local branch on
# CI too. The jsdom canary in particular exists to go red when the lane
# cannot run its 44 browser tests; stripped, it self-skipped everywhere and
# guarded nothing. Forwarding it is what makes those guards real. It is set
# by every CI provider and carries no secret.
TEST_ENV=()
for _test_var in HERMES_TEST_IMAGE HERMES_TEST_WORKERS HERMES_TEST_PATHS \
  HERMES_TEST_FILE_TIMEOUT HERMES_TEST_FILE_RETRIES HERMES_TEST_SLICE CI; do
  if [ -n "${!_test_var:-}" ]; then
