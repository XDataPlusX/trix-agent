...
Output: ``assets/api/model-catalog.json``

Live URL: the raw-content address of the release repo's copy of that file
(see ``RELEASE_RAW_BASE_URL`` in ``hermes_cli/release_source.py``) -- there
is no separate docs site for this fork, so freshness tracks the release
branch directly.
"""
...
OUTPUT_PATH = os.path.join(REPO_ROOT, "assets", "api", "model-catalog.json")
CATALOG_VERSION = 1
