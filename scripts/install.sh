#!/bin/bash
# ============================================================================
# Trix Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   Run the Trix Agent installer provided by XDataPlus.
#
# Or with options:
#   ... | bash -s -- --no-venv --skip-setup
#
# ============================================================================
...
# Configuration
REPO_URL_SSH="git@github.com:xdataplusx/trix-agent.git"
REPO_URL_HTTPS="https://github.com/xdataplusx/trix-agent.git"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Trix Agent installs the code under"
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Trix Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  Run the Trix Agent PowerShell installer provided by XDataPlus."
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if command -v node &> /dev/null && node_satisfies_build "$(node --version)"; then
        ...
        log_warn "min-release-age-exclude) — installing a managed Node $NODE_VERSION instead..."
        install_node
        return
    fi
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ] && node_satisfies_build "$("$HERMES_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$HERMES_HOME/node/bin/node" --version) found (managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Trix Agent requires Node >=26) — installing a managed Node $NODE_VERSION..."
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. The install may complete, but web search and dependency downloads can fail."
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            # A checkout that predates this fix (or one a customer cloned by
            # hand) can still have origin pointed at the upstream repository.
            # Left alone, the `git fetch origin "$BRANCH"` below would run
            # against upstream -- which has no branch named "release" -- and
            # abort the whole repository stage with a raw git error under
            # `set -e`. Repoint origin to the release repository first, same
            # SSH-then-HTTPS preference as a fresh clone. The unrelated
            # history this leaves behind is already handled further down:
            # `git pull --ff-only` will fail (no common ancestor) and the
            # existing fallback resets the checkout to origin/$BRANCH.
            #
            # Matched narrowly against the known upstream org/repo rather
            # than "anything that isn't our exact URL string" -- a customer
            # is free to run a self-hosted mirror or otherwise customize
            # origin, and this must only correct the one specific known-bad
            # case, not every remote we didn't set ourselves.
            local current_origin
            current_origin="$(git remote get-url origin 2>/dev/null || echo "")"
            case "$current_origin" in
                *NousResearch/hermes-agent*)
                    log_warn "origin ($current_origin) is the upstream repository; repointing to the release repository..."
                    git remote set-url origin "$REPO_URL_SSH"
                    if ! GIT_SSH_COMMAND="ssh -o BatchMode=yes -o ConnectTimeout=5" \
                         git ls-remote --exit-code origin "$BRANCH" >/dev/null 2>&1; then
                        log_info "SSH unreachable, using HTTPS instead..."
                        git remote set-url origin "$REPO_URL_HTTPS"
                    fi
                    log_success "origin repointed to $(git remote get-url origin)"
                    ;;
            esac
            ...
            #
            # Checked explicitly rather than left to `set -e`: `--stage`
            # installs run this function with errexit disabled (see
            # run_stage_protocol), so an unchecked failure here would print
            # git's raw error and then silently fall through to `git
            # checkout`/`git pull` against whatever remote-tracking refs
            # happen to already be on disk -- reporting "Repository ready"
            # without ever having fetched the release repository.
            git remote set-branches origin "$BRANCH" 2>/dev/null || true
            if ! git fetch origin "$BRANCH"; then
                log_error "Could not fetch '$BRANCH' from $(git remote get-url origin 2>/dev/null)."
                log_info "Check your network connection and that this machine can reach the repository host over SSH or HTTPS, then re-run the installer."
                exit 1
            fi
            git checkout "$BRANCH"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Trix Agent behaves unexpectedly."
                    else
...
EOF
        ...
        #
        # Sparse-checkout: website/ is the upstream documentation site's
        # source, 27 MB the agent never reads and the customer never sees.
        # It's excluded via clone configuration, not deletion -- deleting it
        # after the fact would conflict with every upstream merge that
        # touches those files. Clone with --no-checkout so no working tree
        # is materialized yet, set the sparse pattern, then check out.
        #
        # Trix Agent ships on the client's own Linux VPS and is driven
        # entirely through Telegram -- the Electron desktop app, its Windows
        # installers, and the Rust bootstrap installer never run on that
        # machine. They're excluded from the client checkout for the same
        # reason as website/: roughly 1600 files and ~200MB of Electron and
        # Tauri source that does nothing here. It also carries upstream
        # branding -- though note that is a side benefit, not the
        # justification: ~1400 files elsewhere in the checkout still say
        # Hermes, and a real rebrand is a separate project. This is a PARKING
        # decision, not a deletion -- the code stays in the repo and keeps
        # working for us. Do NOT exclude apps/shared: the web dashboard
        # (web/) depends on it (`@hermes/shared` in web/package.json), so it
        # must stay in the checkout even though it lives under apps/. When a
        # future project rebrands the desktop/Windows apps, drop the five
        # desktop entries below; the '!/website' exclusion is separate and
        # stays.
        #
        # docs/product/ is our internal product tree -- intake notes, the
        # architecture contract, specs, plans, and a status doc listing
        # known gaps. None of it is meant for the client; exclude it the
        # same way as website/, not by deleting it out of the repo.
        #
        # docs/debug/ is excluded on a DIFFERENT basis than everything
        # above: not weight or irrelevance, but content. A /debug report
        # carries log tails with credentials in them (proxy URLs of the
        # shape http://user:pass@host, which agent/redact.py does not mask
        # -- it covers the colon form only for database schemes), and it
        # belongs to ONE client. The delivery tree is the same for every
        # client, so a published report leaks a secret AND hands one
        # client's data to all the others. Keep this entry even if the
        # repo stops ignoring the directory.
        #
        # docs/progress/ is the same category as docs/product/: session
        # hand-off logs we write for ourselves, appended to by the logging
        # skill. The directory keeps growing, so the directory is excluded
        # rather than any one file.
        log_info "Trying SSH clone..."
        if GIT_SSH_COMMAND="ssh -o BatchMode=yes -o ConnectTimeout=5" \
           git clone --depth 1 --branch "$BRANCH" --no-checkout \
               "$REPO_URL_SSH" "$INSTALL_DIR" 2>/dev/null; then
            log_success "Cloned via SSH"
        else
            rm -rf "$INSTALL_DIR" 2>/dev/null  # Clean up partial SSH clone
            log_info "SSH failed, trying HTTPS..."
            # GIT_TERMINAL_PROMPT=0 is load-bearing, not hygiene: when the
            # clone below fails for ANY reason, git falls back to asking
            # "Username for 'https://github.com'" -- even for a PUBLIC repo.
            # On a tty (an operator running install.sh by hand) that hangs
            # the installer forever on a prompt nobody expects; in cloud-init
            # it prints "could not read Username", which reads like a
            # permissions problem and sends whoever debugs it looking at repo
            # visibility, tokens and deploy keys. Proven on a live client VM
            # 2026-09-03: the repo was public and reachable the whole time.
            if GIT_TERMINAL_PROMPT=0 git clone --depth 1 --branch "$BRANCH" --no-checkout \
                   "$REPO_URL_HTTPS" "$INSTALL_DIR"; then
                log_success "Cloned via HTTPS"
            else
                # HTTP/1.1 retry. Some networks (confirmed in the RU datacenter
                # the product ships into) break git's smart-HTTP POST over
                # HTTP/2: the GET of /info/refs succeeds, then
                # POST /git-upload-pack comes back "HTTP/2 401" with
                # www-authenticate: Basic realm="GitHub". It is NOT about our
                # repo or our account -- cloning github.com/git/git fails
                # identically on the same box, and plain curl to the same
                # endpoint returns 200. Forcing HTTP/1.1 clones fine.
                #
                # Retry rather than always downgrade: HTTP/2 is the better
                # transport wherever it works, and this costs one failed
                # request only on networks that are already broken.
                rm -rf "$INSTALL_DIR" 2>/dev/null
                log_info "HTTPS clone failed, retrying over HTTP/1.1..."
                if GIT_TERMINAL_PROMPT=0 git -c http.version=HTTP/1.1 \
                       clone --depth 1 --branch "$BRANCH" --no-checkout \
                       "$REPO_URL_HTTPS" "$INSTALL_DIR"; then
                    log_success "Cloned via HTTPS (HTTP/1.1)"
                    # Persist it for this checkout: `hermes update` runs
                    # git fetch/pull from here later, on the same network,
                    # and would hit the same wall on the client's machine.
                    git -C "$INSTALL_DIR" config http.version HTTP/1.1 || true
                else
                    rm -rf "$INSTALL_DIR" 2>/dev/null
        ...
        if ! git -C "$INSTALL_DIR" sparse-checkout set --no-cone \
               '/*' '!/website' \
               '!/apps/desktop' '!/apps/bootstrap-installer' \
               '!/scripts/desktop-update*' '!/scripts/install.ps1' '!/scripts/install.cmd' \
               '!/docs/product' '!/docs/debug' '!/docs/progress'; then
            log_warn "sparse-checkout unavailable — excluded paths will be present on disk"
        fi
...
install_deps() {
    ...
    # `uv sync --frozen`. The lockfile records SHA256 hashes for every
    ...
    if [ -f "uv.lock" ]; then
        ...
        #   1. `--extra all --frozen` against a fresh venv has to pull
        ...
        # `--frozen`, NOT `--locked`. The two are not interchangeable here:
        #
        #   --locked  asserts the lockfile is STILL up to date, which makes
        #             uv re-resolve against the live index and compare. With
        #             `exclude-newer = "14 days"` in pyproject.toml (upstream's
        #             supply-chain quarantine) that comparison window slides
        #             every day, and a uv release can change how the recorded
        #             span is interpreted — so the assertion can fail for
        #             reasons that have nothing to do with this machine.
        #             Observed on a clean client VM 2026-09-04: "Resolving
        #             despite existing lockfile due to removal of global
        #             exclude newer" -> "lockfile needs to be updated". The
        #             same tree, same uv, same host passed twenty minutes
        #             later. The client silently lost hash verification and
        #             the only trace was a log line nobody reads.
        #   --frozen  installs EXACTLY what the lockfile records, hashes and
        #             all, without re-resolving. That is what a client install
        #             actually wants: the tree we shipped and tested, not
        #             whatever PyPI resolves to at the moment of install.
        #
        # Freshness of the lockfile is OUR problem, checked where it belongs —
        # in the release gate on our machine (`uv lock --check`), not on the
        # client's, where a stale-lock verdict has no useful action behind it.
        if UV_PROJECT_ENVIRONMENT="$INSTALL_DIR/venv" $UV_CMD sync --extra all --frozen; then
...
install_ddgs_search_backend() {
    # web.search_backend: ddgs is baked into the curated config template
    # (assets/config/trix-config.yaml) -- it's the only search backend that
    # needs neither an API key nor a side service. The `ddgs` PyPI package
    # is declared as its own extra in pyproject.toml (exact-pinned, tracked
    # by LAZY_DEPS["search.ddgs"] in tools/lazy_deps.py) rather than a bare
    # `pip install ddgs` here, so the version pin has one source of truth
    # across the installer and `hermes tools`' own "ddgs" post_setup step
    # (hermes_cli/tools_config.py). Eager-installed here the same way
    # [wake]/[voice] are for desktop in install_desktop_voice_deps() --
    # ".[wake,voice]" there, ".[ddgs]" here.
    #
    # Without this step, _get_capability_backend() in tools/web_tools.py
    # silently falls through to the firecrawl default the instant
    # `import ddgs` fails -- the client doesn't get "install the ddgs
    # package", they get "buy a Firecrawl key / log into Nous Portal"
    # (tools/web_tools.py:287-308). The DDGS provider's search() does NOT
    # self-heal that at runtime, unlike exa/firecrawl/parallel: its
    # is_available() gate checks the same `import ddgs` that would need
    # lazy-installing, so an install-time failure below is never noticed
    # again until the OUTCOME is reported -- see check_ddgs_preflight()
    # near the end of this script (print_success()) for that report, and
    # plugins/web/ddgs/provider.py's search() docstring for why self-heal
    # can't run there. This function is the installer half of the whole
    # fix; tests/test_install_sh_ddgs_search_backend.py is the test-
    # relation half.
    if [ "$DISTRO" = "termux" ]; then
        log_info "Skipping ddgs install on Termux -- run 'hermes tools' afterwards and re-select DuckDuckGo (ddgs) to install it if security.allow_lazy_installs stays enabled"
        return 0
    fi
    if [ -z "${UV_CMD:-}" ]; then
        log_info "Skipping ddgs install (uv unavailable) -- run 'hermes tools' afterwards and re-select DuckDuckGo (ddgs) to install it if security.allow_lazy_installs stays enabled"
        return 0
    fi
    if [ "$USE_VENV" = true ]; then
        export VIRTUAL_ENV="$INSTALL_DIR/venv"
    ...
    log_info "Installing ddgs (DuckDuckGo search backend, no key or service required)..."
    if (cd "$INSTALL_DIR" && $UV_CMD pip install -e ".[ddgs]") >/dev/null 2>&1; then
        log_success "ddgs installed"
    else
        log_warn "ddgs install failed -- web search may fall back to a paid backend until you run: cd $INSTALL_DIR && $UV_CMD pip install -e '.[ddgs]'"
    fi
}
...
setup_path() {
    ...
    if [ ! -x "$HERMES_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$HERMES_ENTRYPOINT" ]; }; then
        log_warn "Trix Agent launcher prerequisites not found"
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Trix Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Trix Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Trix Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    if [ ! -f "$HERMES_HOME/.env" ]; then
        # Prefer our short, curated .env template (assets/config/trix.env.example)
        # over upstream's 496-line, 127-variable .env.example. The choice
        # lives in a tested Python function
        # (hermes_cli/config_template.py::resolve_env_template) rather than
        # as shell logic here — if the venv python is missing for any
        # reason, fall back to the old upstream-example behavior.
        #
        # `|| env_template=""` is load-bearing, same reason as
        # config_template below: this whole script runs under `set -e`, and
        # a plain assignment `x=$(cmd)` aborts the ENTIRE installer the
        # instant `cmd` exits non-zero — e.g. an ImportError inside
        # hermes_cli would otherwise kill the install right here, silently
        # (stderr below is redirected), before .env permissions, config.yaml,
        # SOUL.md, or skills seeding ever run. The guard turns "python
        # failed to resolve" into "fall back to the upstream example", not
        # "installer dies".
        env_template=""
        if [ -x "$INSTALL_DIR/venv/bin/python" ]; then
            env_template=$("$INSTALL_DIR/venv/bin/python" - "$INSTALL_DIR" <<'PYEOF' 2>/dev/null
import sys
from pathlib import Path
from hermes_cli.config_template import resolve_env_template

result = resolve_env_template(Path(sys.argv[1]))
if result:
    print(result)
PYEOF
) || env_template=""
        fi
        if [ -z "$env_template" ] && [ -f "$INSTALL_DIR/.env.example" ]; then
            env_template="$INSTALL_DIR/.env.example"
        fi
        if [ -n "$env_template" ] && [ -f "$env_template" ]; then
            cp "$env_template" "$HERMES_HOME/.env"
            log_success "Created ~/.hermes/.env from $(basename "$env_template")"
        else
    ...
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        # Prefer our short, curated config.yaml template
        # (assets/config/trix-config.yaml) over upstream's 1700+ line
        # cli-config.yaml.example. The choice lives in a tested Python
        # function (hermes_cli/config_template.py::resolve_config_template)
        # rather than as shell logic here — if the venv python is missing
        # for any reason, fall back to the old upstream-example behavior.
        #
        # `|| config_template=""` is load-bearing: this whole script runs
        # under `set -e` (see top of file), and a plain assignment
        # `x=$(cmd)` aborts the ENTIRE installer the instant `cmd` exits
        # non-zero — e.g. an ImportError inside hermes_cli would otherwise
        # kill the install right here, silently (stderr below is
        # redirected), before config.yaml, SOUL.md, or skills seeding ever
        # run. The guard turns "python failed to resolve" into "fall back
        # to the upstream example", not "installer dies".
        config_template=""
        if [ -x "$INSTALL_DIR/venv/bin/python" ]; then
            config_template=$("$INSTALL_DIR/venv/bin/python" - "$INSTALL_DIR" <<'PYEOF' 2>/dev/null
import sys
from pathlib import Path
from hermes_cli.config_template import resolve_config_template

result = resolve_config_template(Path(sys.argv[1]))
if result:
    print(result)
PYEOF
) || config_template=""
        fi
        if [ -z "$config_template" ] && [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
        ...
        if [ -n "$config_template" ] && [ -f "$config_template" ]; then
            ...
            log_success "Created ~/.hermes/config.yaml from $(basename "$config_template")"
        fi
    else
...
You are Trix Agent, an intelligent AI assistant provided by XDataPlus. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations. Respond in Russian by default, and switch to another language only if the user writes to you in it or explicitly asks you to.
SOUL_EOF
    ...
    # Seed the model-catalog disk cache from the freshly-cloned checkout, the
    # same way `hermes update` does after a git pull / zip unpack
    # (seed_cache_from_checkout() in hermes_cli/model_catalog.py). Without
    # this, a fresh install has no disk cache and no network yet run, so
    # model resolution on a fully offline machine has nothing to read until
    # the first successful catalog fetch — which never happens if the box
    # never gets network access at all. Non-fatal: the network fetch path
    # still applies on first use if this is skipped.
    if HERMES_HOME="$HERMES_HOME" "$INSTALL_DIR/venv/bin/python" - "$INSTALL_DIR" <<'PYEOF' 2>/dev/null
import sys
from hermes_cli.model_catalog import seed_cache_from_checkout
sys.exit(0 if seed_cache_from_checkout(sys.argv[1]) else 1)
PYEOF
    then
        log_success "Model catalog cache seeded for offline use"
    else
        log_info "Model catalog cache seed skipped (will fetch on first use)"
    fi
}
...
find_system_browser() {
    ...
    #
    # The check has to run on the RESOLVED path, not on what the user typed.
    # Matching only the raw string let a bare name through: with
    # AGENT_BROWSER_EXECUTABLE_PATH=chromium and a Snap wrapper first on PATH,
    # "chromium" does not start with /snap/, so the guard never fired and this
    # function happily returned /snap/bin/chromium — precisely the binary the
    ...
    if _reject_snap "$override"; then
        return 1
    fi

    if [ -x "$override" ]; then
        _reject_snap "$override" && return 1
        echo "$override"
        return 0
    fi
    if command -v "$override" >/dev/null 2>&1; then
        _resolved="$(command -v "$override")"
        _reject_snap "$_resolved" && return 1
        echo "$_resolved"
        return 0
    fi
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# (Hermes|Trix) Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Trix Agent will use the bundled Chromium instead."
...
run_with_timeout() {
    ...
    if [ "$(type -t "$1" 2>/dev/null)" != "function" ]; then
        ...
        if [ -n "$timeout_bin" ]; then
            ...
            #
            # У `--foreground` есть цена, и она записана в его же man: «children
            # of COMMAND will not be timed out». Сигнал уходит ТОЛЬКО прямому
            # потомку. `agent-browser install` — это `sh`, который запускает
            # загрузчик: `sh` умирает по SIGTERM, загрузчик остаётся жив, держит
            # унаследованный stdout открытым — и установщик, запущенный по
            # конвейеру (`curl | bash`, CI), висит вечно. То есть ровно тот класс
            # зависаний, от которого здесь и стоит сторож.
            #
            # Поэтому `--foreground` включается только когда есть терминал, с
            # которого этот Ctrl+C вообще может прийти. Без терминала гасить
            # нечего, а поведение по умолчанию (своя группа процессов, сигнал
            # всей группе) убивает поддерево целиком — проверено исполнением:
            # с флагом внук переживает таймаут, без флага нет.
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Trix Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        # Ниже стоит безусловное "Browser engine setup complete", и оно
        # печаталось СРАЗУ ПОСЛЕ "browser tools will not work" -- снято с
        # живой машины 2026-09-04, две строки подряд. Читающий лог видел
        # зелёную галочку и шёл дальше; на машине браузера не было.
        BROWSER_ENGINE_OK=true
        strip_snap_browser_override
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            case "$DISTRO" in
                ...
                *)
                    ...
                    cd "$INSTALL_DIR" && run_playwright_install 600 npx playwright install chromium || BROWSER_ENGINE_OK=false
                    ;;
            esac
        fi
        fi
        if [ "${BROWSER_ENGINE_OK:-true}" = true ]; then
            log_success "Browser engine setup complete"
        else
            log_warn "Browser engine setup did NOT complete — see the failure above."
            log_info "The Chromium preflight at the end of this install will try once more."
        fi
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Trix Agent to send/receive messages."
...
check_docker_preflight() {
    ...
    # sandbox. This installer NEVER installs Docker itself: provisioning it
    # is root's job at VM-creation time (see the "Docker" section of
    # docs/product/deployment-requirements.md), and this script always runs
    # as an unprivileged user. Without this check, a VM whose provisioning
    # never ran only finds out the first time the client asks the agent to
    # run a command -- and it's the client who discovers it, not us.
    # Non-fatal either way: an agent without a sandbox still answers
    # questions, so this must never abort the install (see
    # docs/product/specs/2026-08-17-trix-agent-standard-build-design.md
    # §4.5, §10).
    #
    # Called from print_success(), deliberately AFTER the "Installation
    # Complete" banner and every other note (Node.js, ripgrep) -- printing
    # it first would bury it under ~40 lines of green banner the operator
    # has no reason to keep reading past.
    if [ ! -x "$INSTALL_DIR/venv/bin/python" ]; then
        # No venv python (e.g. --no-venv) means the check itself cannot
        # run. Silence here would read as "Docker is fine" to an operator
        # scanning the success banner -- say plainly that nothing was
        # verified instead of implying an all-clear that never happened.
        log_warn "Docker не проверен: нет venv-python установщика (например, при --no-venv). Проверьте вручную: docker info"
        return 0
    fi

    # `local docker_report` (bare declaration) + a separate assignment
    # avoids the `local x=$(cmd)` pitfall where `local`'s own exit status
    # masks the command substitution's exit status under `set -e`.
    # `|| docker_report=""` on the assignment line is ALSO load-bearing,
    # same reason as the template resolvers in copy_config_templates():
    # this whole script runs under `set -e` (line 16), and a plain
    # assignment `x=$(cmd)` would abort the ENTIRE installer -- after the
    # gateway is already started -- the instant this subprocess exits
    # non-zero for any reason (ImportError, a bug in the check itself).
    # The guard turns "the check itself broke" into "skip the check",
    # never "installer dies at the finish line".
    local docker_report
    docker_report=""
    docker_report=$("$INSTALL_DIR/venv/bin/python" - <<'PYEOF' 2>/dev/null
from hermes_cli.docker_preflight import check_docker_backend

result = check_docker_backend()
print("OK" if result.ok else "WARN")
print(result.message)
PYEOF
) || docker_report=""

    if [ -z "$docker_report" ]; then
        log_warn "Docker не проверен: проверка не смогла выполниться. Проверьте вручную: docker info"
        return 0
    fi

    # Parameter expansion instead of `sed`/`printf` subshells: no extra
    # processes, and nothing here can fail under `set -e` the way a
    # subshell whose stubbed/PATH-shadowed `sed` returns non-zero would.
    local docker_status="${docker_report%%$'\n'*}"
    local docker_message="${docker_report#*$'\n'}"

    if [ -z "$docker_message" ] || [ "$docker_message" = "$docker_report" ]; then
        log_warn "Docker не проверен: не удалось разобрать ответ проверки. Проверьте вручную: docker info"
        return 0
    fi
    ...
    if [ "$docker_status" = "OK" ]; then
        ...
        log_warn "$docker_message"
    fi
}

_probe_chromium_backend() {
    # Isolated out of check_chromium_preflight() because that function
    # needs to run this exact probe TWICE -- once for the initial read,
    # once after a repair attempt -- and two independent copies of the
    # heredoc would risk drifting apart over time.
    "$INSTALL_DIR/venv/bin/python" - <<'PYEOF' 2>/dev/null
...
check_chromium_preflight() {
    # browser.backend: "off" in the curated config template selects the
    # built-in browser_* tools running on top of a locally installed
    # Chromium (Ruling 4 of the spec). install_node_deps() already
    # attempts a Chromium install earlier in this script (Playwright), but
    # nothing reported the OUTCOME back to the operator -- and the stakes
    # of silence here are higher than for Docker: the exact check_fn
    # gating browser_navigate/browser_snapshot/etc. in the model's tool
    # schema (tools.browser_tool.check_browser_requirements) returns False
    # whenever Chromium isn't found on disk, so those tools vanish from
    # the schema entirely with no error text anywhere -- proven by direct
    # execution (registry.get_definitions() returns zero browser_* entries
    # with no Chromium reachable). Same failure CLASS as the ddgs-package
    # gap this task also fixes, just with no leftover tool to even show a
    # billing-upsell error. Follows the exact structure of
    # check_docker_preflight() above -- see its comments for the `local`/
    # set -e/parameter-expansion rationale, which applies identically here.
    #
    # Unlike Docker (root's job at VM-provisioning time) and ddgs (a plain
    # pip install), Chromium already has a working unprivileged repair path
    # in THIS script: ensure_browser(), the same function `--ensure browser`
    # drives. A client with no console/SSH cannot act on a bare "Chromium
    # missing" verdict (docs/product/PROMPT-spec15-support-page.md,
    # "Блокирующие зависимости" #3), so on a negative first read we try the
    # repair ONCE, then report the real post-repair state -- never the
    # stale pre-repair one. Exactly one attempt, no retry loop. Skipped
    # entirely when --skip-browser was passed: that is an explicit operator
    # choice install_node_deps() already honored earlier in this run, and
    # this step must not second-guess it. When the repair can't do
    # anything (no Node, no network, npm install fails) ensure_browser()
    # itself just returns non-zero and/or leaves Chromium missing -- that
    # is NOT treated as an installer error here (the `|| true` guard is
    # load-bearing under `set -e`, line 16); we simply fall through to
    # reporting the verdict, same as before this function ever tried to
    # fix anything.
    #
    # root vs user: tools/browser_tool.py's _chromium_installed() looks in
    # PLAYWRIGHT_BROWSERS_PATH when set, else the invoking user's own $HOME
    # Playwright cache. install.sh (and therefore this preflight and the
    # ensure_browser() repair it drives) runs as whatever user invoked it,
    # while the agent in production runs as a separate `user`. The client
    # VM recipe (docs/product/cloud_init/hermes_install_v2.sh:367) papers
    # over this by exporting PLAYWRIGHT_BROWSERS_PATH=/home/user/.cache/
    # ms-playwright before calling install.sh, so that path is fine. But
    # any OTHER invocation of install.sh as root WITHOUT that export --
    # e.g. a manual re-run, a support script, a future recipe variant --
    # downloads Chromium into root's own cache, and this preflight's repair
    # probe, also running as root, will then honestly report OK against
    # root's cache while the agent (running as `user`, with no matching
    # cache and no PLAYWRIGHT_BROWSERS_PATH export in that scenario) still
    # can't find Chromium. Before this task's repair-before-verdict change,
    # that same root/user split just meant a false WARN in an already-
    # working setup; now it can just as easily flip into a false OK,
    # because the repair actively populates root's cache where a plain
    # read wouldn't have. Pre-existing class of bug, not introduced here,
    # and not fixed here -- documented so the next reader knows why "OK"
    # from this function is only trustworthy when the caller's env matches
    # production's (i.e. PLAYWRIGHT_BROWSERS_PATH is exported the same way
    # the client VM recipe does it, or install.sh is run as the same user
    # the agent will run as).
    if [ ! -x "$INSTALL_DIR/venv/bin/python" ]; then
        log_warn "Chromium не проверен: нет venv-python установщика (например, при --no-venv). Проверьте вручную: npx agent-browser install --with-deps"
        return 0
    fi

    local chromium_report
    chromium_report=""
    chromium_report=$(_probe_chromium_backend) || chromium_report=""

    if [ -z "$chromium_report" ]; then
        log_warn "Chromium не проверен: проверка не смогла выполниться. Проверьте вручную: npx agent-browser install --with-deps"
        return 0
    fi

    local chromium_status="${chromium_report%%$'\n'*}"
    local chromium_message="${chromium_report#*$'\n'}"

    if [ -z "$chromium_message" ] || [ "$chromium_message" = "$chromium_report" ]; then
        log_warn "Chromium не проверен: не удалось разобрать ответ проверки. Проверьте вручную: npx agent-browser install --with-deps"
        return 0
    fi

    if [ "$chromium_status" != "OK" ] && [ "$SKIP_BROWSER" != true ]; then
        log_info "Chromium не найден -- пробую доустановить один раз тем же механизмом, что и --ensure browser..."
        # ensure_browser() only ever `return`s, never `exit`s -- but it is
        # guarded here anyway (see the block comment above) because a bare
        # failing statement under `set -e` would otherwise abort the whole
        # installer at the very tail of a run that already started the
        # gateway.
        ensure_browser || true

        # The two guards below (report non-empty, then message-vs-whole-report
        # sane) only ever protect against a malformed/absent retry probe --
        # _probe_chromium_backend()'s real python path always emits exactly
        # two lines (see hermes_cli/browser_preflight.py's
        # check_chromium_backend(), which always returns a
        # BrowserPreflightResult with both fields set). No test has found a
        # reachable input that trips either guard with the real probe; they
        # exist as a parsing safety net, not because a known input defeats
        # them. Covered anyway (test_install_sh_chromium_preflight.py) by
        # stubbing the probe directly with an empty output and a single-line
        # (no-newline) output, so the guards stay exercised even though they
        # are currently unreachable through the real probe.
        #
        # Known asymmetry, left as-is rather than "fixed": if the retry probe
        # itself fails to run at all (chromium_retry_report stays empty --
        # e.g. the venv python became unusable mid-repair), chromium_status/
        # chromium_message are simply never touched, and the code falls
        # through to print the STALE pre-repair verdict below. A repair that
        # actually succeeded on disk, whose verification probe happened to
        # fail, therefore still tells the operator "Chromium не найден...
        # Установите вручную" -- an honest worst case (never a false OK), but
        # a real accuracy gap in a case this function otherwise doesn't
        # reach.
        local chromium_retry_report
        chromium_retry_report=""
        chromium_retry_report=$(_probe_chromium_backend) || chromium_retry_report=""
        if [ -n "$chromium_retry_report" ]; then
            local chromium_retry_status="${chromium_retry_report%%$'\n'*}"
            local chromium_retry_message="${chromium_retry_report#*$'\n'}"
            if [ -n "$chromium_retry_message" ] && [ "$chromium_retry_message" != "$chromium_retry_report" ]; then
                chromium_status="$chromium_retry_status"
                chromium_message="$chromium_retry_message"
            fi
        fi
    fi

    if [ "$chromium_status" = "OK" ]; then
        log_success "$chromium_message"
    else
        log_warn "$chromium_message"
    fi
}

check_ddgs_preflight() {
    # web.search_backend: ddgs in the curated config template is
    # DuckDuckGo search via the `ddgs` PyPI package -- the only search
    # backend that needs neither an API key nor a side service (spec
    # §4.2). install_ddgs_search_backend() (earlier in this script)
    # already attempts to eager-install it, but -- unlike Docker and
    # Chromium below, whose outcomes ARE reported here -- nothing
    # previously reported whether that install actually succeeded.
    # Silence here matters more than it looks: _get_capability_backend()
    # in tools/web_tools.py does not error when ddgs is missing, it
    # silently falls through to the firecrawl default -- so a failed
    # install turns "free DuckDuckGo search" into "buy a Firecrawl key /
    # log into Nous Portal", and the client discovers it, not us. Unlike
    # exa/firecrawl/parallel, the DDGS provider cannot self-heal that at
    # runtime (see plugins/web/ddgs/provider.py's search() docstring), so
    # this install-time report is the only place a failed install is ever
    # surfaced to anyone. Follows the exact structure of
    # check_docker_preflight() / check_chromium_preflight() above -- see
    # check_docker_preflight()'s comments for the local/set -e/parameter-
    # expansion rationale, which applies identically here.
    if [ ! -x "$INSTALL_DIR/venv/bin/python" ]; then
        log_warn "ddgs не проверен: нет venv-python установщика (например, при --no-venv). Проверьте вручную: python -c 'import ddgs'"
        return 0
    fi

    local ddgs_report
    ddgs_report=""
    ddgs_report=$("$INSTALL_DIR/venv/bin/python" - <<'PYEOF' 2>/dev/null
...
PYEOF
    ...
    if [ -z "$ddgs_report" ]; then
        log_warn "ddgs не проверен: проверка не смогла выполниться. Проверьте вручную: python -c 'import ddgs'"
        return 0
    fi

    local ddgs_status="${ddgs_report%%$'\n'*}"
    local ddgs_message="${ddgs_report#*$'\n'}"

    if [ -z "$ddgs_message" ] || [ "$ddgs_message" = "$ddgs_report" ]; then
        log_warn "ddgs не проверен: не удалось разобрать ответ проверки. Проверьте вручную: python -c 'import ddgs'"
        return 0
    fi

    if [ "$ddgs_status" = "OK" ]; then
        log_success "$ddgs_message"
    else
        log_warn "$ddgs_message"
    fi
}
...
print_success() {
    ...
    # Docker + Chromium + ddgs preflight -- deliberately the LAST things
    # print_success() prints (same precedent as the Node.js/ripgrep notes
    # above them: shown inside the success banner, not before it, so they
    # survive on screen instead of scrolling off under ~40 lines of green
    # banner text). See check_docker_preflight() / check_chromium_preflight()
    # / check_ddgs_preflight() for why none of these can just be silent.
    check_docker_preflight
    check_chromium_preflight
    check_ddgs_preflight
}
...
ensure_browser() {
    ...
    # agent-browser кладёт браузер в $HOME того, кто его запустил, и способа
    # указать каталог у него нет (`agent-browser install --help` знает только
    # --with-deps). Провижининг выполняет установщик от root, а агент работает
    # от обычного пользователя — браузер оказывается в /root, где агент его не
    # ищет. Проверено исполнением на чистой клиентской VM 2026-09-03: "Chrome
    # installed successfully / Location: /root/.agent-browser/browsers/
    # chrome-152..." при HERMES_HOME, принадлежащем `user`.
    #
    # PLAYWRIGHT_BROWSERS_PATH, который рецепт для этого экспортирует, здесь не
    # помогает: его знает playwright, а не agent-browser.
    #
    # Владельца берём по HERMES_HOME — это и есть тот, для кого мы ставим.
    # Записка `Root vs user` требует ровно этого: системные библиотеки ставит
    # root, а бинарник браузера кладётся в home пользователя.
    local ab_runner=""
    if [ "$(id -u)" = "0" ] && command -v stat >/dev/null 2>&1; then
        local hermes_owner
        hermes_owner="$(stat -c '%U' "$HERMES_HOME" 2>/dev/null || echo "")"
        if [ -n "$hermes_owner" ] && [ "$hermes_owner" != "root" ] \
           && id "$hermes_owner" >/dev/null 2>&1; then
            ab_runner="$hermes_owner"
            log_info "Browser will be installed for user '$ab_runner' (owner of $HERMES_HOME)"
        fi
    fi

    if [ -x "$ab_bin" ]; then
        # Time-boxed like the npm install a few lines up and like
        # run_playwright_install(): this is the same class of download (a
        # Chromium binary fetch) and the same #35166 apt-release hang
        # applies -- agent-browser install shells out to a Playwright-style
        # browser fetch under the hood. Reuse NODE_DEPS_TIMEOUT rather than
        # inventing a second knob: it is already the "how long may a
        # browser-tooling download run" budget for this function (the npm
        # install above uses it), an operator on a slow link who bumps it
        # gets both steps covered, and check_chromium_preflight() -- the
        # only caller reachable with no operator watching (client VM
        # creation, docs/product/cloud_init/hermes_install_v2.sh:371-372,
        # which passes no --skip-browser) -- now returns instead of hanging
        # the installer indefinitely. A timeout here is treated exactly
        # like any other failure of this step: logged as a warning, `|| true`
        # further up the call chain in check_chromium_preflight(), install.sh's
        # exit code unchanged.
        # -H обязателен: без него sudo сохраняет HOME вызывающего, браузер
        # снова уедет в /root, и вся правка выше окажется бесполезной — молча.
        local ab_cmd=("$ab_bin" install)
        if [ -n "$ab_runner" ]; then
            # Пакет agent-browser ставился npm-ом от root, поэтому его файлы
            # принадлежат root — а запущенный от пользователя инструмент при
            # старте делает свой же бинарник исполняемым и упирается в EPERM
            # ("Cannot make binary executable"). Проверено исполнением на
            # чистой VM 2026-09-03. Рецепт возвращает владельца всему
            # домашнему каталогу, но ПОЗЖЕ — к этому моменту установка
            # браузера уже провалилась бы. Отдаём каталог node сейчас, до
            # запуска: он и так предназначен пользователю.
            chown -R "$ab_runner" "$HERMES_HOME/node" 2>/dev/null || true
            ab_cmd=(sudo -u "$ab_runner" -H "$ab_bin" install)
        fi
        run_with_timeout "$NODE_DEPS_TIMEOUT" "${ab_cmd[@]}" 2>/dev/null || {
