...
# Prefer Trix's curated config.yaml template (assets/config/trix-config.yaml)
# over upstream's cli-config.yaml.example -- same resolver install.sh uses
# (hermes_cli/config_template.py::resolve_config_template), so a container
# image gets the same sandboxed terminal.backend: docker decision a native
# install gets. seed_one() (defined below) takes a src path relative to
# $INSTALL_DIR, so resolve to a relative path rather than an absolute one.
#
# `|| resolved=""` is load-bearing under `set -eu`: a plain assignment
# `x=$(cmd)` aborts the WHOLE stage2 hook the instant `cmd` exits non-zero
# (e.g. an ImportError inside hermes_cli) -- silently killing config.yaml,
# SOUL.md, and every seed/chown/migration step below, not just this one.
# The guard turns "resolution failed" into "fall back to the upstream
# example", not "container never finishes booting".
#
# Gated on config.yaml already being absent: seed_one() below is a no-op on
# every restart of an already-provisioned container (config.yaml exists on
# the persistent volume), so without this outer check this hook would still
# pay for spawning python and importing hermes_cli on EVERY container start
# just to compute a value nothing then uses.
config_src="cli-config.yaml.example"
if [ ! -f "$HERMES_HOME/config.yaml" ] && [ -x "$INSTALL_DIR/.venv/bin/python" ]; then
    resolved=$(s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" - "$INSTALL_DIR" <<'PYEOF' 2>/dev/null
import sys
...
if result:
    print(result.relative_to(sys.argv[1]))
PYEOF
) || resolved=""
    if [ -n "$resolved" ]; then
        config_src="$resolved"
...
# Same resolver pattern for .env: prefer Trix's curated
# assets/config/trix.env.example (3 required variables) over upstream's
# 496-line, 127-variable .env.example -- hermes_cli/config_template.py::
# resolve_env_template(), the same function install.sh's
# copy_config_templates() uses on a native install.
#
# `|| resolved=""` is load-bearing under `set -eu` for the same reason as
# the config.yaml block above -- an ImportError here must not abort the
# rest of the hook.
#
# Gated on .env already being absent, same reasoning as config_src above:
# a no-op on every restart of an already-provisioned container.
env_src=".env.example"
if [ ! -f "$HERMES_HOME/.env" ] && [ -x "$INSTALL_DIR/.venv/bin/python" ]; then
    resolved=$(s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" - "$INSTALL_DIR" <<'PYEOF' 2>/dev/null
import sys
from pathlib import Path
...
seed_one ".env" "$env_src"
seed_one "config.yaml" "$config_src"
