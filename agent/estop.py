...
* new gateway turns get a brief, localized "paused" notice (``paused_reply``)
  instead of an agent run (``gateway/run.py:_handle_message``).
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return t(
            "trix.estop.paused_with_reason",
            reason=reason,
            default=(
                f"⏸️ Paused ({reason}). New work is on hold; "
                "it'll resume automatically once the pause is lifted."
            ),
        )
    return t(
        "trix.estop.paused",
        default=(
            "⏸️ Paused. New work is on hold; "
            "it'll resume automatically once the pause is lifted."
        ),
    )
