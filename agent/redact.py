...
#
# NOT applied by the global pass — see the "Web-URL redaction is intentionally
# OFF" note in redact_sensitive_text. Surfaces that are not round-trip
# workflows opt in explicitly: `redact_cdp_endpoint_for_log` and the /debug
# report collector (`hermes_cli/debug._redact_log_text`).
#
# The scheme list covers proxy transports (socks4/socks5/socks5h), not just
# http/ws/ftp: a proxy URL is a connection string in the same sense as a DSN
# — nothing ever navigates to it — and `socks5://user:pass@host` in a log line
# is exactly how the 2026-09-12 proxy password reached a client's debug report.
# The password group requires 1+ chars, so the legal empty-password form
# `http://user:@host` is left alone rather than collapsed to `***`.
_URL_USERINFO_RE = re.compile(
    r"(https?|wss?|ftp|ftps|sftp|socks[45]h?)://([^/\s:@]+):([^/\s@]+)@",
    re.IGNORECASE,
)
