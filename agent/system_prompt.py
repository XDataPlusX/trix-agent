...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Pointer to the trix-agent skill + bundled docs for product questions.
    stable_parts.append(TRIX_AGENT_HELP_GUIDANCE)

    # Промпт клиента не раздаётся наружу. Стоит в СТАБИЛЬНОЙ части
    # намеренно: текст неизменен, поэтому кэш префикса не ломается, и
    # правило действует с первого хода любого разговора — включая
    # групповые чаты, где спросить может кто угодно, а не только владелец.
    stable_parts.append(TRIX_PROMPT_CONFIDENTIALITY_GUIDANCE)
    ...
    if active_profile == "default":
        # Путь до чужих профилей намеренно НЕ называется: он хостовой, из
        # песочницы недостижим, и это единственное место, где имя чужого
        # продукта просачивалось в системный промпт клиента (Спека 18).
        # Смысл блока — запрет трогать соседний профиль — от пути не зависит.
        post_workspace_parts.append(
            "Active profile: default. Other profiles, if any, have their own "
            "skills, plugins, cron jobs, and memories that affect a different "
        ...
        post_workspace_parts.append(
            f"Active profile: {active_profile}. This session reads and writes "
            f"only that profile's own skills, plugins, cron jobs and memories. "
            f"The default profile's data belongs to a different session run "
            ...
            f"you to. The cross-profile write guard will refuse such writes by "
            f"default; pass cross_profile=True only after explicit direction."
        )
    ...
    # Пояс клиента называется прямо. Без этого настройка, которую он задал
    # в мастере, до модели не доезжала вообще: в промпте стоит одна дата
    # (время намеренно опущено ради стабильности кэша), а на вопрос о
    # времени промпт отправляет в терминал -- то есть в docker-песочницу,
    # часы которой идут по UTC. Модель либо переспрашивала пояс, уже
    # заданный клиентом (вопрос владельца 2026-09-08), либо ставила
    # напоминание на час песочницы. Оговорка про UTC здесь же и по той же
    # причине: без неё `date` в песочнице выглядит как опровержение этой
    # строки.
    #
    # Кэш не страдает: строка меняется только когда клиент сменил
    # настройку, а это всё равно новая сессия.
    try:
        from hermes_time import get_timezone as _hermes_tz

        _tz_name = getattr(_hermes_tz(), "key", "")
    except Exception:
        _tz_name = ""
    if _tz_name:
        timestamp_line += (
            f"\nTimezone: {_tz_name} — the client's own setting. Schedule in it"
            " and answer time questions in it; the sandbox clock is UTC."
        )
    ...
    if agent.model:
        timestamp_line += f"\nModel: {agent.model}"
    if agent.provider:
        timestamp_line += f"\nProvider: {_provider_label(agent.provider)}"
    if agent.platform:
        timestamp_line += f"\nPlatform: {agent.platform}"
...
def _provider_label(provider: str) -> str:
    """Витринное имя провайдера, а не внутренний идентификатор профиля.

    `agent.provider` — это ключ каталога (`zai-coding-plan`,
    `alibaba-coding-plan`, `kimi-coding`), которого клиент нигде не вводил:
    в мастере настройки он выбирал строку «Z.AI (Coding Plan)». Подставляя
    ключ в промпт, мы сами клали перед моделью факт, который тут же
    запрещали произносить, — и живой прогон 2026-09-08 показал, что запрет
    держит не всегда: один ответ из трёх на «кто ты» называл клиенту
    `zai-coding-plan`.

    Имя модели остаётся как есть: его клиент выбрал сам и платит за него
    своим ключом (решение владельца 2026-09-08).

    У провайдера без профиля витринного имени нет — тогда остаётся его
    собственное имя: строка нужнее, чем её отсутствие.
    """
    try:
        ...
        return provider
    return (getattr(profile, "display_name", "") or provider) if profile else provider
