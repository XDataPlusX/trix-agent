...
def _emit_client_terminal_error(agent, *, reason, status_code, is_auth, summary):
    """Одна фраза клиенту о том, почему ход не состоялся.

    Апстрим печатал сюда английскую строку с кодом HTTP. Технические
    подробности (провайдер, модель, адрес, тело ошибки) по-прежнему уходят
    в лог через _vprint ниже по коду — здесь только то, что читает человек.
    """
    from hermes_cli.trix_provider_errors import client_error_message

    agent._emit_status(client_error_message(reason, status_code, is_auth=is_auth))
    if summary:
        try:
            agent._vprint(f"{agent.log_prefix}   ↳ {summary}", force=True)
        except Exception:
            pass


def _client_billing_final_response(*, provider, base_url, model) -> str:
    """Вторая — и последняя — фраза клиенту на отказе по деньгам.

    ``_emit_client_terminal_error`` уже сказало, ЧТО произошло. Эта строка
    уходит в ``final_response``, то есть приходит следующим сообщением, и
    обязана сказать другое: ЧТО ДЕЛАТЬ ДАЛЬШЕ. Раньше обе звали одну и ту же
    ``client_error_message(billing)``, и клиент читал одинаковый текст
    дважды подряд — второй раз с приклеенным английским хвостом и советом
    ``/model <model> --provider <provider>``, который в Telegram не
    набирается.

    Технический хвост (``_billing_or_entitlement_message``) никуда не делся:
    он по-прежнему печатается в лог через
    ``_print_billing_or_entitlement_guidance`` выше по коду и по-прежнему
    едет в структурном ``billing_block`` для остальных поверхностей — просто
    не в тексте, который читает человек.
    """
    from hermes_cli.trix_provider_errors import client_billing_next_step_message

    billing_url = None
    try:
        from agent.billing_links import build_billing_block

        billing_url = build_billing_block(
            provider=provider, base_url=str(base_url), model=model
        ).billing_url
    except Exception:
        billing_url = None
    return client_billing_next_step_message(billing_url=billing_url)


def _buffer_client_retrying_fallback_status(agent, *, reason, status_code, is_auth):
    """Buffer a localized 'trying a different provider' status.

    Fires right before ``agent._try_activate_fallback()`` when a non-retryable
    error has a fallback chain to try. It only ever reaches the client if that
    fallback attempt ALSO fails: ``agent._flush_status_buffer()`` replays it
    immediately ahead of the real terminal message from
    ``_emit_client_terminal_error``; on a successful fallback
    ``agent._clear_status_buffer()`` drops it silently.

    Reuses ``client_error_message`` for the headline so this line is never
    English while the terminal message that may follow it a moment later is
    Russian — before this, only ``content_policy_blocked`` and
    ``ssl_cert_verification`` got dedicated (English) text here; every other
    reason (billing, auth, model_not_found, ...) fell through to a bare
    ``"Non-retryable error (HTTP {code})"``.
    """
    from agent.i18n import t
    from hermes_cli.trix_provider_errors import client_error_message

    headline = client_error_message(reason, status_code, is_auth=is_auth)
    tail = t("trix.errors.fallback.trying_now", default="Trying a fallback provider…")
    agent._buffer_status(f"{headline} {tail}")
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if _runtime_context_error:
            ...
            agent._emit_status(
                t(
                    "trix.agent.ollama_context_too_small",
                    default="❌ Ollama runtime context is too small for Hermes tool use",
                )
            )
            api_call_count -= 1
            agent._api_call_count = api_call_count
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        ...
                        _nous_msg = t(
                            "trix.agent.provider_rate_limit_active",
                            reset_in=_fmt_nous_remaining(_nous_remaining),
                            default=(
                        ...
                        return {
                            "final_response": (
                                f"⏳ {_nous_msg}\n\n"
                                + t(
                                    "trix.agent.provider_rate_limit_no_fallback",
                                    default=(
            ...
            try:
                ...
                if finish_reason == "content_filter":
                    ...
                    if agent._has_pending_fallback():
                        ...
                        agent._buffer_status(
                            t(
                                "trix.agent.safety_refusal_fallback",
                                default=(
                    ...
                    agent._emit_status(
                        t(
                            "trix.agent.safety_refusal_final",
                            default=(
                ...
                if _should_fallback and agent._fallback_index < len(agent._fallback_chain):
                    ...
                    if not pool_may_recover:
                        ...
                        agent._buffer_status(
                            client_fallback_attempt_message(
                                classified.reason, upstream=_upstream_name
                            )
                        )
                ...
                if (
                    ...
                    agent._buffer_status(
                        client_fallback_attempt_message(classified.reason)
                    )
                    if agent._try_activate_fallback(reason=classified.reason):
                        active_system_prompt = _sync_failover_system_message(
                            agent, api_messages, active_system_prompt)
                ...
                if is_payload_too_large:
                    ...
                    agent._buffer_status(
                        t(
                            "trix.agent.payload_too_large_retry",
                            attempt=compression_attempts,
                            max_attempts=max_compression_attempts,
                            default=(
                                f"⚠️  Request payload too large (413) — compression "
                ...
                if is_client_error:
                    ...
                    if agent._has_pending_fallback():
                        _buffer_client_retrying_fallback_status(
                            agent,
                            reason=classified.reason,
                            status_code=status_code,
                            is_auth=bool(getattr(classified, "is_auth", False)),
                        )
                    if agent._try_activate_fallback():
                        active_system_prompt = _sync_failover_system_message(
                            agent, api_messages, active_system_prompt)
                    ...
                    if classified.reason == FailoverReason.billing:
                        # final_response is the second message the client
                        # sees (finalize_turn appends it as a fresh assistant
                        # row) -- it does NOT pass through the gateway's
                        # provider-error rewriter, so leaving it as the raw
                        # English "Billing or credits exhausted: ..." meant a
                        # Russian client got the localized status via
                        # _emit_client_terminal_error above, immediately
                        # followed by an English second reply.
                        #
                        # Routing it through client_error_message(billing) --
                        # the SAME call _emit_client_terminal_error just made
                        # -- fixed the language and introduced a byte-identical
                        # duplicate instead. The two messages are two different
                        # moments and must say two different things: the first
                        # what happened, this one what to do next. The English
                        # guidance still travels in "billing_block" below and in
                        # the logs via _vprint higher up; only the client-facing
                        # text is ours.
                        ...
                        _ce_final = _client_billing_final_response(
                ...
                if retry_count >= max_retries:
                    ...
                    _billing_guidance = ""
                    if classified.reason == FailoverReason.billing:
                        # Same helper the non-retryable-abort branch above
                        # uses (_emit_client_terminal_error) so a Trix client
                        # never reads a raw English "Billing or credits
                        # exhausted" line just because this turn happened to
                        # exhaust retries instead of aborting immediately.
                        from hermes_cli.trix_provider_errors import (
                    ...
                    _billing_block = None
                    if classified.reason == FailoverReason.billing:
                        # Same helper the non-retryable-abort branch above uses
                        # (_client_billing_final_response) so this second,
                        # "what to do next" message is never a raw English
                        # duplicate of the status line just emitted above —
                        # and, per that helper's own history, does NOT get the
                        # English _billing_guidance tail appended: that advice
                        # still travels in the log (above) and in the
                        # structured billing_block below, just not in the text
                        # a human reads.
                        _final_response = _client_billing_final_response(
                            provider=_provider, base_url=_base, model=_model,
                        )
            ...
            if assistant_message.tool_calls:
                ...
                if not agent._has_content_after_think_block(final_response):
                    ...
                    if fallback and getattr(agent, '_last_content_tools_all_housekeeping', False):
                        ...
                        agent._emit_status(
                            t(
                                "trix.agent.empty_after_tools_use_earlier",
                                default=(
                                    "↻ Empty response after tool calls — using "
                                    "earlier content as final answer"
                                ),
                            )
                        )
                        agent._last_content_with_tools = None
                    ...
                    if (
                        ...
                        agent._buffer_status(
                            t(
                                "trix.agent.empty_after_tools_nudge",
                                default=(
                    ...
                    if _truly_empty and (not _has_structured or _prefill_exhausted) and agent._empty_content_retries < 3:
                        ...
                        agent._buffer_status(
                            t(
                                "trix.agent.empty_response_retrying",
                                attempt=agent._empty_content_retries,
                                seconds=f"{wait_time:.0f}",
                                default=(
                    ...
                    if _truly_empty and agent._fallback_chain:
                        ...
                        _pre_fallback_model = agent.model
                        _pre_fallback_provider = agent.provider
                        agent._buffer_status(
                            client_fallback_attempt_message("invalid_response")
                        )
                        if agent._try_activate_fallback():
                            active_system_prompt = _sync_failover_system_message(
                                agent, api_messages, active_system_prompt)
                            agent._empty_content_retries = 0
                            # Its OWN message, not the generic switch line
                            # try_activate_fallback() already buffered a
                            # moment ago (round-5 review). Routing both
                            # through one function made them byte-identical,
                            # so a client whose fallback also failed read the
                            # same sentence twice -- _flush_status_buffer
                            # replays the buffer verbatim, nothing dedupes.
                            # Upstream kept these distinct on purpose: a
                            # generic "switching to fallback" inside the
                            # helper, and this specific post-swap
                            # confirmation for the empty-response path.
                            # Still buffered, so it must not claim the turn
                            # is being answered (round-3 review fix) -- only
                            # that the model swap itself went through.
                            agent._buffer_status(
                                client_fallback_empty_response_switch_message(
                                    _pre_fallback_model,
                                    _pre_fallback_provider,
                                    agent.model,
                                    agent.provider,
                                )
                            )
                    ...
                    if reasoning_text:
                        ...
                        if agent._fallback_chain:
                            agent._emit_status(
                                t(
                                    "trix.agent.no_content_final_with_fallback",
                                    default=(
                                        "❌ Model returned no content after all "
                            ...
                            agent._emit_status(
                                t(
                                    "trix.agent.no_content_final_no_fallback",
                                    default=(
                                        "❌ Model returned no content after all "
                                        "retries. No fallback providers configured."
                                    ),
                                )
                            )
