...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Buffered = only reaches the client if THIS fallback attempt also
        # ends up failing the whole turn (see _flush_status_buffer). Must
        # say "trying", not "answering through" -- claiming a completed
        # switch here would contradict the terminal-failure message that
        # follows it on the same failed turn (round-3 review fix).
        agent._buffer_status(
            client_fallback_switch_attempt_message(
        ...
        agent._pending_fallback_notice = client_fallback_message(
            old_model, old_provider, fb_model, fb_provider
        )
        logger.info(
