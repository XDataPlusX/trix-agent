...
Scope: user-facing static strings Hermes renders itself -- approval
prompts, gateway slash command replies, restart-drain notices.  A caller
that owns its own canonical English (the slash command registry, for
one) can pass it via ``default=`` so a catalog miss degrades to real
copy instead of a key path.  Agent-generated output, log lines, error
tracebacks and tool outputs stay in English.
...
``gateway.approval_expired``).  Missing keys fall back to English; if
English is missing too, the caller-supplied ``default`` is used when given,
else the key path itself is returned so a broken catalog never crashes the
agent.
...
def resolve_language(lang: str | None = None) -> str:
    """Resolve the effective language code the same way :func:`t` does.

    Public wrapper around the ``explicit lang > env/config > default``
    resolution ``t()`` performs internally.  Callers that build
    language-dependent text OUTSIDE the catalog (e.g. deciding whether to
    splice a Russian-only phrase into a ``t()``-rendered string) need this
    to stay in lockstep with what ``t()`` will actually render -- reaching
    for the private ``_normalize_lang`` instead risks the two diverging on
    an alias (``"russian"``, ``"ru-RU"``, ...) and mixing languages inside
    one message.
    """
    return _normalize_lang(lang) if lang else get_language()


def t(key: str, lang: str | None = None, default: str | None = None, **format_kwargs: Any) -> str:
    ...
    default
        Fallback text to use if the key is missing in both the target
        language and English.  A caller that owns its own canonical English
        text (e.g. the slash command registry) passes it here so a catalog
        miss degrades to real copy instead of a raw key path.  ``None``
        (the default) preserves the historical behavior of returning the
        key itself.
    **format_kwargs
        ``str.format`` substitution arguments (``t("gateway.drain", count=3)``
        expects a catalog entry with a ``{count}`` placeholder).  Applied to
        ``default`` the same way as to a catalog value.

    Returns
    -------
    The translated string, the English fallback if the key is missing in the
    target language, ``default`` if the key is missing in English too and a
    default was supplied, or the bare key as a last resort.
    """
    ...
    if value is None:
        # A caller that owns the canonical English text (e.g. the slash
        # command registry) passes it as ``default`` so a catalog miss
        # degrades to real copy instead of a key path.
        if default is not None:
            value = default
        else:
            # Last-ditch: return the key itself.  A broken catalog should
            # not crash anything; it just looks ugly until someone fixes it.
