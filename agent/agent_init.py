...
def _build_codex_gpt5_autoraise_notice(
    ...
    # Через каталог, а не строкой в коде: этот текст доезжает до КЛИЕНТА —
    # он же переигрывается в шлюз через ``status_callback`` (см. ниже по
    # файлу, ``agent._compression_warning``). Клиент читает по-русски.
    #
    # Команды в тексте больше нет намеренно. Она называла `hermes` — то
    # есть имя апстрима в канале, где его быть не должно, — и вдобавок
    # предлагала клиенту консольную команду, которой у него нет: его
    # единственный интерфейс Telegram (решение владельца 2026-09-09,
    # «не давать агенту того, чего он не должен уметь»). Настройка названа
    # по имени: поддержке этого хватает, клиенту не мешает.
    from agent.i18n import t

    return t(
        "trix.compression.codex_autoraise",
        model=model,
        cap=cap,
        to_pct=to_pct,
        from_pct=from_pct,
...
def init_agent(
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Trix Agent.  Choose a model with at least "
