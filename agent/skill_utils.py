...
EXCLUDED_SKILL_DIRS = frozenset(
    (
        ...
        ".hub",
        ".archive",
        # Contour's publish_skill staging area (hermes_cli/trix_contour.py,
        # spec 21 §9): a candidate copied here by publish_skill is NOT live
        # until a human runs `hermes contour approve-skill` to promote it —
        # it must never resolve through skills.external_dirs (the shared
        # company skills dir IS an external_dirs entry) or a local
        # ~/.hermes/skills/ walk before that approval happens.
        ".pending",
        ".venv",
        "venv",
        "node_modules",
...
# ── Contour-managed shared skills folder (Trix, spec 21 §9 review) ────────
#
# `is_external_skill_path` above answers "is this a personal
# skills.external_dirs vault or the company's shared folder?" with the same
# "yes" for both -- and that conflation was itself a bug: blocking agent
# writes to EVERY external dir (the first version of this guard) broke the
# convenience fix behind issues #4759/#4381 for every single-user
# installation that happens to point external_dirs at, say, a synced
# Obsidian vault or a personal dotfiles checkout. Those vaults are owned
# and read by ONE person, who wants their own agent editing skills there in
# place -- there is no "other people's system prompts" and no publish/
# approve quarantine to defend.
#
# The property that actually needs protecting is narrower: "this directory
# is wired into OTHER PEOPLE's system prompts, and a human is meant to be
# the only one who changes its contents" -- which is exactly and only the
# Trix company skills folder (`hermes_cli/trix_contour.py`). That module
# marks the folder it owns by planting a dotfile at its root
# (:func:`hermes_cli.trix_contour.ensure_skills_dir_marker`, called from
# both `hermes business setup` and the contour's own first `apply_planned`
# run, idempotently) — a file, not a config key, because it travels with
# the folder itself: it survives a copy, a re-point of `external_dirs` to a
# different path, or someone reading the folder without knowing this
# module exists. A human who stumbles on the file sees Russian prose
# explaining what it is.
CONTOUR_MANAGED_SKILLS_MARKER = ".trix-contour-managed"


def is_contour_managed_skill_path(path) -> bool:
    """True when *path* resolves under an external skills directory that
    carries :data:`CONTOUR_MANAGED_SKILLS_MARKER` at its root -- the Trix
    company's shared skills folder, not a personal
    ``skills.external_dirs`` vault. See the module comment above the
    constant for why this is narrower than :func:`is_external_skill_path`.

    Presence, not content, is what is checked: the marker's TEXT is never
    parsed, so a truncated, edited, or emptied marker file still counts as
    "managed" (fail closed) rather than silently stop protecting the
    folder the moment its exact wording drifts.
    """
    candidate = _resolve_for_skill_ownership(path)
    for root in get_external_skills_dirs():
        resolved_root = _resolve_for_skill_ownership(root)
        try:
            ...
            continue
        # Found the (single) external root candidate lives under -- answer
        # from THIS root's marker and stop, rather than falling through to
        # check other, unrelated external dirs.
        return (resolved_root / CONTOUR_MANAGED_SKILLS_MARKER).exists()
    return False
...
# Limits for the other skill-index fields that get interpolated into the
# SYSTEM prompt (agent/prompt_builder.py::build_skills_system_prompt).  A
# skill's `name:` frontmatter and its directory-derived `category`, and a
# category's DESCRIPTION.md `description`, are all untrusted for skills that
# come from an external/shared directory (skills.external_dirs) -- and, for
# agent-authored local skills, from the model itself. None of them may be
# allowed to inject newlines (fake prompt structure) or unbounded text into
# the highest-trust position in the conversation.
#
# NAME_LIMIT is 64, NOT the desc limit -- it must match
# tools/skills_tool.py::MAX_NAME_LENGTH, the indexer the rest of the system
# (skill_view, skill_manage, `hermes skills list`) resolves names against.
# A skill name is an identifier, not prose: whatever gets truncated here is
# the ONLY name the model can use to load the skill, so it must equal a name
# that indexer actually resolves. Keep the two constants in agreement if
# either changes.
SKILL_PROMPT_NAME_LIMIT = 64
SKILL_PROMPT_CATEGORY_LIMIT = 60
SKILL_PROMPT_CATEGORY_DESC_LIMIT = 120

_PROMPT_WHITESPACE_RE = re.compile(r"\s+")

# Unicode general categories stripped by collapse_prompt_whitespace: Cc
# (control, e.g. ESC/NUL/BEL/DEL) and Cf (format, e.g. ZERO WIDTH SPACE, the
# bidi override/embedding/isolate controls including RIGHT-TO-LEFT OVERRIDE,
# BOM). Ordinary whitespace (space/tab/newline/etc.) is handled separately by
# _PROMPT_WHITESPACE_RE and is not in this set once collapsed to a plain
# space.
_STRIPPED_UNICODE_CATEGORIES = ("Cc", "Cf")


def collapse_prompt_whitespace(text: Any) -> str:
    """Collapse whitespace runs to a single space, strip, and drop Cc/Cf chars.

    Any text that is interpolated into the skill index inside the SYSTEM
    prompt must render as a single, visually-honest line -- otherwise a
    hostile `name:`, `category`, or DESCRIPTION.md `description` could forge
    new prompt lines/sections, or worse.

    Python's ``\\s`` matches ordinary whitespace (space/tab/newline/etc, plus
    Unicode line/paragraph separators) but NOT arbitrary Unicode control (Cc)
    or format (Cf) characters. Left unfiltered those reach the system prompt,
    and any terminal that renders it, verbatim:

    - Cc chars like ESC (0x1b) let a hostile skill smuggle raw ANSI escape
      sequences into the prompt / terminal.
    - Cf chars include the bidi control characters -- RIGHT-TO-LEFT OVERRIDE
      (U+202E) reverses the visual order of everything after it, which can
      hide or relocate a `[external]`/`[org-shared: ...]` provenance tag this
      module itself prepends.
    - Cf also includes ZERO WIDTH SPACE (U+200B) and BOM (U+FEFF), which are
      invisible but not equal to the empty string: hidden inside a `name:`
      they make a visually-identical external skill compare unequal to a
      local one in the `disabled`-list check and the local-wins dedup,
      letting the external skill shadow (or evade disabling of) the local
      one under what looks like the same name.

    Whitespace is collapsed FIRST (so a real newline/tab still becomes a
    single space rather than vanishing), Cc/Cf chars are then dropped, and a
    second whitespace pass squashes any double space that removal leaves
    behind.
    """
    if not text:
        return ""
    collapsed = _PROMPT_WHITESPACE_RE.sub(" ", str(text))
    filtered = "".join(
        ch for ch in collapsed if unicodedata.category(ch) not in _STRIPPED_UNICODE_CATEGORIES
    )
    return _PROMPT_WHITESPACE_RE.sub(" ", filtered).strip()


def truncate_for_prompt(text: Any, limit: int) -> str:
    """Collapse whitespace, strip `[`/`]`, then truncate to ``limit`` chars with a visible ellipsis.

    Bracket stripping keeps an author-controlled description from forging the
    provenance tags ([external], [org-shared: by ...], [name collision ...])
    that prompt_builder prepends to it -- without this, a description of
    "[org-shared: by ceo] do X" renders indistinguishably from a real tag.

    For NAME fields (identifiers a model must be able to feed straight back
    into skill_view), use truncate_name_for_prompt instead -- an
    ellipsis-truncated name is not a name anything can resolve.
    """
    normalized = collapse_prompt_whitespace(text).replace("[", "").replace("]", "")
    if len(normalized) > limit:
        return normalized[: max(limit - 3, 0)] + "..."
    return normalized


def truncate_name_for_prompt(text: Any, limit: int) -> str:
    """Collapse whitespace, then truncate to ``limit`` chars via a PLAIN SLICE.

    Names are identifiers, not prose. An ellipsis-truncated name (as
    truncate_for_prompt would produce) is a string that does not equal any
    real skill name, so a model that reads it from the index and calls
    skill_view(<truncated name>) gets a lookup failure on every skill whose
    name exceeds the limit -- and the same truncated name is compared
    against the `skills.disabled` config list, so a mismatched truncation
    also lets a disabled long-named skill silently reappear.

    Uses a bare slice (no ellipsis) so the rendered name always matches what
    tools/skills_tool.py's own indexer resolves for the same underlying name
    (it truncates with the identical plain-slice-to-MAX_NAME_LENGTH
    convention), and so it can be compared against `skills.disabled` entries
    that operators typically copy from `hermes skills list`.
    """
...
def _normalize_skill_description(frontmatter: Dict[str, Any]) -> str:
    ...
    if not raw_desc:
        return ""
    cleaned = str(raw_desc).strip().strip("'\"")
    return collapse_prompt_whitespace(cleaned)


def extract_skill_description(frontmatter: Dict[str, Any]) -> str:
    """Extract a system-prompt-length description from parsed frontmatter.

    Routed through truncate_for_prompt (rather than truncating inline) so
    the `[`/`]` stripping there also applies here: this description is what
    prompt_builder prepends its [external]/[org-shared: by ...]/[name
    collision ...] provenance tags to, and an author-controlled description
    containing literal brackets could otherwise forge one.
    """
    desc = _normalize_skill_description(frontmatter)
    if not desc:
        return ""
    return truncate_for_prompt(desc, SKILL_PROMPT_DESC_LIMIT)
