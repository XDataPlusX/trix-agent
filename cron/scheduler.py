...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    if wrap_response:
        ...
        job_id = job.get("id", "")
        delivery_content = format_cron_wrap(task_name, job_id, content)
    else:
    ...
    # A MEDIA: path the host cannot read is an undelivered attachment, the same
    # as one the live adapter timed out on below — record it instead of letting
    # the filter shrink `media_files` in silence. The job's run record is where
    # a cron delivery reports itself; there is no chat turn to answer into.
    if unreachable_media:
        msg = (
            f"{len(unreachable_media)} media attachment(s) not delivered: the path "
            "is not readable from the host (a container-local path outside "
            "/workspace, /root and the mounted cache dirs has no host counterpart)"
        )
        logger.warning("Job '%s': %s", job["id"], msg)
