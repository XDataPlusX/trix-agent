...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        public paste service and return the URL. It now only ever writes
        the report to a local file under ``HERMES_HOME`` — inlined in the
        chat reply when it's short enough to fit, otherwise the customer is
        told plainly that it's too long to inline and where the file lives,
        rather than silently sending a truncated report that looks whole.
        """
        import asyncio
        ...
        def _collect_locally():
            ...
            path = _save_report_locally(report)
        ...
        return await loop.run_in_executor(None, _collect_locally)
