...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        #
        # Only when curation is OFF.  ``hermes_cli.tips.TIPS`` is the
        # upstream corpus: 380 English one-liners written for someone
        # sitting at the terminal.  Against the curated client surface
        # (``hermes_cli.trix_menu``) 193 of them are provably wrong here --
        # they advertise commands we disabled, commands that are cli_only
        # and answer "unknown command" when typed (``/clear``, ``/paste``,
        # ``/copy``), env vars and config.yaml edits the client cannot
        # reach, and keybindings/CLI flags that do not exist in a chat.
        # The remainder are still English under a Russian frame and still
        # name the upstream brand.  Curating the menu, /help and /commands
        # while leaving this third exit open just moved the dead end.
        #
        # ``curated: false`` (our debugging escape hatch on a client
        # machine) restores the upstream behaviour along with the rest of
        # the built-in surface.
        _tip_line = ""
        try:
            from hermes_cli.config import read_raw_config
            from hermes_cli.trix_menu import menu_curation_enabled
            if not menu_curation_enabled(read_raw_config() or {}):
            ...
            for d in delegations[:12]:
                ...
                if status == "stalling":
                    ...
                    if quiet is not None:
                        row += " " + t("trix.cmd.agents.no_progress", seconds=f"{quiet:.0f}")
                elif d.get("seconds_since_progress", 0) >= 60:
                    row += " " + t(
                        "trix.cmd.agents.quiet",
                        seconds=f"{d['seconds_since_progress']:.0f}",
                    )
                if goal:
                    row += f" · {goal}"
                lines.append(row)
                for i, child in enumerate(d.get("children_activity") or []):
                    ...
                    doing = f"`{tool}`" if tool else t("trix.cmd.agents.between_turns")
                    part = t("trix.cmd.agents.child_label", index=i + 1) + t(
                        "trix.cmd.agents.api_calls_line",
                        calls=child.get("api_calls", "?"),
                        doing=doing,
                    )
                    idle = child.get("seconds_since_activity")
                    if idle is not None:
                        part += " " + t("trix.cmd.agents.active_ago", seconds=f"{idle:.0f}")
                    lines.append(part)
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "list":
            lines = [t("trix.cmd.platform.header")]
            connected = sorted(p.value for p in self.adapters.keys())
            if connected:
                lines.append(t("trix.cmd.platform.connected", list=", ".join(connected)))
            else:
                lines.append(t("trix.cmd.platform.connected_none"))
            failed = getattr(self, "_failed_platforms", {}) or {}
            if failed:
                for p, info in failed.items():
                    if info.get("paused"):
                        reason = info.get("pause_reason") or t("trix.cmd.platform.paused_reason_default")
                        lines.append(
                            t("trix.cmd.platform.paused_row", platform=p.value, reason=reason)
                        )
                    else:
                        attempts = info.get("attempts", 0)
                        lines.append(
                            t("trix.cmd.platform.retrying_row", platform=p.value, attempts=attempts)
                        )
            else:
                lines.append(t("trix.cmd.platform.failed_none"))
            return "\n".join(lines)

        if action in {"pause", "resume"}:
            if not target:
                return t("trix.cmd.platform.usage_action", action=action)
            platform = _resolve_platform(target)
            if platform is None:
                return t("trix.cmd.platform.unknown", target=target)
            failed = getattr(self, "_failed_platforms", {}) or {}
            if action == "pause":
                if platform not in failed:
                    return t("trix.cmd.platform.not_in_queue_pause", platform=platform.value)
                if failed[platform].get("paused"):
                    return t("trix.cmd.platform.already_paused", platform=platform.value)
                self._pause_failed_platform(platform, reason="paused via /platform pause")
                return t("trix.cmd.platform.paused_confirm", platform=platform.value)
            # action == "resume"
            if platform not in failed:
                return t("trix.cmd.platform.not_in_queue_resume", platform=platform.value)
            if not failed[platform].get("paused"):
                return t("trix.cmd.platform.already_retrying", platform=platform.value)
            self._resume_paused_platform(platform)
            return t("trix.cmd.platform.resumed", platform=platform.value)

        return t("trix.cmd.platform.usage")
    ...
    async def _handle_setup_wizard_command(self, event: MessageEvent) -> str:
        """Handle /setup — make sure the setup wizard's hosting service is
        running and point the client at its address.

        Permanent-access design (§6, ``docs/product/specs/2026-08-25-trix-
        agent-wizard-permanent-access-design.md``): the wizard now stays up
        permanently and its credentials are the same ones the client got by
        email when the machine was created, so `/setup` no longer mints or
        reveals a password. Its only remaining job is the thing that stays
        useful even with a permanently-open panel — starting the service
        back up if the client (or something else) turned it off, or the
        unit crashed — and telling the client where to point their browser.

        Deliberately does NOT call ``hermes_cli.setup_wizard.cli.open_wizard()``
        (now split — spec §13): the old combined function also issued a
        fresh one-time password as a side effect, which would mutate a
        credential slot from what is now a read-only status command —
        exactly what §6 retires. Instead this calls the public
        ``start_wizard_service()`` half only, which never touches
        ``WizardState`` at all — the other half
        (``issue_temporary_password()``) is reserved for the emergency
        ``hermes setup-wizard open`` path (§9.3).

        Runs off the event loop via ``asyncio.to_thread`` since it does
        blocking I/O (systemctl/subprocess, IP detection, a local TCP
        probe), same as ``/debug`` and ``/update`` above.

        The reply is a plain string, not ``EphemeralReply`` — the previous
        version self-deleted after an hour because the message carried a
        one-time password. There is no secret in this reply anymore, and a
        message that vanishes is actively harmful here: a client coming
        back to `/setup` a month later would find nothing.
        """
        # Lazy import — see the module docstring on `hermes_cli/setup_wizard
        # /cli.py`'s "Import discipline" note; this keeps the gateway's own
        # module-load path from paying for setup_wizard's transitive deps
        # (gateway_ctl -> gateway.status) just to build the slash-command
        # dispatch table.
        from hermes_cli.setup_wizard.cli import start_wizard_service

        try:
            url, reachable = await asyncio.to_thread(start_wizard_service)
        except Exception as exc:
            logger.warning("setup-wizard service (re)start via /setup failed: %s", exc)
            # A systemctl/subprocess exception is Python/English internals —
            # file paths, unit names, traceback-ish blurbs — and never
            # belongs in a chat message to a non-technical client (Task 8,
            # client-command-surface plan). The client gets a short,
            # localized, actionable reply instead; the untruncated
            # exception text already reached the gateway log via the
            # logger.warning() call above, which is where support looks.
            return t("trix.setup_wizard.failed")

        logger.info("setup-wizard service (re)started via /setup")
        reply_text = t("trix.setup_wizard.reply", url=url)
        if not reachable:
            # The local TCP probe came back empty — the service may still
            # be starting up. The client is on the other end of a chat
            # message, so the same honest disclosure has to ride along in
            # the reply itself instead of a terminal only the operator
            # would see.
            reply_text = reply_text + "\n" + t("trix.setup_wizard.port_unreachable")
        return reply_text

    async def _handle_disk_command(self, event: MessageEvent) -> str:
        """Handle /disk — место на диске машины клиента; `clean` убирает служебное.

        Считает и убирает этот процесс, а не агент: HERMES_HOME внутрь
        песочницы не смонтирован (см. hermes_cli/trix_disk.py).

        В отдельном потоке: обход всех данных агента и снос каталогов —
        синхронная работа на секунды, а цикл событий здесь один на все
        разговоры и все платформы. Сделай это прямо в нём — и на время
        замера замолкает весь шлюз, а не только тот, кто спросил.
        """
        from hermes_cli.slash_exec import CommandContext, execute_command

        ctx = CommandContext(surface="gateway", args=event.get_command_args())
        reply = await asyncio.to_thread(execute_command, "disk", ctx)
        return reply.text
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if _cost_warning is not None:
            async def _on_cost_confirm(choice: str) -> str:
                if choice == "cancel":
                    return t(
                        "trix.cmd.model.switch_cancelled",
            ...
            return await self._request_slash_confirm(
                event=event,
                command="model",
                title=t("trix.cmd.model.cost_warning_title"),
                message=t(
                    "trix.cmd.model.cost_warning_body",
                    cost_warning=_cost_warning.message,
                    prefix=_p,
                ),
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            return t("trix.cmd.codex_runtime.config_load_failed", exc=exc)
        cfg = load_config()
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower == "wait" or lower.startswith("wait "):
            ...
            if not wait_arg:
                return t("trix.cmd.goal.wait_usage")
            wtokens = wait_arg.split(None, 1)
            try:
                pid = int(wtokens[0])
            except ValueError:
                return t("trix.cmd.goal.wait_invalid_pid")
            ...
            return t("trix.cmd.goal.wait_parked", pid=pid, reason_part=rtxt)

        # /goal unwait — clear the wait barrier.
        if lower == "unwait":
            if mgr.stop_waiting():
                return t("trix.cmd.goal.unwait_cleared")
            return t("trix.cmd.goal.unwait_none")
        ...
        if lower == "gate" or lower.startswith("gate "):
            ...
            if gate_lower.startswith("remove ") or gate_lower.startswith("rm "):
                ...
                return t("trix.cmd.goal.gate_removed", removed=removed)
            if gate_lower == "clear":
                try:
                    prev = mgr.clear_gates()
                except RuntimeError as exc:
                    return f"/goal gate clear: {exc}"
                key = "trix.cmd.goal.gate_cleared_singular" if prev == 1 else "trix.cmd.goal.gate_cleared_plural"
                return t(key, count=prev)
            return t("trix.cmd.goal.gate_usage")
        ...
        if lower.startswith("draft"):
            ...
            if not objective:
                return t("trix.cmd.goal.draft_usage")
            try:
                import asyncio
        ...
        if state.has_contract():
            label = t("trix.cmd.goal.completion_contract_label")
            return f"{base}\n{label}\n{state.contract.render_block()}"
        if lower.startswith("draft"):
            # Drafting was requested but the aux model couldn't produce one.
            return f"{base}\n" + t("trix.cmd.goal.draft_failed_note")
        return base
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        if mgr is None:
            return t("trix.cmd.heartbeat.unavailable")
        ...
        if lower == "pause":
            state = mgr.pause()
            return (
                t("trix.cmd.heartbeat.paused", prompt=state.prompt)
                if state
                else t("trix.cmd.heartbeat.none_set")
            )

        if lower == "resume":
            state = mgr.resume()
            if state is None:
                return t("trix.cmd.heartbeat.resume_none")
            if quick_key and event.source is not None:
                self._register_heartbeat_watch(quick_key, event.source, mgr.session_id)
            return t(
                "trix.cmd.heartbeat.resumed",
        ...
        if lower in {"clear", "stop", "off"}:
            ...
            return t("trix.cmd.heartbeat.cleared") if had else t("trix.cmd.heartbeat.none_set")
        ...
        if interval is None:
            return t("trix.cmd.heartbeat.usage_no_interval")
        if interval < 0:
            return t("trix.cmd.heartbeat.interval_too_small", min=MIN_INTERVAL_SECONDS)
        if not prompt.strip():
            return t("trix.cmd.heartbeat.prompt_required")

        try:
            state = mgr.set(prompt, interval)
        except ValueError as exc:
            return t("trix.cmd.heartbeat.invalid", exc=exc)
        if quick_key and event.source is not None:
            self._register_heartbeat_watch(quick_key, event.source, mgr.session_id)
        return t(
            "trix.cmd.heartbeat.set",
            interval=format_interval(state.interval_seconds),
            prompt=state.prompt,
        )
    ...
    async def _handle_refine_command(self, event: "MessageEvent") -> str:
        ...
        quick_key = self._session_key_for_source(event.source) if event.source else None
        if not quick_key:
            return t("trix.cmd.refine.unavailable")
        if quick_key in self._running_agents:
            return t("trix.cmd.refine.agent_running")
        ...
        if agent is None:
            return t("trix.cmd.refine.nothing_yet_send")

        snapshot = list(getattr(agent, "_session_messages", None) or [])
        if not snapshot:
            return t("trix.cmd.refine.nothing_yet_empty")
        ...
        try:
            ...
            return t("trix.cmd.refine.failed_to_start", exc=exc)
        tail = t("trix.cmd.refine.focus_tail", focus=args) if args else ""
        return t("trix.cmd.refine.reviewing", tail=tail)
    ...
    async def _handle_subgoal_command(self, event: "MessageEvent") -> str:
        ...
        if mgr is None:
            return t("gateway.goal.unavailable")
        if not mgr.has_goal():
            return t("trix.cmd.subgoal.no_active_goal")
        ...
        if verb == "remove":
            if not rest:
                return t("trix.cmd.subgoal.remove_usage")
            try:
                idx = int(rest.split()[0])
            except ValueError:
                return t("trix.cmd.subgoal.remove_invalid")
            try:
                removed = mgr.remove_subgoal(idx)
            except (IndexError, RuntimeError) as exc:
                return f"/subgoal remove: {exc}"
            return t("trix.cmd.subgoal.removed", index=idx, text=removed)
        ...
        if verb == "clear":
            ...
            if prev:
                key = "trix.cmd.subgoal.cleared_singular" if prev == 1 else "trix.cmd.subgoal.cleared_plural"
                return t(key, count=prev)
            return t("trix.cmd.subgoal.none_to_clear")
        ...
        return t("trix.cmd.subgoal.added", index=idx, text=text)
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        if untracked:
            ...
            more = (
                t("trix.cmd.diff.more_untracked", count=len(untracked) - 15)
                if len(untracked) > 15
                else ""
            )
            label = t("trix.cmd.diff.untracked_label")
            out.append(f"**{label}**\n```\n{shown}{more}\n```")
    ...
    def _fenced_truncated_diff(diff: str, max_lines: int = 60,
        ...
        note = ""
        if truncated:
            note = t("trix.cmd.diff.truncated_note", count=len(diff_lines))
        return f"```diff\n{diff}{note}\n```"
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        if out is None:
            out = t("trix.cmd.memory.unknown_subcommand")
        return out
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        if not gate_on and not wants_toggle and wa.pending_count(wa.SKILLS) == 0:
            return t("trix.cmd.skills.approval_off")
        ...
        if out is None:
            return t("trix.cmd.skills.unknown_subcommand")
        ...
        # version — so we point at whoever can open the pending JSON file
        # server-side, not that command; the Telegram client has no file
        # access to point at directly.)
        if args and args[0].lower() == "diff" and len(out) > 3000:
            out = out[:3000] + t("trix.cmd.skills.diff_truncated")
        return out
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        if requested and not policy.is_admin(event.source.user_id):
            return t("trix.cmd.approvals.admin_only")
            ...
            if _summary_aborted:
                lines.append(
                    t(
                        ...
                        error=(_summary_err or t("trix.cmd.shared.unknown_error")),
                    )
                )
                ...
                lines.append(
                    t(
                        ...
                        error=(_aux_fail_err or t("trix.cmd.shared.unknown_error")),
                    )
                )
            return "\n".join(lines)
        except Exception as e:
    ...
    async def _numbered_session_rows(
        self, source: SessionSource, session_key: str, allow_all: bool
    ) -> tuple[list[dict], bool]:
        """The session rows `/sessions`' default listing renders.

        `/resume <n>`'s numeric resolver calls this too (Task 9f numbering
        fix — see docs/product/plans/2026-09-01-client-command-surface.md):
        before this helper, `/sessions` queried through
        `query_session_listing` (all sessions, named or not) while
        `/resume`'s own numeric index queried a SEPARATE, titled-only list
        (`_list_titled_sessions`), so a row number the client just read off
        `/sessions` could resolve to a different session on `/resume <n>` —
        confirmed by `tests/gateway/test_sessions_listing.py::
        TestResumeNumberingAgainstTheNewSessionsList`. Sharing this one
        query makes the two surfaces agree on both membership and order.

        Deliberately NOT used by `/resume`'s own bare (name-less) listing —
        that surface stays titled-only by design; see
        `tests/hermes_cli/test_trix_session_notices.py::
        test_every_text_that_names_resume_offers_a_second_road`, which pins
        client notice copy on the promise that an untitled conversation
        never appears in THAT list. Only the two numeric paths that must
        agree with each other — `/sessions`' rendered listing and
        `/resume <n>` / `/sessions <n>`'s numeric resolution (the latter
        delegates to the former verbatim) — share this query.

        Returns ``(visible_rows, cross_origin)``. The `_resume_row_visible`
        scoping check is applied here, in the query, before any row is
        handed back for indexing — never skipped, never deferred.
        """
        ...
        visible = [
            row for row in rows
            if await self._resume_row_visible(source, row, allow_all)
        ]
        return visible, cross_origin
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        # Resolve a numbered choice or a title to a session ID. Numeric
        # resolution shares its query with `/sessions`' default listing
        # (`_numbered_session_rows`) rather than the titled-only
        # `_list_titled_sessions()` the bare (name-less) branch above uses
        # -- so a row number typed here, or delegated verbatim from
        # `/sessions <n>`, always means the same session `/sessions` just
        # showed at that position (Task 9f numbering fix).
        if name.isdigit():
            try:
                rows, _cross_origin = await self._numbered_session_rows(
                    source, session_key, allow_all
                )
            except Exception as e:
            ...
            from hermes_cli.session_listing import _format_unnamed_session_label
            name = target.get("title") or _format_unnamed_session_label(target)
        else:
        ...
        if msg_count:
            key = "trix.cmd.resume.message_count_one" if msg_count == 1 else "trix.cmd.resume.message_count_many"
            msg_part = t(key, count=msg_count)
        else:
            msg_part = ""
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        # Task 9f: the client-facing default flips to "show everything".
        # `parse_session_listing_args()` (shared with the CLI) still parses
        # the `full`/`--full` flag exactly as before -- untouched -- but on
        # this surface unnamed sessions are always included regardless of
        # whether the client typed it. Without this, a client who never used
        # `/title` saw an empty `/sessions` on top of real history, with no
        # documented way to discover `/sessions full`.
        include_unnamed = True

        if search_query == "":
            return t("trix.cmd.sessions.usage_search")
        ...
        if search_query:
            ...
            # admin, mirroring the `/resume --all` override. `all` is just a
            # parsed user argument, so without this gate any caller could run
            # `/sessions all` and enumerate other origins' session ids /
            # titles / previews / sources — the enumeration half of the
            # /resume IDOR.
            ...
            rows = await asyncio.to_thread(
                ...
                # Search filters at SQL level, so over-fetch before the
                # visibility cut: origin-invisible matches would otherwise
                # consume the page.
                limit=50,
                exclude_sources=["tool"],
            )
            if not cross_origin:
                # Scope the listing to the caller's own origin on every
                # adapter so session ids/previews from other users/rooms
                # aren't enumerable.
                rows = [
                    row for row in rows
            ...
            rows = rows[:10]
            title = t("trix.cmd.sessions.title_matching", query=search_query)
        else:
            # Task 9f numbering fix: the default listing shares its query
            # with `/resume <n>`'s numeric resolver (`_numbered_session_rows`)
            # so a row number here means the same session on `/resume <n>` /
            # `/sessions <n>` (which delegates to `/resume` verbatim). See
            # docs/product/plans/2026-09-01-client-command-surface.md.
            rows, cross_origin = await self._numbered_session_rows(
                source, session_key, include_all
            )
            rows = rows[:10]
            title = (
                t("trix.cmd.sessions.title_all")
                if include_unnamed
                else t("trix.cmd.sessions.title_named")
            )
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        ...
        lines: list[str] = [t("trix.cmd.topup.balance_header")]
        ...
        if view.topup_url:
            lines.append("")
            lines.append(t("trix.cmd.topup.manage_billing", url=view.topup_url))
            lines.append(t("trix.cmd.topup.manage_hint"))
        return "\n".join(lines)
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return t("trix.cmd.bundles.none", dir=reply.data["dir"])

        lines = [t("trix.cmd.bundles.header", count=len(bundles)), ""]
        for info in bundles:
            skill_count = len(info.get("skills", []))
            desc = info.get("description") or t("trix.cmd.bundles.default_desc", count=skill_count)
            lines.append(
                t("trix.cmd.bundles.entry", slug=info['slug'], desc=desc, count=skill_count)
            )
            for s in info.get("skills", []):
                lines.append(f"    · {s}")
        lines.append("")
        lines.append(t("trix.cmd.bundles.invoke_hint"))
        return "\n".join(lines)
    ...
    # Telegram's hard message-length cap (4096) is the tightest limit among
    # supported platforms; stay well under it so the surrounding text and
    # code-fence markers never push a borderline report over the edge.
    _DEBUG_INLINE_REPORT_LIMIT = 3500

    async def _handle_debug_command(self, event: MessageEvent) -> Optional[str]:
        """Handle /debug — collect a debug report locally; never upload it.

        Trix Agent never ships client logs off the customer's own VM (see
        docs/product/specs/2026-08-13-trix-agent-persona-design.md). This
        used to upload the summary report (system info + log tails) to a
        public paste service and return the URL.

        The report is now **attached to the reply as a file** and also kept
        on disk under ``HERMES_HOME``. Attaching it is the whole point of
        the command: a real report (dump + 200 lines of agent.log + four
        more log tails) runs tens of thousands of characters, so the inline
        branch below effectively never fired on a machine that had been
        used, and the client was left with "the report is on the server,
        ask whoever administers this machine" — who IS the client, with no
        shell. The file is something they can forward to support.

        Text stays the fallback, unchanged, for a platform whose adapter
        cannot send files (or when the upload fails): inline when it fits,
        otherwise the path. Attaching is attempted only when the adapter
        actually overrides ``send_document`` — the base implementation
        "succeeds" by sending an English can't-deliver notice, which would
        otherwise look like delivery here and suppress the fallback.
        """
        import asyncio
        from hermes_cli.debug import (
            _capture_dump, collect_debug_report,
            _save_report_locally, _best_effort_sweep_expired_pastes,
        )

        loop = asyncio.get_running_loop()

        # Run blocking I/O (dump capture, log reads, local file write) in a
        # thread. No log content is ever sent anywhere; the only network
        # call possible here is the legacy-paste sweep below, which just
        # issues DELETEs for pastes an older build may have uploaded — it
        # never creates or uploads a new one.
        def _collect_locally():
            _best_effort_sweep_expired_pastes()
            dump_text = _capture_dump()
            report = collect_debug_report(log_lines=200, dump_text=dump_text)
            return report, _save_report_locally(report)

        report, path = await loop.run_in_executor(None, _collect_locally)

        if await self._attach_debug_report(event, path):
            return None

        if len(report) <= self._DEBUG_INLINE_REPORT_LIMIT:
            return t(
                "trix.cmd.debug.inline",
                platform=event.source.platform.value,
                report=report,
                path=path,
            )

        return t("trix.cmd.debug.too_long", path=path)

    async def _attach_debug_report(self, event: MessageEvent, path) -> bool:
        """Send the collected report as a file. True when it was delivered.

        Every failure mode returns False so the caller falls back to text:
        no adapter (a source built by a test fixture, a transport that is
        gone), an adapter that does not implement native file sending, an
        exception mid-upload, or an adapter that reports failure.
        """
        ...
        if adapter is None:
            return False
        if getattr(type(adapter), "send_document", None) is BasePlatformAdapter.send_document:
            return False
        ...
        except Exception:  # thread metadata is a nicety, not a precondition
            metadata = None
        ...
        try:
            result = await adapter.send_document(
                chat_id=source.chat_id,
                file_path=str(path),
                caption=t("trix.cmd.debug.file_caption"),
                file_name=Path(path).name,
                metadata=metadata,
            )
        except Exception as exc:
            logger.warning("/debug: could not attach the report file: %s", exc)
            return False
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Trix Agent')}"
            ...
            if sys.platform == "win32":
                ...
                helper = textwrap.dedent(
                    ...
                    with open(output_path, "wb") as f:
                        proc = subprocess.Popen(cmd, cwd=workdir, stdout=f,
                ...
                # Каталог задаётся явно и помощнику, и самому обновлению —
                # см. комментарий у posix-ветки ниже. Служба на Windows
                # обычно стартует из C:\\Windows\\System32, и наследовать
                # его обновлению незачем.
                subprocess.Popen(
                    [
                        sys.executable, "-c", helper,
                        str(output_path), str(exit_code_path), str(project_root),
                ...
                # Рабочий каталог задаётся ЯВНО, а не наследуется от шлюза.
                # Унаследованный каталог ломает обновление молча: на стенде
                # 2026-09-09 процесс получил в наследство /root (права 0700,
                # чужой пользователь), обновление ленивых движков упало на
                # чтении /root/uv.toml -- и клиент увидел бодрое
                # «✓ Code updated!», хотя распознавание речи осталось на
                # прежней версии. Установочное дерево -- единственный
                # каталог, который обновлению заведомо и читаем, и осмыслен:
                # там же лежит git, ради которого всё и затевается.
