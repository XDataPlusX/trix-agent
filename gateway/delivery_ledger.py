...
#
# Клиентская формулировка живёт в ``hermes_cli/trix_session_notices.py``:
# апстримный литерал был английским и говорил про «the gateway», слово,
# которое клиенту Trix ничего не объясняет. Имя ``RECOVERED_MARKER``
# сохранено и остаётся модульным атрибутом через ``__getattr__`` (PEP 562):
# и потребитель (``gateway/run.py`` — ``content = RECOVERED_MARKER + content``),
# и апстримный тест обращаются к нему как к обычной константе, но значение
# теперь берётся из каталога при КАЖДОМ обращении — константа, вычисленная
# на импорте, зафиксировала бы язык раньше, чем прочитан config.yaml.
def __getattr__(name: str) -> str:
    if name == "RECOVERED_MARKER":
        from hermes_cli.trix_session_notices import recovered_reply_prefix

        return recovered_reply_prefix()
