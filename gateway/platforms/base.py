...
# -- Cron delivery header/footer wrapper -------------------------------------
#
# ``cron/scheduler.py::_deliver_result`` wraps a job's output in a localized
# header/footer (job name + job_id, then a divider, then a "you can stop or
# manage this from chat" footer) before delivery. Some adapters (currently
# Yuanbao's MessageSender) need to detect and strip that wrapper for a
# cleaner platform-native rendering.
#
# Building and parsing live together in this one module on purpose: the
# wrapper text is localized via ``agent.i18n`` (any of ~17 languages), so a
# parser that hardcodes one language's literal English string silently stops
# recognizing the wrapper the moment the text is translated (or the moment
# ``display.language`` differs from whatever language happened to build the
# message being parsed). Detection here instead reads the *static* prefix of
# the localized template (the text before its first placeholder) for EVERY
# supported language, not just the currently active one -- so it keeps
# working no matter which language built the message, independent of the
# parsing side's own active config.
CRON_WRAP_DIVIDER = "\n-------------\n\n"


def _cron_wrap_prefixes(key: str, placeholder: str) -> list[str]:
    """Static text before ``placeholder`` in the localized template at ``key``,
    across every supported language (deduplicated).

    Untranslated languages resolve to the English catalog value via ``t()``'s
    own fallback, so this naturally collapses to just the languages that
    actually have a distinct translation for ``key`` (today: English and
    Russian) -- no need to track that set separately, and it grows on its own
    if a future translation is added.
    """
    from agent.i18n import t, SUPPORTED_LANGUAGES

    prefixes: list[str] = []
    for candidate in SUPPORTED_LANGUAGES:
        raw = t(key, lang=candidate)
        prefix = raw.split(placeholder, 1)[0]
        if prefix and prefix not in prefixes:
            prefixes.append(prefix)
    return prefixes


def format_cron_wrap(task_name: str, job_id: str, content: str, *, lang: str | None = None) -> str:
    """Build the localized cron-delivery header/footer wrapper around ``content``.

    Uses the active display language by default (``lang=None``); pass an
    explicit ``lang`` to build under a specific one (mainly for tests). See
    ``strip_cron_wrap`` for the matching parser -- keep both in sync through
    this module rather than re-deriving the wrapper's shape elsewhere.
    """
    from agent.i18n import t

    header = t("trix.cron.wrap.header", lang=lang, task_name=task_name)
    job_id_line = t("trix.cron.wrap.job_id", lang=lang, job_id=job_id)
    footer = t("trix.cron.wrap.footer", lang=lang, task_name=task_name)
    return f"{header}\n{job_id_line}{CRON_WRAP_DIVIDER}{content}\n\n{footer}"


def strip_cron_wrap(content: str) -> str:
    """Strip the cron delivery header/footer wrapper built by ``format_cron_wrap``.

    Language-independent: recognizes the wrapper text of every supported
    language, regardless of the active/explicit language that built it or
    the one currently configured on the parsing side. Returns ``content``
    unchanged if it doesn't look like a wrapped cron delivery.
    """
    header_prefixes = _cron_wrap_prefixes("trix.cron.wrap.header", "{task_name}")
    if not any(content.startswith(p) for p in header_prefixes):
        return content
    ...
    job_id_prefixes = _cron_wrap_prefixes("trix.cron.wrap.job_id", "{job_id}")
    if not any(p in header for p in job_id_prefixes):
        return content

    footer_prefixes = _cron_wrap_prefixes("trix.cron.wrap.footer", "{task_name}")
    footer_pos = max((content.rfind(p) for p in footer_prefixes), default=-1)
    if footer_pos < 0 or footer_pos <= divider_pos:
        return content

    body_start = divider_pos + len(CRON_WRAP_DIVIDER)
...
# Why a MEDIA path was refused. The two reasons must stay distinguishable:
# UNREACHABLE is the agent's own mistake (it named a file the host cannot see)
# and the user is told, because the alternative is a silently missing
# attachment. DENIED is the prompt-injection surface (``MEDIA:/etc/shadow``) —
# it stays a silent drop, since echoing the name back into chat would confirm
# the attempt and hand an injected payload a way to write text into the reply.
MEDIA_REJECT_UNREACHABLE = "unreachable"
MEDIA_REJECT_DENIED = "denied"


def classify_media_delivery_path(path: str) -> Tuple[Optional[str], Optional[str]]:
    """Return ``(safe_path, reject_reason)`` for a model-emitted media path.

    Exactly one half is ever set: an accepted path carries no reason, a
    refused one carries :data:`MEDIA_REJECT_UNREACHABLE` or
    :data:`MEDIA_REJECT_DENIED` and no path.
    ...
    """
    if not path:
        return None, MEDIA_REJECT_UNREACHABLE
    ...
    if not _media_delivery_strict_mode():
        if _path_under_denied_prefix(resolved):
            return None, MEDIA_REJECT_DENIED
        return str(resolved), None
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    The reason-free form of :func:`classify_media_delivery_path`, kept because
    most callers only need to know whether the file may be uploaded.
    """
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._authorization_check_takes_thread: bool = False
    ...
    def set_authorization_check(
        ...
        optionally extended with a ``thread_id`` parameter. It is used by
        adapters that pull external context (e.g. Slack thread replies via
        ``conversations.replies``) to flag messages from senders that are not
        on the configured allowlist, so the LLM can treat them as unverified
        background reference rather than authoritative input.

        Whether the callback accepts ``thread_id`` is decided ONCE, here, and
        cached. Probing it per call by catching ``TypeError`` around the
        invocation cannot distinguish "wrong arity" from "the check body
        raised TypeError", and would re-run an authorization decision that may
        not be side-effect free.
        """
        self._authorization_check = callback
        self._authorization_check_takes_thread = self._callback_takes_thread_id(callback)

    @staticmethod
    def _callback_takes_thread_id(callback: Optional[Callable[..., bool]]) -> bool:
        """Return whether ``callback`` declares a ``thread_id`` parameter.

        A ``**kwargs``-style check counts as accepting it. Anything
        un-introspectable (builtins, C callables) is treated as legacy 3-arg.
        """
        if callback is None:
            return False
        try:
    ...
    def _is_sender_authorized(
        ...
        ``thread_id`` is forwarded when the registered check accepts it. It is
        not cosmetic: under ``gateway.profile_routes`` a chat and one of its
        topics can belong to DIFFERENT profiles (the owner's DM chat routes to
        ``default``, thread 570710 inside it routes to ``system_admin``), and
        the profile decides WHICH allowlist the check reads. Dropping the
        thread makes every button in such a topic resolve to the wrong
        profile's allowlist (RAF-176).
        """
        # Read defensively, for the same reason ``set_reaction_handler``
        # assigns defensively: subclasses built with ``object.__new__`` in
        # tests never run ``BasePlatformAdapter.__init__``, so the attribute
        # may be absent rather than ``None``. "Absent" means the same thing
        # as "unset" — no check registered.
        check = getattr(self, "_authorization_check", None)
        if not user_id or check is None:
            return None
        try:
            # ``_authorization_check_takes_thread`` may be absent for the same
            # ``object.__new__`` reason, and for checks assigned straight to
            # ``_authorization_check`` without going through the setter.
            takes_thread = getattr(self, "_authorization_check_takes_thread", None)
            if takes_thread is None:
                takes_thread = self._callback_takes_thread_id(check)
            if takes_thread:
                return bool(check(user_id, chat_type, chat_id, thread_id=thread_id))
            return bool(check(user_id, chat_type, chat_id))
        except Exception:
            logger.warning(
    ...
    # Appended unconditionally at the very end, after the smart-deny note.
    # Empty by default (pass-through for every adapter that doesn't override
    # it), so it is a no-op for the historical English wording. Telegram
    # overrides it with catalog text explaining that silence means the
    # action will NOT run -- see plugins/platforms/telegram/adapter.py.
    _EA_TIMEOUT_NOTE: str = ""
    ...
    def _ea_reason_text(self, description: str) -> str:
        """Hook for platforms that translate the reason. Pass-through by default."""
        return description
    ...
    def _format_exec_approval(
        ...
        ``_EA_SMART_DENY_LINE`` when ``smart_denied``, plus ``_EA_TIMEOUT_NOTE``
        unconditionally at the end. Button construction stays platform-local;
        adapters with additional trailing instructions (e.g. reaction
        legends) append them to this core.
        """
        ...
        text = (
            ...
            f"{self._EA_REASON_LABEL}{self._ea_escape(self._ea_reason_text(description))}"
        )
        if smart_denied:
            text += self._EA_SMART_DENY_LINE
        text += self._EA_TIMEOUT_NOTE
        return text
    ...
    async def _notify_media_delivery_failure(
        ...
        *,
        ...
        ``include_file_name`` must be False whenever *media_path* did NOT pass
        ``validate_media_delivery_path``. A validated path names a real host
        file the agent produced, and showing its name helps. An unvalidated one
        is raw model output: echoing even its basename puts attacker-chosen
        text into the reply for an injected ``MEDIA:`` tag, and leaks fragments
        of a path the user was never meant to see.
        """
    ...
    def partition_media_delivery_paths(
        ...
        """Split MEDIA paths into ``(deliverable, unreachable)``.

        Explicit ``MEDIA:`` tags are the agent stating that it is attaching a
        file. When such a path names nothing the host can read, dropping it
        silently leaves the user with a reply that promises an attachment and
        carries none, and the agent never learns (delivery runs after its turn
        has ended). The second list exists so the caller can say so out loud —
        same contract as a failed upload (#66797).

        Paths refused by the denylist / strict mode are NOT returned: they are
        the prompt-injection surface, and naming them back into chat would
        turn an injected ``MEDIA:`` tag into a way to write text into the
        reply. Those stay a logged, silent drop.
        """
        safe_media: List[Tuple[str, bool]] = []
        unreachable: List[Tuple[str, bool]] = []
        for media_path, is_voice in media_files or []:
            ...
            if safe_path:
                ...
                continue
            if reason == MEDIA_REJECT_UNREACHABLE:
                logger.warning(
                    "MEDIA directive path is not readable from the host: %s "
                    "(a container-local path outside /workspace, /root and the "
                    "mounted cache dirs has no host counterpart)",
                    _log_safe_path(raw),
                )
                unreachable.append((raw, bool(is_voice)))
            else:
                logger.warning("Skipping unsafe MEDIA directive path: %s", _log_safe_path(raw))
        return safe_media, unreachable

    @staticmethod
    def filter_media_delivery_paths(media_files) -> List[Tuple[str, bool]]:
        """Drop unsafe MEDIA paths and normalize accepted paths."""
        return BasePlatformAdapter.partition_media_delivery_paths(media_files)[0]

    async def notify_unreachable_media(
        self,
        chat_id: str,
        unreachable,
        *,
        ...
        """Tell the user about MEDIA attachments that never left the sandbox.

        Call it AFTER the reply's text and real attachments have been sent, so
        the notice reads as a footnote to the message rather than replacing it.

        These paths never validated, so the notice names no file — see
        ``_notify_media_delivery_failure``'s ``include_file_name``.
        """
        for media_path, is_voice in unreachable or []:
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            if not cmd:
                ...
                # Spec 19, Ruling 6 — the SAME deadlock this block already
                # closes for clarify replies applies to a secret-capture
                # reply: the agent thread is blocked in
                # tools.secret_capture_gateway.wait_for_response(), and
                # without this bypass the client's reply (the key itself)
                # would sit in _pending_messages until the agent times out.
                # This is one of the two guards Ruling 6 requires — missing
                # THIS one (not just the run.py-side intercept) is exactly
                # the mistake the spec's first revision made.
                _has_pending_secret = False
                if not _has_text_clarify:
                    try:
                        from tools import secret_capture_gateway as _secret_mod
                        _has_pending_secret = _secret_mod.has_pending(session_key)
                    except Exception:
                        _has_pending_secret = False

                if _has_text_clarify or _has_pending_secret:
                    logger.debug(
                        "[%s] Routing message to %s text-intercept for %s",
                        self.name,
                        "clarify" if _has_text_clarify else "secret-capture",
                        session_key,
                    )
                    try:
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                media_files, _unreachable_media = self.partition_media_delivery_paths(
                    media_files
                )
                ...
                # An explicit MEDIA: tag naming a file the host cannot read is
                # the same user-visible failure as an upload that errors out:
                # the reply announces an attachment that isn't there. Said last,
                # after the text and the attachments that did work.
                await self.notify_unreachable_media(
                    event.source.chat_id,
                    _unreachable_media,
                    metadata=_final_thread_metadata,
                )
