...
class MessageSender:
    ...
    def strip_cron_wrapper(content: str) -> str:
        ...
        Delegates to ``gateway.platforms.base.strip_cron_wrap``, which builds
        and parses the wrapper from the same localized templates
        (``cron/scheduler.py`` uses the matching ``format_cron_wrap``) so
        detection keeps working regardless of ``display.language``.
        """
        return strip_cron_wrap(content)
