...
def _describe_reaped_workers(
    pids: "list[int]",
    classify: "Callable[[int], tuple[str, Optional[int]]]",
) -> str:
    """Render reaped worker pids with their exit classification.

    Classifies each reaped pid via ``classify`` (wired to
    ``hermes_cli.kanban_db._classify_worker_exit`` at the call site, which
    reads the exit registry the reaper itself just populated) so the
    dispatcher's reap log is actionable instead of a bare pid list:
    ``clean_exit`` = protocol violation, ``rate_limited`` = provider quota
    wall, ``signaled`` = OOM/SIGKILL.  A classifier failure degrades to
    ``unknown`` for that pid instead of dropping the log line.
    """
    details = []
    for pid in pids:
        try:
            kind, code = classify(pid)
        except Exception:
            kind, code = "unknown", None
        suffix = f" ({code})" if code is not None else ""
        details.append(f"{pid}:{kind}{suffix}")
    return ", ".join(details)
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        while self._running:
            try:
                ...
                if pids:
                    logger.info(
                        "kanban dispatcher: reaped %d zombie worker(s): %s",
                        len(pids),
                        _describe_reaped_workers(pids, _kb._classify_worker_exit),
                    )
            except Exception:
