...
# ``hermes-acp`` is an installed entry point (pyproject [project.scripts],
# laid down by scripts/install.sh) and hosts a full agent session, so it
# needs the same redirect as ``hermes`` and the gateway — before anything
# down the import chain reaches ``tempfile.gettempdir()`` and caches it.
#
# No-op in shared-host; HOME of this process is never touched. See the
# contained-profile-root plan §2.2.
try:
