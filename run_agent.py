...
# ``hermes-agent`` is an installed entry point (pyproject [project.scripts],
# laid down by scripts/install.sh) — a first-class way to start the engine,
# not a helper. Without this it ran the whole session on the real ``/tmp``
# while every other entry point was contained, because ``import tempfile``
# below caches ``gettempdir()`` for the life of the process.
#
# Placed above that import on purpose. No-op in shared-host; HOME of this
# process is never touched. See the contained-profile-root plan §2.2.
try:
    from hermes_constants import bootstrap_process_isolation as _bootstrap_isolation

    _bootstrap_isolation()
except Exception:
    # Изоляция не имеет права мешать процессу стартовать.
    pass
