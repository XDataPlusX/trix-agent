...
TOOLSETS = {
    ...
    # Spec 19 — asking the chat itself for a missing secret. Messaging-only
    # (gated to specific platforms via hermes_cli/tools_config.py's
    # _TOOLSET_PLATFORM_RESTRICTIONS): CLI/desktop/TUI sessions already have a
    # local masked-input secret prompt and never see this toolset. The value
    # never reaches the model — see tools/secret_capture_gateway.py.
    "secrets": {
        "description": "Ask the current chat for a missing API key/secret by name and purpose (value never reaches the model; messaging sessions only)",
        "tools": ["secret_request"],
        "includes": []
    },

    # Spec 20 — connecting a remote HTTP MCP server described in chat.
    # Deliberately NOT in _HERMES_CORE_TOOLS (every upstream user would pay
    # for the schema on every call) and gated to messaging sessions the same
    # way as "secrets" above: a session with a real human to ask a token
    # from and to wait through a gateway restart. See
    # tools/mcp_connect_gateway.py for why a captured token never reaches
    # this toolset's tool result.
    "mcp": {
        "description": "Connect a remote HTTP MCP server described in chat by URL (no arbitrary local commands; token never reaches the model; messaging sessions only)",
        "tools": ["mcp_connect"],
        "includes": []
    },
    ...
    "hermes-telegram": {
        ...
        # secret_request (spec 19): Telegram-only clients have no shell and no
        # host filesystem access from the sandbox, so this is their one path
        # to hand the agent an API key. See toolsets["secrets"].
        "tools": _HERMES_CORE_TOOLS + ["secret_request"],
        "includes": []
    },
