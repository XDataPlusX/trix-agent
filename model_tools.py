...
def _compute_tool_definitions(
    ...
    # Hide session_search's cross-profile `profile` parameter from the model
    # unless session_search.allow_cross_profile is enabled in config.yaml
    # (default off). The parameter's mere presence in the schema is a
    # standing invitation to try reading another profile's conversation
    # history, which every profile's isolation guarantee forbids by default.
    # This function is memoized on a config-mtime fingerprint (see
    # get_tool_definitions above), so the rebuilt schema stays byte-stable
    # for the life of a conversation/process unless config.yaml itself
    # changes — same caching contract as the execute_code/discord blocks
    # above.
    if "session_search" in available_tool_names:
        try:
            from tools.session_search_tool import get_session_search_schema
            dynamic_schema = get_session_search_schema()
        except Exception:
            dynamic_schema = None
        if dynamic_schema is not None:
            for i, td in enumerate(filtered_tools):
                if td.get("function", {}).get("name") == "session_search":
                    filtered_tools[i] = {"type": "function", "function": dynamic_schema}
                    break
