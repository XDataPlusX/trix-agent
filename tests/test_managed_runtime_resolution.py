...
from tests._source_tree import prune_top_level, walk_files
...
_EXEMPT_DIRS = (
    ...
    "venv",
    ".worktrees",
    # Agent-tool scratch checkouts — a second copy of this repo on disk.
    # Without this the scan reports every call site twice, once at a path
    # nobody edits (see ``.worktrees`` above, same class, older location).
    ".claude",
)
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("hermes_cli/setup_wizard/tools_view.py", "uv"): (
        "Fallback after resolve_uv() in the wizard's blocked-row check — a "
        "user-installed uv on PATH is as good as a managed one for deciding "
        "whether a uv-dependent row can install."
    ),
...
def _source_files() -> list[Path]:
    # prune_top_level(), not prune_names(): _EXEMPT_DIRS is a top-level
    # exemption, and pruning those names at any depth would quietly shrink
    # what this guard covers. walk_files() keeps the same file set while
    # staying out of the excluded subtrees and surviving a __pycache__ that
    # a parallel worker deletes mid-walk (RAF-171).
    return [
        path
        for _, path in walk_files(
            REPO_ROOT, match="*.py", prune=prune_top_level(_EXEMPT_DIRS)
        )
    ]
