...
def _make_conflicted_managed_checkout(tmp_path: Path, branch: str = "release") -> Path:
    """Create a managed checkout whose autostash conflicts with its origin.

    ``branch`` defaults to "release" — neither installer accepts a
    caller-supplied branch anymore (install.sh dropped --branch in Task 7,
    install.ps1 dropped -Branch in Task 8), so each one's own hardcoded
    default branch is the only ref it will ever fetch/checkout, and the
...
def test_install_sh_repository_stage_recovers_from_autostash_conflict(
    ...
    managed = _make_conflicted_managed_checkout(tmp_path, branch="release")
    env = os.environ | {
...
def test_install_sh_repository_stage_clean_apply_drops_stash(
    ...
    _git(seed, "add", "tracked.txt")
    _git(seed, "commit", "-m", "base")
    _git(seed, "branch", "-M", "release")

    remote = tmp_path / "origin.git"
    _git(tmp_path, "init", "--bare", str(remote))
    _git(seed, "remote", "add", "origin", str(remote))
    _git(seed, "push", "-u", "origin", "release")

    managed = tmp_path / "hermes-agent"
    _git(tmp_path, "clone", "--branch", "release", str(remote), str(managed))

    # Local edit on a file upstream will NOT touch — no conflict on apply.
    (managed / "local-only.txt").write_text("local edit\n", encoding="utf-8")

    upstream = tmp_path / "upstream"
    _git(tmp_path, "clone", "--branch", "release", str(remote), str(upstream))
    (upstream / "tracked.txt").write_text("upstream edit\n", encoding="utf-8")
    _git(upstream, "commit", "-am", "upstream")
    _git(upstream, "push", "origin", "release")
