...
# ровно то, что печатает раннер для FLAKY-файла. Здесь файл ОДИН, и
# раннер в этом случае печатает "1 FLAKY file" в единственном числе
# (run_tests_parallel.py склоняет по len(_FLAKY_RESULTS)) — заголовок
# обязан быть дословным, иначе тест разойдётся с реальным выводом.
FLAKY_SECTION = """=== ⚠ 1 FLAKY file (failed once, passed on retry — fix these) ===
...
def test_flaky_section_is_found_for_both_singular_and_plural_header():
    """Заголовок секции FLAKY склоняется по числу файлов, и разбор обязан
    понимать обе формы.

    Регрессия: ``_FLAKY_HEADER_RE`` требовал только "FLAKY files", а
    раннер для одного файла печатает "1 FLAKY file". Секция не находилась,
    и падение первой (отменённой повтором) попытки уезжало в настоящие —
    релиз-гейт краснел на зелёном прогоне. Фикстура выше теперь
    единственного числа, так что этот тест закрывает множественное.
    """
    plural = FLAKY_SECTION.replace(
        "=== ⚠ 1 FLAKY file (", "=== ⚠ 2 FLAKY files ("
    )
    assert plural != FLAKY_SECTION
    output = _wrap_run(SUMMARY_LINE, plural, FAILURE_OUTPUT_HEADER, FINAL_SUMMARY)
    assert parse_real_failures(output) == []
...
# ---------------------------------------------------------------------------
#
# Сверка переехала из bash в этот модуль после того, как гейт остановил
# релиз 0.1.38 вердиктом «новые падения» на двух тестах, которые в базовой
# линии ЕСТЬ. В bash она делалась `comm` по двум спискам, отсортированным
# по-разному: базовая линия — системным `sort` (порядок зависит от локали
# машины, режущей релиз), список падений — питоновским `sorted()`.


# Та самая пара, на которой это вылезло: под en_US.UTF-8 системный sort
# игнорирует пунктуацию и ставит _callback ПЕРЕД .py, а sorted() сравнивает
# кодовые точки ('.' = 0x2E < '_' = 0x5F) и ставит наоборот.
WECOM = "tests/gateway/test_wecom.py::TestMediaUpload::test_blocks_rebind"
WECOM_CALLBACK = "tests/gateway/test_wecom_callback.py::TestPollLoop::test_dispatch"


def test_baseline_entries_are_not_reported_as_new_whatever_their_order():
    """Порядок входных списков на результат не влияет — вообще никак."""
    from hermes_cli.release_gate import compare_to_baseline

    baseline = {WECOM, WECOM_CALLBACK}
    for current in ([WECOM, WECOM_CALLBACK], [WECOM_CALLBACK, WECOM]):
        new_failures, fixed = compare_to_baseline(current, baseline)
        assert new_failures == []
        assert fixed == []


def test_a_real_new_failure_is_reported_even_next_to_the_tricky_pair():
    """Обратная сторона того же дефекта: старая сверка могла СЪЕСТЬ новое
    падение, попавшее в рассогласованный участок, и выпустить релиз."""
    from hermes_cli.release_gate import compare_to_baseline

    real_new = "tests/gateway/test_wecom_callback.py::TestPollLoop::test_broken"
    new_failures, fixed = compare_to_baseline(
        [WECOM_CALLBACK, real_new, WECOM], {WECOM, WECOM_CALLBACK}
    )
    assert new_failures == [real_new]
    assert fixed == []


def test_baseline_failure_that_now_passes_is_reported_as_fixed():
    from hermes_cli.release_gate import compare_to_baseline

    new_failures, fixed = compare_to_baseline([WECOM], {WECOM, WECOM_CALLBACK})
    assert new_failures == []
    assert fixed == [WECOM_CALLBACK]


def test_read_baseline_drops_comments_and_blank_lines():
    from hermes_cli.release_gate import read_baseline

    text = (
        "# Tests that already fail on a tree we consider good.\n"
        "#   tests/commented/test_out.py::test_x\n"
        "\n"
        f"{WECOM}\n"
        f"  {WECOM_CALLBACK}  \n"
    )
    assert read_baseline(text) == {WECOM, WECOM_CALLBACK}


def test_cli_compare_baseline_matches_the_function(tmp_path):
    """Гейт зовёт этот путь из bash — он обязан вести себя так же."""
    baseline_file = tmp_path / "known-test-failures.txt"
    ...
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "hermes_cli.release_gate",
            "--compare-baseline",
            str(baseline_file),
        ],
        # Порядок на входе — «неудобный»: callback перед wecom.
        input=f"{WECOM_CALLBACK}\n{real_new}\n",
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stderr
