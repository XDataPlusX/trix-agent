...
class TestRunConversation:
    ...
    def test_empty_response_retry_backoff_status(self, agent, monkeypatch):
        ...
        # This test asserts an exact English sentence. The status line moved
        # behind agent.i18n.t() (Trix localization, agent/conversation_loop.py
        # ~7300) whose catalog default is "ru" for this fork when no
        # language is pinned -- tests/gateway/ has an autouse fixture for
        # this (see tests/gateway/conftest.py), tests/run_agent/ does not.
        # Pin English explicitly so this test keeps checking what it always
        # checked (the wait-time/retry-count formatting), not translation.
        ...
        # The full sentence, deliberately. The string moved into the `t()`
        # catalog when this notice was localized, but the suite pins
        # HERMES_LANGUAGE=en (tests/conftest.py) and the English catalog entry
        # is byte-identical to the literal that used to live here — so this
        # assertion still means exactly what it always meant, and weakening it
        # to "contains (1/3) and an 8 somewhere" would have bought nothing
        # while accepting "in 18s" and "0.8s" as passes.
        #
        # The Russian rendering of the same call site is covered by
        # tests/agent/test_conversation_loop_l10n.py, which sets the language
        # per case; this test's job is the retry/backoff behavior underneath.
        #
        # Checked rather than assumed: gateway/run.py's
        # _TELEGRAM_NOISY_STATUS_RE suppresses `retrying in <n>`, which this
        # string does NOT match — the "(1/3)" sits between the two words. So
        # the notice really does reach the user.
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Trix Agent" in agent._cached_system_prompt
