...
async def test_stale_marker_older_than_5min_does_not_block(tmp_path, monkeypatch):
    ...
    # Сверяемся с каталогом, а не с прозой: текст уведомления —
    # клиентский, его переписывают (2026-09-10 из него убрали консольную
    # команду). Проверяемое здесь — что ответ ИМЕННО это уведомление,
    # а не заглушка дедупликации; сам факт перезапуска держит
    # request_restart.assert_called_once() строкой ниже.
    assert t("gateway.restart.restarting") in str(result)
