...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # trix.cmd.skill.installed_disabled points a Telegram-only client at
    # "whoever administers this machine" rather than a `hermes skills
    # config` command it has no terminal to run.
    assert "administers this machine" in msg
    assert "hermes skills config" not in msg
