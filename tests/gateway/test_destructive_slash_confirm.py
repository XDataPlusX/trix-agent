...
async def test_resolve_always_persists_opt_out_and_runs_execute(monkeypatch):
    ...
    # trix.cmd.confirm.always_saved points the Telegram-only client at
    # "whoever administers this machine" rather than a config.yaml edit
    # it cannot make -- assert the opt-out note landed without pinning to
    # the upstream config.yaml literal.
    assert "will run without confirmation" in resolved
