...
def _pin_english_ui_language(monkeypatch):
    """Gateway tests assert literal English copy for static UI strings
    (approval prompts, /voice, /status, /credits, restart notices, etc.)
    rendered through ``agent.i18n.t()``. Trix Agent's ``DEFAULT_CONFIG``
    default for ``display.language`` is ``"ru"`` (the product ships to a
    Russian-speaking customer), so without this fixture the very same code
    legitimately renders Russian in a temp ``HERMES_HOME`` with no
    ``config.yaml`` -- which is correct product behavior, but not what these
    tests are about. Pin the gateway test environment to English by default;
    a test that specifically exercises localization can still monkeypatch
    ``HERMES_LANGUAGE`` (or write ``display.language`` into its own config)
    to override this.
    """
...
def _gateway_test_files() -> list[Path]:
    """Every gateway test module, without racing the parallel workers.

    walk_files() rather than ``rglob``: the runner spawns ~18 concurrent
    pytest subprocesses over this very directory, and rglob descends into
    the ``__pycache__`` they create and delete — raising FileNotFoundError
    mid-walk (RAF-171). See tests/_source_tree.py.
    """
    return [path for _, path in walk_files(_GATEWAY_DIR, match="test_*.py")]
_GUARD_HINT = (
