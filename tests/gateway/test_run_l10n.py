...
class TestWatchUpdateProgressFinished:
    ...
    async def test_failure_ru(self, monkeypatch, tmp_path):
        ...
        assert sent == (
            "❌ Обновление не выполнено (код выхода 3). "
            "Сообщите тому, кто администрирует эту машину."
        )
...
class TestSendUpdateNotificationLocalized:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("output", ["log line", ""], ids=["с логом", "без лога"])
    async def test_finished_en(self, monkeypatch, tmp_path, output):
        """Вердикт не зависит от того, остался ли на машине лог: он в чат
        не попадает ни в каком виде."""
        sent = await self._run(monkeypatch, tmp_path, output=output, exit_code="0", lang="en")
        assert sent == "✅ Update finished."
        assert "log line" not in sent

    @pytest.mark.asyncio
    @pytest.mark.parametrize("output", ["log line", ""], ids=["с логом", "без лога"])
    async def test_finished_ru(self, monkeypatch, tmp_path, output):
        sent = await self._run(monkeypatch, tmp_path, output=output, exit_code="0", lang="ru")
        assert sent == "✅ Обновление завершено."
        assert "log line" not in sent

    @pytest.mark.asyncio
    @pytest.mark.parametrize("output", ["log line", ""], ids=["с логом", "без лога"])
    async def test_failed_en(self, monkeypatch, tmp_path, output):
        ...
        mention either."""
        sent = await self._run(monkeypatch, tmp_path, output=output, exit_code="1", lang="en")
        assert sent == (
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("output", ["log line", ""], ids=["с логом", "без лога"])
    async def test_failed_ru(self, monkeypatch, tmp_path, output):
        sent = await self._run(monkeypatch, tmp_path, output=output, exit_code="1", lang="ru")
        assert sent == (
            "❌ Обновление не выполнено (код выхода 1). "
            "Сообщите тому, кто администрирует эту машину."
        )
        assert "log line" not in sent
    ...
    @pytest.mark.parametrize("lang,expected", [
        ("en", "auxiliary components"),
        ("ru", "вспомогательных компонентов"),
    ])
    async def test_partial_verdict_matches_the_watcher(self, monkeypatch, tmp_path, lang, expected):
        """Код 2 — частичный успех и на запасном пути тоже.

        Апстримный запасной путь знал только «успех/отказ» и объявлял
        частично применённое обновление отказом. Клиент видел разный
        исход одного и того же обновления в зависимости от того, пережил
        ли шлюз перезапуск.
        """
        sent = await self._run(monkeypatch, tmp_path, output="", exit_code="2", lang=lang)
