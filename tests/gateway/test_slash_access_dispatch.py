...
# ---------------------------------------------------------------------------
# /whoami response shape — proves _handle_whoami_command's own tier/allowed-
# commands listing still reflects the access policy. /whoami itself is now
# in hermes_cli.trix_menu.DISABLED_COMMANDS (client-command-surface plan,
# decision "/whoami — убрать": it always answered "full access", the user is
# always the owner, and there is nothing else to diagnose), so
# _handle_message() no longer dispatches to it — that dispatch-layer gate is
# covered by tests/gateway/test_disabled_commands.py. The handler itself is
# still real, working code (reachable when the client-command-surface
# curation is off), so this test calls it directly rather than through
# _handle_message — a deliberate, narrower boundary than before, not a loss
# of coverage.
# ---------------------------------------------------------------------------
...
async def test_whoami_non_admin_lists_runnable_commands():
    ...
    result = await runner._handle_whoami_command(_make_event("/whoami", _make_source(user_id="999")))
...
async def test_non_admin_with_empty_user_commands_gets_floor_only():
    ...
    # /help still works (always-allowed floor). /whoami is the floor's other
    # member but is itself disabled at the client-command-surface layer now
    # (see the comment above test_whoami_non_admin_lists_runnable_commands),
    # so it is no longer a valid vehicle for "reached the real handler,
    # unblocked by the access gate" — /help still is.
    runner._handle_help_command = AsyncMock(return_value="help-reachable")
    help_result = await runner._handle_message(_make_event("/help", _make_source(user_id="999")))
    assert help_result == "help-reachable"
...
async def test_group_only_gating_leaves_dm_unrestricted():
    ...
    # /restart is admin-only under an active gate but reachable by anyone in
    # backward-compat mode (no admin list for this scope) — a stronger proof
    # of "no gating at all" than an always-allowed-floor command like the
    # old /whoami vehicle would be: a floor command is reachable by non-admins
    # even WITH an active gate, so it can't distinguish "gate is active but
    # this command is exempt" from "there is no gate here at all".
    runner._handle_restart_command = AsyncMock(return_value="restart-reachable")
    result = await runner._handle_message(
        _make_event("/restart", _make_source(user_id="anyone", chat_type="dm"))
    )
    assert result == "restart-reachable"
...
async def test_gating_isolated_per_platform():
    ...
    # Same user_id on Telegram → must be unrestricted (Telegram has no admin
    # list). /restart (not an always-allowed-floor command, unlike the old
    # /whoami vehicle) proves this platform has no gate at all, not merely
    # that this particular command is exempt from one.
    runner._handle_restart_command = AsyncMock(return_value="restart-reachable")
    tg_src = _make_source(platform=Platform.TELEGRAM, user_id="999", chat_id="t1")
    result = await runner._handle_message(_make_event("/restart", tg_src))
    assert result == "restart-reachable"
