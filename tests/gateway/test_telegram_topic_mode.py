...
async def test_topic_root_command_lists_unlinked_sessions_for_restore(tmp_path, monkeypatch):
    """/topic is now in hermes_cli.trix_menu.DISABLED_COMMANDS
    (client-command-surface plan Task 1, hint="replace" -> /sessions:
    explaining Telegram topics before explaining sessions-on-top-of-topics
    was one layer too many for a client, and /sessions + /resume already
    cover "see my other conversations"). _handle_message() no longer
    dispatches to _handle_topic_command -- that dispatch-layer gate is
    covered by tests/gateway/test_disabled_commands.py. The handler's own
    unlinked-sessions listing logic this test actually exercises is still
    real, working code, so it is called directly rather than through
    _handle_message."""
    import gateway.run as gateway_run
