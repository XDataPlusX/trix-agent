...
from gateway.run import (
    GatewayRunner,
    TurnRunner,
    _client_provider_resolution_error_message,
...
class TestClientProviderResolutionErrorMessage:
    """Defect: a client machine with no inference provider configured yet
    got the raw ``hermes_cli.auth.AuthError`` text verbatim in Telegram --
    "No inference provider configured. Run 'hermes model' ... in
    ~/.hermes/.env." -- naming a shell command, a file path, and the
    upstream binary. The client has no shell; only the setup wizard.
    """

    def _no_provider_auth_error(self):
        from hermes_cli.auth import AuthError

        return AuthError(
            "No inference provider configured. Run 'hermes model' to choose "
            "a provider and model, or set an API key (OPENROUTER_API_KEY, "
            "OPENAI_API_KEY, etc.) in ~/.hermes/.env.",
            code="no_provider_configured",
        )

    def test_wrapped_runtime_error_is_detected_via_cause(self):
        # This is the real shape: _resolve_runtime_agent_kwargs re-raises
        # the AuthError as a plain RuntimeError via `raise ... from auth_exc`.
        auth_exc = self._no_provider_auth_error()
        try:
            raise RuntimeError(str(auth_exc)) from auth_exc
        except RuntimeError as wrapped:
            msg = _client_provider_resolution_error_message(wrapped)
        assert_no_unreachable_advice(msg)
        assert "run 'hermes" not in msg.lower()
        assert "openrouter_api_key" not in msg.lower()
        assert "wizard" in msg.lower() or "мастер" in msg.lower()

    def test_bare_auth_error_is_also_detected(self):
        msg = _client_provider_resolution_error_message(self._no_provider_auth_error())
        assert_no_unreachable_advice(msg)
        assert "wizard" in msg.lower() or "мастер" in msg.lower()

    def test_unrelated_exception_keeps_previous_behavior(self):
        exc = RuntimeError("some other resolution failure")
        msg = _client_provider_resolution_error_message(exc)
        assert "some other resolution failure" in msg

    def test_other_auth_error_codes_are_not_misrouted(self):
        from hermes_cli.auth import AuthError

        exc = AuthError("key rejected", code="invalid_provider")
        msg = _client_provider_resolution_error_message(exc)
        # Falls through to the generic (pre-existing) path, not the new
        # "not configured" wizard message -- this fix is scoped to the one
        # reproduced leak, not a general AuthError rewrite.
