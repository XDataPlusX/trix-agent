...
def test_telegram_final_response_redacts_auth_secrets():
    ...
    # Two separate claims, and the second one is the easy one to get wrong.
    #
    # (a) No key material survives.
    assert "sk-live" not in sanitized
    assert "abcdefghijklmnopqrstuvwxyz1234567890" not in sanitized

    # (b) The AUTH branch is the one that answered — not merely "some branch
    # that mentions a key". _gateway_provider_error_reply has five branches
    # and the billing one also says "the key is out of funds", is also != raw,
    # and also carries no secret. A guard phrased as `"key" in sanitized`
    # would therefore stay green while an auth failure got misrouted into
    # billing and the user was told their balance ran out instead of that
    # their key was rejected — a wrong diagnosis they would act on.
    #
    # Built by calling the same catalog the product calls, so rewording the
    # message is free and only a wrong BRANCH fails. The literal English
    # sentence is not frozen here; the wording itself is exercised in
    # tests/gateway/test_errors_l10n.py.
    from agent import i18n

    expected = i18n.t("trix.errors.provider.auth_failed")
    assert sanitized == expected, (
        f"expected the auth-failure reply, got {sanitized!r}"
    )
    assert sanitized != i18n.t("trix.errors.provider.billing", status="")
