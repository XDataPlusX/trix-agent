...
class TestNewInHelp:
    ...
    def test_new_hint_is_localized_in_russian(self, monkeypatch):
        """The English fixture above is pinned to `en` by the module-level
        `_pin_english_ui_language` autouse fixture and therefore never
        exercises the Russian path -- pin `ru` explicitly here so that path
        has a test too."""
    ...
    def test_alias_note_is_localized_in_russian(self, monkeypatch):
        """The ``(alias: …)`` fragment is appended by ``gateway_help_lines``
        itself rather than coming from the registry, so it needs its own
        test: reverting it to an f-string literal leaves every other test
        in this suite green.  It rides 42 of the 60 help lines, which makes
        it the most-repeated single fragment the customer reads.
        """
        from agent import i18n
        from hermes_cli.commands import gateway_help_lines
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        ...
        try:
            ...
            assert "(псевдоним: `/reset`)" in line
        finally:
            i18n.reset_language_cache()

    def test_alias_note_is_unchanged_in_english(self, monkeypatch):
        """The English rendering must stay byte-identical to pre-i18n."""
        from agent import i18n
        from hermes_cli.commands import gateway_help_lines
        ...
        try:
            line = next(l for l in gateway_help_lines() if l.startswith("`/new "))
            assert line.endswith(" (alias: `/reset`)")
        finally:
            i18n.reset_language_cache()
