...
class TestHandleUpdateCommand:
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting the Trix Agent update" in result
...
class TestSendUpdateNotification:
    ...
    async def test_deferred_notification_delivers_after_reconnect(self, tmp_path):
        ...
        # Клиенту приходит вердикт, а не эхо лога обновления: строка
        # "✓ Update complete!" выше — это вывод `hermes update`, его место
        # в gateway.log (см. hermes_cli/trix_update_notices.py).
        assert sent_text == "✅ Update finished."
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        mock_adapter.send.assert_called_once()
        sent_text = mock_adapter.send.call_args[0][1]
        # Битые байты больше не могут добраться до клиента — лог в чат не
        # пересылается вовсе, — но декодировать их всё равно приходится,
        # чтобы записать лог в журнал. Проверяем, что это не роняет путь
        # доставки и что вердикт ушёл.
        assert sent_text == "✅ Update finished."
        assert "ok before" not in sent_text
        assert "continued after" not in sent_text
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        # Наблюдатель декодирует лог, чтобы записать его в журнал, —
        # битые байты не должны ронять ни это, ни доставку вердикта. В чат
        # лог не уходит.
        assert sent == "✅ Update finished."
        assert "ok before" not in sent
        assert "continued after" not in sent
        assert "Hermes" not in sent
        assert not (hermes_home / ".update_pending.json").exists()


# ---------------------------------------------------------------------------
# Рабочий каталог обновления
# ---------------------------------------------------------------------------


class TestUpdateRunsFromTheInstallTree:
    """`/update` из чата запускается в установочном дереве, а не где придётся.

    Найдено на стенде 2026-09-09: процесс обновления наследовал рабочий
    каталог от шлюза, получил `/root` (0700, чужой пользователь), и
    обновление ленивых движков упало на чтении `/root/uv.toml`. Клиент при
    этом увидел «✓ Code updated!» — то есть поломка не выглядела поломкой,
    а распознавание голосовых осталось на прежней версии.

    Проверяются обе ветки запуска: наследование каталога от родителя не
    зависит от того, нашёлся `setsid` или нет.
    """

    @staticmethod
    def _install_tree() -> str:
        from pathlib import Path as _P

        import gateway.slash_commands as _sc

        return str(_P(_sc.__file__).parent.parent.resolve())

    @pytest.mark.asyncio
    @pytest.mark.parametrize("has_setsid", [True, False], ids=["setsid", "без setsid"])
    async def test_popen_gets_an_explicit_cwd(self, tmp_path, has_setsid):
        runner = _make_runner()
        event = _make_event()
        ...
        def which(name):
            if name == "setsid":
                return "/usr/bin/setsid" if has_setsid else None
            return "/usr/bin/hermes"
        ...
        assert mock_popen.call_args is not None, "обновление вообще не запустилось"
        assert mock_popen.call_args[1].get("cwd") == self._install_tree()

    @pytest.mark.windows_only
    @pytest.mark.asyncio
    async def test_windows_helper_gets_the_same_cwd(self, tmp_path):
        """Ветка Windows — тот же класс: помощник наследовал каталог службы.

        Клиента это не касается (у него Linux), но каталог наследуется
        одинаково на обеих ветках, и служба на Windows обычно стартует из
        ``C:\\Windows\\System32``. Проверяется здесь, а не подменой
        ``sys.platform``: ветку выбирает сам обработчик, и подделка хоста в
        этом проекте запрещена.
        """
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._hermes_home", hermes_home), \
             patch("shutil.which", return_value="hermes.exe"), \
             patch("subprocess.Popen", mock_popen):
            await runner._handle_update_command(event)

        tree = self._install_tree()
        assert mock_popen.call_args[1].get("cwd") == tree
        # Каталог передаётся и внутрь помощника — он запускает обновление
        # своим Popen, и наследование там такое же.
        assert tree in mock_popen.call_args[0][0]
