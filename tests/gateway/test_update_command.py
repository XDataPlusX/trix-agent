...
class TestHandleUpdateCommand:
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting Trix Agent update" in result
...
class TestSendUpdateNotification:
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        # trix.update.finished_with_output de-brands the upstream literal
        # ("Hermes update finished") -- assert the success marker survives
        # and the upstream product name does not leak back in.
        assert "Update finished" in sent_text
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        # trix.update.finished de-brands the upstream literal ("Hermes
        # update finished") -- assert the success marker survives and the
        # upstream product name does not leak back in.
        assert "Update finished" in sent
