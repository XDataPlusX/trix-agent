...
class TestDebug:
    """Тексты запасного пути.

    С 2026-09-07 отчёт уходит файлом, и эти строки клиент видит только
    там, где приложить файл нечем: адаптера нет, платформа файлов не умеет
    или отправка сорвалась. Поэтому здесь адаптер отсутствует намеренно —
    сама доставка файлом проверяется в
    tests/gateway/test_debug_command_delivers_file.py.
    """

    def _run(self, monkeypatch, *, report_len):
        stub = _stub()
        stub._DEBUG_INLINE_REPORT_LIMIT = 3500
        stub._adapter_for_source = lambda source: None
        report = "x" * report_len
        monkeypatch.setattr(
