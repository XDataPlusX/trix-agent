...
class TestRecoveryDoesNotLeakMediaFragments:
    ...
    async def test_spaced_media_path_does_not_leak_fragment(self, monkeypatch, caplog):
        ...
        # Patch the partition entry point the dispatch loop actually calls —
        # returning it as unreachable also pins that the resulting
        # "couldn't deliver" notice names no file (it would otherwise leak the
        # very fragment this test forbids).
        monkeypatch.setattr(
            type(adapter),
            "partition_media_delivery_paths",
            staticmethod(lambda m: ([], list(m or []))),
        )
