...
@pytest.mark.asyncio
async def test_auto_registered_builtin_commands_use_localized_description_and_hint(adapter):
    """COMMAND_REGISTRY commands that aren't hand-registered above (e.g.
    /branch) go through the auto-register loop, which must feed
    _build_auto_slash_command the *localized* description/args_hint --
    not the raw registry English -- via command_description()/
    command_args_hint() (hermes_cli/commands.py). Nothing else in this
    file exercises that loop's description/hint values, so reverting
    those two call sites to cmd_def.description/.args_hint leaves the
    rest of the suite green."""
    from hermes_cli.commands import resolve_command

    branch_cmd = resolve_command("branch")
    assert branch_cmd is not None
    # Confirm /branch actually flows through the auto-register loop (i.e.
    # it isn't one of the hand-written @tree.command entries above).
    assert "branch" not in adapter._client.tree.commands

    with patch(
        "hermes_cli.commands.command_description",
        side_effect=lambda cmd: f"LOCALIZED-DESC-{cmd.name}",
    ) as mock_desc, patch(
        "hermes_cli.commands.command_args_hint",
        side_effect=lambda cmd: f"LOCALIZED-HINT-{cmd.name}" if cmd.args_hint else "",
    ) as mock_hint:
        adapter._register_slash_commands()

    # The helpers were actually invoked with the /branch CommandDef -- proves
    # the loop reads through the localization layer instead of the raw
    # registry fields.
    assert any(call.args[0].name == "branch" for call in mock_desc.call_args_list)
    assert any(call.args[0].name == "branch" for call in mock_hint.call_args_list)

    branch_entry = adapter._client.tree.commands["branch"]
    assert branch_entry.description == "LOCALIZED-DESC-branch"
