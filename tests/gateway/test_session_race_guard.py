...
@pytest.mark.asyncio
async def test_update_confirms_without_interrupt():
    """/update mid-run asks for confirmation, but still neither interrupts
    the running agent nor gets queued as pending text.

    The confirmation itself (both dispatch paths, and that a prior "always
    approve" on /new cannot waive it) lives in
    tests/gateway/test_update_confirmation.py. What this file still owns is
    the race-guard half: the command answers immediately and leaves the
    running agent alone.
    """
    runner = _make_runner()
    ...
    # Confirmation gate ran instead of the handler...
    runner._handle_update_command.assert_not_awaited()
    assert result is not None
    # ...and the running agent was left completely alone.
    fake_agent.interrupt.assert_not_called()
    assert session_key not in runner.adapters[Platform.TELEGRAM]._pending_messages
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # /update left this list when spec 12 Task 9a gave it a confirmation
        # step: mid-run it now returns the confirmation prompt instead of the
        # handler's own result, so the "handler was awaited" shape below no
        # longer fits. It still must not interrupt and must not be queued --
        # that half is pinned by test_update_confirms_without_interrupt below,
        # and the confirmation contract itself by
        # tests/gateway/test_update_confirmation.py.
        # /profile was the fourth case here until the client-command-surface
        # plan disabled it (hermes_cli.trix_menu.DISABLED_COMMANDS) -- a
        # single profile, its home directory meaning nothing to a client
        # with no file access. /disk replaces it as a bypass command that
        # is still client-facing: see its own comment in gateway/run.py
        # ("Free space is asked about precisely when something has ground
        # to a halt, so /disk must answer mid-run").
        ("/disk", "_handle_disk_command", "Disk text"),
    ],
)
