...
class TestWatchUpdateProgress:
    ...
    @pytest.mark.asyncio
    async def test_update_log_never_reaches_the_client(self, tmp_path, caplog):
        """Ход обновления клиенту не транслируется — только вердикт.

        Апстрим слал сюда сырой вывод `hermes update` кусками по 3500
        символов. Клиент читать его не должен (он поимённо называет
        Hermes и его пакеты, а решений по нему клиент не принимает), но
        и потерять лог нельзя: поддержка читает его в gateway.log.
        """
        runner = _make_runner()
        hermes_home = tmp_path / "hermes"
        hermes_home.mkdir()
        ...
        # Ровно одно сообщение клиенту — вердикт.
        assert mock_adapter.send.call_count == 1
        sent = mock_adapter.send.call_args_list[0].args[1]
        assert sent == "✅ Update finished."
        assert "Fetching updates" not in sent
        assert "Code updated" not in sent

        # …и тот же лог целиком лежит в журнале шлюза.
        logged = "\n".join(r.getMessage() for r in caplog.records)
        assert "Fetching updates" in logged
        assert "Code updated" in logged
