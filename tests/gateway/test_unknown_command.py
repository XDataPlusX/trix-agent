...
@pytest.mark.asyncio
async def test_egress_slash_command_answers_disabled_explanation(monkeypatch):
    """/egress used to reach its real proxy-status handler here; it is now
    in hermes_cli.trix_menu.DISABLED_COMMANDS (client-command-surface plan,
    decision "/egress — убрать": the proxy is off by default and the report
    was always the same uninformative string on a client machine). It must
    neither hit the unknown-command guard (it IS a known command) nor its
    old handler -- see tests/gateway/test_disabled_commands.py for the
    dedicated dispatch-layer coverage of that gate on both session states;
    this file only needs to stop asserting the old, now-incorrect,
    behavior."""
    runner = _make_runner()
    status_fn = MagicMock(return_value="Egress proxy status\nEnabled: no")
    monkeypatch.setattr("hermes_cli.proxy_cli.format_status_text", status_fn)

    result = await runner._handle_message(_make_event("/egress"))

    assert result is not None
    status_fn.assert_not_called()
    assert "Egress proxy status" not in result
    assert "Unknown command" not in result
    assert "Unknown command" not in result
