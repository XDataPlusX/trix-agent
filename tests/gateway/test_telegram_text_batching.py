...
def _make_event(text: str, chat_id: str = "12345", *, message_id: str | None = None) -> MessageEvent:
    return MessageEvent(
        text=text,
...
class TestMergedMessageIdTracking:
    """Spec 19, Ruling 5 — the flushed event must carry EVERY merged message
    id, not just the first, so a secret-capture reply that arrives as
    "here's the key" + the key itself (two Telegram updates merged into one
    event) can have both deleted instead of only the harmless first one."""

    @pytest.mark.asyncio
    async def test_single_message_batch_records_its_own_id(self):
        adapter = _make_adapter()
        adapter._enqueue_text_event(_make_event("the key itself", message_id="100"))

        await asyncio.sleep(0.2)

        dispatched = adapter.handle_message.call_args[0][0]
        assert dispatched.metadata["telegram_merged_message_ids"] == ["100"]

    @pytest.mark.asyncio
    async def test_two_message_batch_records_both_ids_in_order(self):
        adapter = _make_adapter()
        adapter._enqueue_text_event(_make_event("here's the key", message_id="100"))
        await asyncio.sleep(0.02)
        adapter._enqueue_text_event(_make_event("sk-abc123secret", message_id="101"))

        await asyncio.sleep(0.2)

        dispatched = adapter.handle_message.call_args[0][0]
        assert dispatched.metadata["telegram_merged_message_ids"] == ["100", "101"]
        # And the merged text still keeps the FIRST message's id as the
        # legacy single-id field — deletion code must not rely on this
        # alone (see the metadata list above).
        assert dispatched.message_id == "100"

    @pytest.mark.asyncio
    async def test_three_message_batch_records_all_three_ids(self):
        adapter = _make_adapter()
        adapter._enqueue_text_event(_make_event("chunk 1", message_id="1"))
        await asyncio.sleep(0.02)
        adapter._enqueue_text_event(_make_event("chunk 2", message_id="2"))
        await asyncio.sleep(0.02)
        adapter._enqueue_text_event(_make_event("chunk 3", message_id="3"))

        await asyncio.sleep(0.2)

        dispatched = adapter.handle_message.call_args[0][0]
        assert dispatched.metadata["telegram_merged_message_ids"] == ["1", "2", "3"]
