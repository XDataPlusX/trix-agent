...
def test_persist_dm_topic_thread_id_preserves_config_on_write_failure(tmp_path):
    ...
    # _persist_dm_topic_thread_id writes through
    # hermes_cli.config.atomic_config_write, which (since the setup-wizard
    # comment-preserving-write fix) routes through
    # utils.atomic_roundtrip_yaml_save -> ruamel's round-trip dumper, not
    # plain PyYAML yaml.dump. Patching "yaml.dump" here no longer intercepts
    # anything on that path — the write would silently succeed and this test
    # would pass without checking anything. Patch the dumper actually used.
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"HERMES_HOME": str(tmp_path / ".hermes")}), \
