...
class TestSlashCommands:
    ...
    @pytest.mark.asyncio
    async def test_verbose_is_disabled(self, adapter, platform):
        """/verbose is disabled for the client surface (client-command-
        surface plan Task 1: its config-gate is closed in this build, so it
        always answered "edit config.yaml" -- a dead end for a client with
        no file access). It must answer with an explanation, never the old
        mode-cycle output, and never the unknown-command guard (it IS a
        known, registered command -- just not a client-facing one)."""
        send = await send_and_capture(adapter, "/verbose", platform)

        send.assert_called_once()
        response_text = send.call_args[1].get("content") or send.call_args[0][1]
        assert "tool_progress" not in response_text
        assert "unknown command" not in response_text.lower()
    ...
    @pytest.mark.asyncio
    async def test_personality_is_disabled(self, adapter, platform):
        """/personality is disabled for the client surface (client-command-
        surface plan Task 1: ready-made personas don't fit a business
        product). It must answer with an explanation, never the old
        persona-listing output, and never the unknown-command guard."""
    ...
    @pytest.mark.asyncio
    async def test_yolo_is_disabled(self, adapter, platform):
        """/yolo is disabled for the client surface (client-command-surface
        plan Task 1 -- it removes dangerous-command approval with no second
        question, a protection spec 9 put there on purpose; see also
        tests/gateway/test_disabled_commands.py and
        test_running_agent_session_toggles.py for the dispatch-layer
        coverage on both session states). It must answer with an
        explanation, never toggle real YOLO state, and never the
        unknown-command guard."""
        send = await send_and_capture(adapter, "/yolo", platform)

        send.assert_called_once()
        response_text = send.call_args[1].get("content") or send.call_args[0][1]
        assert "yolo mode" not in response_text.lower()
        assert "unknown command" not in response_text.lower()
