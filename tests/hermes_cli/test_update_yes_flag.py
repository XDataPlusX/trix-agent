...
def _make_run_side_effect(
    ...
    default. ``branch`` still only feeds the "current branch" simulation --
    the guard itself doesn't read the branch (controller ruling, fix round
    1: only origin carries the security guarantee).
    """
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    sequence (e.g. a non-UTF-8 locale, or an embedded terminal). Two
    interactive update prompts call input() directly -- the config-
    migration prompt and the stash-restore prompt -- and each must fail
    safe (skip, don't crash) rather than let the exception escape and
    crash `hermes update` mid-flight.
    """
