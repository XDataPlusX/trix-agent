...
def test_prewarm_warms_the_active_custom_endpoint_for_the_next_open(monkeypatch):
    ...
    # This test's fakes cover the custom-endpoint probe (the #72762
    # regression under test); list_authenticated_providers() also
    # unconditionally resolves the curated "nous" model list, which isn't
    # this test's concern. Pin the manifest fetch to "unavailable" so that
    # resolution deterministically falls back to the static in-repo list
    # instead of reaching the real catalog network address.
    monkeypatch.setattr(
        model_catalog,
        "_fetch_manifest_with_fallback",
        lambda url, timeout, fallback_urls=(): None,
    )
