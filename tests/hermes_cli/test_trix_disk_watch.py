...
def _report(pct: float, *, removable=None, total_gb: int = 20) -> DiskReport:
    """Отчёт с заданной занятостью раздела. Числа сходятся между собой.

    Диск намеренно небольшой. Раньше стенд был на сто гигабайт, и «занято
    82 %» означало восемнадцать свободных — то есть не «мало места», а
    вполне спокойный диск. Тревога теперь смотрит и на запас (клиент
    прислал 2026-09-04 предупреждение при 84 свободных гигабайтах), и на
    таком стенде её попросту нет. Двадцать гигабайт при 82 % — это три с
    половиной свободных, и вот это уже тесно по-настоящему.
    """
    total = total_gb * GB
...
def watch_home(tmp_path, monkeypatch):
    ...
    # Свободные байты берутся тем же дешёвым вызовом и решают наравне с
    # процентом (тревоги нет, пока запаса вдоволь). Стенд обязан отвечать
    # согласованно с процентом, иначе он проверял бы не то: настоящий
    # временный каталог отдал бы свободное место МАШИНЫ, а не стенда.
    monkeypatch.setattr(
        td,
        "partition_free_bytes",
        lambda h: int(20 * GB * (100 - percent["cheap"]) / 100),
    )
    monkeypatch.setattr(
        td, "_wall_clock",
...
class TestTheCheapGate:
    ...
    def test_a_failed_cheap_probe_stays_quiet(self, watch_home, monkeypatch):
        ...
        monkeypatch.setattr(td, "partition_free_bytes", lambda h: None)
        sent = _Sent()
        _tick(monkeypatch, sent)
        assert sent.messages == []
        assert watch_home.scans == []
