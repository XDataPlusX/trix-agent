...
const rawScenario = process.argv[4];
const scenarioParts = rawScenario.split('|');
const scenario = scenarioParts[0];
const scenarioArg = scenarioParts[1] || '';

// Выбор строки в <select> так, как это делает человек: по ВИДИМОЙ
// подписи, а не по внутреннему значению опции. Значение с 2026-09-11 —
// идентичность строки каталога, а не имя бэкенда (две строки Firecrawl
// несут один бэкенд), и сценарий, который присваивает select.value имя
// бэкенда, просто молча не меняет выбор.
function pickOptionByLabel(select, label) {
  ...
  throw new Error('нет опции с подписью ' + label + ': ' +
    Array.prototype.map.call(select.options, function (o) { return o.textContent; }).join(' / '));
}
...
const dom = new JSDOM(html, {
  ...
  beforeParse(window) {
    ...
    // Опрос статуса входа по аккаунту тикает раз в три секунды. Ждать их
    // по-настоящему значило бы держать сценарий десятки секунд, поэтому
    // интервал перехватывается: сценарий сам решает, когда «прошло три
    // секунды». Это подмена ТАЙМЕРА, а не проверяемого кода — тело
    // обработчика исполняется настоящее.
    const realSetInterval = window.setInterval;
    window.__intervals = [];
    window.setInterval = function (fn, ms) {
      const handle = { fn: fn, ms: ms, cleared: false };
      window.__intervals.push(handle);
      return handle;
    };
    window.clearInterval = function (handle) {
      if (handle && typeof handle === 'object') handle.cleared = true;
      else if (handle) realSetInterval && void 0;
    };
  },
});
const { window } = dom;
...
// «Прошло три секунды»: исполнить тела всех незакрытых интервалов один
// раз. Возвращает, сколько их сработало.
async function tickDevicePoll() {
  const live = (window.__intervals || []).filter((h) => !h.cleared);
  live.forEach((h) => h.fn());
  await flush();
  return live.length;
}
...
async function scenarioInstallStagePendingForWebExtract() {
  ...
  pickOptionByLabel(select, 'Tavily');
  await flush();
...
async function scenarioFirecrawlDuplicateBackendSubmitsConsistentEnv() {
  ...
  // Какую из ДВУХ строк Firecrawl выбрать — приходит аргументом сценария.
  pickOptionByLabel(select, scenarioArg || 'Firecrawl');
  await flush();
  ...
  return {
    ...
    submittedSearchBackend: (submitReq.body && submitReq.body.search_backend) || null,
  };
}
...
// Запасной провайдер настраивается ТЕМ ЖЕ блоком, что и основной
// (владелец 2026-09-10). Сценарий проходит по запасному слоту руками, как
// клиент: раскрывает список, выбирает группу по её видимому имени, при
// необходимости выбирает способ подключения — и возвращает, что реально
// появилось на экране.
function slotRowByName(slotIds, name) {
  const rows = document.getElementById(slotIds.group).querySelectorAll('.p');
  for (const row of rows) {
    const b = row.querySelector('b');
    if (b && b.textContent === name) return row;
  }
  return null;
}

function openSlotPicker(slotIds) {
  const link = document.getElementById(slotIds.group).querySelector('a');
  if (link) link.click();
}

// Раскрыть список до конца: сначала «выбрать другого» (если слот свёрнут
// на одной строке), затем «Показать остальные N →».
function expandSlotPickerFully(slotIds) {
  for (let i = 0; i < 3; i++) {
    const link = document.getElementById(slotIds.group).querySelector('a');
    if (!link) return;
    link.click();
...
function slotGroupNames(slotIds) {
  return Array.from(document.getElementById(slotIds.group).querySelectorAll('.p'))
    .map((row) => (row.querySelector('b') || {}).textContent);
}

const FALLBACK_IDS = {
  block: 'fallback-block', group: 'fallback_group', api_key_block: 'fallback-api-key-block',
  api_key: 'fallback_api_key', base_url: 'fallback_base_url', base_url_details: 'fallback-base-url-details',
  model: 'fallback_model', device_block: 'fallback-device-code-block',
  device_start: 'fallback-device-login-start', auth_options: 'fallback-auth-options',
  key_verdict: 'fallback-key-verdict', current_hint: 'fallback-current-hint',
};
const PROVIDER_IDS = { group: 'provider_group' };

// Основной слот, от пустого состояния до payload. Заменяет пачку
// тестов, которые раньше проверяли ЭТО ЖЕ чтением исходника страницы
// регекспами (запрещено CLAUDE.md: такой тест зелен на сломанной
// реализации и красен на честном рефакторинге — ровно это и случилось,
// когда блок провайдера стал общим для двух слотов).
async function scenarioPrimarySlotEndToEnd() {
  await boot();
  const ids = { group: 'provider_group', auth_choice: 'provider-auth-choice',
    auth_options: 'provider-auth-options', api_key_block: 'provider-api-key-block',
    api_key: 'provider_api_key', base_url: 'provider_base_url', model: 'provider_model',
    device_block: 'provider-device-code-block', key_verdict: 'key-verdict' };

  // 1. Ничего не выбрано: ни одной подсвеченной строки.
  const preselected = document.getElementById(ids.group).querySelectorAll('.p.sel').length;
  // 2. Рекомендованные сверху, остальные — за ссылкой.
  const beforeExpand = slotGroupNames(ids).length;
  const moreLink = document.getElementById(ids.group).querySelector('a');
  const moreLinkText = moreLink ? moreLink.textContent : null;
  if (moreLink) moreLink.click();
  await flush();
  const afterExpand = slotGroupNames(ids).length;

  // 3. Выбор группы с одним вариантом подключения не показывает радио.
  const target = scenarioArg || 'DeepSeek';
  const row = slotRowByName(ids, target);
  if (!row) throw new Error('нет группы ' + target + ' среди ' + slotGroupNames(ids).join(' / '));
  row.click();
  await flush();
  const singleVariantAuthHidden = document.getElementById(ids.auth_choice).hidden;
  const apiBlockHidden = document.getElementById(ids.api_key_block).hidden;

  // 4. Ключ с пробелами по краям + автопроверка по change.
  const key = document.getElementById(ids.api_key);
  key.value = '  sk-primary-key  ';
  key.dispatchEvent(new window.Event('change', { bubbles: true }));
  await flush();
  const keyReq = takePending('/api/check/key', 'POST');
  const keyCheckSentValue = keyReq ? keyReq.body.value : null;
  if (keyReq) { keyReq.resolve(resOk({ ok: true, reachable: true, checked: true })); await flush(); await flush(); }
  const modelsReq = takePending('/api/models', 'POST');
  if (modelsReq) { modelsReq.resolve(resOk({ models: ['deepseek-chat'] })); await flush(); }
  const verdictHiddenAfterCheck = document.getElementById(ids.key_verdict).hidden;

  // 5. Переключение провайдера чистит ключ и вердикт.
  const otherLink = document.getElementById(ids.group).querySelector('a');
  if (otherLink) otherLink.click();
  await flush();
  const other = slotGroupNames(ids).filter((n) => n && n !== target)[0];
  slotRowByName(ids, other).click();
  await flush();
  const authOptions = document.getElementById(ids.auth_options).querySelectorAll('.p');
  if (authOptions.length) { authOptions[0].click(); await flush(); }
  const keyAfterSwitch = document.getElementById(ids.api_key).value;
  const verdictHiddenAfterSwitch = document.getElementById(ids.key_verdict).hidden;

  // 6. Вернуться на исходную группу и отправить.
  const backLink = document.getElementById(ids.group).querySelector('a');
  if (backLink) backLink.click();
  await flush();
  slotRowByName(ids, target).click();
  await flush();
  document.getElementById(ids.api_key).value = '  sk-primary-key  ';
  document.getElementById('telegram_token').value = '  123:TOKEN  ';
  document.getElementById('done').click();
  await flush();
  const submitReq = takePending('/api/submit', 'POST');
  if (!submitReq) throw new Error('expected /api/submit');
  ...
  return {
    ...
    singleVariantAuthHidden: singleVariantAuthHidden,
    apiBlockHidden: apiBlockHidden,
    keyCheckSentValue: keyCheckSentValue,
    verdictHiddenAfterCheck: verdictHiddenAfterCheck,
    keyAfterSwitch: keyAfterSwitch,
    verdictHiddenAfterSwitch: verdictHiddenAfterSwitch,
    submittedProvider: submitReq.body.provider,
    submittedTelegramToken: submitReq.body.telegram_token,
  };
}

// Вариант со входом по аккаунту у ОСНОВНОГО слота: что уезжает в payload
// (ни ключа, ни адреса — вход уже произошёл), и что опрос статуса не
// принимает чужой ответ.
async function scenarioPrimaryDeviceCodePayload() {
  await boot();
  const ids = { group: 'provider_group', auth_options: 'provider-auth-options',
    api_key: 'provider_api_key', base_url: 'provider_base_url',
    device_block: 'provider-device-code-block', api_key_block: 'provider-api-key-block',
    model_device: 'provider_model_device', device_status: 'device-login-status',
    err_device: 'err_device_login' };
  expandSlotPickerFully(ids);
  await flush();

  // Сначала выбрать вариант С КЛЮЧОМ и напечатать в него — чтобы поймать
  // ровно тот дефект, ради которого ветка device_code чистит поля.
  const apiGroup = scenarioArg ? scenarioArg.split(',')[1] : '';
  if (apiGroup) {
    const r = slotRowByName(ids, apiGroup);
    if (r) {
      r.click(); await flush();
      const opts = document.getElementById(ids.auth_options).querySelectorAll('.p');
      if (opts.length) { opts[0].click(); await flush(); }
      document.getElementById(ids.api_key).value = 'sk-stale';
      document.getElementById(ids.base_url).value = 'https://stale.example';
      expandSlotPickerFully(ids); await flush();
    }
  }

  const deviceGroup = scenarioArg ? scenarioArg.split(',')[0] : 'OpenAI (ChatGPT)';
  const row = slotRowByName(ids, deviceGroup);
  if (!row) throw new Error('нет группы ' + deviceGroup);
  row.click();
  await flush();
  const opts = document.getElementById(ids.auth_options).querySelectorAll('.p');
  for (const o of opts) {
    const label = (o.querySelector('b') || {}).textContent || '';
    if (/аккаунт|подписк/i.test(label)) { o.click(); break; }
  }
  await flush();
  const deviceBlockHidden = document.getElementById(ids.device_block).hidden;
  const apiBlockHidden = document.getElementById(ids.api_key_block).hidden;

  // Запустить вход и ответить статусом ЧУЖОГО входа — его обязаны
  // проигнорировать.
  document.getElementById('device-login-start').click();
  await flush();
  const startReq = takePending('/api/device/start', 'POST');
  if (startReq) {
    startReq.resolve(resOk({ verification_url: 'https://auth.example/device', user_code: 'ABCD-1234', login_id: 'mine' }));
    await flush();
  }
  const statusAfterStart = document.getElementById(ids.device_status).textContent;

  // Ответ статуса от ЧУЖОГО входа (другой login_id) обязан быть
  // проигнорирован: иначе успех чужой сессии покрасил бы наш блок.
  await tickDevicePoll();
  const statusReq = takePending('/api/device/status', 'GET');
  if (statusReq) {
    statusReq.resolve(resOk({ state: 'ok', login_id: 'someone-else', provider: 'other' }));
    await flush();
    await flush();
  }
  const statusAfterForeignOk = document.getElementById(ids.device_status).textContent;
  const modelBlockHiddenAfterForeignOk = document.getElementById('device-model-block').hidden;
  ...
  return {
    deviceBlockHidden: deviceBlockHidden,
    apiBlockHidden: apiBlockHidden,
    statusAfterStart: statusAfterStart,
    statusAfterForeignOk: statusAfterForeignOk,
    modelBlockHiddenAfterForeignOk: modelBlockHiddenAfterForeignOk,
    foreignStatusAnswered: !!statusReq,
    submittedProvider: submitReq.body.provider,
  };
}

// Свой OpenAI-совместимый эндпоинт: клиент вставляет адрес и ключ.
// Проходим блок руками и смотрим, что отрисовалось, куда ушла проверка
// ключа и что уехало в payload.
async function scenarioCustomEndpoint() {
  await boot();
  const ids = { group: 'provider_group', auth_options: 'provider-auth-options',
    api_key: 'provider_api_key', base_url: 'provider_base_url',
    base_url_details: 'provider-base-url-details', model: 'provider_model',
    model_custom: 'provider_model_custom', key_verdict: 'key-verdict',
    api_key_block: 'provider-api-key-block' };
  expandSlotPickerFully(ids);
  await flush();

  const row = slotRowByName(ids, scenarioArg || 'Свой сервер (OpenAI-совместимый)');
  if (!row) throw new Error('нет строки своего сервера среди ' + slotGroupNames(ids).join(' / '));
  row.click();
  await flush();
  const opts = document.getElementById(ids.auth_options).querySelectorAll('.p');
  if (opts.length) { opts[0].click(); await flush(); }

  // Поле адреса обязано быть раскрыто сразу, а не спрятано за «изменить».
  const details = document.getElementById(ids.base_url_details);
  const modelFirstOption = document.getElementById(ids.model).options[0].textContent;

  // Клиент печатает ключ ПЕРВЫМ (поле выше) — проверка уходит без адреса.
  const key = document.getElementById(ids.api_key);
  key.value = 'sk-my-endpoint';
  key.dispatchEvent(new window.Event('change', { bubbles: true }));
  await flush();
  const earlyReq = takePending('/api/check/key', 'POST');
  const earlyBody = earlyReq ? earlyReq.body : null;
  if (earlyReq) { earlyReq.resolve(resOk({ ok: true, checked: false })); await flush(); await flush(); }

  // Теперь адрес — и проверка обязана перезапуститься сама.
  const baseUrl = document.getElementById(ids.base_url);
  baseUrl.value = 'https://llm.example.org/v1';
  baseUrl.dispatchEvent(new window.Event('change', { bubbles: true }));
  await flush();
  const secondReq = takePending('/api/check/key', 'POST');
  const secondBody = secondReq ? secondReq.body : null;
  if (secondReq) {
    // Ровно тот ответ, который отдаёт наш зонд при недоступном сервере
    // клиента: проверка выполнена, но достучаться не удалось.
    secondReq.resolve(resOk({ ok: true, checked: true, reachable: false, message: '' }));
    await flush(); await flush();
  }
  const unreachableVerdict = document.getElementById(ids.key_verdict).textContent;

  // И теперь успешный ответ — оба текста в одном сценарии.
  baseUrl.dispatchEvent(new window.Event('change', { bubbles: true }));
  await flush();
  const thirdReq = takePending('/api/check/key', 'POST');
  if (thirdReq) {
    thirdReq.resolve(resOk({ ok: true, checked: true, reachable: true }));
    await flush(); await flush();
  }
  const okVerdict = document.getElementById(ids.key_verdict).textContent;
  // Список моделей уходит следом за проверкой ключа; на этом тике их
  // может оказаться два (первый — от ранней проверки без адреса), берём
  // последний: он и несёт введённый адрес.
  let modelsBody = null;
  for (let req = takePending('/api/models', 'POST'); req; req = takePending('/api/models', 'POST')) {
    modelsBody = req.body;
    req.resolve(resOk({ models: ['my-model-7b'] }));
    await flush();
  }

  const model = document.getElementById(ids.model);
  for (let i = 0; i < model.options.length; i++) {
    if (model.options[i].value === 'my-model-7b') { model.value = 'my-model-7b'; break; }
  }
  ...
  return {
    baseUrlOpen: details ? details.open : null,
    baseUrlSummary: details ? details.querySelector('summary').textContent : null,
    modelFirstOption: modelFirstOption,
    earlyCheckBaseUrl: earlyBody ? earlyBody.base_url : null,
    earlyCheckEnvVar: earlyBody ? earlyBody.env_var : null,
    secondCheckFired: !!secondReq,
    secondCheckBaseUrl: secondBody ? secondBody.base_url : null,
    verdictHidden: document.getElementById(ids.key_verdict).hidden,
    unreachableVerdict: unreachableVerdict,
    okVerdict: okVerdict,
    modelsRequestBaseUrl: modelsBody ? modelsBody.base_url : null,
    modelOptions: Array.from(model.options).map((o) => o.value),
    submittedProvider: submitReq.body.provider,
  };
}

async function scenarioFallbackSlotParity() {
  await boot();
  // Запасной слот стартует свёрнутым на строке «Не настраивать».
  const collapsed = slotGroupNames(FALLBACK_IDS);
  expandSlotPickerFully(FALLBACK_IDS);
  await flush();
  const expanded = slotGroupNames(FALLBACK_IDS);

  // Тот же список групп, что и у основного слота, плюс строка отказа.
  expandSlotPickerFully(PROVIDER_IDS);
  await flush();
  const primaryNames = slotGroupNames(PROVIDER_IDS);

  const target = scenarioArg || 'Z.AI (GLM)';
  const row = slotRowByName(FALLBACK_IDS, target);
  if (!row) throw new Error('нет строки ' + target + ' среди ' + expanded.join(' / '));
  row.click();
  await flush();

  // Если у группы несколько способов подключения — берём первый.
  const authOptions = document.getElementById(FALLBACK_IDS.auth_options).querySelectorAll('.p');
  if (authOptions.length) { authOptions[0].click(); await flush(); }

  const apiBlock = document.getElementById(FALLBACK_IDS.api_key_block);
  const deviceBlock = document.getElementById(FALLBACK_IDS.device_block);
  const baseUrl = document.getElementById(FALLBACK_IDS.base_url);
  const model = document.getElementById(FALLBACK_IDS.model);
  return {
    collapsedRows: collapsed,
    expandedRows: expanded,
    primaryRows: primaryNames,
    apiBlockHidden: apiBlock.hidden,
    deviceBlockHidden: deviceBlock.hidden,
    baseUrlValue: baseUrl ? baseUrl.value : null,
    baseUrlEditable: !!baseUrl && !baseUrl.disabled && baseUrl.type === 'text',
    modelOptionCount: model ? model.options.length : 0,
    blockText: document.getElementById(FALLBACK_IDS.block).textContent,
  };
}

// Тот же слот — до конца: ключ, автопроверка, список моделей, правка
// адреса и сборка payload. Это и есть «всё так же, как в первом разе».
async function scenarioFallbackSlotSubmits() {
  await boot();
  openSlotPicker(FALLBACK_IDS);
  await flush();
  const row = slotRowByName(FALLBACK_IDS, scenarioArg || 'Z.AI (GLM)');
  if (!row) throw new Error('нет строки запасного провайдера');
  row.click();
  await flush();
  const authOptions = document.getElementById(FALLBACK_IDS.auth_options).querySelectorAll('.p');
  if (authOptions.length) { authOptions[0].click(); await flush(); }

  const key = document.getElementById(FALLBACK_IDS.api_key);
  key.value = 'sk-fallback-key';
  key.dispatchEvent(new window.Event('change', { bubbles: true }));
  await flush();

  const keyReq = takePending('/api/check/key', 'POST');
  const keyCheckEnvVar = keyReq ? keyReq.body.env_var : null;
  if (keyReq) {
    keyReq.resolve(resOk({ ok: true, reachable: true, checked: true }));
    await flush();
    await flush();
  }
  const modelsReq = takePending('/api/models', 'POST');
  const modelsRequestedFor = modelsReq ? modelsReq.body.provider : null;
  if (modelsReq) {
    modelsReq.resolve(resOk({ models: ['glm-4.6', 'glm-4.5-air'] }));
    await flush();
  }
  ...
  for (let i = 0; i < model.options.length; i++) {
    if (model.options[i].value === 'glm-4.6') { model.value = 'glm-4.6'; modelPicked = 'glm-4.6'; break; }
  }
  model.dispatchEvent(new window.Event('change', { bubbles: true }));

  const baseUrl = document.getElementById(FALLBACK_IDS.base_url);
  const defaultBaseUrl = baseUrl.value;
  baseUrl.value = 'https://api.z.ai/api/coding/paas/v4';
  ...
  return {
    keyCheckEnvVar: keyCheckEnvVar,
    modelsRequestedFor: modelsRequestedFor,
    modelOptions: Array.from(model.options).map((o) => o.value),
    modelPicked: modelPicked,
    defaultBaseUrl: defaultBaseUrl,
    verdictHidden: document.getElementById(FALLBACK_IDS.key_verdict).hidden,
    fallback: submitReq.body.fallback,
    provider: submitReq.body.provider,
  };
}

// Вход по аккаунту для ЗАПАСНОГО слота — то, чего в мастере не было
// вовсе: строка device_code в списке запасного пряталась именно потому,
// что завершить вход было негде.
async function scenarioFallbackDeviceLogin() {
  await boot();
  openSlotPicker(FALLBACK_IDS);
  await flush();
  const row = slotRowByName(FALLBACK_IDS, scenarioArg || 'OpenAI (ChatGPT)');
  if (!row) throw new Error('нет строки ' + scenarioArg + ' в списке запасного');
  row.click();
  await flush();
  const authOptions = document.getElementById(FALLBACK_IDS.auth_options).querySelectorAll('.p');
  let deviceVariantPicked = false;
  for (const opt of authOptions) {
    const label = (opt.querySelector('b') || {}).textContent || '';
    if (/аккаунт|подписк/i.test(label)) { opt.click(); deviceVariantPicked = true; break; }
  }
  if (!deviceVariantPicked && authOptions.length) { authOptions[0].click(); }
  await flush();

  const deviceBlock = document.getElementById(FALLBACK_IDS.device_block);
  const startBtn = document.getElementById(FALLBACK_IDS.device_start);
  startBtn.click();
  await flush();
  const startReq = takePending('/api/device/start', 'POST');
  ...
  return {
    deviceBlockHidden: deviceBlock.hidden,
    deviceStartRequestedFor: startReq ? startReq.body.provider : null,
    primaryDeviceBlockHidden: document.getElementById('provider-device-code-block').hidden,
  };
}
...
async function scenarioTimezoneReachesThePayload() {
  ...
  return { timezone: submitReq.body.timezone };
    ...
    else if (scenario === 'primary_slot_end_to_end') result = await scenarioPrimarySlotEndToEnd();
    else if (scenario === 'primary_device_code_payload') result = await scenarioPrimaryDeviceCodePayload();
    else if (scenario === 'custom_endpoint') result = await scenarioCustomEndpoint();
    else if (scenario === 'fallback_slot_parity') result = await scenarioFallbackSlotParity();
    else if (scenario === 'fallback_slot_submits') result = await scenarioFallbackSlotSubmits();
    else if (scenario === 'fallback_device_login') result = await scenarioFallbackDeviceLogin();
...
@pytest.mark.parametrize(
    "row_label,expected_field_type,expected_env",
    [
        ("Firecrawl", "password", "FIRECRAWL_API_KEY"),
        ("Firecrawl Self-Hosted", "text", "FIRECRAWL_API_URL"),
    ],
)
def test_firecrawl_duplicate_web_backend_submits_the_rendered_rows_env(
    logged_in, tmp_path, row_label, expected_field_type, expected_env
):
    """Две строки каталога с одним ``web_backend`` ("firecrawl") — облачная
    и self-hosted.

    Finding 11 (ревью 2026-08-26) закрыл первую половину: показанное поле
    и отправляемая переменная обязаны совпадать. Вторая половина осталась
    открытой до 2026-09-11 и нашлась владельцем на живой машине: пока
    значением <option> было имя бэкенда, обе строки давали ОДИНАКОВЫЙ
    value, `select.value` отдавал первую, и выбрать вторую было нельзя
    физически. Теперь значение — идентичность строки, и тест выбирает
    строку так же, как клиент: по видимой подписи.

    Проверяется обе стороны сразу: облачная строка даёт поле-пароль и
    FIRECRAWL_API_KEY, self-hosted — текстовое поле адреса и
    FIRECRAWL_API_URL; наружу обе отдают один и тот же ``firecrawl``."""
    from hermes_cli.setup_wizard.page import render_page
    ...
    out = _run_scenario(
        tmp_path, html, catalog, "firecrawl_duplicate_backend_consistent_env|" + row_label
    )
    assert out["renderedFieldType"] == expected_field_type, out
    assert out["submittedSearchEnvKey"] == expected_env, out
    assert out["submittedSearchBackend"] == "firecrawl", out
...
@requires_jsdom
def test_fallback_slot_expands_exactly_like_the_primary_one(logged_in, tmp_path):
    """Требование владельца 2026-09-10, дословно: «выбираешь запасного
    провайдера, и там всё так же раскрывается, как и в первом разе».

    До этого запасной слот был одиноким <select> с плоским списком имён
    провайдеров плюс полем ключа: ни описаний, ни выбора модели, ни
    адреса API, ни автопроверки. Упор, на котором это нашлось: основным
    выбран ChatGPT, запасным нужен GLM, а у GLM два варианта с РАЗНЫМИ
    адресами — и поменять адрес было негде.

    Сценарий проходит блок руками, как клиент, и смотрит на реально
    отрисованный DOM."""
    from hermes_cli.setup_wizard.page import render_page

    html = render_page()
    catalog = logged_in.get("/api/form").json()
    group_names = [g["display_name"] for g in catalog["provider_groups"]]
    target = next((n for n in group_names if "GLM" in n or "Z.AI" in n), None)
    if target is None:
        pytest.skip("в живом каталоге нет группы GLM/Z.AI")

    out = _run_scenario(tmp_path, html, catalog, "fallback_slot_parity|" + target)

    # Стартовое состояние: одна строка отказа, список свёрнут.
    assert out["collapsedRows"] == ["Не настраивать"], out["collapsedRows"]
    # Раскрытый список запасного — те же группы, что и у основного, плюс
    # строка отказа. Именно «те же», а не «похожие»: один каталог.
    # Запасной слот = основной минус строки со своим адресом эндпоинта,
    # плюс собственная строка отказа. Свой сервер в запасном не
    # предлагается намеренно: его ключ живёт в model.api_key, одном на
    # конфиг, и второму адресу там места нет.
    endpoint_groups = {
        g["display_name"]
        for g in catalog["provider_groups"]
        if any(v.get("endpoint_from_client") for v in g["variants"])
    }
    assert endpoint_groups, "в каталоге не стало строки со своим эндпоинтом — проверка обесценилась"
    assert set(out["expandedRows"]) == (
        set(out["primaryRows"]) - endpoint_groups
    ) | {"Не настраивать"}, out
    assert endpoint_groups <= set(out["primaryRows"]), out
    assert not (endpoint_groups & set(out["expandedRows"])), out
    assert len(out["primaryRows"]) > 5, out["primaryRows"]

    # Выбранная группа раскрывает тот же под-блок, что и у основного.
    assert out["apiBlockHidden"] is False, out
    assert out["baseUrlEditable"] is True, out
    assert out["baseUrlValue"], out
    assert out["modelOptionCount"] >= 2, out


@requires_jsdom
def test_fallback_slot_checks_its_key_loads_models_and_submits_its_own_base_url(logged_in, tmp_path):
    """Второй конец того же требования: у запасного слота работают
    автопроверка ключа, живой список моделей и правка адреса API, и всё
    три доезжают до payload.

    Раньше форма слала для запасного ``base_url: ""`` и ``model: ""``
    ЖЁСТКО, хотя сервер обе величины принимает и пишет в
    ``fallback_providers``. Здесь проверяется именно отправленное тело."""
    from hermes_cli.setup_wizard.page import render_page

    html = render_page()
    catalog = logged_in.get("/api/form").json()
    target = next(
        (g["display_name"] for g in catalog["provider_groups"]
         if "GLM" in g["display_name"] or "Z.AI" in g["display_name"]),
        None,
    )
    if target is None:
        pytest.skip("в живом каталоге нет группы GLM/Z.AI")

    out = _run_scenario(tmp_path, html, catalog, "fallback_slot_submits|" + target)

    # Ключ запасного проверяется своей переменной окружения, а не чужой.
    assert out["keyCheckEnvVar"], out
    assert out["keyCheckEnvVar"] != "OPENAI_API_KEY", out
    # Список моделей запрошен для ЗАПАСНОГО провайдера.
    assert out["modelsRequestedFor"], out
    assert "glm-4.6" in out["modelOptions"], out
    assert out["verdictHidden"] is False, out

    fallback = out["fallback"]
    assert fallback and fallback["name"], out
    assert fallback["model"] == "glm-4.6", out
    assert fallback["base_url"] == "https://api.z.ai/api/coding/paas/v4", out
    assert fallback["api_key"] == "sk-fallback-key", out
    # Основной слот при этом не затронут ничем из перечисленного.
    assert out["provider"]["api_key"] == "", out


@requires_jsdom
def test_fallback_slot_can_log_in_by_account(logged_in, tmp_path):
    """Вход по аккаунту (ChatGPT, MiniMax OAuth) для ЗАПАСНОГО слота.

    Раньше эти провайдеры из списка запасного вырезались, и в блоке
    стояла честная сноска: «вход по аккаунту мастер для запасного пока не
    настраивает». Причина была ровно одна — на экране негде было
    завершить вход. Теперь под-блок у слота свой, и сноска снята.

    Проверяется, что кнопка запускает вход ИМЕННО для запасного
    провайдера, а под-блок основного слота при этом не раскрывается."""
    from hermes_cli.setup_wizard.page import render_page

    html = render_page()
    catalog = logged_in.get("/api/form").json()
    device_names = {p["name"] for p in catalog["providers"] if p["kind"] != "api_key"}
    if not device_names:
        pytest.skip("в живом каталоге нет провайдера со входом по аккаунту")
    group = next(
        (g["display_name"] for g in catalog["provider_groups"]
         if any(v["name"] in device_names for v in g["variants"])),
        None,
    )
    if group is None:
        pytest.skip("ни одна группа не содержит варианта со входом по аккаунту")

    out = _run_scenario(tmp_path, html, catalog, "fallback_device_login|" + group)

    assert out["deviceBlockHidden"] is False, out
    assert out["deviceStartRequestedFor"] in device_names, out
    assert out["primaryDeviceBlockHidden"] is True, out
...
# ---------------------------------------------------------------------------
# Блок провайдера — ИСПОЛНЕНИЕМ
#
# Эти проверки раньше жили в test_setup_wizard_page.py регекспами по
# исходнику страницы ("assert 'name: providerRow ? providerRow.name' in
# html" и подобное). CLAUDE.md такие тесты запрещает прямо, и перевод
# блока провайдера на общий для двух слотов код показал почему: ни одна
# из них не заметила бы сломанного поведения, зато все восемнадцать
# покраснели на честном рефакторинге с неизменным поведением.
# ---------------------------------------------------------------------------


@requires_jsdom
def test_primary_provider_slot_from_empty_to_payload(logged_in, tmp_path):
    """Основной слот целиком: ничего не выбрано заранее, рекомендованные
    сверху и «показать остальные» за ссылкой, группа с одним способом
    подключения не показывает радио, ключ обрезается от пробелов и
    проверяется сам, переключение провайдера чистит и ключ, и вердикт, а
    в payload уезжает имя выбранного варианта."""
    from hermes_cli.setup_wizard.page import render_page
    ...
    single = next(
        (g["display_name"] for g in catalog["provider_groups"]
         if g.get("recommended") and len(g["variants"]) == 1),
        None,
    )
    if single is None:
        pytest.skip("в живом каталоге нет рекомендованной группы с одним вариантом")

    out = _run_scenario(tmp_path, html, catalog, "primary_slot_end_to_end|" + single)

    assert out["preselectedRows"] == 0, out
    assert "Показать остальные" in (out["moreLinkText"] or ""), out
    assert out["afterExpand"] > out["beforeExpand"], out
    assert out["singleVariantAuthHidden"] is True, out
    assert out["apiBlockHidden"] is False, out
    # Ключ уходит на проверку обрезанным — ровно тем, что потом сохранится.
    assert out["keyCheckSentValue"] == "sk-primary-key", out
    assert out["verdictHiddenAfterCheck"] is False, out
    # Смена провайдера: чужой ключ и чужой вердикт не переезжают.
    assert out["keyAfterSwitch"] == "", out
    assert out["verdictHiddenAfterSwitch"] is True, out

    provider = out["submittedProvider"]
    assert provider["name"], out
    assert provider["api_key"] == "sk-primary-key", out
    assert provider["env_var"], out
    # Токен бота обрезается тем же правилом, что и ключи.
    assert out["submittedTelegramToken"] == "123:TOKEN", out


@requires_jsdom
def test_primary_device_code_variant_submits_no_key_and_no_base_url(logged_in, tmp_path):
    """Вариант со входом по аккаунту: поля ключа и адреса лежат в скрытом
    под-блоке, и остаток от ранее выбранного варианта С КЛЮЧОМ не должен
    уехать в payload (вход уже произошёл через /api/device/*, а адрес
    сервер подставляет сам)."""
    from hermes_cli.setup_wizard.page import render_page
    ...
    api_group = next(
        (g["display_name"] for g in catalog["provider_groups"]
         if g["display_name"] != device_group
         and all(v["name"] not in device_names for v in g["variants"])),
        None,
    )
    if device_group is None or api_group is None:
        pytest.skip("нужны обе группы: со входом по аккаунту и по ключу")

    out = _run_scenario(
        tmp_path, html, catalog, "primary_device_code_payload|%s,%s" % (device_group, api_group)
    )

    assert out["deviceBlockHidden"] is False, out
    assert out["apiBlockHidden"] is True, out
    assert "Ожидаем" in out["statusAfterStart"], out

    # Ответ статуса ЧУЖОГО входа (другой login_id, другой провайдер) не
    # должен покрасить наш блок успехом. Опрос при этом реально идёт —
    # иначе проверка была бы пустой.
    assert out["foreignStatusAnswered"] is True, out
    assert out["statusAfterForeignOk"] == out["statusAfterStart"], out
    assert out["modelBlockHiddenAfterForeignOk"] is True, out

    provider = out["submittedProvider"]
    assert provider["name"] in device_names, out
    assert provider["api_key"] == "", out
    assert provider["base_url"] == "", out


@requires_jsdom
def test_custom_endpoint_row_asks_for_address_and_probes_the_key_against_it(
    logged_in, tmp_path
):
    """Свой OpenAI-совместимый эндпоинт: вставил адрес и ключ — работает.

    Три вещи, которые здесь легко сделать неправильно, и каждая ломает
    ровно тот сценарий, ради которого строку и завели:

    1. Адрес спрятан за «изменить». У остальных провайдеров он
       подставляется из каталога и правится редко; здесь подставлять
       нечего, и спрятанное поле — это спрятанная половина настройки.
    2. «по умолчанию» в списке моделей. У своего сервера каталога моделей
       нет вовсе, подставить нечего, и сервер такую отправку не примет —
       первая опция обязана требовать выбор, а не обещать умолчание.
    3. Ключ проверяется по АДРЕСУ. Поле ключа стоит выше поля адреса, то
       есть естественный порядок ввода — сначала ключ. Первая проверка
       уходит без адреса и возвращается «не проверяли»; если правка
       адреса не перезапускает её, клиент не узнает об опечатке в ключе
       никогда — до первого молчания бота."""
    from hermes_cli.setup_wizard.page import render_page
    ...
    if group is None:
        pytest.skip("в каталоге нет провайдера со своим адресом эндпоинта")

    out = _run_scenario(tmp_path, html, catalog, "custom_endpoint|" + group)

    # 1. Адрес раскрыт сразу и подписан как обязательное поле.
    assert out["baseUrlOpen"] is True, out
    assert "вашего сервера" in (out["baseUrlSummary"] or ""), out

    # 2. Умолчания нет — есть требование выбрать.
    assert "по умолчанию" not in (out["modelFirstOption"] or ""), out
    assert "выберите модель" in (out["modelFirstOption"] or "").lower(), out

    # 3. Первая проверка ушла без адреса (его ещё не ввели) и без
    #    переменной окружения — у своего сервера её нет.
    assert out["earlyCheckEnvVar"] is None, out
    assert out["earlyCheckBaseUrl"] == "", out
    # ...а правка адреса перезапустила проверку, теперь уже с адресом.
    assert out["secondCheckFired"] is True, out
    assert out["secondCheckBaseUrl"] == "https://llm.example.org/v1", out
    assert out["verdictHidden"] is False, out
    # Недоступный сервер клиента — честный текст про АДРЕС, а не общее
    # «провайдер не проверяется автоматически»: проверка существует и была
    # выполнена, не ответил чужой сервер.
    assert "не ответил" in out["unreachableVerdict"], out["unreachableVerdict"]
    assert "адрес" in out["unreachableVerdict"].lower(), out["unreachableVerdict"]
    assert "не проверяется автоматически" not in out["unreachableVerdict"], out
    assert "Ключ принят" in out["okVerdict"], out["okVerdict"]

    # Список моделей запрашивается у самого эндпоинта клиента.
    assert out["modelsRequestBaseUrl"] == "https://llm.example.org/v1", out
    assert "my-model-7b" in out["modelOptions"], out

    provider = out["submittedProvider"]
    assert provider["name"] == "custom", out
    assert provider["env_var"] is None, out
    assert provider["api_key"] == "sk-my-endpoint", out
    assert provider["base_url"] == "https://llm.example.org/v1", out
    assert provider["model"] == "my-model-7b", out
