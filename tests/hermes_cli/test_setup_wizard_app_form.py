...
def test_check_key_forwards_the_form_proxy(logged_in, monkeypatch):
    ...
    assert r.json() == {"ok": True, "checked": True}
    # Четвёртый аргумент — адрес своего эндпоинта; для провайдера с
    # известной переменной он всегда None (см. маршрут: адрес принимается
    # только в паре с env_var=None).
    assert calls == [("OPENROUTER_API_KEY", "k", "http://h:8080", None)]
...
def test_form_extract_env_picks_the_row_whose_key_is_actually_set(tmp_path, monkeypatch):
    """Две строки каталога несут один и тот же ``web_backend: firecrawl``
    — «Firecrawl Self-Hosted» (FIRECRAWL_API_URL) и «Firecrawl» (облако,
    FIRECRAWL_API_KEY). В config.yaml лежит только имя бэкенда, поэтому
    какая из строк была выбрана, решает сохранённая переменная.

    Раньше сервер брал ПЕРВУЮ строку с таким бэкендом, и возвращающийся
    клиент с облачным ключом получал в мастере поле «Адрес сервера»
    вместо поля ключа. Строка self-hosted видна только когда на машине
    что-то слушает 3002 — поэтому поднимаем пробу живости, иначе обеих
    строк одновременно не бывает и дефект не воспроизводится."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_constants import get_hermes_home
    from hermes_cli.config import save_env_value_secure
    ...
    monkeypatch.setattr(
        tv, "_local_service_alive", lambda url, timeout=1.0: url == "http://localhost:3002"
    )

    home = get_hermes_home()
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("web:\n  extract_backend: firecrawl\n", encoding="utf-8")
    save_env_value_secure("FIRECRAWL_API_KEY", "fc-real-secret")
    ...
    payload = client.get("/api/form").json()

    rows = [
        b for b in payload["tools"] if b["category"] == "web_extract"
    ][0]["rows"]
    names = [r["name"] for r in rows]
    assert "Firecrawl Self-Hosted" in names and "Firecrawl" in names, names

    extract_env = payload["current"]["extract_env"]
    assert extract_env["env_var"] == "FIRECRAWL_API_KEY", extract_env
    assert extract_env["is_set"] is True


def test_form_returns_the_saved_custom_endpoint_key_as_set(tmp_path, monkeypatch):
    """Возврат клиента со своим эндпоинтом: ключ лежит в config.yaml
    (`model.api_key`), а не в `.env`, и `/api/form` обязан показать его
    сохранённым.

    Без этой ветки мастер читал бы ключ только из переменной окружения,
    которой у `custom` нет вовсе, и возвращающийся клиент видел бы «ключ
    не сохранён» при сохранённом — то есть вводил бы его заново на каждом
    заходе, а пустое поле означает «оставить как было» и молча ничего бы
    не меняло."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_constants import get_hermes_home
    ...
    r = client.get("/api/form")
    current = r.json()["current"]["provider"]
    assert current["name"] == "custom"
    assert current["base_url"] == "https://llm.example.org/v1"
    assert current["model"] == "my-model-7b"
    assert current["api_key"]["is_set"] is True, current["api_key"]
    # Сам ключ наружу не уезжает ни при каких обстоятельствах.
    assert "sk-my-endpoint" not in r.text


def test_check_key_probes_a_client_endpoint_only_without_an_env_var(logged_in, monkeypatch):
    """Адрес принимается ТОЛЬКО в паре с отсутствующей переменной.

    Иначе форма могла бы попросить сервер сходить с ключом известного
    провайдера на произвольный адрес — ровно то, что закрытый список
    разрешённых переменных и не пускает."""
    from hermes_cli.setup_wizard import app as wapp

    calls = []
    monkeypatch.setattr(
        wapp, "check_provider_key", lambda *a: calls.append(a) or {"ok": True, "checked": True}
    )

    logged_in.post(
        "/api/check/key",
        json={"env_var": None, "value": "k", "proxy": "", "base_url": "https://llm.example.org/v1"},
    )
    assert calls[-1] == (None, "k", None, "https://llm.example.org/v1"), calls[-1]
    ...
    assert calls[-1] == ("OPENROUTER_API_KEY", "k", None, None), calls[-1]
