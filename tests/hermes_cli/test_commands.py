...
class TestCommandDescriptionsDoNotNameTheUpstreamProduct:
    """The Telegram client's /help and BotCommand menu render CommandDef
    descriptions verbatim (see gateway_help_lines / telegram_bot_commands).
    They must present this product's own name, not the upstream fork's."""

    # No commands are exempt any more: /wake's description was rewritten
    # so it does not need to quote the wake phrase (the phrase and the
    # bundled model in tools/wake_word.py are unchanged).
    _EXEMPT = set()

    _UPSTREAM_TOKENS = ("Hermes Agent", "Hermes", "Nous Research", "Nous")

    def test_no_gateway_visible_description_says_hermes_agent(self):
        for cmd in COMMAND_REGISTRY:
            assert "Hermes Agent" not in cmd.description, cmd.name

    def test_no_gateway_visible_description_names_hermes(self):
        for cmd in COMMAND_REGISTRY:
            if cmd.name in self._EXEMPT:
                continue
            assert "Hermes" not in cmd.description, cmd.name

    def test_version_and_update_present_trix_agent(self):
        by_name = {cmd.name: cmd for cmd in COMMAND_REGISTRY}
        assert "Trix Agent" in by_name["version"].description
        assert "Trix Agent" in by_name["update"].description

    def test_slack_reserved_hermes_entry_does_not_name_hermes(self):
        entries = slack_native_slashes()
        hermes_entry = next(e for e in entries if e[0] == "hermes")
        assert "Hermes" not in hermes_entry[1]

    def test_no_description_names_any_upstream_brand(self):
        for cmd in COMMAND_REGISTRY:
            if cmd.name in self._EXEMPT:
                continue
            for token in self._UPSTREAM_TOKENS:
                assert token not in cmd.description, f"/{cmd.name}: {token}"

    def test_no_args_hint_names_any_upstream_brand(self):
        """args_hint is rendered next to the description in gateway /help,
        so it is the same customer-facing surface. The original guard only
        looked at `description`, which is how `/debug [nous|local]` kept
        the upstream brand in the client's help text."""
        for cmd in COMMAND_REGISTRY:
            for token in self._UPSTREAM_TOKENS:
                assert token.lower() not in cmd.args_hint.lower(), f"/{cmd.name}: {token}"

    def test_no_russian_description_names_any_upstream_brand(self):
        """The Russian catalog is a second copy of the same user-facing copy;
        the guard has to cover it or the brand leak just moves."""
        import yaml, pathlib
        from agent import i18n
        raw = yaml.safe_load(
            (pathlib.Path(__file__).resolve().parents[2] / "locales" / "ru.yaml").read_text(encoding="utf-8")
        )
        for name, entry in (raw.get("commands") or {}).items():
            for value in (entry or {}).values():
                for token in self._UPSTREAM_TOKENS:
                    assert token not in value, f"commands.{name}: {token}"


class TestRenderedHelpAndCommandsSurfaceDoNotLeakBrand:
    """The guard above (``TestCommandDescriptionsDoNotNameTheUpstreamProduct``)
    only ever looks at ``CommandDef.description``/``args_hint`` -- the
    registry. Client-command-surface plan Task 9: the actual leak went
    through a name the registry never carries at all -- an *installed
    skill's* auto-generated slash command. ``/hermes-agent-skill-authoring``
    is a REAL skill this repo ships
    (``optional-skills/software-development/hermes-agent-skill-authoring/SKILL.md``);
    its frontmatter ``name`` literally embeds the upstream fork's brand, and
    ``agent.skill_commands.scan_skill_commands()`` turns that verbatim into
    a slash-command slug. A registry-only guard can never see it -- only
    rendering the actual ``/help`` and ``/commands`` bodies the client
    receives can, so that is what this class checks.

    Seeds the skill by copying the REAL on-disk SKILL.md into a fake
    ``SKILLS_DIR`` (patching ``tools.skills_tool.SKILLS_DIR``, the same seam
    ``tests/hermes_cli/test_trix_menu.py::
    TestRealSkillCommandsDoNotReachTheCuratedMenu`` uses, ``realpath``-wrapped
    for the same macOS ``/var`` -> ``/private/var`` reason documented there)
    rather than inventing a synthetic SKILL.md -- an empty temp HERMES_HOME
    would make this test green on broken code too (global constraint:
    "пустой HERMES_HOME делает тесты про навыки бессмысленными").

    Mutation proof (Task 9 step 1): ``hermes_cli/slash_exec.py``'s
    ``_exec_help``/``_exec_commands`` only append ``skill_cmds`` inside
    ``if not curated:``. Temporarily deleting that guard so skills are
    always appended (i.e. reverting the Task 3 fix this test now stands
    guard over) turns BOTH assertions below red on exactly
    ``hermes-agent-skill-authoring`` -- verified by hand and reverted; a
    tree-editing test would itself be the "read source in a test"
    antipattern the plan's global constraints forbid, so the proof lives in
    the task report, not in this file.
    """

    _UPSTREAM_TOKENS = ("Hermes Agent", "Hermes", "Nous Research", "Nous")
    _REAL_SKILL_DIR = (
        Path(__file__).resolve().parents[2]
        # Переехал в optional-skills 2026-09-07 (кураторство поставки).
        # Тесту нужен настоящий SKILL.md из чекаута, а не активность
        # скилла: он копирует каталог во временный HOME.
        / "optional-skills" / "software-development" / "hermes-agent-skill-authoring"
    )

    def _rendered_bodies(self, monkeypatch, tmp_path) -> list:
        real_home = Path(os.path.realpath(str(tmp_path)))
        monkeypatch.setenv("HERMES_HOME", str(real_home))
        # No config.yaml at all -- curation defaults to on, matching a
        # fresh client install (menu_curation_enabled's own default).

        skills_dir = real_home / "skills"
        shutil.copytree(self._REAL_SKILL_DIR, skills_dir / "hermes-agent-skill-authoring")

        with patch("tools.skills_tool.SKILLS_DIR", skills_dir):
            from agent.skill_commands import scan_skill_commands

            skill_cmds = scan_skill_commands()
            assert "/hermes-agent-skill-authoring" in skill_cmds, (
                "seeding failed -- the real skill did not scan in, which "
            ...
            commands_reply = execute_command(
                "commands",
                CommandContext(surface="gateway", options={"page_size": 100}),
            )
        return [help_reply.text, commands_reply.text]

    def test_default_curated_surface_never_shows_the_skill_slug(self, tmp_path, monkeypatch):
        for text in self._rendered_bodies(monkeypatch, tmp_path):
            assert "hermes-agent-skill-authoring" not in text

    def test_default_curated_surface_carries_no_upstream_brand_token(self, tmp_path, monkeypatch):
        for text in self._rendered_bodies(monkeypatch, tmp_path):
            for token in self._UPSTREAM_TOKENS:
                assert token not in text, f"{token!r} leaked into rendered client surface: {text!r}"
...
class TestTelegramBotCommands:
    ...
    def test_default_cap_covers_every_gateway_command(self, tmp_path, monkeypatch):
        """Invariant, not a snapshot: the *uncurated* Telegram menu cap must
        never fall below the number of gateway-visible commands.

        Regression: adding /setup (Task 11) grew the registry to 61 while
        ``_DEFAULT_TELEGRAM_MENU_MAX_COMMANDS`` was still 60 — the
        uncurated (``curated: false``) menu silently dropped /version off
        the visible list with no error anywhere, because
        ``telegram_menu_commands()``'s clamp just stops appending once it
        hits the cap. Asserted as a relationship, not a literal count, so
        the NEXT command added trips this test loudly instead of quietly
        reproducing the same regression.

        ``curated: false`` is pinned explicitly (client-command-surface
        Task 1b): Trix defaults to curation ON, and once curated, the cap
        is deliberately derived from ``len(CLIENT_MENU_COMMANDS)`` instead
        of this registry-sized default -- a smaller number on purpose, see
        ``hermes_cli.commands._telegram_command_menu_config``. This test's
        invariant is specifically about the uncurated escape hatch (its own
        docstring always said so), which is why it needs the explicit
        override rather than relying on an ambient default.
        """
...
class TestCommandDescriptionLocalization:
    """Descriptions render through the catalog but fall back to the
    registry's canonical English, so a missing translation degrades to
    real copy rather than a key path."""

    def test_falls_back_to_registry_english_when_no_translation(self, monkeypatch):
        """A synthetic CommandDef whose key can never appear in any locale
        catalog proves the fallback independent of which commands ru.yaml
        currently translates -- unlike a real command name, this can't be
        invalidated by a future task adding its translation."""
        from agent import i18n
        from hermes_cli.commands import CommandDef, command_description
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
        cmd = CommandDef(name="__nope__", description="Fallback text", category="Session")
        assert command_description(cmd) == cmd.description

    def test_english_renders_registry_text_verbatim(self, monkeypatch):
        from agent import i18n
        from hermes_cli.commands import command_description, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "en")
        i18n.reset_language_cache()
        cmd = resolve_command("new")
        assert command_description(cmd) == cmd.description

    def test_translation_is_used_when_present(self, monkeypatch, tmp_path):
        """Prove the lookup is live, without depending on ru.yaml content."""
        from agent import i18n
        from hermes_cli.commands import command_description, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
        cmd = resolve_command("new")
        i18n._catalog_cache["ru"] = {f"commands.{cmd.name}.description": "ТЕСТОВОЕ ОПИСАНИЕ"}
        try:
            assert command_description(cmd) == "ТЕСТОВОЕ ОПИСАНИЕ"
        finally:
            i18n.reset_language_cache()

    def test_help_lines_use_the_localized_description(self, monkeypatch):
        from agent import i18n
        ...
        i18n._catalog_cache["ru"] = {f"commands.{cmd.name}.description": "ТЕСТОВОЕ ОПИСАНИЕ"}
        try:
            line = next(l for l in gateway_help_lines() if l.startswith("`/new"))
            assert "ТЕСТОВОЕ ОПИСАНИЕ" in line
        finally:
            i18n.reset_language_cache()

    def test_telegram_menu_uses_the_localized_description(self, monkeypatch):
        from agent import i18n
        from hermes_cli.commands import telegram_bot_commands, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
        cmd = resolve_command("new")
        i18n._catalog_cache["ru"] = {f"commands.{cmd.name}.description": "ТЕСТОВОЕ ОПИСАНИЕ"}
        try:
            desc = dict(telegram_bot_commands())["new"]
            assert desc == "ТЕСТОВОЕ ОПИСАНИЕ"
        finally:
            i18n.reset_language_cache()

    def test_cli_surfaces_stay_english(self, monkeypatch):
        """COMMANDS feeds CLI help and autocomplete — our surfaces, not the
        client's. They must not pick the translation up."""
        from agent import i18n
        from hermes_cli.commands import COMMANDS, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
        cmd = resolve_command("new")
        assert cmd.description in COMMANDS["/new"]


class TestCommandArgsHintLocalization:
    """args_hint translates for plain hints, but Ruling 2 (a hint listing
    literal subcommands must never be translated -- the customer has to
    type the literal word back, and a translated word the parser rejects
    is worse than an English one) is enforced in code, not by convention,
    so a stray `commands.<name>.args_hint` catalog key cannot override
    it."""

    def test_piped_hint_ignores_a_catalog_key(self, monkeypatch):
        from agent import i18n
        from hermes_cli.commands import command_args_hint, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
        cmd = resolve_command("topic")
        assert cmd is not None and "|" in cmd.args_hint
        i18n._catalog_cache["ru"] = {f"commands.{cmd.name}.args_hint": "ТЕСТ"}
        try:
            assert command_args_hint(cmd) == cmd.args_hint
        finally:
            i18n.reset_language_cache()

    def test_plain_hint_translates(self, monkeypatch):
        from agent import i18n
        from hermes_cli.commands import command_args_hint, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
        cmd = resolve_command("new")
        assert cmd is not None and cmd.args_hint and "|" not in cmd.args_hint
        i18n._catalog_cache["ru"] = {f"commands.{cmd.name}.args_hint": "[имя]"}
        try:
            assert command_args_hint(cmd) == "[имя]"
        finally:
            i18n.reset_language_cache()

    def test_english_hint_is_byte_identical(self, monkeypatch):
        from agent import i18n
        from hermes_cli.commands import command_args_hint, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "en")
        i18n.reset_language_cache()
    ...
    def test_help_lines_render_the_localized_hint(self, monkeypatch):
        from agent import i18n
        from hermes_cli.commands import gateway_help_lines, resolve_command
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
    ...
    def test_piped_and_plain_hints_partition_every_gateway_command_with_a_hint(self):
        """Derive the piped/plain split instead of hardcoding a snapshot
        count -- the invariant is that every gateway-visible command with an
        args_hint falls into exactly one bucket, and both buckets are
        non-empty (Ruling 2 has real work to do on this registry). A new
        command added to either bucket should not need this test touched."""
        from hermes_cli.commands import (
        ...
        with_hint = [
            ...
            if _is_gateway_available(c, overrides) and c.args_hint
        ]
        piped = [c for c in with_hint if "|" in c.args_hint]
        plain = [c for c in with_hint if "|" not in c.args_hint]
        assert len(piped) + len(plain) == len(with_hint)
        assert piped, "expected at least one piped (literal-subcommand) args_hint"
        assert plain, "expected at least one plain, translatable args_hint"


    def test_literal_subcommand_hints_without_a_pipe_stay_english(self, monkeypatch):
        """`/deny [all]` and `/context [all]` are compared VERBATIM by their
        handlers, so a Russian rendering would be typed and then rejected.

        The `"|"` heuristic is only a syntactic proxy for "holds a literal
        subcommand"; these two hold one without a pipe and slipped through it
        once already.  /deny is the expensive case: a customer with several
        pending dangerous commands who follows a translated hint denies ONE
        of them, leaves the rest blocking, and hands the agent a garbled
        reason -- silent wrong behaviour on the approval surface.
        """
        from agent import i18n
        from hermes_cli.commands import command_args_hint, resolve_command
        ...
        i18n.reset_language_cache()
        try:
            for name in ("deny", "context"):
                cmd = resolve_command(name)
                assert command_args_hint(cmd) == cmd.args_hint, name
        finally:
            i18n.reset_language_cache()

    def test_no_literal_subcommand_hint_is_translated_in_the_catalog(self):
        """Belt and braces: even if the code guard were removed, shipping a
        translation for these keys is itself the defect."""
        import pathlib as _pl

        import yaml as _yaml

        from hermes_cli.commands import _LITERAL_SUBCOMMAND_HINTS

        raw = _yaml.safe_load(
            (_pl.Path(__file__).resolve().parents[2] / "locales" / "ru.yaml")
            .read_text(encoding="utf-8")
        ) or {}
        entries = raw.get("commands") or {}
        for name in _LITERAL_SUBCOMMAND_HINTS:
            assert "args_hint" not in (entries.get(name) or {}), name
...
# ---------------------------------------------------------------------------
# suggest_gateway_command — typo rescue for the gateway's unknown-command
# notice (/debuf → /debug, observed twice in the 2026-09-12 debug report)
# ---------------------------------------------------------------------------


def test_suggest_gateway_command_rescues_single_char_typo():
    """/debuf must suggest /debug — the exact slip from the debug report."""
    assert suggest_gateway_command("debuf") == "debug"


def test_suggest_gateway_command_normalizes_underscores():
    """The gateway dispatch check hyphen-normalizes; the suggestion must too,
    so a /new_session-style token can still suggest a hyphenated command."""
    assert suggest_gateway_command("stat_us") == "status"


def test_suggest_gateway_command_returns_none_for_garbage():
    """Random junk must not produce a confident-looking wrong suggestion."""
    assert suggest_gateway_command("zzzzzzzz") is None
    assert suggest_gateway_command("") is None
    assert suggest_gateway_command(None) is None


def test_suggest_gateway_command_only_suggests_gateway_commands():
    """Suggestions come from GATEWAY_KNOWN_COMMANDS — a cli_only command with
    no gateway gate must never be suggested in the gateway."""
    suggestion = suggest_gateway_command("skin")
    if suggestion is not None:
        assert suggestion in GATEWAY_KNOWN_COMMANDS
