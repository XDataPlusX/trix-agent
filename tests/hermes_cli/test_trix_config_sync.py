...
def test_inline_parent_is_skipped_not_inserted(tmp_path):
    """``media_retention_hours: {}`` — родитель с ИНЛАЙН-значением. Под него
    блочных детей дописать нельзя: файл перестал бы парситься. Глубина тут
    больше не при чём (потолок в два уровня снят), а защита формы осталась."""
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    from hermes_cli.trix_config_sync import _SKIP_INLINE_PARENT
    ...
    assert (
        _skipped_reason(skipped, "gateway.media_retention_hours.documents")
        == _SKIP_INLINE_PARENT
    )
...
def test_crlf_client_file_is_preserved_byte_for_byte(tmp_path):
    ...
    assert _old_lines_are_untouched(
        before_bytes.decode("utf-8"), after_bytes.decode("utf-8")
    ), "существующие строки не тронуты"
    # Проверка идёт по ВСЕМУ файлу, а не только по дописанному хвосту: с
    # полным шаблоном вставки приходят и внутрь уже существующих секций,
    # так что «хвост» выделить нельзя — зато утверждение стало сильнее.
    assert b"\r\n" in after_bytes
    assert b"\n" not in after_bytes.replace(b"\r\n", b""), (
        "в файле не должно остаться ни одного голого \\n — весь файл обязан "
        "говорить на одном языке переводов строк"
    )
    assert "Что клиент видит в Telegram" in after_bytes.decode("utf-8")
...
def test_inline_display_value_is_never_corrupted(tmp_path, display_line, expected_display):
    ...
    after = path.read_text(encoding="utf-8")
    # Nothing was spliced under the display line. Note the invariant is
    # "no existing line changed", NOT "the old text is a prefix of the new
    # one": the shipped template now carries every DEFAULT_CONFIG section,
    # so siblings legitimately land INSIDE sections the client already has
    # (terminal, web, ...). Only the inline-valued parent must stay shut.
    assert _old_lines_are_untouched(before, after), (
        "existing content (including the display line itself) changed — "
        "nothing should have been spliced under an inline-valued parent"
    )
    assert display_line in after, "сама инлайн-строка осталась как была"
...
def test_multiline_scalar_that_looks_like_a_root_key_is_never_written(tmp_path, quote):
    ...
    # Причина отказа теперь называет БЕДУ, а не одну на оба случая: у этого
    # теста две ветки ровно про две разные беды, что и написано в его
    # докстринге. Двойные кавычки — файл не разбирается; одинарные —
    # разбирается, но значение клиента переписано.
    expected = (
        _SKIP_VERIFY_UNPARSEABLE if quote == '"' else _SKIP_VERIFY_FAILED
    )
    assert expected in _skipped_reasons(skipped), (
        "отказ обязан быть объяснён верной причиной, а не одной на оба случая: "
        f"получено {set(_skipped_reasons(skipped))}"
    )
...
def test_verify_rejects_a_result_that_changes_an_existing_value():
    ...
    ``added`` carries key TUPLES, not dotted strings — the module stopped
    using "." as a path separator once keys that contain a dot showed up in
    DEFAULT_CONFIG (``monitoring…resource_attributes."service.name"``).
    """
    ...
    # Unparseable -> rejected with its OWN verdict, not "value changed".
    from hermes_cli.trix_config_sync import _UNPARSEABLE
...
def test_verify_resolves_a_dotted_key_name_as_one_key(tmp_path):
    """A key whose NAME contains a dot must not be read as two path steps."""
    from hermes_cli.trix_config_sync import _verify

    client = {"monitoring": {"attrs": {}}}
    text = "monitoring:\n  attrs:\n    service.name: hermes\n"
    assert _verify(text, client, [("monitoring", "attrs", "service.name")]) == []
    # The naive split would have looked for monitoring.attrs.service.name.
    assert _verify(text, client, [("monitoring", "attrs", "service", "name")]) == [
        ("monitoring", "attrs", "service", "name")
    ]
...
def test_every_missing_path_is_accounted_for(tmp_path, monkeypatch):
    ...
    expected = {
        ...
        for path in _collect_missing_paths(template_data, client_data)
    }
...
def test_update_prints_the_notice_when_it_seeds_ports(client_config, monkeypatch, capsys):
    ...
    # Отчёт называет РАЗДЕЛ, а не путь: с полным шаблоном список путей — это
    # 5000 символов в одну строку, а `update` ходит и в Телеграм, где предел
    # сообщения 4096 (см. added_summary).
    assert "terminal" in out
    assert "docker_extra_args" not in out, "отчёт снова сыплет путями"
...
def test_doctor_prints_the_notice_when_it_seeds_ports(client_config, capsys):
    ...
    out = capsys.readouterr().out
    assert fixed == 1
    # Как и в update: раздел, а не путь (см. added_summary).
    assert "terminal" in out
    assert "docker_extra_args" not in out, "отчёт снова сыплет путями"
    assert "пересоздайте песочницу" in out.lower()
...
# ---------------------------------------------------------------------------
# Полный конфиг (решение владельца, сентябрь 2026): шаблон теперь содержит
# ВСЕ секции DEFAULT_CONFIG, а не только те, где мы отошли от дефолта.
# Требование «у всех одинаково» означает, что уже работающая машина после
# `hermes update` обязана получить ровно тот же набор ключей, что и свежая
# установка. Ограничение «два уровня» это требование нарушало: у клиента,
# у которого agent.agent_cache уже есть (мы задаём в нём memory_high_mb),
# соседние ключи третьего уровня не доезжали никогда.
#
# Глубина в DEFAULT_CONFIG доходит до 5 (moa.presets.default.aggregator.model),
# и там же есть ключи С ТОЧКОЙ ВНУТРИ ИМЕНИ
# (monitoring…resource_attributes."service.name") — поэтому путь внутри
# модуля представлен кортежем ключей, а не строкой через точку.
# ---------------------------------------------------------------------------


def test_third_level_key_is_inserted_under_an_existing_block_parent(tmp_path):
    """Ровно случай agent.agent_cache: родитель у клиента есть и НЕ пуст,
    отсутствует соседний ключ третьего уровня. Он обязан доехать."""
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    assert "agent.agent_cache.max_size" in added
    assert "agent.agent_cache.protect_recent" in added
    after = path.read_text(encoding="utf-8")
    assert _old_lines_are_untouched(before, after)
    data = yaml.safe_load(after)
    assert data["agent"]["agent_cache"]["max_size"] == 128
    assert data["agent"]["agent_cache"]["protect_recent"] == 8
    assert data["agent"]["agent_cache"]["memory_high_mb"] == 2560


def test_fourth_level_key_is_inserted(tmp_path):
    """display.platforms.telegram.streaming — четвёртый уровень, и он есть
    в нашем шаблоне живьём."""
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    assert "display.platforms.telegram.chunk_size" in added
    data = yaml.safe_load(path.read_text(encoding="utf-8"))
    assert data["display"]["platforms"]["telegram"]["chunk_size"] == 3500
    assert data["display"]["platforms"]["telegram"]["streaming"] is False


def test_deep_insert_follows_the_clients_own_indent(tmp_path):
    """Клиент отбивает четырьмя пробелами на уровень, шаблон — двумя.
    Форму задаёт клиент: вставка обязана лечь на его отступ."""
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    assert "agent.agent_cache.max_size" in added, skipped
    text = path.read_text(encoding="utf-8")
    assert "        max_size: 128" in text, text
    assert yaml.safe_load(text)["agent"]["agent_cache"]["max_size"] == 128


def test_a_key_whose_name_contains_a_dot_is_inserted_not_misparsed(tmp_path):
    """monitoring…resource_attributes."service.name": точка — часть ИМЕНИ
    ключа, а не разделитель пути. Наивный split(".") искал бы ключ
    ``service`` и не нашёл ничего."""
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    data = yaml.safe_load(path.read_text(encoding="utf-8"))
    attrs = data["monitoring"]["gateway_health_export"]["resource_attributes"]
    assert attrs["deployment.environment.name"] == "production", skipped
    assert attrs["service.name"] == "hermes-gateway"


def test_inline_parent_deeper_than_two_levels_is_still_refused(tmp_path):
    """Снятие ограничения по глубине НЕ снимает защиту формы: под
    ``media_retention_hours: {}`` блочных детей дописать нельзя — файл
    перестал бы парситься. Причина теперь «инлайн-родитель», а не «глубоко»."""
    from hermes_cli.trix_config_sync import (
        _SKIP_INLINE_PARENT,
        sync_missing_client_sections,
    )
...
def test_the_shipped_template_syncs_into_an_old_client_with_nothing_skipped(tmp_path):
    """Сквозная проверка требования «у всех одинаково»: короткий конфиг
    старого клиента + текущий шаблон = тот же набор ключей, что у свежей
    установки. Ни одного пропуска."""
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    assert skipped == [], f"после досева остались пропуски: {skipped}"
    client = yaml.safe_load(path.read_text(encoding="utf-8"))
    template = yaml.safe_load(TEMPLATE.read_text(encoding="utf-8"))

    def _same_keys(t, c, prefix=()):
        gaps = []
        for key, tval in t.items():
            if key not in (c or {}):
                gaps.append(".".join(str(p) for p in prefix + (key,)))
                continue
            if isinstance(tval, dict):
                gaps += _same_keys(tval, (c or {}).get(key) or {}, prefix + (key,))
        return gaps

    assert _same_keys(template, client) == [], _same_keys(template, client)


# ---------------------------------------------------------------------------
# Найдено ревью после снятия потолка глубины.
#
# Отступ вставки в ПУСТОЙ блок брался из шаблона (`t_indent`). На двух
# уровнях это было безопасно: родитель стоял на отступе 0, а любой отступ
# шаблона больше нуля. На глубине 3+ родитель сам отбит, и отступ шаблона
# может оказаться НЕ БОЛЬШЕ родительского — тогда вставка перестаёт быть
# потомком, файл не разбирается, и проверка результата отклоняет ВЕСЬ проход.
# То есть один неудачный путь уносил все остальные: до снятия потолка такие
# пути просто пропускались по одному, а всё прочее доезжало.
# ---------------------------------------------------------------------------


def test_wide_client_indent_with_an_empty_deep_parent_does_not_abort_the_run(tmp_path):
    """Клиент отбивает четырьмя пробелами, глубокий родитель пуст.

    Отступ шаблона (2) меньше отступа родителя (8) — вставка обязана уйти
    ПРАВЕЕ родителя, а не сорвать весь досев.
    """
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    assert added, (
        f"досев не доставил ничего, пропущено {len(skipped)} — один "
        "неудачный путь не имеет права уносить весь проход"
    )
    text = path.read_text(encoding="utf-8")
    data = yaml.safe_load(text)  # обязан разбираться
    assert data["terminal"]["backend"] == "docker"

    # Потомки, дописанные в пустой блок `telegram:` (он отбит на 8), обязаны
    # стоять ПРАВЕЕ него. Проверяем именно строки этого блока, а не все
    # строки файла: `streaming` встречается и в других секциях, дописанных
    # целыми блоками на своих отступах.
    lines = text.splitlines()
    telegram_idx = next(
        i for i, line in enumerate(lines) if line == "        telegram:"
    )
    children = []
    for line in lines[telegram_idx + 1:]:
        if not line.strip():
            continue
    ...
    assert children, "пустой блок telegram: не заполнился вовсе"
    for indent, line in children:
        assert indent > 8, f"строка левее своего родителя: {line!r}"
    assert (data["display"]["platforms"] or {}).get("telegram")


@pytest.mark.parametrize("width", [3, 4, 6])
def test_any_client_indent_width_survives_an_empty_deep_parent(tmp_path, width):
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    added, _skipped = sync_missing_client_sections(path, TEMPLATE)

    assert added, f"при отступе {width} досев не доставил ничего"
    assert yaml.safe_load(path.read_text(encoding="utf-8"))["terminal"]["backend"] == "docker"


def test_an_unparseable_result_is_not_reported_as_a_changed_value(tmp_path, monkeypatch):
    """Две разные беды — две разные причины.

    ``_verify`` возвращает ``None`` и когда результат не разбирается, и когда
    врезка испортила значение клиента. Единственная причина
    ``_SKIP_VERIFY_FAILED`` утверждала второе, и саппорт шёл искать порчу
    значений там, где файл просто не распарсился.
    """
    from hermes_cli.trix_config_sync import (
        _SKIP_VERIFY_FAILED,
        _SKIP_VERIFY_UNPARSEABLE,
    ...
    assert _SKIP_VERIFY_UNPARSEABLE != _SKIP_VERIFY_FAILED
    assert "не разбир" in _SKIP_VERIFY_UNPARSEABLE.lower()

    path = tmp_path / "config.yaml"
    path.write_text("web:\n  search_backend: ddgs\n", encoding="utf-8")

    import hermes_cli.trix_config_sync as mod

    # Врезка, после которой текст перестаёт быть YAML.
    monkeypatch.setattr(mod, "_reindent_block", lambda block, f, t: ["  {{{"])
    added, skipped = sync_missing_client_sections(path, TEMPLATE)

    assert added == []
    reasons = set(_skipped_reasons(skipped))
    assert _SKIP_VERIFY_UNPARSEABLE in reasons, reasons
    assert _SKIP_VERIFY_FAILED not in reasons


# ---------------------------------------------------------------------------
# Отчёт о досеянном (найдено ревью).
#
# Вызывающие печатали ", ".join(added). Пока шаблон был дельтой, путей было
# единицы; с полным шаблоном первый досев даёт 238 путей — строку в 5243
# символа. Она уезжает не только в консоль: `update` есть в клиентском меню
# Телеграма, а предел сообщения там 4096 символов.
# ---------------------------------------------------------------------------


def test_added_summary_fits_in_a_telegram_message():
    """Отчёт о полном досеве обязан влезать в сообщение мессенджера."""
    from hermes_cli.trix_config_sync import added_summary
    ...
    def _walk(data, prefix=()):
        for key, value in (data or {}).items():
            if isinstance(value, dict) and value:
                ...
                paths.append(".".join(str(p) for p in prefix + (key,)))

    _walk(template)
    assert len(paths) > 200, "шаблон подозрительно мал — тест потерял смысл"

    line = added_summary(paths)

    assert len(line) < 500, f"строка отчёта {len(line)} символов: {line[:120]}…"
    assert str(len(paths)) in line, "счёт настроек обязан быть назван"


def test_added_summary_names_sections_not_paths():
    from hermes_cli.trix_config_sync import added_summary

    line = added_summary(["terminal.backend", "terminal.cwd", "web.search_backend"])
    assert "3 настроек" in line
    assert "terminal" in line and "web" in line
    assert "terminal.backend" not in line, "путями отчёт не сыплет"


def test_added_summary_of_nothing_is_empty():
    from hermes_cli.trix_config_sync import added_summary

    assert added_summary([]) == ""


def test_a_comment_in_the_TEMPLATE_does_not_decide_the_child_indent(tmp_path):
    """Инвариант «комментарий формы не несёт» действует и для шаблона.

    Найдено ревью: спуск по клиентскому файлу считал отступ по содержательным
    строкам (``_client_child_indent``), а спуск по ШАБЛОНУ — по первой
    непустой, комментарии включительно. Один кураторский комментарий,
    отбитый иначе, чем его соседи, уводил отступ шаблона в сторону, и все
    глубокие ключи этого раздела молча уходили в пропуск — видно только в
    ``hermes logs``. В шаблоне на 1250 строк, который ведут руками, это
    вопрос времени.
    """
    from hermes_cli.trix_config_sync import sync_missing_client_sections
    ...
    template.write_text(
        "display:\n"
        "      # пояснение, отбитое не как соседи\n"
        "  platforms:\n"
        "    telegram:\n"
        "      streaming: false\n"
        "      chunk_size: 3500\n",
    ...
    assert "display.platforms.telegram.chunk_size" in added, (
        f"комментарий в шаблоне решил вопрос формы: {skipped}"
    )
    data = yaml.safe_load(path.read_text(encoding="utf-8"))
    assert data["display"]["platforms"]["telegram"]["chunk_size"] == 3500
