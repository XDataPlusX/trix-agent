...
from tests._source_tree import prune_names, walk_files
...
ALLOWLIST = {
    ...
    # Line-preserving write-back round-trip, same class as
    # hermes_cli.config.read_user_config_raw() -- mirrors provider/model
    # settings from the default profile's config.yaml into the
    # system_admin profile's, comparing PARSED values to decide what to
    # rewrite (hermes_cli/trix_business.py's `_apply_edits`/`_ensure_value`
    # pattern does the identical thing but isn't caught by this heuristic
    # only because none of its safe_load call sites happen to sit within
    # PROXIMITY lines of a literal "config.yaml" string).
    "hermes_cli/trix_admin_profile_mirror.py",
}
...
EXCLUDED_DIR_PARTS = {
    ...
    # Agent-tool scratch checkouts. A git worktree under ``.claude/worktrees/``
    # is a SECOND copy of this repository on disk, so every scanner that walks
    # the tree reports its files as fresh offenders — the same finding twice,
    # attributed to a path nobody edits. ``.worktrees`` above covers the older
    # location; this covers where the tooling actually puts them now.
    ".claude",
}
...
def _iter_source_files():
    # walk_files() prunes the excluded directories BEFORE descending into
    # them. The old ``rglob`` + filter-the-result form walked into every one
    # of them anyway, and under the gate's ~18 parallel pytest subprocesses
    # it died with FileNotFoundError on a neighbour's ``__pycache__`` that
    # vanished mid-scan (RAF-170/RAF-171). See tests/_source_tree.py.
    return walk_files(REPO_ROOT, match="*.py", prune=prune_names(EXCLUDED_DIR_PARTS))
