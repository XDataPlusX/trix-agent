...
class TestImport:
    ...
    def test_interrupted_restore_preserves_existing_file(self, tmp_path, monkeypatch):
        """An interrupted restore (crash, disk full, ...) must never leave an
        existing target file truncated or partially overwritten.

        This simulates the crash at the atomic-replace step (the point where
        the freshly-written temp file is swapped into place) and asserts the
        pre-existing file on disk is untouched -- restore is all-or-nothing
        per file, never a partial write in place.
        """
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        ...
        original_content = "model:\n  provider: original-should-survive\n"
        (hermes_home / "config.yaml").write_text(original_content)

        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            "config.yaml": "model:\n  provider: new-from-backup\n",
        })
        ...
        monkeypatch.setattr(_utils, "atomic_replace", _boom)

        from hermes_cli.backup import run_import
        run_import(args)  # per-file errors are collected, not fatal

        restored = (hermes_home / "config.yaml").read_text()
        assert restored == original_content, (
            "config.yaml was corrupted/replaced by an interrupted restore: "
            f"expected the original content to survive, got {restored!r}"
        )
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_an_unreadable_live_file_says_so_instead_of_declining_silently(
        self, tmp_path, caplog
    ):
        """Declining is correct here. Declining *quietly* is how the net dies.

        Executed chain, not reasoning: an unreadable live file makes this
        function decline (it must not overwrite a file it cannot parse), and
        the NEXT update's pre-update snapshot copies that same unreadable
        file — so from then on the snapshot is unreadable too and the net can
        never fire again. Nothing anywhere mentions it. The one thing that
        breaks the chain is saying it out loud once, while a good snapshot
        still exists.
        """
        import logging
        ...
        # Truncated mid-write — what a non-atomic writer leaves behind.
        jobs_path.write_text('{"jobs": [{"id": "a"}, {"id"')

        with caplog.at_level(logging.WARNING):
            result = restore_cron_jobs_if_emptied(snap_id, hermes_home=hermes_home)

        # Still declines: a corrupt file must surface, not be papered over.
        assert result is None
        assert jobs_path.read_text() == '{"jobs": [{"id": "a"}, {"id"'

        # But it is no longer silent, and it names what could have saved them.
        warning = " ".join(r.getMessage() for r in caplog.records)
        assert "unreadable" in warning
        assert snap_id in warning
        assert "3" in warning

    def test_a_missing_live_file_stays_quiet(self, tmp_path, caplog):
        """No file is not the same as a broken file.

        A fresh install has no ``cron/jobs.json`` at all, and warning every
        such user about a safety net they have never needed would train them
        ...
        assert "unreadable" not in " ".join(r.getMessage() for r in caplog.records)
...
class TestMemoryProviderExternalPaths:
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        # HOME as well as the Path.home attribute — the same pair the restore
        # test below needs, and for the same reason: the external anchor goes
        # through ``user_home_path()``, which reads the environment. With only
        # the attribute patched, the anchor stayed at the real ``$HOME`` of
        # whoever runs the suite, and this test silently stopped testing
        # anything: it passed because pytest's tmp_path happened to sit in
        # ``/tmp``, outside that home. Point TMPDIR inside the real home —
        # which is all a release gate has to do — and the "outside" path was
        # classified as internal and archived. The assertion below is about a
        # credential leak, so it has to bind to the anchor the code uses.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        # HOME as well as the Path.home attribute: the external anchor now
        # goes through ``user_home_path()``, which reads the environment —
        # that is exactly what makes it follow the profile under
        # containment. Patching only the attribute would leave this restore
        # aimed at the real ``$HOME`` of whoever runs the suite.
        monkeypatch.setenv("HOME", str(dst_home))
