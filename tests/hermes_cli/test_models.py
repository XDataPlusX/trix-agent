...
class TestDetectProviderForModel:
    ...
    def test_custom_provider_not_overridden_by_static_catalog(self, monkeypatch):
        ...
        Static-catalog resolution is step 1 and should already return
        (via the custom-provider guard) before step 2 ever reaches the
        OpenRouter catalog — but `_find_openrouter_slug` runs regardless,
        so pin it to the static fallback list instead of touching the
        network, same as the sibling ``test_short_alias_resolves_to_static_model``.
        """
        monkeypatch.setattr(
            _models_mod, "fetch_openrouter_models", lambda **kw: OPENROUTER_MODELS
        )
