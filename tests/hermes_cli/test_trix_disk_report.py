...
class TestTemporaryFilesPromiseIsQualified:
    """Единственная строка плана, где обещание больше того, что освободится.

    ``_removable_bytes`` меряет ``<root>/tmp`` целиком, а уборка не трогает
    записи моложе ``MIN_AGE_SECONDS``. Разойтись молча этим числам нельзя
    (правило 3 в шапке модуля), и сказать об этом надо клиенту, а не
    докстрингу.
    """

    # Метка берётся из таблицы модуля, а не из имени оговорки: тест обязан
    # падать на пропавшей оговорке, а не на пропавшей константе.
    @staticmethod
    def _label() -> str:
        from hermes_cli.trix_disk import _REMOVABLE_LABELS

        return _REMOVABLE_LABELS["tmp"]

    @classmethod
    def _tmp_item(cls, size: int = 3 * GB) -> RemovableItem:
        return RemovableItem(cls._label(), Path("/x/tmp"), size)

    def test_the_temporary_files_line_carries_the_age_caveat(self):
        text = format_report(_report(removable=[self._tmp_item()]))
        assert _size(3 * GB) in _line_starting(text, f"• {self._label()}")
        assert "старше" in text
        assert "освободится меньше" in text

    def test_the_caveat_names_the_threshold_the_cleanup_actually_uses(self):
        from hermes_cli.trix_profile_tmp import MIN_AGE_SECONDS

        # Число, а не слово: порог можно подвинуть, и тогда падать должна
        # оговорка, разъехавшаяся с кодом, а не согласование «минуты/минут».
        text = format_report(_report(removable=[self._tmp_item()]))
        assert f"старше {round(MIN_AGE_SECONDS / 60)} мину" in text

    def test_other_directories_are_not_qualified(self):
        # Обещание по медиакэшу сходится с фактом — оговорка там была бы
        # ложной осторожностью и обесценила бы её там, где она нужна.
        text = format_report(
            _report(removable=[RemovableItem("медиакэш", Path("/x/cache/images"), 3 * GB)])
        )
        assert "старше" not in text

    def test_the_caveat_is_silent_while_the_disk_is_roomy(self):
        text = format_report(
            _report(
                used=10 * GB,
                free=90 * GB,
