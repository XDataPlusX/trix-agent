...
class TestRegisterTranscriptionProvider:
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        # Invariant, not a snapshot: does NOT assert list_providers() == [] —
        # PluginManager() here scans the real (unmocked) bundled plugins/
        # tree, so legitimate bundled STT backends (e.g. plugins/stt/nexara,
        # kind: backend) are expected to register alongside this test's
        # rejected entry. What must hold regardless of how many real
        # backends exist is that every registered entry is a real
        # TranscriptionProvider — the rejected 'not a provider' string
        # never becomes one.
        from agent.transcription_provider import TranscriptionProvider
        assert all(
            isinstance(p, TranscriptionProvider)
            for p in transcription_registry.list_providers()
        )
