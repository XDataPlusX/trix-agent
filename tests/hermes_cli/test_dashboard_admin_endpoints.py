...
class TestDebugShareEndpoint:
    """POST /api/ops/debug-share collects the redacted bundle and returns
    its content directly — it never uploads anywhere. Customer logs stay on
    their own VM; the dashboard (the customer's own browser talking to
    their own server) renders/downloads the returned file content instead
    of a paste-service URL."""
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr(dbg, "upload_to_pastebin", self._boom)
        monkeypatch.setattr(dbg, "_schedule_auto_delete", self._boom)
        ...
        assert "agent line" in body["files"]["report"]
    ...
    def test_never_uploads_even_if_pastebin_would_fail(self, monkeypatch):
        """The old endpoint surfaced a broken paste service as a 502. There
        is no upload left to fail: even with the pastebin client wired to
        blow up, the endpoint succeeds because it's never called."""
        import hermes_cli.debug as dbg
    ...
    def test_response_keeps_neutral_legacy_keys_for_old_frontend(self, monkeypatch):
        """The response must carry the real (new) ``files``/``redacted``
        keys AND the old ``urls``/``failures``/``auto_delete_seconds`` keys
        with neutral values, exact same names/types the pre-privacy-fix
        upload response used. An un-updated dashboard frontend that still
        reads ``shareResult.urls`` / ``.auto_delete_seconds`` must see
        "nothing to show" (empty dict/list/zero), not a missing key that
        throws a TypeError. If a future cleanup drops these neutral keys,
        this test should catch it before it ships another frontend crash.
        """
        import hermes_cli.debug as dbg
        ...
        # New, real contract.
        assert "report" in body["files"]
        assert isinstance(body["redacted"], bool)

        # Legacy contract, neutralised — same key names/types as before,
        # never populated with real data (nothing is ever uploaded).
        assert body["urls"] == {}
