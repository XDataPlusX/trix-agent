...
class TestClientMenuCommandsResolve:
    def test_every_name_resolves_in_the_registry(self):
        """Catches a typo or an upstream rename of one of the 32 names."""
...
class TestServiceCommandsResolve:
    ...
    def test_exactly_start_sethome_and_debug(self):
        """Ruling 1a fixes the service layer by name, not by size: /start
        (the platform's own ping), /sethome (the home-chat escape hatch)
        and /debug (the support channel, moved here from the menu by the
        owner on 2026-09-04). Any other name showing up here is a product
        decision that needs its own review, not something that should slip
        in silently alongside these three."""
        assert set(SERVICE_COMMANDS) == {"start", "sethome", "debug"}

    def test_debug_is_service_only_not_menu_not_disabled(self):
        """The 2026-09-04 decision, stated as the three facts it consists
        of: /debug is gone from the client's menu (and therefore from
        /help, which reads client_surface_commands()), it is NOT disabled,
        and it still dispatches. Removing any one of the three breaks the
        decision -- putting it back in the menu re-advertises exporting the
        client's own conversation, and disabling it leaves support with no
        diagnostic channel at all until the automatic error channel exists."""
        assert "debug" not in CLIENT_MENU_COMMANDS
        assert "debug" not in DISABLED_COMMANDS
        assert "debug" in SERVICE_COMMANDS
        assert not is_disabled_in_gateway("debug")
        assert "debug" in GATEWAY_KNOWN_COMMANDS
        assert "debug" not in client_surface_commands()
...
class TestDisabledCommandsResolve:
    ...
    def test_exactly_twenty_nine_entries(self):
        """Pinned to the spec's own machine-checked total (32 + 3 + 29 =
...
# ---------------------------------------------------------------------------
# /debug's description must name what the report actually contains.
#
# History, because the assertion inverted: spec 4 §8.4 required the
# description to WARN that the report carries fragments of the client's own
# conversation -- `agent/turn_context.py` logs `msg='<first 80 chars>'` at
# INFO and the report shipped that verbatim. On 2026-09-06 that leak was
# closed at the upload boundary (`_redact_log_text`) and pinned by
# tests/hermes_cli/test_debug_report_excludes_conversation.py, which runs a
# real conversation turn and asserts neither side's text survives into a
# report.
#
# So the warning became false in the other direction, and a false warning is
# not the safe side here: it tells the client that the one channel they have
# for reaching support will hand us their private messages, which is exactly
# the thing that stops them from using it. The description must still say
# what IS in there (log tails), and must no longer claim conversation
# content -- that claim now has a test proving it wrong.
# ---------------------------------------------------------------------------

_RU_CONVERSATION_MARKERS = ("переписк", "разговор")
_EN_CONVERSATION_MARKERS = ("conversation", "chat message")


class TestDebugDescriptionMatchesTheReport:
    def test_russian_names_the_logs_and_claims_no_conversation(self, monkeypatch):
        from agent import i18n

        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        i18n.reset_language_cache()
        try:
            text = command_description(resolve_command("debug")).lower()
            assert "журнал" in text or "лог" in text, (
                f"ru /debug description doesn't say the report carries logs: {text!r}"
            )
            leftover = [m for m in _RU_CONVERSATION_MARKERS if m in text]
            assert not leftover, (
                f"ru /debug description still claims conversation content {leftover} — "
                f"the leak it warned about is closed and tested: {text!r}"
            )
        finally:
            i18n.reset_language_cache()

    def test_english_names_the_logs_and_claims_no_conversation(self, monkeypatch):
        from agent import i18n
        ...
        try:
            text = command_description(resolve_command("debug")).lower()
            assert "log" in text, (
                f"en /debug description doesn't say the report carries logs: {text!r}"
            )
            leftover = [m for m in _EN_CONVERSATION_MARKERS if m in text]
            assert not leftover, (
                f"en /debug description still claims conversation content {leftover}: {text!r}"
            )
        finally:
            i18n.reset_language_cache()
