...
class TestCreateProfile:
    ...
    def test_seeds_placeholder_env_file(self, profile_env):
        ...
        bare comment-only stub. Still no real credentials leak in: every
        variable in the template is present but unfilled."""
        import stat
        ...
        assert "TELEGRAM_BOT_TOKEN" in lines, "expected template not seeded"
        # Every variable the template ships is unfilled — no credential
        # leaks in from anywhere — EXCEPT the non-secret operational hosts
        # (NO_PROXY/no_proxy) the template pre-fills on purpose. Asserted
        # as a blanket rule, not a named list, so a future template
        # addition that ships pre-filled with a real value fails this test
        # immediately instead of silently slipping through an outdated
        # named check.
        non_secret_prefilled = {"NO_PROXY", "no_proxy"}
        for var, value in lines.items():
            if var in non_secret_prefilled:
                continue
            assert value == "", f"{var} must ship unfilled in a fresh profile, got {value!r}"
