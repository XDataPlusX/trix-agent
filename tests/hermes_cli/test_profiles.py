...
class TestCreateProfile:
    ...
    def test_seeds_placeholder_env_file(self, profile_env):
        ...
        Since Task 10 of the client-profile-review spec, this is seeded
        from the same curated template (assets/config/trix.env.example)
        install.sh / doctor --fix use via resolve_env_template() — not a
        bare comment-only stub. Still no real credentials leak in.

        Since RAF-176 the template's unfilled blanks arrive COMMENTED OUT
        (``comment_out_empty_assignments``): under
        ``gateway.multiplex_profiles`` this file becomes the profile's
        authoritative secret scope, where ``KEY=`` reads as a configured
        empty value rather than as a blank nobody has filled in yet. The
        template's guidance text still lands verbatim — only the assignment
        itself is commented."""
        import stat
        ...
        # The template was seeded — its blanks are in the file, commented.
        assert "# TELEGRAM_BOT_TOKEN=" in content, "expected template not seeded"
        assert "TELEGRAM_BOT_TOKEN" not in lines
        # Nothing is left assigned-but-empty: the two meanings of ``KEY=``
        # (unfilled blank vs configured empty) must not share a spelling.
        assert not [var for var, value in lines.items() if not value.strip()]
        # What DOES remain assigned carries no credential — only the
        # non-secret operational hosts (NO_PROXY/no_proxy) the template
        # pre-fills on purpose. Asserted as a blanket rule, not a named
        # list, so a future template addition that ships pre-filled with a
        # real value fails this test immediately instead of silently
        # slipping through an outdated named check. (This profile_env
        # fixture's default install has no .env, so nothing is inherited
        # here either — that path has its own tests in
        # test_profile_env_authz_seed.py.)
        non_secret_prefilled = {"NO_PROXY", "no_proxy"}
        assert set(lines) <= non_secret_prefilled, (
            "unexpected pre-filled variables in a fresh profile: "
            f"{sorted(set(lines) - non_secret_prefilled)}"
        )
