...
class TestWebExtractPairedWithExtractBackend:
    ...
    def test_no_backend_configured_means_no_web_extract(self, template):
        # The KEY is present (the shipped template carries every
        # DEFAULT_CONFIG setting since the full-config switch) -- what must
        # hold is that its VALUE is empty, i.e. no paid extract backend is
        # configured. Asserting the key's absence was a proxy for this and
        # stopped meaning anything once the template became complete.
        assert not (template.get("web") or {}).get("extract_backend"), (
            "trix-config.yaml now configures web.extract_backend -- the other "
