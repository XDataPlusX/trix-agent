...
def test_no_open_coded_venv_layout_remains_in_hermes_cli():
    ...
    offenders = []
    # walk_files(), not rglob: this scan died on hermes_cli/__pycache__ under
    # the 18-worker run (RAF-171). See tests/_source_tree.py.
    for rel_py, py in walk_files(pkg, match="*.py"):
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Constant) and node.value == "Scripts":
                offenders.append(f"{rel_py}:{node.lineno}")
