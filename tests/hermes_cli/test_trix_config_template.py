...
# ---------------------------------------------------------------------------
# Полный шаблон (решение владельца, сентябрь 2026)
#
# Прежний инвариант этого файла был обратный: «шаблон обязан быть чистой
# дельтой — ни одно значение не равно дефолту». Он жил вместе с таблицей
# исключений ``_UPSTREAM_DRIFT_PINS``, куда попадало всё, что клиенту надо
# видеть в файле, даже совпадая с дефолтом. Владелец развернул политику:
# клиент должен видеть ВСЕ настройки в одном файле, а не только наши
# отклонения — поэтому и инвариант, и таблица исключений удалены, а их
# место занимают две проверки ниже.
#
# Что они закрывают вместо удалённого:
#   * полнота — в шаблоне есть каждый путь DEFAULT_CONFIG (раньше это не
#     проверялось никак, потому что считалось нарушением);
#   * действенность — наши отклонения от апстрима всё ещё отклонения.
#     Раньше это следовало из «ничего не равно дефолту»; теперь, когда
#     равно почти всё, нужна явная проверка, иначе пересборка шаблона,
#     молча сбросившая наше значение на дефолтное, прошла бы незамеченной.
# ---------------------------------------------------------------------------
...
#: Наши отклонения от апстрима: путь -> ожидаемое значение в шаблоне.
#:
#: Это НЕ снимок значений DEFAULT_CONFIG (такой тест краснел бы на каждом
#: релизе апстрима) — это список продуктовых решений. Каждая строка
#: проверяется дважды: значение в шаблоне равно нашему И отличается от
#: апстримного. Вторая половина не даёт списку тихо выродиться в перечень
#: строк, которые уже ничего не меняют.
_OUR_DEVIATIONS = {
    ("terminal", "backend"): "docker",
    ("terminal", "cwd"): "/workspace",
    ("terminal", "container_memory"): 3072,
    ("terminal", "container_cpu"): 2,
    ("terminal", "docker_extra_args"): ["-p", "18000-18009:18000-18009"],
    ("web", "search_backend"): "ddgs",
    ("browser", "backend"): "off",
    ("gateway", "media_retention_hours", "documents"): 0,
...
class TestTemplatePointsOnlyAtThingsTheClientHas:
    """Шаблон не отправляет клиента в файл, которого у него нет.

    ``docs/product`` вырезается из дерева витрины, поэтому ссылка туда для
    клиента ведёт в никуда — а в момент чтения комментария он как раз ищет
    объяснение и упирается в отсутствующий файл. Тот же класс дефекта уже
    сторожится для ``.env``
    (``test_does_not_point_at_a_file_the_customer_does_not_have``); здесь та
    же проверка для ``config.yaml``, потому что ровно этот дефект в нём и
    нашёлся при осмотре собранного дерева витрины.
    """

    def test_no_reference_to_our_internal_docs(self):
        text = TRIX_TEMPLATE_PATH.read_text(encoding="utf-8")
        assert "docs/product" not in text, (
            "шаблон ссылается на нашу внутреннюю документацию — у клиента "
            "этого каталога нет, ссылка ведёт в никуда"
        )

    def test_every_local_path_it_mentions_ships_to_the_client(self):
        """Любой упомянутый путь репозитория обязан быть в поставке.

        Проверяются только пути, похожие на файл нашего дерева: внешние
        ссылки (``https://…``) и пути внутри контейнера (``/workspace``) под
        правило не попадают.
        """
        import re

        text = TRIX_TEMPLATE_PATH.read_text(encoding="utf-8")
        repo_root = TRIX_TEMPLATE_PATH.parents[2]
        candidates = set(
            re.findall(r"(?<![\w/.])([a-z_][a-z0-9_]*(?:/[\w.-]+)+\.(?:md|py|yaml|yml|sh))", text)
        )
        missing = [
            path
        ...
        assert not missing, f"упомянуты несуществующие файлы: {missing}"


class TestTemplateIsComplete:
    """Шаблон — полный конфиг, а не дельта."""

    def test_every_default_config_path_is_in_the_template(self, template):
        have = set(_all_paths(template))
        missing = [path for path in _all_paths(DEFAULT_CONFIG) if path not in have]
        assert not missing, (
            f"в шаблоне не хватает {len(missing)} путей из DEFAULT_CONFIG "
            f"(первые: {[ '.'.join(map(str, p)) for p in missing[:10] ]}). "
            "Апстрим добавил настройки — довести шаблон: "
            "python3 scripts/build_trix_config.py"
        )

    def test_the_template_adds_no_root_upstream_cannot_read(self, template):
        """Корень, которого апстрим не читает, — опечатка: он не применится."""
        from hermes_cli.config import _KNOWN_ROOT_KEYS

        unknown = sorted(set(template) - set(_KNOWN_ROOT_KEYS))
        assert not unknown, f"корни, которых апстрим не читает: {unknown}"

    def test_the_template_adds_no_nested_path_upstream_cannot_read(self, template):
        """То же для ВЛОЖЕННЫХ путей.

        Проверка по корням мимо этого проходит: опечатка внутри секции
        (`terminal.contaner_memory`) даёт известный корень и неизвестный ключ,
        то есть настройку, которая молча не применяется никогда. Найдено
        ревью: прежний тест обещал ловить «путь, которого в апстриме нет
        вовсе», а сверял только верхний уровень.

        Список ниже — пути, которые читает не ``DEFAULT_CONFIG``, а шлюз
        (верхнеуровневые формы из ``_EXTRA_KNOWN_ROOT_KEYS``) либо
        ``gateway/display_config.py``. Каждый обязан быть перечислен здесь
        явно: свободное «ну эти можно» вернуло бы дыру.
        """
        ...
        # Внутри `platforms` живёт наша собственная схема (`telegram.extra.*`),
        # апстрим её читает в gateway/config.py, а не через DEFAULT_CONFIG.
        unknown = []
        for path, _value in _collect_leaves(template, ()):
            if path in allowed_outside_default_config or path[:1] == ("platforms",):
                continue
            if _is_free_form_mapping_leaf(path):
                continue
            exists, _default = _lookup(DEFAULT_CONFIG, path)
            if exists:
                continue
            if len(path) == 2 and path[0] == "display" and path[1] in _gateway_display_defaults():
                continue
            unknown.append(".".join(map(str, path)))
        assert not unknown, (
            f"путей, которых апстрим не читает: {unknown} — такая настройка "
            "молча не применится никогда (опечатка?)"
        )

    @pytest.mark.parametrize(
        "path, expected",
        sorted(_OUR_DEVIATIONS.items()),
        ids=lambda v: ".".join(map(str, v)) if isinstance(v, tuple) else repr(v),
    )
    def test_our_deviation_is_in_the_template_and_still_deviates(
        self, template, path, expected
    ):
        in_template, template_value = _lookup(template, path)
        dotted = ".".join(map(str, path))
        assert in_template, f"{dotted} пропал из шаблона"
        assert template_value == expected and type(template_value) is type(expected), (
            f"{dotted}={template_value!r}, ожидали {expected!r} — наше "
            "решение сброшено (похоже на пересборку шаблона поверх дефолта)"
        )

        in_default, default_value = _lookup(DEFAULT_CONFIG, path)
        assert in_default, (
            f"{dotted} исчез из DEFAULT_CONFIG — апстрим переименовал ключ, "
            "наша строка больше ни на что не влияет"
        )
        assert template_value != default_value or type(template_value) is not type(
            default_value
        ), (
            f"{dotted} совпал с апстримным дефолтом ({default_value!r}) — "
            "строка больше ничего не переопределяет. Проверьте объяснение "
            "над ней: оно почти наверняка стало неправдой."
        )
...
class TestTerminalBackendIsDocker:
    ...
    catch a regression in the shipped file. Since the template became
    complete, a replacement with ``local`` is indistinguishable by shape
    from any other value in the file -- nothing but this test catches it,
...
class TestToolLoopGuardrails:
    ...
    def test_hard_stop_is_a_real_override_of_the_upstream_default(self):
        ...
        перестанет что-либо менять, а объяснение над ней («по умолчанию
        только предупреждение») станет неправдой. С полным шаблоном по
        форме файла этого не видно — все значения выписаны одинаково, —
        поэтому расхождение проверяется здесь отношением.
        """
        assert (
