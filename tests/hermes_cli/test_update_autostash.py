...
def _make_update_side_effect(
    ...
    Origin always answers as our own release repo, so callers pass the
    release-source guard's origin check (``_assert_release_source``) by
    default -- that guard is enforced unconditionally. ``current_branch``
    still only feeds the "current branch" simulation; the guard doesn't
    read the branch (controller ruling, fix round 1: only origin carries
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "remote" in joined and "get-url" in joined and "origin" in joined:
            return SimpleNamespace(stdout=f"{RELEASE_REPO_HTTPS}\n", stderr="", returncode=0)
