...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    ...
    Restricted to the subset of ``_RECENTLY_SHIPPED_TOOLSETS`` actually
    allowed on "cli" — a toolset scoped away from cli by
    ``_toolset_allowed_for_platform`` (e.g. spec 19's ``secrets``, which only
    makes sense on a messaging platform with no other way to hand the agent
    a credential) is correctly ABSENT from both sides here; its own
    platform-scoped recovery is covered by
    ``test_platform_restricted_recently_shipped_toolset_recovers_on_its_platform``.
    """
    from hermes_cli.tools_config import _toolset_allowed_for_platform

    cli_relevant = {
        ts for ts in _RECENTLY_SHIPPED_TOOLSETS
        if _toolset_allowed_for_platform(ts, "cli")
    }
    if not cli_relevant:
        pytest.skip("no recently-shipped toolset is allowed on cli right now")
    ...
    assert cli_relevant <= (on_composite & on_saved_list)


@_requires_recently_shipped
def test_platform_restricted_recently_shipped_toolset_recovers_on_its_platform():
    """Spec 19 — a toolset scoped to one platform via
    ``_TOOLSET_PLATFORM_RESTRICTIONS`` must still reach an already-saved
    explicit list on THAT platform, exactly like an unrestricted
    recently-shipped toolset does on cli. Skips cleanly if the current
    release has no platform-restricted entry in the set (steady state)."""
    from hermes_cli.tools_config import _toolset_allowed_for_platform

    restricted = {
        ts for ts in _RECENTLY_SHIPPED_TOOLSETS
        if not _toolset_allowed_for_platform(ts, "cli")
    }
    if not restricted:
        pytest.skip("no platform-restricted toolset is currently in its first release")

    for ts_key in restricted:
        home_platform = next(
            p for p in ("telegram", "discord", "slack")
            if _toolset_allowed_for_platform(ts_key, p)
        )
        enabled = _get_platform_tools(
            _saved_list_from_before(home_platform),
            home_platform,
            include_default_mcp_servers=False,
        )
        assert ts_key in enabled, (ts_key, home_platform)
        # And it must never leak onto cli, restriction or not.
        cli_enabled = _get_platform_tools(
            _saved_list_from_before("cli"), "cli", include_default_mcp_servers=False,
        )
        assert ts_key not in cli_enabled
