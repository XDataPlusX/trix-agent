...
def test_apply_preserves_client_template_comments(tmp_path, monkeypatch):
    ...
    *why* each non-default setting is set. Before the round-trip-YAML fix,
    ...
    assert "Секреты (ключи, токены, пароли) сюда не пишутся" in new_text
...
def test_custom_endpoint_key_goes_to_config_not_env(tmp_path, monkeypatch):
    """Свой OpenAI-совместимый эндпоинт — ЕДИНСТВЕННЫЙ случай, когда ключ
    провайдера уезжает не в `.env`, а в `config.yaml` (`model.api_key`).

    Причина структурная, а не вкусовая: у провайдера `custom` нет своей
    переменной окружения (`env_vars=()`), писать ключ в `.env` просто не
    во что, а рантайм читает его из конфига (источник `model_config`,
    agent/credential_sources.py). Решение владельца 2026-09-12 — принять
    апстримный способ вместо своего моста в резолве ключей.

    Тест сторожит обе стороны: ключ появился в конфиге И не появился в
    `.env` (иначе он лежал бы в двух местах и расходился при смене)."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_constants import get_hermes_home
    ...
    assert "model.api_key" in result["written"], result

    import yaml

    cfg = yaml.safe_load((get_hermes_home() / "config.yaml").read_text(encoding="utf-8"))
    assert cfg["model"]["api_key"] == "sk-my-endpoint", cfg["model"]
    assert cfg["model"]["provider"] == "custom", cfg["model"]
    assert cfg["model"]["base_url"] == "https://llm.example.org/v1", cfg["model"]
    assert cfg["model"]["default"] == "my-model-7b", cfg["model"]

    env_path = get_hermes_home() / ".env"
    env_text = env_path.read_text(encoding="utf-8") if env_path.exists() else ""
    assert "sk-my-endpoint" not in env_text, env_text


def test_known_provider_key_still_goes_to_env_not_config(tmp_path, monkeypatch):
    """Обратная сторона: исключение для своего эндпоинта не должно
    расползтись на обычных провайдеров. У них ключ по-прежнему в `.env`,
    и в `config.yaml` его быть не должно."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_constants import get_hermes_home
    ...
    cfg = yaml.safe_load((get_hermes_home() / "config.yaml").read_text(encoding="utf-8"))
    assert "api_key" not in (cfg.get("model") or {}), cfg.get("model")
    env_text = (get_hermes_home() / ".env").read_text(encoding="utf-8")
    assert "OPENROUTER_API_KEY=sk-or-secret" in env_text
