...
class TestFallbackChain:
    """``_fetch_manifest_with_fallback`` walks ``fallback_urls`` when the
    primary URL fails. It used to walk a second host by default (the Nous
    docs site behind Vercel, which occasionally returned HTTP 403 +
    x-vercel-mitigated: challenge for urllib, with raw.githubusercontent.com
    as the fallback). This product has no separate docs site: the primary
    URL (``DEFAULT_CATALOG_URL``) already IS the release repo's raw-content
    address, so a fallback list containing that same URL would just
    double-fetch the one host that's already down — ``DEFAULT_CATALOG_FALLBACK_URLS``
    is therefore empty by default. The walking mechanism itself stays
    generic (an operator can still configure a real second host), so it's
    exercised directly here with synthetic URLs rather than the removed
    two-host default.
    """

    PRIMARY = "http://primary.example/model-catalog.json"
    FALLBACK = "http://mirror.example/model-catalog.json"

    def test_default_fallback_chain_is_empty(self, isolated_home):
        """No second host to fall back to by default — see class docstring."""
        from hermes_cli import model_catalog
    ...
    def test_falls_through_to_configured_fallback_on_primary_failure(self, isolated_home):
        """The walking mechanism still works when an operator configures a
        real fallback chain — it just isn't populated by default anymore."""
        from hermes_cli import model_catalog
        calls: list[str] = []
    ...
    def test_get_catalog_returns_empty_when_primary_fails_and_no_fallback_configured(
        self, isolated_home
    ):
        """End-to-end with the real (empty) default fallback chain: a primary
        failure with no disk cache and no configured fallback yields an empty
        catalog rather than crashing — callers fall back to in-repo lists."""
        from hermes_cli import model_catalog
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        manifest = json.loads(
            (repo_root / "assets" / "api" / "model-catalog.json").read_text()
        )
...
# the shipped manifest at assets/api/model-catalog.json (formerly published
# under website/static/api/ before the docs site stopped shipping to
# clients; see Task 11 of the installer/updater plan).
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["nous"] in commit
# 9dd6e5510 but the manifest was not regenerated for weeks, so free-tier
# users on a new install fetched a stale manifest and the free-tier picker
# showed "No free models currently available." even though the Portal was
# serving qwen/qwen3.6-plus as free. CI must catch this.
# -----------------------------------------------------------------------------
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        commit the regenerated ``assets/api/model-catalog.json``.
        """
        # Resolve the repo root from this test file's location.
        repo_root = Path(__file__).resolve().parents[2]
        manifest_path = repo_root / "assets" / "api" / "model-catalog.json"

        # Assert rather than skip: this test is the drift guard between the
        # shipped manifest and the code that generates it. If the manifest
        # moves again, a skip would quietly stop guarding anything -- which
        # is exactly how the previous move went unnoticed until it broke
        # offline resolution on a client.
        assert manifest_path.exists(), (
            f"manifest missing at {manifest_path} -- if it moved, update this "
            "test and every producer/consumer of the path together"
        )
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            "assets/api/model-catalog.json is out of sync with "
            "_PROVIDER_MODELS['nous'] / OPENROUTER_MODELS. "
            "Run: python scripts/build_model_catalog.py && "
            "git add assets/api/model-catalog.json"
        )
