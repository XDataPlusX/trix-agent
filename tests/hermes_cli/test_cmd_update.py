...
class TestUnfinishedUpdateTailIsFinishedOnRerun:
    """Обрыв в ХВОСТЕ обновления лечится повторным ``hermes update``.

    ``hermes update`` ставит новый код первым делом, а миграцию конфига, досев
    секций и перезапуск шлюза делает после. Процесс, умерший в этом хвосте —
    на машине клиента это обычная закрывшаяся SSH-сессия — оставлял установку
    с новым кодом, старым конфигом и шлюзом на прежних модулях. Повторный
    запуск положение не исправлял: видя свежий checkout, он уходил на
    «Already up to date» раньше хвоста. То есть единственное действие, которое
    оператор предпримет первым, не работало (поймано живой раскаткой
    2026-09-10).
    """

    @pytest.fixture
    def marker(self, tmp_path, monkeypatch):
        """Подменённый путь крошки: живой checkout трогать нельзя."""
        path = tmp_path / ".post-update-incomplete"
        monkeypatch.setattr(
            "hermes_cli.main._post_update_marker_path", lambda: path
        )
        return path

    @staticmethod
    def _stale_client_config():
        from hermes_cli.config import get_config_path
        from hermes_cli.config_defaults import DEFAULT_CONFIG
        ...
        path.write_text(
            "terminal:\n  backend: docker\n  cwd: /workspace\n"
            "\nweb:\n  search_backend: ddgs\n"
            f"\n_config_version: {current_ver}\n",
            encoding="utf-8",
        )
        return path, current_ver

    def _run_up_to_date(self, mock_args, current_ver):
        seen = []
    ...
    def test_marker_present_means_the_tail_runs(self, mock_args, capsys, marker):
        config_path, current_ver = self._stale_client_config()
        marker.write_text("started=1\npid=1\n", encoding="utf-8")

        seen = self._run_up_to_date(mock_args, current_ver)

        out = capsys.readouterr().out
        assert "не довело дело до конца" in out, out[-600:]
        data = yaml.safe_load(config_path.read_text(encoding="utf-8"))
        assert "display" in data, "досев секций в хвосте не отработал"
        assert any("gateway restart" in c for c in seen), (
            "шлюз не перезапущен — он остался бы на прежних модулях"
        )
        assert not marker.exists(), "крошка не снята после успешной доделки"
        assert "Update complete" in out

    def test_nothing_to_do_stays_a_plain_no_op(self, mock_args, capsys, marker):
        """Машине, у которой всё на месте, обновление ничего не делает.

        Конфиг здесь — поставляемый шаблон целиком, поэтому досеивать нечего, и
        путь обязан остаться тем же честным «Already up to date», каким был: ни
        перезапуска шлюза, ни разговоров про недоделанное обновление.

        Случай «крошки нет, а досеять есть что» — отдельный класс ниже
        (``TestUpToDateStillSyncsConfigWithoutAMarker``): там досев отрабатывает,
        и это новая задумка, а не регрессия.
        """
        ...
        template = (
            ...
            / "assets" / "config" / "trix-config.yaml"
        )
        config_path = get_config_path()
        config_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        out = capsys.readouterr().out
        assert "Already up to date" in out, out[-400:]
        assert "не довело дело до конца" not in out
        assert not any("gateway restart" in c for c in seen), (
            "обычный пустой запуск не имеет права трогать работающий шлюз"
        )

    def test_a_failed_restart_still_finishes_and_tells_the_operator(
        self, mock_args, capsys, marker
    ):
        _config_path, current_ver = self._stale_client_config()
        marker.write_text("started=1\npid=1\n", encoding="utf-8")

        def side_effect(cmd, **kwargs):
            joined = " ".join(str(c) for c in cmd)
            if "gateway" in joined and "restart" in joined:
        ...
        out = capsys.readouterr().out
        assert "hermes gateway restart" in out, "оператору не сказали, что делать руками"
        assert not marker.exists(), (
            "крошка должна сниматься и при неудачном перезапуске: конфиг уже "
            "доделан, и повторять доделку в цикле смысла нет"
        )


class TestRestartGatewaysViaCli:
    def test_it_calls_the_documented_command(self):
        from hermes_cli.update_cmd import _restart_gateways_via_cli

        with patch("subprocess.run") as run:
            run.return_value = subprocess.CompletedProcess([], 0, stdout="", stderr="")
            assert _restart_gateways_via_cli(quiet=True) is True

        argv = [str(a) for a in run.call_args[0][0]]
        assert argv[-2:] == ["gateway", "restart"]
        assert argv[1].endswith("hermes")

    def test_a_crash_in_the_subprocess_is_not_fatal(self):
        from hermes_cli.update_cmd import _restart_gateways_via_cli

        with patch("subprocess.run", side_effect=OSError("boom")):
            assert _restart_gateways_via_cli(quiet=True) is False


class TestPostUpdateMarkerLifecycle:
    """Крошка ставится после pull и снимается, когда хвост дошёл до конца.

    Если её забыть снять, КАЖДЫЙ следующий пустой ``hermes update`` будет
    доделывать хвост и дёргать работающий шлюз — то есть лекарство станет
    хуже болезни.
    """

    def test_a_full_successful_update_leaves_no_marker(
        self, mock_args, tmp_path, monkeypatch, capsys
    ):
        from hermes_cli.config import get_config_path
        from hermes_cli.config_defaults import DEFAULT_CONFIG

        marker = tmp_path / ".post-update-incomplete"
        monkeypatch.setattr("hermes_cli.main._post_update_marker_path", lambda: marker)
        ...
        assert not marker.exists(), (
            "крошка пережила успешное обновление — каждый следующий пустой "
            "запуск будет зря дёргать работающий шлюз"
        )

    def test_the_helper_reads_the_marker(self, tmp_path, monkeypatch):
        from hermes_cli.update_cmd import _post_update_tail_unfinished

        marker = tmp_path / ".post-update-incomplete"
        monkeypatch.setattr("hermes_cli.main._post_update_marker_path", lambda: marker)

        assert _post_update_tail_unfinished() is False
        marker.write_text("started=1\n", encoding="utf-8")
        assert _post_update_tail_unfinished() is True


class TestUpdateMarkersAreGitIgnored:
    """Крошки обновления обязаны игнорироваться git.

    Незаигнорированная крошка показывается как локальное изменение, и автостэш
    обновления её прячет и восстанавливает, печатая клиенту «Local changes were
    restored on top of the updated codebase — review git diff» на установке,
    где он ничего не менял. Наблюдалось живьём 2026-09-10 на
    ``.post-update-incomplete``; у двух её соседей это уже учтено, и
    ``.gitignore`` прямо называет причину.

    Проверяется настоящим ``git check-ignore``, а не чтением файла: важно
    поведение git, а не текст правила.
    """

    @pytest.mark.parametrize(
        "marker",
        [".update-incomplete", ".lazy-refresh-incomplete", ".post-update-incomplete"],
    )
    def test_marker_is_ignored(self, marker):
        ...
        assert result.returncode == 0, (
            f"{marker} не игнорируется: он попадёт в «Local changes detected», "
            "и автостэш обновления напугает клиента несуществующими правками"
        )


class TestUpToDateStillSyncsConfigWithoutAMarker:
    """Досев секций на пути «Already up to date» — без всякой крошки.

    Парный случай к ``TestCmdUpdateSyncsTrixConfigWithoutMigration``, который
    гоняет тот же сценарий при НАЛИЧИИ новых коммитов. Когда коммитов нет,
    штатный досев не вызывался вовсе: машина на свежем коде, которой досев ни
    разу не делали, через ``hermes update`` не лечилась никогда.

    Шлюз при этом не трогается: досеянные секции стоят на своих же дефолтах и
    поведения не меняют, а перезапуск обрывает живой разговор клиента.
    """

    def test_sections_arrive_and_the_gateway_is_left_alone(
        self, mock_args, tmp_path, monkeypatch, capsys
    ):
        from hermes_cli.config import get_config_path
        from hermes_cli.config_defaults import DEFAULT_CONFIG
        ...
        marker = tmp_path / ".post-update-incomplete"
        monkeypatch.setattr("hermes_cli.main._post_update_marker_path", lambda: marker)
        assert not marker.exists(), "крошки быть не должно — проверяем путь без неё"
        ...
        data = yaml.safe_load(config_path.read_text(encoding="utf-8"))
        assert "display" in data, (
            "секции не доехали: машина на свежем коде со старым конфигом "
            "так и осталась бы непочиненной"
        )
        assert not any("gateway restart" in c for c in seen), (
            "шлюз перезапущен из-за досеянных дефолтов — это обрывает живой "
            "разговор клиента без причины"
        )
        out = capsys.readouterr().out
        assert "не довело дело до конца" not in out, (
            "обрыва не было — незачем говорить клиенту про недоделанное обновление"
        )
