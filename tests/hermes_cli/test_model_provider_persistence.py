...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_roundtrip_yaml_save(self, config_home):
        """Provider switches should delegate config writes to the
        comment-preserving atomic_roundtrip_yaml_save (setup-wizard fix:
        the plain atomic_yaml_write/yaml.dump path silently dropped every
        comment in a client's config.yaml on the first provider switch)."""
        from hermes_cli.auth import _update_config_for_provider

        config_path = config_home / "config.yaml"
        original_text = config_path.read_text(encoding="utf-8")

        def _boom(path, new_state, **kwargs):
            assert path == config_path
            assert new_state["model"]["provider"] == "nous"
            assert new_state["model"]["base_url"] == "https://inference.example.com/v1"
            assert new_state["model"]["default"] == "some-old-model"
            raise OSError("simulated atomic write failure")

        with patch("utils.atomic_roundtrip_yaml_save", side_effect=_boom) as mock_write:
