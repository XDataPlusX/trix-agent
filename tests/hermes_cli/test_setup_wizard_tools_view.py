...
def test_firecrawl_self_hosted_row_appears_only_in_extract_when_alive(monkeypatch):
    """Живой localhost:3002 поднимает строку Firecrawl Self-Hosted в
    «Чтении страниц» — и НЕ поднимает её в «Поиске в интернете».

    Своего поиска у self-hosted Firecrawl нет: его /search проксирует
    fire-engine (только облако), SearXNG или DuckDuckGo. Каталог апстрима
    этого не различает — обе строки Firecrawl несут один
    ``web_backend: "firecrawl"``, и капабилити резолвятся по имени
    бэкенда, где ответ «умеет искать» верен только для облачной строки.
    Отсюда именная запись в EXCLUDED_ROWS[("web", ...)]."""
    from hermes_cli.setup_wizard import tools_view as tv
    ...
    row = next((r for r in extract_block["rows"] if r["name"] == "Firecrawl Self-Hosted"), None)
    assert row is not None, "Firecrawl Self-Hosted должен быть в «Чтении страниц», когда localhost:3002 жив"
    ...
    assert "Firecrawl Self-Hosted" not in {r["name"] for r in web_block["rows"]}, (
        "self-hosted Firecrawl не поисковик — в «Поиске в интернете» его быть не должно"
    )
    # Облачный Firecrawl поиском остаётся: у него поиск свой.
    assert "Firecrawl" in {r["name"] for r in web_block["rows"]}


def test_self_hosted_probe_runs_once_per_url(monkeypatch):
    """Finding 10 (review 2026-08-26, owner-approved fix): один вызов
    /api/form не должен щупать один и тот же локальный адрес дважды —
    каждая проба стоит до секунды таймаута.

    Раньше цена платилась строкой Firecrawl Self-Hosted, которая
    рендерилась и в "web", и в "web_extract". Сейчас в поиске её нет
    (EXCLUDED_ROWS — своего поиска у неё нет), но инвариант тот же и
    формулируется без опоры на то, какая строка в скольких категориях
    живёт: каждый РАЗЛИЧНЫЙ адрес щупается не больше одного раза."""
    from hermes_cli.setup_wizard import tools_view as tv
    ...
    assert calls, "проба вообще не запускалась — правило self-hosted перестало работать"
    assert len(calls) == len(set(calls)), calls

    # Обе self-hosted строки живы — каждая на своём месте и ровно там.
    web_names = {r["name"] for b in blocks if b["category"] == "web" for r in b["rows"]}
    extract_names = {r["name"] for b in blocks if b["category"] == "web_extract" for r in b["rows"]}
    assert "SearXNG" in web_names and "SearXNG" not in extract_names
    assert "Firecrawl Self-Hosted" in extract_names and "Firecrawl Self-Hosted" not in web_names
...
def test_row_names_are_unique_within_every_block(monkeypatch):
    """Имя строки — это её ИДЕНТИЧНОСТЬ для страницы: значение <option> в
    блоках «Поиск в интернете»/«Чтение страниц» строится из имени, потому
    что имени бэкенда не хватает (две строки Firecrawl несут один
    бэкенд). Два одинаковых имени в одном блоке снова сделали бы выбор
    неоднозначным — на этот раз молча. Пробу живости поднимаем, чтобы в
    выборку попали и self-hosted строки."""
    from hermes_cli.setup_wizard import tools_view as tv

    monkeypatch.setattr(tv, "_local_service_alive", lambda url, timeout=1.0: True)
    for block in tv.wizard_tool_blocks():
        names = [row["name"] for row in block["rows"]]
        assert len(names) == len(set(names)), (block["category"], names)
