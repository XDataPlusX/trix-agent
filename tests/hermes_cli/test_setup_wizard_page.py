...
# ---------------------------------------------------------------------------
# Двенадцать проверок блока провайдера ПЕРЕЕХАЛИ отсюда в
# test_setup_wizard_page_jsdom.py и стали исполняемыми (2026-09-11).
#
# Они читали исходник страницы регекспами: «assert 'name: providerRow ?
# providerRow.name' in html». CLAUDE.md такие тесты запрещает прямо, и
# перевод блока провайдера на общий для основного и запасного слота код
# показал, почему запрет правильный: поведение не изменилось ни в чём, а
# покраснели все двенадцать разом — они держались за форму кода, а не за
# то, что он делает. Сломанную реализацию с той же формой они бы
# пропустили.
#
# Что и где теперь проверяется исполнением в jsdom:
#   test_single_variant_group_skips_the_radio_and_goes_straight_to_its_subblock
#     -> test_primary_provider_slot_from_empty_to_payload
#   test_provider_api_key_field_resets_on_provider_switch
#     -> test_primary_provider_slot_from_empty_to_payload
#   test_fallback_provider_key_resets_on_selection_change
#     -> test_fallback_slot_checks_its_key_loads_models_and_submits_its_own_base_url
#   test_nothing_preselected_in_the_new_provider_list_on_first_entry
#     -> test_primary_provider_slot_from_empty_to_payload
#   test_provider_list_recommended_first_rest_collapsed_behind_a_link
#     -> test_primary_provider_slot_from_empty_to_payload
#   test_provider_payload_reads_name_from_chosen_variant_row
#     -> test_primary_provider_slot_from_empty_to_payload
#   test_device_code_variant_never_submits_stale_api_key_block_values
#     -> test_primary_device_code_variant_submits_no_key_and_no_base_url
#   test_device_model_select_resets_when_switching_device_code_providers
#     -> test_primary_device_code_variant_submits_no_key_and_no_base_url
#   test_device_status_poll_ignores_mismatched_login_id_and_provider
#     -> test_primary_device_code_variant_submits_no_key_and_no_base_url
#   test_provider_key_autochecks_on_change_and_blur_without_blocking_next
#     -> test_primary_provider_slot_from_empty_to_payload
#   test_provider_key_verdict_reset_on_provider_switch_and_empty_field
#     -> test_primary_provider_slot_from_empty_to_payload
#   test_telegram_and_provider_keys_are_trimmed_before_submit
#     -> test_primary_provider_slot_from_empty_to_payload
# ---------------------------------------------------------------------------
...
def test_provider_list_is_rows_not_a_bare_select():
    ...
    # Тот же контейнер-оболочка есть и у запасного слота: блок общий.
    fallback_idx = html.index('id="fallback_group"')
    fallback_tag = html[max(0, fallback_idx - 20) : fallback_idx + 80]
    assert "<select" not in fallback_tag
    assert 'class="prov"' in fallback_tag
...
def test_no_fetch_models_button_anywhere():
    ...
    # То, что список моделей и правда подтягивается сам после проверки
    # ключа, проверяется исполнением — см.
    # test_setup_wizard_page_jsdom.py::test_primary_provider_slot_from_empty_to_payload
    # и ..._fallback_slot_checks_its_key_loads_models_and_submits_its_own_base_url.
...
def test_fallback_block_lives_inside_the_provider_step():
    """Запасной провайдер стоит в разметке внизу шага 4 (Провайдер), а не
    приезжает туда переносом узла из шага 5.

    Раньше блок рисовался в «Дополнительно» и переезжал функцией
    relocateFallbackBlockForSteps(). С 2026-09-11 запасной слот — это тот
    же блок провайдера, что и основной, и он просто стоит на своём месте
    статикой: переносить нечего."""
    from hermes_cli.setup_wizard.page import render_page

    html = render_page()
    slot_idx = html.index('id="step4-fallback-slot"')
    slot_end = html.index('<div class="step-nav">', slot_idx)
    slot_html = html[slot_idx:slot_end]
    assert 'id="fallback-block"' in slot_html
    assert 'id="fallback_group"' in slot_html
    assert 'id="fallback_api_key"' in slot_html
    assert 'id="fallback_base_url"' in slot_html
    assert 'id="fallback_model"' in slot_html
    assert "relocateFallbackBlockForSteps" not in html
...
def test_advanced_step_holds_exactly_the_seven_catalog_categories_in_order():
    ...
    # Запасной провайдер уехал из «Дополнительно» на шаг 4 (он теперь тот
    # же блок, что и основной), поэтому границей служит конец самого
    # контейнера, а не бывшая строка #advanced-fallback.
    rows_chunk = html[advanced_idx : html.index('<div class="step-nav">', advanced_idx)]
...
def test_fallback_field_errors_have_matching_err_elements():
    ...
    html = render_page()
    assert 'id="err_fallback_name"' in html
    assert 'id="err_fallback_api_key"' in html
    # И у основного слота — те же два, под его собственными именами:
    # блок один и тот же, различаются только идентификаторы.
    assert 'id="err_provider_name"' in html
    assert 'id="err_provider_api_key"' in html
...
def _call_extracted_fn(
    html: str, fn_name: str, *, dom: dict, call: str, extra: str = "", deps: tuple = ()
) -> object:
    ...
    #
    # ``deps`` — имена ДРУГИХ функций страницы, которые вызывает
    # проверяемая. Берём их НАСТОЯЩИЕ тела, а не пишем заглушку: смысл
    # этих тестов в том, что исполняется реальный код страницы, и
    # заглушка на месте зависимости ровно эту ценность и убивает.
    body = "".join(_function_body(html, name) + "\n}\n" for name in deps)
...
def test_image_video_gen_choice_untouched_states_never_clear(fn_name, select_id, current_key):
    ...
    html = render_page()
    deps = ("webRowByChoiceValue",) if fn_name == "extractBackendChoiceValue" else ()
...
def test_step4_section_titles_read_bigger_than_a_field_label_but_smaller_than_a_step_title():
    ...
    # Заголовок ЗАПАСНОГО слота — тот же элемент с тем же классом: с
    # 2026-09-11 это один и тот же блок, отрисованный дважды, а не
    # самодельный <h3>, который раньше строил renderAdvancedFallback().
    assert '<label id="fallback_group_label" class="section-title" for="fallback_group">' in html

    # Размер задаёт ровно одно правило CSS, и в его списке селекторов
    # есть класс, который носят оба заголовка.
...
# --- Две строки каталога с ОДНИМ бэкендом ---------------------------------
#
# «Firecrawl» (облако, FIRECRAWL_API_KEY) и «Firecrawl Self-Hosted»
# (FIRECRAWL_API_URL) несут один и тот же ``web_backend: "firecrawl"``.
# Пока значением <option> было имя бэкенда, в «Чтении страниц»
# получались два пункта с ОДИНАКОВЫМ value: выбрать второй было нельзя
# физически, и клиент, ткнувший в облачный Firecrawl, получал поле
# «Адрес сервера» вместо поля ключа. Значение опции — идентичность
# строки; ниже исполняется настоящий код страницы, а не его текст.

_TWO_FIRECRAWL_ROWS = """
var toolBlockFor = function (category) {
  if (category !== "web_extract") return null;
  return { rows: [
    { name: "Firecrawl Self-Hosted", web_backend: "firecrawl",
      env_vars: [{ key: "FIRECRAWL_API_URL" }] },
    { name: "Exa", web_backend: "exa", env_vars: [{ key: "EXA_API_KEY" }] },
    { name: "Firecrawl", web_backend: "firecrawl",
...
@requires_node
@pytest.mark.parametrize(
    "chosen_row,expected_env",
    [
        ("Firecrawl Self-Hosted", "FIRECRAWL_API_URL"),
        ("Firecrawl", "FIRECRAWL_API_KEY"),
    ],
)
def test_extract_env_follows_the_chosen_row_not_the_backend_name(chosen_row, expected_env):
    """Ключ уходит в переменную ВЫБРАННОЙ строки. До правки обе ветки
    отдавали FIRECRAWL_API_URL — переменную первой строки."""
    from hermes_cli.setup_wizard.page import render_page

    out = _call_extracted_fn(
        render_page(),
        "extractEnvPayload",
        dom={"extract_choice": chosen_row, "extract_env_value": "секрет"},
        extra=_TWO_FIRECRAWL_ROWS + "var state = { current: {} };",
        call="extractEnvPayload()",
        deps=("webRowChoiceValue", "webRowByChoiceValue"),
    )
    assert out == {"key": expected_env, "value": "секрет"}


@requires_node
@pytest.mark.parametrize("chosen_row", ["Firecrawl Self-Hosted", "Firecrawl"])
def test_extract_backend_is_the_chosen_rows_backend(chosen_row):
    """Наружу (в config.yaml) по-прежнему уходит ИМЯ БЭКЕНДА — обе строки
    дают ``firecrawl``, различие живёт только в переменной окружения."""
    from hermes_cli.setup_wizard.page import render_page

    out = _call_extracted_fn(
        render_page(),
        "extractBackendChoiceValue",
        dom={"extract_choice": chosen_row},
        extra=_TWO_FIRECRAWL_ROWS + "var state = { current: {} };",
        call="extractBackendChoiceValue()",
        deps=("webRowChoiceValue", "webRowByChoiceValue"),
    )
    assert out == "firecrawl"


@requires_node
@pytest.mark.parametrize(
    "saved_env_var,expected_row",
    [
        ("FIRECRAWL_API_KEY", "Firecrawl"),
        ("FIRECRAWL_API_URL", "Firecrawl Self-Hosted"),
        (None, "Firecrawl Self-Hosted"),
    ],
)
def test_saved_backend_resolves_to_the_row_whose_key_is_actually_set(saved_env_var, expected_row):
    """Возврат клиента: в конфиге лежит только имя бэкенда. Какая из двух
    строк была выбрана, решает сохранённая переменная — то же правило,
    что и на сервере в _current_web_capability_env(). Без сохранённой
    переменной — первая строка, как и было."""
    from hermes_cli.setup_wizard.page import render_page
    ...
    current_env = "null" if saved_env_var is None else '{ env_var: "%s" }' % saved_env_var
    out = _call_extracted_fn(
        render_page(),
        "webRowValueForSavedBackend",
        dom={},
        extra="var ROWS = %s; var CUR = %s;" % (rows, current_env),
        call='webRowValueForSavedBackend(ROWS, "firecrawl", CUR)',
        deps=("webRowChoiceValue",),
    )
    assert out == expected_row


def _element_id_tags(html: str) -> dict:
    """``id`` -> имя тега для КАЖДОГО элемента страницы.

    Разбором, а не регекспом по тексту: имена идентификаторов встречаются
    и внутри <style>/<script> (комментарии, обращения byId), и поиск
    подстрибой посчитал бы их за элементы.
    """
    from html.parser import HTMLParser
...
def _element_id_counts(html: str) -> dict:
    from html.parser import HTMLParser
...
def test_primary_provider_block_markup_is_unchanged_by_the_slot_template():
    """Разметка ОСНОВНОГО слота обязана совпадать с той, что была до
    появления общего шаблона, — иначе весь внешний код и CSS, которые
    держатся за её идентификаторы, тихо отвалятся.

    Проверяется не текстом-эталоном (он бы устарел на первой же правке),
    а свойством: шаблон, заполненный картой имён основного слота, даёт
    ровно то, что лежит в готовой странице, и каждый идентификатор из
    карты в ней присутствует ровно один раз."""
    import re
    ...
    # Ни один идентификатор слота не продублирован — ни внутри слота, ни
    # между слотами (карты не должны пересекаться). Считаем настоящие
    # ЭЛЕМЕНТЫ разбором, а не поиском подстроки: текст внутри <style> и
    # <script> тоже содержит эти имена (комментарии в CSS и коде), и
    # регексп по всей странице ловил бы их как элементы.
    assert not (set(_PRIMARY_SLOT_IDS.values()) & set(_FALLBACK_SLOT_IDS.values()))
    counts = _element_id_counts(html)
    for ids in (_PRIMARY_SLOT_IDS, _FALLBACK_SLOT_IDS):
        for element_id in ids.values():
            assert counts.get(element_id, 0) == 1, (element_id, counts.get(element_id, 0))


def test_both_provider_slots_render_the_same_set_of_elements():
    """Полнота паритета структурно: у запасного слота есть КАЖДЫЙ элемент
    основного — поле ключа, выбор модели, адрес API, под-блок входа по
    аккаунту и место под вердикт проверки ключа.

    До 2026-09-11 запасной слот был плоским <select> плюс полем ключа, и
    ровно этого списка ему не хватало."""
    import re
    ...
    tags = _element_id_tags(html)
    for name in _PRIMARY_SLOT_IDS:
        primary_id, fallback_id = _PRIMARY_SLOT_IDS[name], _FALLBACK_SLOT_IDS[name]
        assert primary_id in tags, primary_id
        assert fallback_id in tags, fallback_id
        assert tags[primary_id] == tags[fallback_id], (name, tags[primary_id], tags[fallback_id])


def test_slot_id_map_in_the_script_is_the_python_one(tmp_path):
    """Карта имён элементов слота на странице обязана БЫТЬ питоновой, а не
    её копией.

    Разметка обоих слотов собирается из `_PRIMARY_SLOT_IDS` /
    `_FALLBACK_SLOT_IDS`, а javascript адресует элементы через ту же карту.
    Пока она была вклеена в текст скрипта копией, добавление одного поля в
    шаблон требовало правки в двух местах — и расхождение здесь означает
    не падение, а молча неработающий элемент у одного из слотов.
    """
    import json
    import re
    ...
    html = render_page()
    match = re.search(r"var PROVIDER_SLOT_IDS = (\{.*?\n  \});", html, re.DOTALL)
    assert match, "карта слотов не найдена в скрипте страницы"
    rendered = json.loads(match.group(1))

    assert rendered == {"provider": _PRIMARY_SLOT_IDS, "fallback": _FALLBACK_SLOT_IDS}

    # И каждое имя из карты — настоящий элемент страницы, по одному разу.
    counts = _element_id_counts(html)
    for slot_ids in rendered.values():
        for element_id in slot_ids.values():
            assert counts.get(element_id, 0) == 1, (element_id, counts.get(element_id, 0))
