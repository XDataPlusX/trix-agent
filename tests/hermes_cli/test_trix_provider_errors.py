...
from hermes_cli.trix_provider_errors import (
    ...
    client_provider_not_configured_message,
)
...
class TestClientProviderNotConfiguredMessage:
    """Defect: hermes_cli.auth's raw AuthError("No inference provider
    configured. Run 'hermes model' ... in ~/.hermes/.env.") used to reach
    the client verbatim through gateway/run.py's `provider_auth_exception`
    catch-all. The client has no shell and no file access — only the setup
    wizard (spec 18/1)."""

    def test_says_nothing_about_files_commands_or_upstream_branding(self):
        msg = client_provider_not_configured_message()
        assert "hermes " not in msg.lower()
        assert ".hermes" not in msg
        assert "~/" not in msg
        # No raw env var names.
        for token in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY"):
            assert token not in msg

    def test_points_at_the_setup_wizard(self):
        msg = client_provider_not_configured_message().lower()
        assert "мастер" in msg or "wizard" in msg

    def test_survives_a_broken_catalog(self, monkeypatch):
        monkeypatch.setattr(i18n_mod, "_load_catalog", lambda lang: {})
        msg = client_provider_not_configured_message()
        assert "trix.errors.provider" not in msg
