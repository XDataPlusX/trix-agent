...
def test_steady_state_housekeeping_trim_stays_quiet_by_default(monkeypatch, caplog, tmp_path):
    """Default config must keep sub-1MB housekeeping trims (the per-minute
    gateway housekeeping spam from the 2026-09-12 debug report) out of
    agent.log, while a meaningful release still logs once."""
    from hermes_constants import reset_hermes_home_override, set_hermes_home_override

    monkeypatch.setattr(mem_trim.gc, "collect", lambda: None)
    monkeypatch.setattr(mem_trim, "_malloc_trim", lambda _pad: 1)
    # Advance the clock past the 60s cooldown between the two trims.
    ...
    # before/after snapshots per trim (KiB): trim 1 releases 760 (noise),
    # trim 2 releases 26248 (a real release).
    snapshots = iter(
        (
            {"rss_kib": 165896, "rss_anon_kib": 135988, "thread_count": 11},
            {"rss_kib": 165136, "rss_anon_kib": 135228, "thread_count": 11},
            {"rss_kib": 165136, "rss_anon_kib": 135228, "thread_count": 11},
            {"rss_kib": 138888, "rss_anon_kib": 108980, "thread_count": 11},
        )
    )
    ...
    monkeypatch.setattr(mem_trim, "collect_memory_snapshot", fake_snapshot)

    with caplog.at_level("INFO", logger="hermes_cli.mem_trim"):
        assert mem_trim.trim_memory(reason="messaging gateway housekeeping") is True
        assert mem_trim.trim_memory(reason="startup") is True

    messages = [record.getMessage() for record in caplog.records]
    housekeeping = [m for m in messages if "reason=messaging gateway housekeeping" in m]
    startup = [m for m in messages if "reason=startup" in m]
    assert not housekeeping, "sub-1MB steady-state trims must not INFO-log by default"
    assert startup, "a ~26MB release must still log"


def test_default_threshold_matches_config_default():
    """The code fallback and DEFAULT_CONFIG must agree — the module default is
    what runs when the user config omits the key."""
    from hermes_cli.config_defaults import DEFAULT_CONFIG

    configured = DEFAULT_CONFIG["context"]["memory_trim"]["info_log_min_delta_mb"]
    assert mem_trim._DEFAULT_INFO_LOG_MIN_DELTA_MB == configured
