...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("Trix Agent", "Install directory:", "Python:", "OpenAI SDK:"):
        assert field in out, f"fast --version output missing {field!r}:\n{out}"
    assert "Hermes Agent" not in out, "fast --version must not name the upstream product"
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    assert "Trix Agent" in result.stdout
    assert "Hermes Agent" not in result.stdout, "fast --version must not name the upstream product"
