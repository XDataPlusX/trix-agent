...
class TestProviderEnvDetection:
    """``_has_provider_env_config`` must tell a USABLE secret apart from a
    hint variable NAME merely appearing in the file -- an empty/placeholder
    value or a commented-out line is not "configured", even though the
    variable name is present in the raw text either way.
    """
    ...
    def test_provider_var_present_but_empty_is_not_configured(self):
        """The exact shape Trix's own curated .env template ships before
        the customer fills it in: OPENROUTER_API_KEY= with no value."""
        content = "TELEGRAM_BOT_TOKEN=\nOPENROUTER_API_KEY=\nGLM_API_KEY=\n"
        assert not _has_provider_env_config(content)

    def test_provider_var_commented_out_is_not_configured(self):
        """The exact shape upstream's .env.example ships every provider
        hint in -- commented out, not a real assignment."""
        content = "# OPENROUTER_API_KEY=your_api_key_here\n# OPENAI_API_KEY=sk-...\n"
        assert not _has_provider_env_config(content)

    def test_provider_var_with_placeholder_value_is_not_configured(self):
        content = "OPENROUTER_API_KEY=changeme\n"
        assert not _has_provider_env_config(content)

    def test_provider_var_with_plausible_value_is_configured(self):
        content = "OPENROUTER_API_KEY=sk-or-v1-1234567890abcdef\n"
        assert _has_provider_env_config(content)

    def test_falls_back_correctly_when_dotenv_values_is_unimportable(self, monkeypatch):
        """Several test modules (test_fireworks_provider.py,
        test_gmi_provider.py, ...) replace sys.modules["dotenv"] with a
        bare stub exposing only load_dotenv(), to keep .env auto-loading
        a no-op during collection. That breaks
        `from dotenv import dotenv_values` for every OTHER caller sharing
        that process, not just theirs -- including doctor.run_doctor(),
        which crashed outright the first time this fix shipped.
        _has_provider_env_config must fall back to _parse_dotenv_lines()
        and still get the right answer, not raise or silently return
        ...
        assert _has_provider_env_config("OPENROUTER_API_KEY=sk-or-v1-1234567890abcdef\n")
        assert not _has_provider_env_config("OPENROUTER_API_KEY=\n")
        assert not _has_provider_env_config("# OPENROUTER_API_KEY=your_api_key_here\n")


class TestParseDotenvLines:
    """``_parse_dotenv_lines`` is the dependency-free fallback parser --
    relation tests against real .env syntax, not a snapshot of one shape.
    """

    def test_skips_comments_and_blank_lines(self):
        from hermes_cli.doctor import _parse_dotenv_lines

        values = _parse_dotenv_lines("# comment\n\nKEY=value\n")
        assert values == {"KEY": "value"}

    def test_strips_matching_quotes(self):
        from hermes_cli.doctor import _parse_dotenv_lines

        values = _parse_dotenv_lines('KEY="quoted value"\nOTHER=\'single\'\n')
        assert values == {"KEY": "quoted value", "OTHER": "single"}

    def test_tolerates_export_prefix(self):
        from hermes_cli.doctor import _parse_dotenv_lines

        values = _parse_dotenv_lines("export KEY=value\n")
        assert values == {"KEY": "value"}

    def test_empty_assignment_yields_empty_string_not_missing_key(self):
        from hermes_cli.doctor import _parse_dotenv_lines

        values = _parse_dotenv_lines("KEY=\n")
        assert values == {"KEY": ""}
...
def test_doctor_surfaces_the_trix_config_sync_sidecar_location_when_it_exists(monkeypatch, tmp_path):
    """Review round 2, minor finding: nobody knows the anti-resurrection
    sidecar exists. A client who deleted a setting we once seeded has no
    way to find out that removing this file resets that protection —
    ``hermes doctor`` must say so, unconditionally (not just under --fix).

    Round 3 tightened this to require the sidecar to actually be on disk
    (doctor must not claim a fact it never checked) — this test creates it
    to prove the message still fires in the case it exists for."""
    home = tmp_path / ".hermes"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text(
        "terminal:\n  backend: docker\n_config_version: 34\n", encoding="utf-8"
    )
    (home / "trix_config_sync_state.json").write_text("{}", encoding="utf-8")
    ...
    out = buf.getvalue()
    assert "trix_config_sync_state.json" in out
    assert str(home) in out


def test_doctor_stays_silent_about_the_sidecar_when_it_does_not_exist(monkeypatch, tmp_path):
    """Review round 3, minor finding: doctor must not claim a fact it never
    checked. On a machine where trix_config_sync has never added anything
    (or never run at all), the sidecar file genuinely doesn't exist —
    doctor must not print a sentence about a file that isn't there."""
    home = tmp_path / ".hermes"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text(
        "terminal:\n  backend: docker\n_config_version: 34\n", encoding="utf-8"
    )
    assert not (home / "trix_config_sync_state.json").exists()
    ...
    out = buf.getvalue()
    assert "trix_config_sync_state.json" not in out
...
def test_run_doctor_termux_does_not_mark_browser_available_without_agent_browser(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "_DHH", str(home))
    monkeypatch.setattr(doctor_mod.shutil, "which", lambda cmd, *_a, **_kw: "/data/data/com.termux/files/usr/bin/node" if cmd in {"node", "npm"} else None)
...
class TestDoctorCodexCliHintPlacement:
    ...
    def _run(self, monkeypatch, tmp_path, *, codex_logged_in: bool, codex_cli_present: bool) -> str:
        ...
        monkeypatch.setattr(
            doctor_mod.shutil,
            "which",
            lambda cmd, *_a, **_kw: ("/usr/local/bin/codex" if codex_cli_present else None) if cmd == "codex" else real_which(cmd),
        )
...
class TestTelegramLinkHealthReport:
    """`hermes doctor` — единственное место, где сегодня видно, сколько раз
    рвалась связь с Телеграмом. Без него счётчик писал бы в никуда."""

    def test_silent_when_nothing_was_recorded(self, monkeypatch, tmp_path, capsys):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))

        doctor._report_telegram_link_health()

        assert capsys.readouterr().out == ""

    def test_reports_the_daily_count(self, monkeypatch, tmp_path, capsys):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        from plugins.platforms.telegram import link_health

        for _ in range(14):
            link_health.record(link_health.KIND_POOL_TIMEOUT)

        doctor._report_telegram_link_health()

        out = capsys.readouterr().out
        assert "14" in out
        assert link_health.KIND_POOL_TIMEOUT in out

    def test_a_quiet_day_after_a_bad_week_still_shows_the_week(
        self, monkeypatch, tmp_path, capsys
    ):
        """«Вчера всё сломалось, сегодня тихо» — самый частый разговор с
        поддержкой; недельное число не должно исчезать вместе с суточным."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        ...
        for _ in range(5):
            link_health.record(link_health.KIND_INIT_RETRY, now=old)

        doctor._report_telegram_link_health()

        out = capsys.readouterr().out
        assert "No link drops in the last 24h" in out
        assert "5 in the last 7 days" in out
