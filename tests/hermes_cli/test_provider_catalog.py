...
def test_profileless_providers_still_present():
    ...
    lmstudio / tencent-tokenhub / xai-oauth have no profile on main; they
    exist only as registry + canonical entries. The catalog must not require
    a profile to include a provider. (openai-api used to be in this list —
    it now has a profile at plugins/model-providers/openai-api/ and is
    covered by test_openai_api_provider_uses_its_profile_metadata below.)
    """
    by = provider_catalog_by_slug()
    for slug in ("lmstudio", "tencent-tokenhub", "xai-oauth"):
...
def test_openai_api_provider_uses_its_profile_metadata():
    """openai-api now has a bundled ProviderProfile — the catalog must read
    its label/description from the profile, not fall back to canonical/env
    placeholders (regression guard for the profile being added and then the
    catalog silently not picking it up, or being dropped again later)."""
    by = provider_catalog_by_slug()
    assert "openai-api" in by
    d = by["openai-api"]
    assert d.label == "OpenAI API"
    # Profile-sourced description, distinct from the canonical-entry
    # fallback string — proves the catalog read the profile, not the
    # profile-less fallback path.
    assert d.description == "OpenAI - direct API access with your own key"
    assert d.auth_type == "api_key"
    assert "OPENAI_API_KEY" in d.api_key_env_vars
