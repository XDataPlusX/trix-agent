...
# ---------------------------------------------------------------------------
# Шаблон конфига ПРЕДЫДУЩЕГО релиза — отправная точка для проверок
# обновления. Живёт здесь, потому что нужен двум файлам тестов
# (test_trix_config_comments.py и test_trix_upgrade_parity.py).
#
# Раньше каждый читал `git show HEAD:...`. Пока работа не была закоммичена,
# это давало старый шаблон и всё выглядело правильно. После коммита `HEAD`
# стал НОВЫМ шаблоном — и проверки, существующие ровно для того, чтобы
# сторожить путь обновления, начали стартовать с уже обновлённого файла: то
# есть проходить, не проверив ничего. Поэтому точка привязана к ТЕГУ
# последнего релиза (что и лежит на машинах клиентов), а не к HEAD, и
# отдельно утверждается, что взятый файл действительно СТАРЫЙ.
# ---------------------------------------------------------------------------

_PREVIOUS_RELEASE_TAG = "trix-v0.1.29"

#: Признак нового шаблона. Если он нашёлся в «предыдущем» — точка привязки
#: соскользнула на текущее дерево, и тест обязан упасть, а не позеленеть.
_NEW_TEMPLATE_MARKER = "Здесь ВСЕ настройки агента"


@pytest.fixture(scope="session")
def previous_release_config_template() -> str:
    """Текст ``assets/config/trix-config.yaml`` из последнего релиза."""
    import subprocess
    from pathlib import Path

    repo_root = Path(__file__).resolve().parents[2]
    out = subprocess.run(
        ["git", "show", f"{_PREVIOUS_RELEASE_TAG}:assets/config/trix-config.yaml"],
        capture_output=True,
        text=True,
        cwd=repo_root,
    )
    if out.returncode != 0 or not out.stdout:
        pytest.skip(
            f"тег {_PREVIOUS_RELEASE_TAG} недоступен "
            "(shallow clone или не репозиторий)"
        )
    text = out.stdout
    assert _NEW_TEMPLATE_MARKER not in text, (
        f"шаблон из {_PREVIOUS_RELEASE_TAG} содержит признак НОВОГО шаблона — "
        "точка привязки соскользнула, и проверки обновления стартуют с уже "
        "обновлённого файла, то есть не проверяют ничего"
    )
    return text
