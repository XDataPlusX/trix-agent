...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    # Опознаём строку провайдера ЦЕЛИКОМ, а не по куску подписи.
    # Кусок ловил соседей: у профиля подписки описание начиналось с
    # подписи основного профиля ("Z.AI / GLM Coding Plan…" против
    # "Z.AI / GLM"), и тест падал не на сломанном исключении, а на
    # появлении в каталоге второго профиля того же вендора — то есть на
    # штатном событии. Строка формируется из того же описания, из
    # которого её строит и сам выбор.
    from hermes_cli.models import CANONICAL_PROVIDERS

    target_row = next(
        (p.tui_desc for p in CANONICAL_PROVIDERS if p.slug == target_slug),
        None,
    )
    assert target_row, f"нет описания для {target_slug}"

    # Baseline: the provider appears without exclusion.
    _write_config(config_home)
    baseline = _capture_provider_labels(config_home)
    assert target_row in baseline, (
        f"sanity: {target_slug} ({target_row!r}) should appear by "
    ...
    assert target_row not in excluded_labels, (
