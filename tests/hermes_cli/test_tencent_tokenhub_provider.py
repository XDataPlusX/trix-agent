...
class TestTencentTokenhubModelCatalogJSON:
    """Verify tencent/hy3:free and tencent/hy3 are present in assets/api/model-catalog.json."""
    ...
    def test_in_model_catalog_json(self):
        ...
        # Assert rather than skip: a skip here would silently stop checking
        # the catalog the moment the manifest moves, and this file already
        # went quiet once that way.
        assert os.path.isfile(catalog_path), (
            f"model-catalog.json not found at {catalog_path} -- if the "
            "manifest moved, update this path with the other consumers"
        )
