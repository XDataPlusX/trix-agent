...
# ``docs/product/configs_for_user/`` used to sit here: it held the owner's
# personal working config, a reference copy that was never a customer
# surface. The directory was removed on 2026-09-12 (the curated template
# that ships is ``assets/config/trix-config.yaml``), so the exemption has
# nothing left to exempt and was dropped with it -- an exclusion prefix for
# a path that no longer exists silently widens the day someone recreates
# the name for an unrelated purpose.
...
def _iter_repo_scanned_files():
    # walk_files() prunes the excluded directories before descending. The
    # old rglob form entered every one of them and could die mid-walk on a
    # __pycache__ a parallel worker removed (RAF-171, tests/_source_tree.py).
    prune = prune_names(EXCLUDED_DIR_NAMES)
    for ext in SCANNED_EXTENSIONS:
        for rel, path in walk_files(REPO_ROOT, match=f"*{ext}", prune=prune):
