...
def _forbid_is_chat_available(monkeypatch):
    from hermes_cli.setup_wizard import support_chat as sc

    def _boom(*a, **k):
        raise AssertionError("is_chat_available must not be called: the pass's own provider_key check already answers this")

    monkeypatch.setattr(sc, "is_chat_available", _boom)


def test_working_key_gives_chat_available_true_with_no_second_call(logged_in, monkeypatch):
    _forbid_is_chat_available(monkeypatch)
