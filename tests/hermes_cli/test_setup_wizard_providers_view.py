...
def test_rendered_rows_carry_form_fields():
    """Каждая строка с ключом обязана называть переменную, в которую этот
    ключ пишется, — иначе сабмит молча потеряет кредентиал.

    Исключение ровно одно и оно структурное: у своего эндпоинта клиента
    (``CLIENT_SUPPLIED_ENDPOINT_PROVIDERS``) переменной нет вовсе, его
    ключ уезжает в ``config.yaml`` (``model.api_key``). Проверяется не
    «у custom можно», а «нет переменной ⇔ это провайдер с клиентским
    адресом»: новый провайдер без переменной, забытый в этом наборе,
    тест уронит."""
    from hermes_cli.setup_wizard.providers_view import (
    ...
    for row in wizard_providers():
        ...
        if row["kind"] != "api_key":
            continue
        if row["env_var"]:
            assert not row["endpoint_from_client"], row["name"]
        else:
            assert row["name"] in CLIENT_SUPPLIED_ENDPOINT_PROVIDERS, row["name"]
            assert row["endpoint_from_client"], row["name"]
