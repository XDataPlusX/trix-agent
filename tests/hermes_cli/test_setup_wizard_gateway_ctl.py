...
# ---------------------------------------------------------------------------
# Третья ступень: юнита шлюза нет вовсе.
#
# Установщик ставит его ТОЛЬКО интерактивно — требует открывающийся
# /dev/tty и задаёт вопрос. Рецепт исполняется без терминала, поэтому на
# клиентской машине юнита не появляется. Дальше клиент проходит мастер,
# обе прежние ступени падают, и он читает «Не удалось перезапустить или
# запустить шлюз». Починить это ему нечем: SSH и консоли у него нет.
#
# Проверено исполнением 2026-09-04 на trix-testing6.ru: тот же самый
# конфиг после ручного `gateway install` поднялся, и бот подключился к
# Telegram (getUpdates снаружи стал отдавать 409 — опрос занят шлюзом).
# ---------------------------------------------------------------------------


def _cli_verb(cmd) -> str:
    """Последний аргумент вызова `hermes gateway <verb>`."""
    return cmd[-1] if isinstance(cmd, (list, tuple)) else str(cmd).split()[-1]


def test_install_is_the_last_resort_when_no_unit_exists():
    from hermes_cli.setup_wizard import gateway_ctl as g
    calls = []
    ...
    def fake_run(cmd, **kw):
        verb = _cli_verb(cmd)
        calls.append(verb)
        # Ни restart, ни start не работают без установленного юнита.
        return MagicMock(returncode=0 if verb == "install" else 1, stdout="", stderr="")

    with patch.object(g.subprocess, "run", side_effect=fake_run):
        out = g.restart_gateway()

    assert out["ok"] is True, out
    assert calls == ["restart", "start", "install"], calls


def test_install_is_not_attempted_when_start_already_worked():
    """Порядок ступеней — не косметика: `install` на живой машине
    переписал бы юнит без нужды."""
    from hermes_cli.setup_wizard import gateway_ctl as g
    calls = []
    ...
    def fake_run(cmd, **kw):
        verb = _cli_verb(cmd)
        calls.append(verb)
        return MagicMock(returncode=1 if verb == "restart" else 0, stdout="", stderr="")

    with patch.object(g.subprocess, "run", side_effect=fake_run):
        out = g.restart_gateway()

    assert out["ok"] is True
    assert "install" not in calls, calls


def test_failure_is_still_reported_when_install_also_fails():
    """Установка — не способ отрапортовать успех. Если и она не прошла,
    клиент обязан увидеть отказ, а не зелёную галочку."""
    from hermes_cli.setup_wizard import gateway_ctl as g
    ...
    assert out["ok"] is False
    assert out["message"] == g._MSG_FAILED


def test_install_exception_is_not_a_success():
    from hermes_cli.setup_wizard import gateway_ctl as g
