...
def test_fallback_device_code_requires_a_live_login(logged_in, monkeypatch):
    """Запасной провайдер со входом по аккаунту обязан подтвердить вход —
    зеркало тех же ворот у основного слота.

    С 2026-09-11 запасной слот настраивается тем же блоком, что и
    основной, значит и вариант со входом по аккаунту у него доступен. У
    такого провайдера нет ни env_var, ни api_key, которые поймали бы
    более поздние проверки: без этих ворот «запасной ChatGPT» сохранился
    бы в fallback_providers, ни разу не подтвердив вход, и отказал бы
    ...
    form = dict(GOOD_FORM)
    form["fallback"] = {"name": "openai-codex", "env_var": None, "api_key": "", "base_url": "", "model": ""}

    r = logged_in.post("/api/submit", json=form)

    assert r.status_code == 422, r.text
    assert r.json()["errors"]["fallback.name"], r.text
    assert applied == []


def test_fallback_device_code_passes_and_applies_when_the_login_is_live(logged_in, monkeypatch):
    """Обратная сторона тех же ворот: подтверждённый вход пропускает
    запасного дальше, и он доезжает до apply_settings — вместе с моделью,
    которую клиент выбрал в том же блоке."""
    from hermes_cli.setup_wizard import app as wapp

    _ok_stack(monkeypatch, wapp)
    monkeypatch.setattr(wapp, "device_login_is_valid", lambda name: True)
    applied = []
    monkeypatch.setattr(wapp, "apply_settings", lambda f: applied.append(f) or {"ok": True, "written": [], "warnings": []})

    form = dict(GOOD_FORM)
    form["fallback"] = {
        "name": "openai-codex", "env_var": None, "api_key": "",
        "base_url": "", "model": "gpt-5-codex",
    }

    r = logged_in.post("/api/submit", json=form)

    assert r.status_code == 200, r.text
    assert applied, "форма не доехала до apply_settings"
    assert applied[0]["fallback"]["name"] == "openai-codex"
    assert applied[0]["fallback"]["model"] == "gpt-5-codex"


def test_fallback_base_url_and_model_reach_apply(logged_in, monkeypatch):
    """Адрес API и модель ЗАПАСНОГО провайдера доезжают до apply.

    Это и есть тот упор, с которого началась правка (владелец,
    2026-09-10): основным выбран ChatGPT, запасным нужен GLM, у GLM два
    варианта с разными адресами — и поменять адрес было негде. Форма
    слала для запасного base_url и model жёстко пустыми, хотя сервер обе
    величины принимает и пишет в fallback_providers."""
    from hermes_cli.setup_wizard import app as wapp
    ...
    form = dict(GOOD_FORM)
    form["fallback"] = {
        # env_var намеренно не задан: сервер подставляет канонический сам
        # (у zai их три синонима — GLM_API_KEY/ZAI_API_KEY/Z_AI_API_KEY).
        "name": "zai", "env_var": None, "api_key": "zai-key",
        "base_url": "https://api.z.ai/api/coding/paas/v4", "model": "glm-4.6",
    }
    ...
    assert r.status_code == 200, r.text
    assert applied, "форма не доехала до apply_settings"
    fallback = applied[0]["fallback"]
    assert fallback["base_url"] == "https://api.z.ai/api/coding/paas/v4", fallback
    assert fallback["model"] == "glm-4.6", fallback


def _custom_form(**over):
    provider = {
        "name": "custom",
        "env_var": None,
        "api_key": "sk-my-endpoint",
        "base_url": "https://llm.example.org/v1",
        "model": "my-model-7b",
    }
    provider.update(over)
    form = dict(GOOD_FORM)
    form["provider"] = provider
    return form


def test_custom_endpoint_requires_an_address(logged_in, monkeypatch):
    """Без адреса своему эндпоинту неоткуда взяться: каталог про него не
    знает ничего, подставить нечего. Ворота только здесь — у остальных
    провайдеров адрес приходит из каталога, поэтому общей проверки нет."""
    from hermes_cli.setup_wizard import app as wapp
    ...
    r = logged_in.post("/api/submit", json=_custom_form(base_url=""))

    assert r.status_code == 422, r.text
    assert r.json()["errors"]["provider.base_url"], r.text
    assert applied == []


def test_custom_endpoint_requires_a_model(logged_in, monkeypatch):
    """Модель тоже обязательна: у своего сервера нет ни каталога моделей,
    ни умолчания. Без неё агент уехал бы с пустым model.default и отказал
    бы на первом сообщении — то есть отказ переехал бы из мастера в чат,
    где клиенту с ним делать нечего."""
    from hermes_cli.setup_wizard import app as wapp
    ...
    r = logged_in.post("/api/submit", json=_custom_form(model=""))

    assert r.status_code == 422, r.text
    assert r.json()["errors"]["provider.model"], r.text
    assert applied == []


def test_custom_endpoint_key_is_probed_against_the_typed_address(logged_in, monkeypatch):
    """Ключ своего эндпоинта проверяется ПО ВВЕДЁННОМУ АДРЕСУ.

    До правки проверка была коротким замыканием: `check_provider_key`
    возвращает «ок, не проверяли» при `env_var=None`, а у `custom` он
    всегда None. То есть опечатка в ключе доживала до первого молчания
    бота."""
    from hermes_cli.setup_wizard import app as wapp
    ...
    monkeypatch.setattr(
        wapp,
        "check_provider_key",
        lambda *a: calls.append(a) or {"ok": True, "reachable": True, "checked": True},
    )
    monkeypatch.setattr(wapp, "apply_settings", lambda f: {"ok": True, "written": [], "warnings": []})

    r = logged_in.post("/api/submit", json=_custom_form())

    assert r.status_code == 200, r.text
    assert calls, "проверка ключа не вызывалась вовсе"
    env_var, value, _proxy, base_url = calls[0]
    assert env_var is None, calls[0]
    assert value == "sk-my-endpoint", calls[0]
    assert base_url == "https://llm.example.org/v1", calls[0]


def test_custom_endpoint_is_not_offered_for_the_fallback_slot(logged_in):
    """Запасным свой эндпоинт пока не предлагается: его ключ живёт в
    model.api_key — одном на конфиг, и второму адресу там места нет.
    Строка отсутствует в каталоге групп для запасного слота на клиенте;
    здесь сторожим само правило и его причину, записанную рядом."""
    from hermes_cli.setup_wizard.providers_view import (
        CLIENT_SUPPLIED_ENDPOINT_PROVIDERS,
        CUSTOM_ENDPOINT_UNSUPPORTED_SLOTS,
    )

    assert "fallback" in CUSTOM_ENDPOINT_UNSUPPORTED_SLOTS
    assert CUSTOM_ENDPOINT_UNSUPPORTED_SLOTS["fallback"].strip()
    assert "custom" in CLIENT_SUPPLIED_ENDPOINT_PROVIDERS
