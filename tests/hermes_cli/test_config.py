...
class TestSaveConfigAtomicity:
    ...
    def test_no_partial_write_on_crash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            # Simulate a crash mid-write by making the YAML dumper raise
            # after the temp file is created but before replace. save_config
            # writes through utils.atomic_roundtrip_yaml_save (comment-
            # preserving ruamel round-trip mode, not plain PyYAML) since the
            # setup-wizard fix — patch ruamel's dumper, not utils.yaml.dump.
    ...
    def test_no_leftover_temp_files(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            # save_config writes through utils.atomic_roundtrip_yaml_save
            # (ruamel round-trip dumper) since the setup-wizard fix — patch
            # that dumper, not utils.yaml.dump (which save_config no longer
            # calls; patching it here would silently test nothing).
            with patch("ruamel.yaml.YAML.dump", side_effect=OSError("disk full")):
                try:
                    save_config(config)
                except OSError:
...
# ---------------------------------------------------------------------------
# save_config обещает: «Keys the user explicitly set are always preserved».
# Два способа, которыми это обещание не выполнялось, — оба найдены на живом
# клиентском конфиге при переходе на полный шаблон (сентябрь 2026).
#
# Симптом был не теоретический: в поставляемом шаблоне секция
#
#     platforms:
#       telegram:
#         extra:
#           command_menu:        # <- голый ключ, под ним 20 строк объяснения
#
# исчезала из файла клиента при ЛЮБОЙ миграции, унося с собой всю свою
# документацию. Причина — ``None`` служил в ``_strip_default_values``
# одновременно значением и признаком «выкушено»: сохранённый путь со
# значением ``None`` возвращался как ``None``, вызывающий код читал это как
# «ребёнок выкушен», родитель оставался пустым и тоже выкусывался.
# ---------------------------------------------------------------------------


def test_a_bare_key_the_user_wrote_survives_a_save(tmp_path, monkeypatch):
    """Голый ``key:`` (значение None) остаётся в файле после save_config."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    assert "platforms" in raw, (
        "секция с голым ключом исчезла целиком — вместе со своими "
        "комментариями, которые и есть документация продукта"
    )
    assert raw["platforms"]["telegram"]["extra"] == {"command_menu": None}


def test_an_empty_section_the_user_wrote_survives_a_save(tmp_path, monkeypatch):
    """Пустая секция (``hooks: {}``), написанная пользователем, остаётся.

    ``_explicit_config_paths`` обходил словарь по детям, поэтому у ПУСТОГО
    словаря не записывал ни одного пути — и сохранять было нечего. Формально
    значение равно дефолту, но пользователь его написал: файл — это в том
    числе список того, что вообще можно настроить.
    """
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_cli.config import get_config_path, read_raw_config, save_config
    ...
    raw = read_raw_config()
    assert "hooks" in raw and "quick_commands" in raw


def test_a_default_section_the_user_never_wrote_is_still_not_materialised(
    tmp_path, monkeypatch
):
    """Обратная сторона: чего пользователь не писал, того и не появляется.

    Страховка от «починили сохранение — получили полный дамп DEFAULT_CONFIG в
    каждом конфиге», то самое поведение, из-за которого у апстрима
    существует ``strip_defaults``.
    """
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_cli.config import get_config_path, read_raw_config, save_config

    get_config_path().write_text("timezone: Europe/Moscow\n", encoding="utf-8")

    save_config(read_raw_config())

    raw = read_raw_config()
    assert raw.get("timezone") == "Europe/Moscow"
    for untouched in ("compression", "delegation", "auxiliary", "discord"):
        assert untouched not in raw, (
            f"{untouched} материализовалась в конфиг, которого пользователь "
            "не писал — strip_defaults перестал работать"
        )


def test_explicit_paths_include_an_empty_container():
    """Единичная проверка причины: пустой словарь даёт путь."""
    from hermes_cli.config import _explicit_config_paths

    paths = _explicit_config_paths({"hooks": {}, "model": {"default": "x"}})
    assert ("hooks",) in paths
    assert ("model", "default") in paths


def test_a_bare_section_survives_a_save_of_the_MERGED_config(tmp_path, monkeypatch):
    """Голый ``key:`` выживает и когда сохраняют СЛИТЫЙ с дефолтами конфиг.

    Так делают `save_config(load_config())` — путь `hermes_cli/agent_import.py`
    и `hermes_cli/claw.py`. В слитом конфиге на месте голого ``display:`` уже
    стоит непустой словарь дефолтов, поэтому обход уходит внутрь, все дети
    равны дефолту, поддерево пустеет — и раздел исчезал целиком. Вместе с
    ним осиротевал комментарий над ним: он оставался в файле, но уже над
    ЧУЖИМ ключом.

    Материализовать дефолты вместо этого нельзя (у апстрима есть шрам
    «hermes update blows up my config» ровно от этого), поэтому раздел
    остаётся ПУСТЫМ — как его и написал пользователь.
    """
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_cli.config import (
        get_config_path,
    ...
    assert "display" in raw, (
        "раздел, который написал пользователь, исчез — а его комментарий "
        "остался в файле над чужим ключом"
    )
    assert not raw["display"], (
        f"раздел материализовался дефолтами ({len(raw['display'])} ключей) — "
        "это то самое «hermes update blows up my config»"
    )
    # комментарий остался над своим ключом
    lines = [line for line in text.splitlines() if line.strip()]
    idx = lines.index("# моё пояснение про раздел")
    assert lines[idx + 1].startswith("display"), lines[idx : idx + 2]


def test_saving_a_merged_config_does_not_materialise_untouched_sections(
    tmp_path, monkeypatch
):
    """Обратная сторона того же: чего пользователь не писал — не появляется."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from hermes_cli.config import (
        get_config_path,
        load_config,
    ...
    raw = read_raw_config()
    assert raw.get("timezone") == "Europe/Moscow"
    for untouched in ("compression", "delegation", "auxiliary", "discord", "moa"):
        assert untouched not in raw, (
            f"{untouched} материализовалась из дефолтов в конфиг, которого "
