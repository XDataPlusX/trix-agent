...
class TestModelSwitchSkewGuard:
    ...
    def test_guard_message_names_revs_and_restart(self, monkeypatch):
        ...
        # The invariant is "the message says a RESTART is what fixes this",
        # not "the message hands the reader a shell command". The literal
        # `hermes gateway restart` was removed on purpose: this guard fires in
        # a Telegram chat, and the only person reading it is a customer with
        # no terminal on the machine. Telling them to run a command they
        # cannot run is the same defect class this product has now fixed five
        # times over.
