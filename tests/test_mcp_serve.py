...
def _bump_mtime(path) -> None:
    """Сдвинуть mtime ВПЕРЁД на заведомо различимую величину.

    ``os.utime(path, None)`` ставит «сейчас», а «сейчас» у файловых отметок
    времени в Linux — грубые часы ядра с шагом в тик (миллисекунды, не
    наносекунды). Запись файла и такой «перенос» укладываются в один тик:
    mtime не меняется, и mtime-калитка ``_poll_once()`` совершенно честно
    решает, что с прошлого раза ничего не произошло.

    Тест из-за этого падал не всегда, а только когда успевал, — то есть
    описывал скорость машины, а не поведение моста. Сдвиг задаётся явно.
    """
