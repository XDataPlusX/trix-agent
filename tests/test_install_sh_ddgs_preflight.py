...
def _plant_ddgs_import_blocker(blocker_dir: Path) -> None:
    """Make ``import ddgs`` fail in the check's subprocess, whatever this
    machine's venv happens to contain.

    The WARN branch needs the package ABSENT. Relying on the dev venv to
    supply that absence made the test a statement about the machine rather
    than about the installer: ``ddgs`` is a real optional extra
    (``pyproject.toml``'s ``[ddgs]``), so any worktree where someone ran
    ``pip install -e '.[ddgs]'`` turned this test red while the preflight it
    covers was working perfectly. Measured 2026-09-14: the full release gate
    failed here for exactly that reason -- the extra was installed in the
    gate's venv.

    A meta-path finder that raises ``ModuleNotFoundError`` is indistinguishable
    from a missing package at the chokepoint this preflight actually consults:
    ``DDGSWebSearchProvider.is_available()`` (``plugins/web/ddgs/provider.py``)
    does ``import ddgs`` under ``except ImportError``, and
    ``ModuleNotFoundError`` is an ``ImportError``. Mirror image of
    :func:`_plant_fake_ddgs_module`, which supplies presence for the OK branch
    the same way -- neither branch now depends on what is installed here.
    """
    ...
    (blocker_dir / "sitecustomize.py").write_text(
        ...
        "        if fullname == 'ddgs' or fullname.startswith('ddgs.'):\n"
        "            raise ModuleNotFoundError(\"No module named 'ddgs'\", name=fullname)\n"
        "        return None\n"
        "\n"
        "\n"
...
def test_missing_ddgs_routes_to_log_warn_with_the_truth_not_a_firecrawl_ad(tmp_path):
    ...
    execution against the REAL dev venv with the `ddgs` package made
    unimportable: the installer must warn, in Russian, with an actionable
    ...
    Absence is imposed by the test (see :func:`_plant_ddgs_import_blocker`),
    not borrowed from whatever this machine has installed -- `ddgs` is a
    genuine optional extra, so a venv that happens to carry it used to turn
    this red while the installer was behaving correctly."""
    install_dir = tmp_path / "install"
    install_dir.mkdir()
    _symlink_real_venv_python(install_dir)

    blocker_dir = tmp_path / "no_ddgs"
    _plant_ddgs_import_blocker(blocker_dir)

    result = _run_check_ddgs_preflight(install_dir, extra_pythonpath=blocker_dir)
