...
def _isolated_home(isolated_user_home):
    """Дом уводится через ``isolated_user_home``, а не подменой ``Path.home``.

    Hindsight ищет ``~/.hindsight`` через ``hermes_constants.user_home_path()``.
    Патч ``Path.home`` перестал перекрывать этот путь и перестал молча: тесты
    писали ПЛАЙНТЕКСТОВЫЙ ключ (``sk-hindsight-secret``) в настоящий
    ``~/.hindsight/profiles/hermes/.env`` разработчика, а тот из них, что
    создаёт каталог сам, падал на уже существующем настоящем каталоге.
    """
