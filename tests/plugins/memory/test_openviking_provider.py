...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
    # HOME как переменная, а не только атрибут Path.home: путь к ovcli.conf
    # теперь идёт через ``user_home_path()``, который читает окружение —
    # именно это и даёт ему следовать за профилем под containment.
