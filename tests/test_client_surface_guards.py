...
def _skill_texts():
    ...
    """
    # walk_files(), not rglob: skills/*/scripts/ carries __pycache__ that the
    # parallel workers churn (RAF-171). See tests/_source_tree.py.
    for _, path in walk_files(SKILLS, match="*.md"):
...
class TestProductSkillDoesNotLinkOutOfTheDelivery:
    ...
    def test_every_related_skill_is_actually_delivered(self):
        delivered = {
            p.parent.name for _, p in walk_files(SKILLS, match="SKILL.md")
        }
