...
def _skill_paths():
    # walk_files(), not a ``**`` glob: the recursive form descends into
    # skills/*/scripts/__pycache__ and dies when a parallel worker removes
    # one mid-walk (RAF-171). See tests/_source_tree.py.
    return sorted(
        path
        for top in ("skills", "optional-skills")
        for _, path in walk_files(REPO / top, match="SKILL.md")
    )
