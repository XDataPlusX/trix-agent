...
def _run_with_timeout_dispatch(*, supports_foreground: bool, tty: bool = True) -> dict:
    """Прогнать реальный ``run_with_timeout`` с поддельным ``timeout``.

    ``tty`` управляет тем, есть ли у скрипта управляющий терминал: от этого
    зависит, имеет ли смысл ``--foreground`` (см. комментарий выше). Терминал
    подкладывается настоящим pty, а не флагом — проверять надо то же условие,
    что читает продакшн-код.
    """
    with tempfile.TemporaryDirectory() as td:
...
"""
        ...
        try:
            ...
            proc = subprocess.run(
                ...
                text=True,
                timeout=30,
                stdin=stdin_slave if tty else subprocess.DEVNULL,
            )
        finally:
            for fd in (stdin_slave, stdin_master):
                if fd is not None:
                    os.close(fd)
...
def test_run_with_timeout_uses_foreground_kill_flags_on_a_terminal() -> None:
    """С терминалом ``--foreground`` нужен: без него Ctrl+C не дойдёт до
    команды и загрузка будет выглядеть намертво зависшей (#35166)."""
    r = _run_with_timeout_dispatch(supports_foreground=True, tty=True)
...
def test_run_with_timeout_drops_foreground_without_a_terminal() -> None:
    """Без терминала ``--foreground`` не приобретение, а потеря.

    Ctrl+C взяться неоткуда (``curl | bash``, CI, тест), а флаг выключает
    единственное, ради чего сторож существует: по его же man'у дети команды
    под таймаут не попадают. Зависший загрузчик переживает SIGTERM, держит
    унаследованный stdout открытым — и установщик висит вечно.
    """
    r = _run_with_timeout_dispatch(supports_foreground=True, tty=False)

    assert len(r["calls"]) == 2, r
    assert r["calls"][0] == "--foreground -k 10 1 true", r  # probe still attempted
    assert r["calls"][1].startswith("-k 10 5 "), (
        "without a TTY the guarded call must drop --foreground so `timeout` "
        f"signals the whole process group: {r}"
    )
    assert r["final_rc"] == 0, r
