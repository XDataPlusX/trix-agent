...
class TestLogicalLineStreaming:
    ...
    def test_unbreakable_run_stays_single_line(self, cli_stub):
        ...
        cli._flush_stream()
        # Exclude the response-box header/footer chrome (identified
        # structurally by their box-drawing corners, not by content) before
        # counting -- the active skin's response label (e.g. "Trix") can
        # itself contain the probe character, which must not be confused
        # with the streamed content.
        content_lines = [
            _strip_ansi(e) for e in emitted
            if not _strip_ansi(e).strip().startswith(("╭", "╰"))  # ╭ ╰
        ]
        plain = "\n".join(content_lines)
        assert plain.count("x") == 300
        content = [line for line in content_lines if "x" in line]
