...
def test_paused_reply_surfaces_reason_and_resume_hint(hermes_home):
    """The client reading this reply cannot run any CLI command, so the
    notice must not tell them to (nor name the upstream product)."""
    ...
    assert "hermes resume" not in notice
    assert "hermes" not in notice.lower()
...
def _fake_source():
    """Настоящий MessageSource, а не самодельная заглушка.

    С 2026-09-08 перехват ответа с секретом (спека 19) стоит ПЕРВЫМ в
    `_handle_message`, до гейта паузы, и считает ключ сессии. Заглушка с
    `platform = None` падала на `source.platform.value`, то есть тест
    мерил собственную неполноту, а не паузу. Настоящий dataclass не даст
    разойтись с продуктом и на следующем добавленном поле.
    """
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(hermes_home, monkeypatch):
    # This test asserts the actual reply text of _handle_pause_command, which
    # is now rendered through agent.i18n.t() (Trix Agent l10n, trix.busy.*).
    # tests/test_estop.py lives outside tests/gateway/, so the
    # `_pin_english_ui_language` autouse fixture in tests/gateway/conftest.py
    # does not reach it -- pin the language explicitly so this test keeps
    # asserting the English copy it was written for. See
    # test_gateway_pause_command_engages_and_resumes_ru below for the
    # Russian-path coverage this gap left missing.
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes_ru(hermes_home, monkeypatch):
    """Russian-path counterpart to the English-pinned test above -- covers
    the same /pause -> /pause -> /pause off -> /pause off sequence so the
    Russian reply text isn't left untested by pinning the other test to en.
    """
    from agent import i18n
    ...
    try:
        ...
        reply = await runner._handle_pause_command(_FakePauseEvent("off"))
        assert "возобновлено" in reply.lower()
        assert estop.is_engaged() is False

        reply = await runner._handle_pause_command(_FakePauseEvent("off"))
        assert "не был на паузе" in reply.lower()
    finally:
        i18n.reset_language_cache()
