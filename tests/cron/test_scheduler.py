...
class TestDeliverResultWrapping:
    ...
    def test_delivery_wraps_content_with_header_and_footer(self):
        """Delivered content carries a header/footer wrapper the user can
        both read (job name is visible) and that a platform adapter can
        reliably strip back down to the clean agent output -- regardless of
        which language the wrapper text is localized into."""
        from gateway.config import Platform
        from gateway.platforms.base import strip_cron_wrap
        ...
        raw_output = "Here is today's summary."
        ...
        with patch("gateway.config.load_gateway_config", return_value=mock_cfg), \
            ...
            _deliver_result(job, raw_output)

        send_mock.assert_called_once()
        sent_content = send_mock.call_args.kwargs.get("content") or send_mock.call_args[0][-1]
        # The wrapper actually changed the content (it's not delivered raw)...
        assert sent_content != raw_output
        # ...the job's identifying info made it into the wrapper...
        assert "daily-report" in sent_content
        assert "test-job" in sent_content
        # ...the clean output is still in there verbatim...
        assert raw_output in sent_content
        # ...and a platform that needs the unwrapped output back (Yuanbao)
        # can always recover it, independent of the active display language.
        assert strip_cron_wrap(sent_content) == raw_output
    ...
    def test_unreachable_media_is_recorded_as_a_delivery_error(self, tmp_path, monkeypatch):
        """A MEDIA: path the host cannot read is an undelivered attachment.

        A cron run has no chat turn to answer into, so the job's own error
        string is where the drop has to surface — the same place the live
        adapter's confirmation timeout already reports skipped attachments.
        """
        from gateway.config import Platform
        from concurrent.futures import Future
        ...
        assert result is not None and "not delivered" in result
        # The text still goes out — only the attachment was lost.
        adapter.send.assert_called_once()
        assert "Report attached" in adapter.send.call_args[0][1]
...
class TestCronDeliveryMirror:
    ...
    def test_delivery_mirrors_clean_content_not_wrapped(self):
        ...
        # Clean content made it through (mirroring adds its own "[Cron
        # delivery: ...]" replay label ahead of it -- unrelated to the cron
        # header/footer wrapper this test guards against).
        assert raw_output in mirrored_text
        # No cron wrapper was applied, in any language: unwrapping a string
        # with no wrapper is a structural no-op, checked rather than via an
        # English literal.
        assert strip_cron_wrap(mirrored_text) == mirrored_text
