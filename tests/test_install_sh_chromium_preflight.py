...
def _browserless_bin(tmp_path: Path, *tools: str) -> str:
    """PATH-каталог без единого браузера, но с нужными утилитами.

    Тот же урок, что записан у ``path_entries=[str(empty_bin)]`` ниже, только
    доведённый до конца: тест, который обязан доказать «Chromium не найден»,
    не может держать в PATH ``/usr/bin`` — на машине разработчика (и на
    ubuntu-раннере) там лежит ``google-chrome``, и ``_chromium_installed()``
    находит его вторым же шагом, ещё до кэша Playwright. Тогда исправили
    только один случай из четырёх: оставшиеся три продолжали проверять не
    поведение установщика, а состав ``/usr/bin`` на конкретной коробке.

    Утилиты (``chmod`` для заглушки, которая «чинит» браузер) прокидываются
    симлинками поимённо — так в PATH не может случайно приехать ничего, о чём
    тест не просил.
    """
    bin_dir = tmp_path / "browserless-bin"
    bin_dir.mkdir(exist_ok=True)
    import shutil as _shutil

    for tool in tools:
        resolved = _shutil.which(tool)
        assert resolved, f"в системе нет {tool!r} — заглушке теста его не хватит"
        link = bin_dir / tool
        if not link.exists():
...
"""
    result = _run_check_chromium_preflight(
        ...
        hermes_home=hermes_home,
        home=tmp_path / "home",
        # Первое чтение обязано быть отрицательным — значит, браузера в PATH
        # быть не должно (см. _browserless_bin). `chmod` заглушке нужен.
        path_entries=[_browserless_bin(tmp_path, "chmod")],
...
"""
    result = _run_check_chromium_preflight(
        ...
        hermes_home=hermes_home,
        home=tmp_path / "home",
        # Вердикт обязан остаться WARN — браузера в PATH быть не должно
        # (см. _browserless_bin); заглушке хватает встроенного `echo`.
        path_entries=[_browserless_bin(tmp_path)],
        extra_env_lines=extra_env_lines,
    )
