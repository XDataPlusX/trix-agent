...
def test_config_set_model_explicit_provider_surfaces_selected_provider_errors(monkeypatch, caplog):
    """The point of this test is that the EXPLICITLY-selected provider's own
    failure is what surfaces -- not the unrelated default-config provider's
    (``requested is None`` raises a different, distinguishing message the
    test also arms as a trap) and not a generic catch-all. It does not
    require the raw exception text verbatim: client-command-surface plan
    Task 8 changed ``hermes_cli.model_switch.switch_model()``'s credential-
    resolution failure to return a short, localized, actionable message
    instead of interpolating the raw exception (which -- via this same
    shared pipeline -- also reaches the Telegram client's chat, where a
    keyring/subprocess exception's internals do not belong). The untruncated
    exception still reaches the log, which this test now pins instead of
    the old literal chat-text assertion."""
    import logging
    ...
    try:
        ...
        # Not the unrelated default-config provider's error, and not the raw
        # exception text -- see this test's docstring (Task 8).
        assert "broken default provider" not in resp["error"]["message"].lower()
        assert "missing anthropic api key" not in resp["error"]["message"].lower()
        assert seen["build"] == 0
        assert seen["wait"] == 0
        # The real reason is not lost -- it reached the log.
        assert any(
            "missing anthropic API key" in record.getMessage() for record in caplog.records
        )
    finally:
        server._sessions.pop("sid", None)
