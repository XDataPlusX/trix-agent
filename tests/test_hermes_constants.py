...
class TestIsContainer:
    ...
    # ── mountinfo: признак «мы ВНУТРИ», а не «рантайм есть на машине» ──────
    #
    # Оба случая ниже содержат ОДИН И ТОТ ЖЕ маркер ``containerd``. Разница
    # только в том, к какой точке монтирования он относится, и именно её
    # детектор обязан различать.

    def _fake_mountinfo(self, monkeypatch, tmp_path, text):
        self._reset_cache(monkeypatch)
        monkeypatch.setattr(os.path, "exists", lambda p: False)
        ...
        def _open(path, *args, **kwargs):
            ...
            if path == "/proc/1/cgroup":
                raise OSError("cgroup v2 — маркера рантайма здесь нет")
            return real_open(path, *args, **kwargs)

        monkeypatch.setattr("builtins.open", _open)

    def test_containerd_rootfs_means_we_are_inside(self, monkeypatch, tmp_path):
        """Наш собственный корень смонтирован рантаймом — это контейнер."""
        self._fake_mountinfo(monkeypatch, tmp_path, (
            "1 0 0:30 / / rw,relatime shared:1 - overlay overlay rw,"
            "lowerdir=/var/lib/containerd/io.containerd.snapshotter.v1."
            "overlayfs/snapshots/9/fs\n"
        ))
        assert is_container() is True

    def test_container_host_is_not_a_container(self, monkeypatch, tmp_path):
        """Хост, который САМ запускает контейнеры, контейнером не является.

        Регрессия: сканирование mountinfo целиком находило оверлеи чужого
        демона (``/var/lib/containerd/...``) на любой машине разработчика с
        Docker. ``hermes doctor`` из-за этого печатал «Running inside a
        container» и ЦЕЛИКОМ пропускал диагностику терминального бэкенда.
        """
        self._fake_mountinfo(monkeypatch, tmp_path, (
            "30 1 259:2 / / rw,relatime shared:1 - ext4 /dev/nvme0n1p2 rw\n"
            "119 30 0:42 / /var/lib/docker/rootfs/overlayfs/8fbe424f rw,"
            "relatime shared:116 - overlay overlay rw,"
            "lowerdir=/var/lib/containerd/io.containerd.snapshotter.v1."
            "overlayfs/snapshots/178/fs\n"
        ))
