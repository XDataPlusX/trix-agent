...
#: Настоящий ``open``, снятый ДО любого патча.
#:
#: Шим, который в своей последней строке зовёт просто ``open``, под
#: ``patch("builtins.open")`` зовёт сам себя — и так до RecursionError. Пока
#: код под тестом не открывал ничего, кроме подменённого ``/proc/version``,
#: этой ветки просто не достигали. Теперь достигают: подготовка окружения для
#: подпроцесса (``hermes_subprocess_env``) читает маркер режима изоляции, шим
#: уходит в рекурсию, а вызывающий код ловит это как «плеер не запустился» —
#: то есть тест сообщал о поведении, которого не было.
_REAL_OPEN = open


def _proc_version_open(text: str):
    """Шим ``open``: подменяет ТОЛЬКО ``/proc/version``, остальное — настоящее."""
...
class TestWSL2PowerShellFallback:
    ...
    def test_wsl2_unique_temp_filename(self, monkeypatch, tmp_path, sample_wav):
        ...
        _fake_open = _proc_version_open("Linux Microsoft WSL2")
    ...
    def test_non_wsl_skips_powershell_fallback(self, monkeypatch, sample_wav):
        ...
        _fake_open = _proc_version_open("Linux version 5.15.0-generic #72-Ubuntu")
