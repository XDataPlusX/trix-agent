...
class TestCrossProfileRead:
    """Cross-profile reads are gated by session_search.allow_cross_profile
    (default off, see tests/tools/test_session_search_profile_gate.py for the
    gate's own contract). These tests cover the mechanism when a person has
    opted it in — the desktop-app case — so each forces the gate open."""
    ...
    def _allow_cross_profile(self, monkeypatch):
        import tools.session_search_tool as sst
        monkeypatch.setattr(sst, "session_search_allow_cross_profile", lambda: True)
    ...
    def test_bare_id_locates_across_profiles(self, db, tmp_path, monkeypatch):
        ...
        self._allow_cross_profile(monkeypatch)
