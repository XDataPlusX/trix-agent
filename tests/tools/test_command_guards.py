...
class TestContainerSkip:
    def test_docker_no_longer_skips_rm_rf_root(self):
        """Trix-specific, deliberate divergence from upstream Hermes (see
        hermes_cli/trix_sandbox_guard.py). Docker's /workspace and /root are
        host directories bind-mounted by the gateway outside docker_volumes,
        so an isolated docker sandbox no longer silently bypasses `rm -rf /`
        — it now reaches the hardline floor like any other environment with
        real host impact. This test used to assert the OLD upstream
        behavior (silently approved); it now asserts the NEW one.
        """
