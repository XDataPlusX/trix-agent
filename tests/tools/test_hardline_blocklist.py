...
def test_container_backends_still_bypass(clean_session):
    """Ephemeral remote sandboxes remain bypass-approved — they can't touch
    the host at all, so hardline never needs to run for them.

    Docker is the deliberate Trix exception (hermes_cli/
    trix_sandbox_guard.py): its /workspace and /root ARE host directories
    bind-mounted by the gateway outside docker_volumes, so `rm -rf /`
    there is no longer silently bypassed — it now reaches the hardline
    floor exactly like `local`/`ssh` (see test_hardline_runs_before_
    dangerous_detection below for the equivalent local-backend assertion).
    """
    for env in ("singularity", "modal", "daytona", "vercel_sandbox"):
    ...
    d1 = check_dangerous_command("rm -rf /", "docker")
    assert d1["approved"] is False, "docker should no longer bypass rm -rf /"
    assert d1.get("hardline") is True
    d2 = check_all_command_guards("rm -rf /", "docker")
    assert d2["approved"] is False, "docker should no longer bypass rm -rf /"
    assert d2.get("hardline") is True
