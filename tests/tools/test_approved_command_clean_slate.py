...
def test_approved_command_genuine_interrupt_after_start_still_kills(tmp_path):
    ...
    def worker():
        # sleep 60, а не 5: единственное, что этот тест измеряет, — что
        # прерывание доходит до УЖЕ ЗАПУЩЕННОЙ команды. Пятисекундное окно
        # делало из этого гонку — под нагруженным раннером барьер и
        # планировщик съедали его целиком, команда успевала завершиться
        # сама, и код возврата выходил нулевым вместо 130. Длинный сон
        # ничего не стоит: прерывание убивает команду сразу, а верхнюю
        # границу теста держит join(timeout=15) ниже.
        holder["result"] = tt.terminal_tool(
            command=f"touch {sentinel}; sleep 60; echo DONE", force=True
        )
...
def test_approved_note_enriched_not_misleading_on_interrupt(monkeypatch, tmp_path):
    ...
    def worker():
        # sleep 60 по той же причине, что и в тесте выше: пятисекундное
        # окно превращало проверку прерывания в гонку с самой командой.
        # Сосед проиграл эту гонку на релизном прогоне; этот выиграл, но
        # устроен точно так же — чиним класс, а не экземпляр.
        holder["result"] = tt.terminal_tool(command=f"touch {sentinel}; sleep 60; echo DONE")
