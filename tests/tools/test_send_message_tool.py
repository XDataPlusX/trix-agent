...
class TestSendMessageTool:
    ...
    def test_unreachable_media_path_is_reported_back_to_the_agent(self, tmp_path):
        """send_message is a TOOL call, so the agent is still in its loop and
        can fix the path. Gateway delivery can only tell the user; here the
        drop must not be silent to the caller either."""
        config, _telegram_cfg = _make_config()
        missing = tmp_path / "report.pdf"  # never written
        ...
        assert result["attachments_not_sent"] == [str(missing)]
        assert "readable from the host" in result["attachments_not_sent_reason"]

    def test_denied_media_path_is_not_reported_back(self):
        """A denylisted path is the prompt-injection surface. It stays a silent
        drop on every surface, including this one."""
        config, _telegram_cfg = _make_config()
        ...
        assert "attachments_not_sent" not in result

    def test_deliverable_media_adds_no_failure_fields(self, tmp_path):
        config, _telegram_cfg = _make_config()
