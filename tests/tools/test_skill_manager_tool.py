...
def _two_roots(local_dir: Path, external_dir: Path):
    ...
    Also patches ``get_external_skills_dirs()`` to agree: the external-dir
    write guard (``_external_dir_write_guard`` -> ``is_contour_managed_skill_path``)
    reads that function directly rather than ``get_all_skills_dirs()``, so a
    test that only patches the latter would silently fail to trigger it.
    """
...
def _mark_as_contour_managed(external_dir: Path) -> Path:
    """Plant the same marker ``hermes_cli.trix_contour.ensure_skills_dir_marker``
    writes at the root of the folder it owns, so tests can exercise the
    narrow (marked-folder-only) guard without importing the whole
    ``trix_contour`` module. Content is irrelevant to the guard (presence,
    not content, is what is checked — see
    ``agent.skill_utils.is_contour_managed_skill_path``); a short
    placeholder is enough.
    """
    from agent.skill_utils import CONTOUR_MANAGED_SKILLS_MARKER

    marker = external_dir / CONTOUR_MANAGED_SKILLS_MARKER
    marker.write_text("Managed by hermes contour.\n", encoding="utf-8")
    return marker


class TestPersonalVaultKeepsWorking:
    """No marker -> a personal ``skills.external_dirs`` vault (an Obsidian
    folder, a synced dotfiles checkout) -> writes still land in place.

    This is the #4759/#4381 contract: an earlier version of
    ``_external_dir_write_guard`` refused ALL of ``skills.external_dirs``
    regardless of marker, which silently regressed this exact behavior for
    every single-user installation, not just Trix business machines (spec
    21 §9 review, blocker 1, second pass — narrowed after the coordinator
    flagged the regression). These tests exist so that regression cannot
    come back quietly a second time.
    """

    def test_patch_personal_vault_skill_writes_in_place(self, tmp_path):
        ...
        # No duplicate in local — the fix for #4759/#4381 is "edit in
        # place", not "fork a local copy".
        assert not (local / "ext-skill").exists()

    def test_edit_personal_vault_skill_succeeds(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
    ...
    def test_write_file_personal_vault_skill_succeeds(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        _write_external_skill(external)
        ...
        with _two_roots(local, external):
            result = _write_file("ext-skill", "references/notes.md", "hi")

        assert result["success"] is True, result
        assert (external / "ext-skill" / "references" / "notes.md").is_file()

    def test_remove_file_personal_vault_skill_succeeds(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        skill_dir = _write_external_skill(external)
        ref = skill_dir / "references" / "keep.md"
        ref.parent.mkdir(parents=True)
        ref.write_text("keep me")
    ...
    def test_delete_personal_vault_skill_succeeds(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        skill_dir = _write_external_skill(external)
...
class TestContourManagedFolderRefuses:
    """Marker present -> this is the Trix company's shared skills folder
    (``hermes_cli/trix_contour.py``), wired into every profile's system
    prompt. Every agent-invoked mutation is refused here, regardless of
    which profile calls (spec 21 §9 review, blocker 1).
    """

    def test_patch_refused(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        skill_dir = _write_external_skill(external)
        _mark_as_contour_managed(external)
    ...
    def test_write_file_refused(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        _write_external_skill(external)
        ...
        assert result["success"] is False, result
        assert not (external / "ext-skill" / "references" / "notes.md").exists()

    def test_remove_file_refused(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        skill_dir = _write_external_skill(external)
    ...
    def test_delete_refused(self, tmp_path):
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        skill_dir = _write_external_skill(external)
    ...
    def test_read_via_find_skill_still_resolves(self, tmp_path):
        """Reading/viewing a company skill must keep working — only
        mutation is blocked."""
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
        ...
        with _two_roots(local, external):
            ...
            found = _find_skill("ext-skill")

        assert found is not None
        assert found["path"] == skill_dir

    def test_refused_even_for_system_admin_profile_with_real_config(self, tmp_path, monkeypatch):
        """End-to-end, without mocking ``get_external_skills_dirs`` — a
        real ``config.yaml`` shaped exactly like the ``system_admin``
        profile's (``skills.external_dirs`` pointing at the marked company
        folder). The guard is purely path-based (it never asks "which
        profile is this"), so even the profile that legitimately
        administers the contour, and carries the ``skills`` toolset, gets
        refused from inside a session exactly like any other profile —
        the only sanctioned path to change this folder is
        ``publish_skill`` + ``hermes contour approve-skill``, which never
        goes through ``skill_manage`` at all.
        """
        from agent import skill_utils as su

        system_admin_home = tmp_path / "system_admin_home"
        system_admin_home.mkdir()
        company_skills = tmp_path / "srv-trix-skills"
        company_skills.mkdir()
        skill_dir = _write_external_skill(company_skills)
        _mark_as_contour_managed(company_skills)

        (system_admin_home / "config.yaml").write_text(
            "skills:\n"
            f"  external_dirs:\n    - {company_skills}\n",
            encoding="utf-8",
        )
        monkeypatch.setenv("HERMES_HOME", str(system_admin_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        su._external_dirs_cache_clear()
        monkeypatch.setattr(
            "tools.skill_manager_tool.SKILLS_DIR", system_admin_home / "skills"
        )
...
class TestSetupCreatesMarkerIdempotently:
    """``hermes business setup`` and the contour's own first run both plant
    the marker — an existing deployment (developed before this guard
    existed) must gain it without manual work."""

    def test_ensure_skills_dir_marker_creates_and_is_idempotent(self, tmp_path):
        from agent.skill_utils import CONTOUR_MANAGED_SKILLS_MARKER
        from hermes_cli.trix_contour import ensure_skills_dir_marker

        root = tmp_path / "srv" / "trix"
        marker = ensure_skills_dir_marker(root)
        assert marker == root / "skills" / CONTOUR_MANAGED_SKILLS_MARKER
        assert marker.is_file()
        first_content = marker.read_text(encoding="utf-8")
        assert first_content.strip()

        # Idempotent: a human who edited the file keeps their edit — the
        # second call must not clobber it.
        marker.write_text("человек это уже поправил\n", encoding="utf-8")
        ensure_skills_dir_marker(root)
        assert marker.read_text(encoding="utf-8") == "человек это уже поправил\n"

    def test_apply_planned_backfills_marker_on_a_pre_existing_deployment(self, tmp_path):
        """A machine where the contour ran before this marker existed —
        the very next apply must backfill it, not require a manual step."""
        from agent.skill_utils import CONTOUR_MANAGED_SKILLS_MARKER
        from hermes_cli import trix_contour as tc

        root = tmp_path / "srv" / "trix"
        (root / "skills").mkdir(parents=True)  # pre-existing, no marker yet
        planned, _ = tc.validate_request(
            {"when": "now", "ops": [{"op": "create_folder", "folder": "company"}]},
            root=root,
        )
        tc.apply_planned(planned, root)
        assert (root / "skills" / CONTOUR_MANAGED_SKILLS_MARKER).is_file()

    # `hermes business setup` also creates the marker (idempotently) — see
    # test_business_setup_creates_marker_idempotently in
    # tests/hermes_cli/test_trix_business.py, which already has the full
    # run_setup() fixture; not duplicated here.
