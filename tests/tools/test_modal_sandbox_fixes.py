...
class TestDockerHostBindApproval:
    ...
    def test_isolated_docker_keeps_fast_path_for_everyday_commands(self, monkeypatch):
        """Isolated Docker still bypasses dangerous-command approval for
        non-delete commands."""
        import tools.approval as A
        ...
        res = A.check_all_command_guards("npm install", "docker",
                                         has_host_access=False)
        assert res["approved"] is True

    def test_isolated_docker_deletes_require_approval(self, monkeypatch):
        """Trix-specific, deliberate divergence from upstream Hermes (see
        hermes_cli/trix_sandbox_guard.py): even in an ISOLATED docker
        sandbox with no declared ``has_host_access``, a delete inside
        ``/workspace`` no longer takes the silent fast path.

        ``has_host_access`` only tracks EXPLICIT ``docker_volumes``/
        ``host_cwd`` bind mounts — it says nothing about ``/workspace``
        itself, which the gateway bind-mounts from the host unconditionally,
        outside that mechanism. This test used to assert the OLD upstream
        behavior (silently approved even for a delete); it now asserts the
        NEW one, matching the sibling
        ``test_host_bound_docker_requires_approval`` below (has_host_access
        is irrelevant to whether /workspace deletes get silently approved).
        """
        import tools.approval as A
        self._isolate_approval_state(monkeypatch)
        monkeypatch.setenv("HERMES_EXEC_ASK", "1")
