...
class TestDogfoodSkill:
    ...
    def _skill_dir(self):
        # Скилл ищется в ОБОИХ деревьях поставки. `dogfood` -- методичка
        # разработчика: решением владельца 2026-09-07 она уехала из
        # активной поставки в `optional-skills/` (79 активных скиллов
        # стало 53). Жёсткий путь на `skills/` пережил этот перенос
        # красным и три недели никем не читался. Курирование состава --
        # штатная операция в обе стороны, и тест обязан проверять, что
        # скилл цел, а не где именно он сейчас лежит.
        repo = os.path.join(os.path.dirname(__file__), "..", "..")
        for tree in ("skills", "optional-skills"):
            candidate = os.path.join(repo, tree, "software-development", "dogfood")
            if os.path.isdir(candidate):
                self.skill_dir = candidate
                return
        pytest.fail("скилл dogfood не найден ни в skills/, ни в optional-skills/")
