...
class TestCronModeInteractions:
    ...
    def test_container_env_auto_approves_everyday_but_not_deletes(self, monkeypatch):
        """Docker/sandbox environments bypass approvals for EVERYDAY commands
        regardless of cron_mode — but no longer for deletes.

        Trix-specific, deliberate divergence from upstream Hermes (see
        ``hermes_cli/trix_sandbox_guard.py``). Upstream's
        ``_should_skip_container_guards()`` bypassed ALL approval checks —
        including the hardline floor — unconditionally for an isolated
        docker sandbox with no host mounts. That is unsafe for Trix: the
        sandbox's ``/workspace`` and ``/root`` are host directories
        bind-mounted by the gateway outside ``docker_volumes``, so a
        container ``rm -rf`` inside them destroys real client data with no
        recovery (checkpoints do not work with the docker sandbox backend).
        We therefore let delete commands fall through to the normal guard
        chain even inside an isolated container. This test used to assert
        the OLD upstream behavior (``rm -rf /`` silently approved in
        docker); it now asserts the NEW one: everyday commands keep the
        fast path, deletes do not.
        """
        monkeypatch.setenv("HERMES_CRON_SESSION", "1")
        monkeypatch.delenv("HERMES_INTERACTIVE", raising=False)
        monkeypatch.delenv("HERMES_GATEWAY_SESSION", raising=False)
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("tools.approval._get_cron_approval_mode", return_value="deny"):
            # Everyday command: unaffected, still bypassed exactly as before.
            everyday = check_dangerous_command("npm install", "docker")
            assert everyday["approved"]
            assert everyday["message"] is None

            # `rm -rf /` no longer skips the guard chain — it now reaches
            # the hardline floor, which blocks it unconditionally (cron_mode
            # is irrelevant to a hardline match; the floor runs before
            # cron_mode is even consulted).
            root_delete = check_dangerous_command("rm -rf /", "docker")
            assert root_delete["approved"] is False
            assert root_delete.get("hardline") is True

            # A non-hardline delete inside the client's own working folder
            # now reaches the cron path instead of being skipped, and
            # cron_mode=deny blocks it because no human is present to
            # approve it — "no human, no delete".
            workspace_delete = check_dangerous_command(
                "rm -rf /workspace/proj", "docker"
            )
            assert workspace_delete["approved"] is False
            assert "cron jobs run without a user present" in workspace_delete["message"]
