...
class TestDDGSProviderSearch:
    ...
    def test_missing_package_returns_honest_error_without_attempting_install(self, monkeypatch):
        """search()'s ImportError branch must NOT call
        tools.lazy_deps.ensure() (there used to be a "self-heal like exa/
        firecrawl/parallel" call here -- dead code, since is_available()
        gates provider selection on this exact same import and would
        already have returned False, so search() is never reached with the
        package genuinely missing except via a caller that bypasses
        _get_capability_backend()). Proves the honest error path: no
        install attempt, no Firecrawl/Nous mention, an actionable message
        instead."""
        import sys
        ...
        monkeypatch.delitem(sys.modules, "tools.lazy_deps", raising=False)
        ensure_called = []
        try:
            import tools.lazy_deps as lazy_deps

            monkeypatch.setattr(
                lazy_deps, "ensure", lambda *a, **k: ensure_called.append((a, k))
            )
        except ImportError:
            pass
        ...
        assert "firecrawl" not in result["error"].lower()
        assert "nous" not in result["error"].lower()
        assert not ensure_called, "search() must not attempt a lazy install -- see docstring"
