...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_built_prompt(self, fake_hermes, monkeypatch):
        """Строка про активный профиль проверяется в СОБРАННОМ промпте.

        Прежняя редакция читала исходник `agent/system_prompt.py` и искала
        в нём слова «Active Hermes profile» — запрещённый в этом проекте
        антипаттерн (CLAUDE.md, «Never read source code in tests»). Он же
        и подвёл: спека 18 намеренно переименовала строку в «Active
        profile» и так же намеренно убрала из неё хостовый путь
        `~/.hermes/profiles/` — имя чужого продукта в промпте клиента.
        Тест краснел с того дня и не читался никем; поведение при этом
        было верным.

        Отсутствие пути теперь тоже проверяется — это решение спеки 18, а
        не побочный эффект.
        """
        import importlib.util

        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_resolve_active_profile_name", lambda: "hermes-security")

        from agent.system_prompt import build_system_prompt_parts

        # Помощник грузится ПО ПУТИ: каталог тестов в sys.path заслоняет
        # настоящий hermes_cli одноимённым тестовым подкаталогом.
        spec = importlib.util.spec_from_file_location(
            "_sysprompt_helper",
            Path(__file__).resolve().parent.parent / "agent" / "test_system_prompt.py",
        )
        helper = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(helper)

        agent = helper._make_agent(platform="telegram", valid_tool_names=["terminal"])
        parts = build_system_prompt_parts(agent)
        prompt = "\n".join(str(v) for v in (
            parts.values() if isinstance(parts, dict) else [parts]))

        assert "Active profile: hermes-security" in prompt
        assert "Do NOT modify another profile" in prompt
        assert "~/.hermes/profiles/" not in prompt, (
            "хостовый путь до чужих профилей вернулся в промпт клиента (спека 18)"
        )
