...
class TestFindAllSkills:
    ...
    def test_newline_in_name_and_description_cannot_add_a_line(self, tmp_path):
        """A hostile/untrusted SKILL.md must not be able to turn one skill
        entry into multiple rendered lines wherever this list is displayed
        (skills_list, the CLI banner, etc.) — whitespace runs (newlines
        included) collapse to a single space before the length cap applies.
        """
        ...
        # The injected phrase is collapsed inline (not stripped) -- what
        # matters is that it can never start its own line.
        assert not entry["name"].startswith("SYSTEM:")
        assert not entry["description"].startswith("SYSTEM:")

    def test_long_name_and_description_are_truncated_after_whitespace_collapse(
        self, tmp_path
    ):
        d = tmp_path / "long-skill"
        d.mkdir(parents=True)
        long_name = "z" * 200  # well over MAX_NAME_LENGTH
        long_desc = "w " * 700  # well over MAX_DESCRIPTION_LENGTH once collapsed
        (d / "SKILL.md").write_text(
        ...
        assert len(skills) == 1
        entry = skills[0]
        # Existing limits are unchanged (coordinator's instruction: don't
        # touch the MAX_* numbers, just collapse whitespace beforehand).
