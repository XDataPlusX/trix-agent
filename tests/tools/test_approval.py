...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_offers_only_once_and_deny(self, monkeypatch, capsys):
        # This asserts the literal English choice labels, so pin the
        # language explicitly -- DEFAULT_CONFIG's display.language default is
        # "ru", and without this the prompt would legitimately render the
        # Russian labels instead (see the neighboring locale-specific test).
