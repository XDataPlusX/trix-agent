...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # Containment mode (contained-profile-root plan, 2026-09-14). A developer
    # shell with isolation switched on would otherwise repaint the whole suite:
    # HOME, TMPDIR and the default cwd of every spawned subprocess move under
    # the profile root, and tests that describe upstream shared-host behavior
    # start failing with no code change behind it.
    "HERMES_ISOLATION",
    "HERMES_TMP_DIR",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
#: Настоящий дом пользователя ОС, снятый ДО того, как любая фикстура успела
#: подменить ``HOME``. Нужен ровно для одной проверки — «а точно ли мы увели
#: приватные данные профиля от разработчика» (см. ``isolated_user_home``).
_REAL_OS_HOME = Path(os.path.expanduser("~")).resolve()


@pytest.fixture()
def isolated_user_home(tmp_path, monkeypatch):
    """Увести ПРИВАТНЫЕ данные профиля (``~/.claude``, ``~/.codex``, …) в tmp.

    Зачем отдельная фикстура, если есть ``_hermetic_environment``: тот
    намеренно НЕ трогает ``HOME`` — подмена ломала подпроцессы в CI. Поэтому
    тест, который читает или ПИШЕТ учётные данные внешних CLI, обязан
    попросить изоляцию явно.

    Почему не ``monkeypatch.setattr(mod.Path, "home", ...)``, как писали
    раньше: приватные данные профиля движок ищет через
    ``hermes_constants.user_home_path()``, а не через ``Path.home()``. После
    перехода на резолвер патч ``Path.home`` перестал что-либо перекрывать —
    молча, не уронив ни одного теста в тот же день. Тесты учётных данных
    Claude Code при этом читали и ПЕРЕЗАПИСЫВАЛИ настоящий
    ``~/.claude/.credentials.json`` разработчика.

    Здесь уводится сам источник, а не его бывший синоним:

    * ``shared-host`` — ``HOME``. Его же читает ``Path.home()``, так что
      апстримный путь остаётся ровно тем же и совместимость не страдает;
    * ``contained`` — дом профиля лежит под корнем, а корень уже песочница
      (``HERMES_HOME`` подменён в ``_hermetic_environment``).

    Домом становится сам ``tmp_path`` — тест продолжает раскладывать файлы
    туда же, куда и раньше. Возвращается при этом каталог, который РЕАЛЬНО
    вернёт ``user_home_path()``: спрашивать надо резолвер, а не собственное
    представление о нём. Утверждение ниже обязательно — без него следующий
    переезд резолвера вернёт утечку так же тихо, как её вернул прошлый.
    """
    from hermes_constants import user_home_path

    # HERMES_REAL_HOME сильнее HOME в _iter_real_home_candidates: оставить его
    # значило бы подменить дом, который никто не читает.
    monkeypatch.delenv("HERMES_REAL_HOME", raising=False)
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("USERPROFILE", str(tmp_path))

    resolved = user_home_path().resolve()
    assert resolved != _REAL_OS_HOME and _REAL_OS_HOME not in resolved.parents, (
        f"user_home_path() всё ещё указывает в дом разработчика ({resolved}) — "
        "тест собирается читать и писать настоящие учётные данные"
    )
...
@pytest.fixture(autouse=True)
def _pin_english_ui_language(monkeypatch):
    """Default the whole suite to English UI copy, suite-wide and on purpose.

    This fork ships ``display.language: "ru"`` as the DEFAULT -- the product
    is sold to a Russian-speaking customer. So the moment a string moves out
    of a hardcoded English literal and into the ``t()`` catalog, it starts
    rendering Russian in a temp ``HERMES_HOME`` with no ``config.yaml``. That
    is correct product behavior and a failing upstream test that says nothing
    whatsoever about the behavior under test.

    That collision bit FOUR times in one localization pass, in four different
    directories, one test at a time: ``tests/gateway/`` (pinned long ago, in
    its own conftest), then ``tests/cli/``, then ``tests/run_agent/``, then
    ``tests/tui_gateway/`` and root-level ``tests/``. Pinning per directory
    just moves the next discovery one directory over, so the pin belongs
    here, once, where every test sees it.

    What this does NOT weaken:

    * ``test_default_display_language_is_russian`` reads ``DEFAULT_CONFIG``
      directly, not through ``t()`` -- the product invariant "we ship
      Russian" is still asserted and still fails if someone flips it.
    * The l10n suites (``test_slash_commands_l10n``, ``test_run_l10n``,
      ``test_conversation_loop_l10n``, ``test_errors_l10n``, ...) set
      ``HERMES_LANGUAGE`` themselves per case and execute both languages at
      the real call site. Overriding a default still works.
    * ``test_locale_placeholders_agree_across_languages`` covers what an
      English-by-default suite would otherwise miss: a Russian string whose
      ``{placeholder}`` set has drifted from its English twin, which would
      only ever blow up on a customer's machine.
    """
...
_ALLOW_CATALOG_FETCH_MARK = "allow_catalog_fetch"


class CatalogNetworkGuardViolation(RuntimeError):
    """Бросается, когда тест дотягивается до настоящего адреса каталога моделей.

    История: раньше это исключение намеренно наследовало ``BaseException``,
    именно чтобы НЕ тонуть в продуктовом ``except Exception: remote = None``
    (вызовы ``get_curated_openrouter_models()`` / ``get_curated_nous_model_ids()``
    в ``hermes_cli/models.py``). Цена этого решения оказалась намного выше
    защиты: полный прогон сюиты показал 79 новых падений примерно в тридцати
    файлах -- все пикеры моделей и перечисление провайдеров
    (``test_model_switch_custom_providers.py``, ``test_vertex_model_picker.py``,
    ``test_bedrock_model_picker.py`` и другие). Все они уходили в ТУ ЖЕ самую
    офлайн-ветку, которую продукт честно проходил и до появления сторожа
    (адрес витрины отвечает 404, пока витрины нет) -- но ``BaseException``
    пролетает сквозь ``except Exception`` насквозь и роняет тест вместо
    того, чтобы дать продукту откатиться в офлайн так же, как он откатывался
    на настоящий 404.

    Поэтому исключение сделано **поглощаемым** (наследник ``Exception`` /
    ``RuntimeError``) намеренно. Настоящая задача сторожа -- доказать, что
    сюита никогда не достаёт до настоящей сети -- не зависит от того, какой
    у исключения базовый класс; она зависит от перехвата ``urlopen`` до
    открытия сокета, а этот перехват остаётся на месте без изменений.
    Поглощение исключения продуктовым офлайн-фолбэком воспроизводит ровно
    то поведение, под которое были написаны эти 79 тестов до появления
    сторожа.

    НЕ возвращай ``BaseException`` обратно, чтобы "починить" пойманное
    нарушение, не прогнав перед этим полную сюиту и не посмотрев на дельту
    -- именно эта правка стоила 79 зелёных тестов в прошлый раз. Видимость
    поглощённого нарушения сохраняется через ``warnings.warn`` в месте
    броска ниже (pytest показывает его в сводке warnings даже когда
    продуктовый код ловит исключение) -- не через непоглощаемость.
    """
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        "markers",
        f"{_ALLOW_CATALOG_FETCH_MARK}: тесту нужен путь загрузки каталога "
        "модели (подделкой, не сетью).",
    )
...
def _live_system_guard(request, monkeypatch):
    ...
    """
    # Каталог моделей ходит по адресу нашей витрины (see
    # ``hermes_cli.release_source.RELEASE_RAW_BASE_URL``). Пока витрины
    # нет, это 404 и тишина; в день её создания -- 200 и настоящие данные
    # из сети, плюс фоновый SWR-поток
    # (``model_catalog._spawn_catalog_swr_refresh``), пишущий кэш на диск
    # посреди чужого теста. Один и тот же тест начинает то падать, то
    # проходить без единой правки. Ходить в сеть за каталогом сюите нельзя
    # ни при каком состоянии витрины.
    #
    # Перехват стоит на ``urllib.request.urlopen``, а не на
    # ``hermes_cli.model_catalog._fetch_manifest``: несколько тестов
    # (``test_model_catalog.py``, ``test_model_catalog_offline_seed.py``)
    # делают ``importlib.reload(model_catalog)`` в своей фикстуре, что
    # тихо возвращает настоящую ``_fetch_manifest`` на место и снимает
    # атрибутный сторож. ``urllib.request`` не перезагружается вместе с
    # ``model_catalog``, так что перехват здесь переживает reload -- он
    # ловит запрос по НАЗНАЧЕНИЮ (адрес витрины), а не по имени функции,
    # и переживёт и переименование внутренностей каталога.
    #
    # Этот блок стоит ВЫШЕ early-return по ``live_system_guard_bypass``:
    # тот маркер разрешает только настоящие os.kill/subprocess-сигналы, к
    # каталогу моделей отношения не имеет и не должен молча снимать заодно
    # и его. У каталога свой отдельный выход -- ``allow_catalog_fetch``.
    if not request.node.get_closest_marker(_ALLOW_CATALOG_FETCH_MARK):
        import urllib.request as _urllib_request
        import warnings as _warnings

        from hermes_cli import model_catalog as _model_catalog_mod
        from hermes_cli.release_source import RELEASE_RAW_BASE_URL

        _real_urlopen = _urllib_request.urlopen

        def _guarded_catalog_urlopen(url, *args, **kwargs):
            target = (
                url.full_url
                if isinstance(url, _urllib_request.Request)
                else str(url)
            )
            if target.startswith(RELEASE_RAW_BASE_URL):
                message = (
                    "tests/conftest.py live-system guard: заблокирован "
                    f"сетевой запрос за каталогом моделей ({target}). "
                    "Тесту нужен ответ каталога -- подделайте его: "
                    "monkeypatch на hermes_cli."
                    "model_catalog._fetch_manifest_with_fallback, либо "
                    "мок на get_catalog(). Настоящая сеть в сюите "
                    "запрещена: адрес витрины отвечает по-разному до и "
                    "после её создания. Если тесту сетевой путь каталога "
                    "нужен по существу -- пометь его "
                    f"@pytest.mark.{_ALLOW_CATALOG_FETCH_MARK} и подделай "
                    "сервер сам, не ходи в настоящую сеть."
                )
                # CatalogNetworkGuardViolation -- поглощаемое исключение
                # (наследник RuntimeError, см. класс выше), и продуктовый
                # код может поймать его собственным ``except Exception`` и
                # тихо уйти в офлайн-фолбэк. warnings.warn здесь -- та самая
                # видимость взамен непоглощаемости: pytest печатает
                # предупреждение в сводке в конце прогона независимо от
                # того, поймал ли его продуктовый код, так что разработчик
                # всё равно видит объяснение и адрес эскейп-хэтча, а не
                # только зелёную галочку.
                _warnings.warn(message, stacklevel=2)
                raise CatalogNetworkGuardViolation(message)
            return _real_urlopen(url, *args, **kwargs)

        monkeypatch.setattr(_urllib_request, "urlopen", _guarded_catalog_urlopen)

        # Фоновый SWR-поток не джойнится и переживает teardown теста -- если
        # он доедет до сети уже после отката monkeypatch (процесс/поток
        # ещё жив в момент), это будет настоящий сетевой запрос из
        # "ничьего" контекста. Превращаем его старт в пустышку по
        # умолчанию; тесты, которым нужен сам механизм SWR
        # (``tests/hermes_cli/test_model_cache_swr.py``), патчат
        # ``_spawn_catalog_swr_refresh`` заново в своём теле -- их патч
        # ставится позже и перекрывает этот no-op на время теста.
        monkeypatch.setattr(
            _model_catalog_mod, "_spawn_catalog_swr_refresh", lambda url: None
        )

        # Второй рубеж для файлов, которые делают
        # ``importlib.reload(model_catalog)`` (``test_model_catalog.py``,
        # ``test_model_catalog_offline_seed.py``): reload заново связывает
        # настоящую ``_spawn_catalog_swr_refresh`` и снимает no-op выше тем
        # же механизмом, которым мы обошли перехват сети. ``threading`` не
        # перезагружается вместе с ``model_catalog``, так что перехват
        # рождения потока здесь переживает reload -- подменяем
        # ``threading.Thread`` не функцией-фабрикой, а настоящим
        # подклассом ``Thread``: сторонние библиотеки (например, ``rich``
        # делает ``class _RefreshThread(Thread)`` в ``rich/live.py``)
        # наследуются от того, что лежит в ``threading.Thread`` в момент
        # определения класса, а от функции унаследоваться нельзя --
        # получали TypeError при импорте ``rich`` и всего, что его тянет.
        # Подкласс ведёт себя как обычный ``Thread`` для всех, и только
        # для потока с именем ``model-catalog-swr`` подменяет ``start()``
        # на пустышку, чтобы поток вовсе не рождался. Любой другой поток
        # (включая тесты, которые сами патчат ``threading.Thread`` внутри
        # своего тела, например ``test_swr_refresh_dedupes_inflight``) не
        # затронут.
        import threading as _threading

        _real_thread_cls = _threading.Thread

        class _GuardedThread(_real_thread_cls):
            def start(self):
                if self.name == "model-catalog-swr":
                    return
                super().start()

        monkeypatch.setattr(_threading, "Thread", _GuardedThread)
    ...
    _NPM_INSTALL_VERBS = ("install", "ci", "i")

    def _npm_verb_tokens(cmd) -> list:
        """Tokenize ``cmd`` for the npm-install check WITHOUT round-tripping
        an argv list through ``_cmd_to_string`` + ``_shlex.split``.

        That round-trip (join argv into one string, then re-split it in
        POSIX mode) EATS backslashes: ``['C:\\nodejs\\npm.cmd', 'install']``
        collapses to ``['C:nodejsnpm.cmd', 'install']`` (undetectable), and
        any argv element containing a space (``'C:\\Program Files\\nodejs\\
        npm.cmd'``) gets split into multiple tokens. ``find_node_executable
        ("npm")`` — the exact call site this guard was written for — returns
        precisely such a ``.cmd`` path on Windows. So: a real argv list
        (list/tuple) is used AS-IS, one token per element, no re-splitting.

        The one exception is a wrapper executable (``bash -c "..."`` etc.):
        there the payload is itself a shell string, so it still needs
        ``shlex.split`` to see the words inside it. A plain shell string
        (``cmd`` is ``str``/``bytes``) is always shlex-split, matching the
        semantics of the shell that will actually run it.
        """
        if isinstance(cmd, (list, tuple)):
            raw = [
            ...
            if not raw:
                return []
            head0 = raw[0].rsplit("/", 1)[-1].rsplit("\\", 1)[-1]
            if head0 not in _WRAPPER_COMMANDS:
                return raw
            # Wrapper case, e.g. ["bash", "-c", "cd /tmp && npm install"].
            # We deliberately do NOT parse a leading ``cd <dir> &&`` out of
            # the wrapped string to guess the shell's effective cwd — the
            # outer ``cwd=`` kwarg (compared against PROJECT_ROOT by the
            # caller) is still what decides whether this fires. We just
            # can't know statically where a shell string actually lands,
            # so a wrapped npm-install is judged against the *subprocess*
            # cwd only, same as every other command. Blocking on that
            # (rather than trying to be clever about "npm install" being
            # unreachable from a different `cd`) is the safe direction.
            tokens = []
            for part in raw[1:]:
                try:
                    tokens.extend(_shlex.split(part))
                except ValueError:
                    tokens.extend(part.split())
            return tokens
        cmd_str = _cmd_to_string(cmd)
        if "npm" not in cmd_str:
            return []
        try:
    ...
    def _is_npm_install(cmd) -> bool:
        tokens = _npm_verb_tokens(cmd)
        if not tokens:
            return False
        # Scan for the npm executable itself (bare "npm"/"npm.cmd", or a
        # full ``find_node_executable("npm")`` path — the real diagnosed
        # call site passes an absolute path, not a bare "npm") anywhere in
        # the token stream, so a wrapper (``bash -c "... npm install"``)
        # is still caught. The install VERB must be the first non-flag
        # token following npm — not merely present somewhere later — so
        # ``npm run ci``/``npm run build`` (a script name, not an install)
        # and prose like ``git commit -m "fix npm install flakiness"``
        # (where "npm" only appears inside one argv string, never as its
        # own token) don't false-positive.
        for idx, tok in enumerate(tokens):
            head = tok.rsplit("/", 1)[-1].rsplit("\\", 1)[-1]
            # ``npm``, ``npm.cmd`` (Windows shim), or an absolute
            # find_node_executable("npm") path all end in this token.
            if head in ("npm", "npm.cmd"):
                for t in tokens[idx + 1 :]:
                    if t.startswith("-"):
                        continue
                    if t in _NPM_INSTALL_VERBS:
                        return True
                    break
        return False

    def _resolved_cwd(cwd) -> Path:
        try:
    ...
    def _check_subprocess_cmd(name, cmd, cwd=None):
        # `npm install`/`npm ci`/`npm i` against the REAL checkout is
        # exactly how a `pytest` run silently rewrote the tracked
        # package-lock.json (a wizard-submit test that reached an
        # unmocked ``run_tool_install("camofox")`` — see
        # tests/hermes_cli/test_setup_wizard_app_submit.py's install-stage
        # guards) and blocked scripts/release_trix.sh, which refuses to
        # release with a dirty tracked file. The repo tree is not a
        # scratch directory a test gets to `npm install` into — a real
        # install belongs in a tmp dir the test creates itself.
        if _is_npm_install(cmd) and _resolved_cwd(cwd) == PROJECT_ROOT.resolve():
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}, cwd={cwd!r}) — this would run "
                "a REAL `npm install`/`ci` against the real repository "
                "root, which is not a scratch directory. This is almost "
                "always an unmocked installer path (e.g. a setup-wizard "
                "submit test that didn't mock wizard_tool_blocks/"
                "run_tool_install and reached the real post_setup install "
                "hook) — mock the installer call in the test instead, run "
                "npm against a tmp_path the test creates, or mark with "
                "@pytest.mark.live_system_guard_bypass if genuinely needed."
            )
    ...
    try:
        ...
        async def _guarded_async_exec(program, *args, **kwargs):
            _check_subprocess_cmd(
                "asyncio.create_subprocess_exec", [program, *args], cwd=kwargs.get("cwd")
            )
            return await real_async_exec(program, *args, **kwargs)

        async def _guarded_async_shell(cmd, *args, **kwargs):
            _check_subprocess_cmd(
                "asyncio.create_subprocess_shell", cmd, cwd=kwargs.get("cwd")
            )
