...
async def test_slow_push_is_waited_for(tmp_path: Path):
    ...
    try:
        ...
        # 5 с, а не 2: сервер-заглушка отвечает через 0.8 с, и на занятой
        # машине запуск процесса съедал остаток бюджета — тест падал в
        # релизном гейте 2026-09-09, проходя три раза из трёх в одиночку.
        # Смысл проверки в том, что медленный ответ ДОЖИДАЕТСЯ, а не в
        # том, за сколько именно; нижняя граница здесь ничего не стережёт.
        assert await client.wait_for_diagnostics(str(f), v0, mode="document", timeout=5.0)
