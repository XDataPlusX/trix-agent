...
def _pool_never_sees_the_developers_credentials(isolated_user_home):
    """Пул досевает синглтоны из ``~/.claude/.credentials.json``.

    ``_seed_from_singletons`` зовёт ``read_claude_code_credentials()``, а тот
    ищет файл через ``user_home_path()``. Подмены ``HERMES_HOME`` на это не
    хватает: дом профиля — отдельный источник. Без изоляции тест получал в
    пул ЛИШНЮЮ запись, досеянную из живого логина разработчика, и размер
    пула переставал совпадать с тем, что тест сам положил в ``auth.json`` —
    одноэлементный пул выглядел двухэлементным и «успешно» ротировался.

    Автоматическая, а не точечная: подсев одинаково искажает любой тест,
    который рассуждает о составе пула.
    """
    return isolated_user_home
