...
class TestTerminalFormatting:
    def test_terminal_format_has_sections(self, populated_db):
        ...
        assert "Trix Agent Insights" in text
...
class TestNoUpstreamBrandLeak:
    """Task 9d: /insights is disabled for the client, but format_terminal()
    and format_gateway() are the rendered surface any future caller (CLI,
    gateway, a re-enabled command) would show a client. The upstream
    fork's name must never appear in that rendered text, regardless of
    which command wires these formatters up."""

    def test_terminal_report_does_not_name_upstream_brand(self, populated_db):
        engine = InsightsEngine(populated_db)
        report = engine.generate(days=30)
        text = engine.format_terminal(report)
    ...
    def test_gateway_report_does_not_name_upstream_brand(self, populated_db):
        engine = InsightsEngine(populated_db)
        report = engine.generate(days=30)
        text = engine.format_gateway(report)
