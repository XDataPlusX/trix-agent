...
class TestGuidanceConstants:
    ...
    def test_session_search_guidance_demands_global_scope_disclosure(self):
        """Scoped search contract (spec 22): the guidance must (a) say the
        default search is current-conversation-first and (b) require the
        model to tell the user when a search ran globally."""
        assert "current conversation" in SESSION_SEARCH_GUIDANCE
        assert "global scope" in SESSION_SEARCH_GUIDANCE
        assert "scope" in SESSION_SEARCH_GUIDANCE
...
# Права каталога ничего не значат под root (обход проверки доступа) и на
# Windows (другая модель прав) — там каталог 0000 всё равно читается.
_needs_unix_permissions = pytest.mark.skipif(
    os.name == "nt" or (hasattr(os, "geteuid") and os.geteuid() == 0),
    reason="нужны юниксовые права: под root и на Windows каталог 0000 всё равно читается",
)


@pytest.fixture
def unreadable_parent(tmp_path):
    """``tmp_path/outer`` с правами 0000 и каталогом ``inner`` внутри.

    Стенд для запуска из каталога, чей родитель принадлежит другому
    пользователю (`/root` с правами 0700, чужой домашний каталог):
    stat любого пути внутри ``outer`` возвращает EACCES.
    """
...
@_needs_unix_permissions
class TestWalkSurvivesUnreadableParents:
    """Нечитаемый родитель — «здесь ничего не видно», а не падение запуска."""

    def test_find_git_root_does_not_raise(self, unreadable_parent):
        result = _find_git_root(unreadable_parent)
        if result is not None:
            assert (result / ".git").exists()

    def test_find_git_root_reaches_readable_root_above(
        self, tmp_path, unreadable_parent
    ):
        (tmp_path / ".git").mkdir()
        assert _find_git_root(unreadable_parent) == tmp_path

    def test_find_hermes_md_walks_past_unreadable_dirs(
        self, tmp_path, unreadable_parent
    ):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".trix.md").write_text("правила репозитория")
        assert _find_hermes_md(unreadable_parent) == tmp_path / ".trix.md"

    @pytest.mark.parametrize("from_inner", [True, False])
    def test_context_prompt_builds_from_unreadable_cwd(
        self, unreadable_parent, from_inner
    ):
        """Сам сценарий находки: запуск из чужого каталога (`cwd=/root`).

        Хелперы обхода — только первый stat на этом пути; за ними идут
        AGENTS.md / CLAUDE.md / .cursorrules, и каждый из них ронял
        запуск тем же EACCES. Проверяем весь сбор контекста целиком, а
        не одну функцию: и когда нечитаем родитель, и когда нечитаем сам
        cwd.
        """
        cwd = unreadable_parent if from_inner else unreadable_parent.parent
        result = build_context_files_prompt(
            cwd=str(cwd), skip_soul=True, allow_install_tree_fallback=True
        )
