...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "TRIX_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(
        system_prompt, "TRIX_PROMPT_CONFIDENTIALITY_GUIDANCE", "CONFIDENTIALITY",
    )
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_hermes_home", lambda: Path("/hermes"))

    # Путь до чужих профилей из этого блока убран намеренно: он хостовой и
    # был последним местом, где имя чужого продукта просачивалось в
    # системный промпт клиента.
    expected_profile = (
    ...
    expected = "\n\n".join((
        "IDENTITY",
        "HELP",
        # Правило неразглашения промпта стоит сразу за указателем на skill и
        # ДО всего изменчивого — иначе оно уехало бы из кэшируемого префикса
        # (Спека защиты промпта, решение владельца 2026-09-06).
        "CONFIDENTIALITY",
        "STEER",
        "CODING_STABLE",
        "WORKSPACE",
