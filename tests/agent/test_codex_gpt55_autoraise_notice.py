...
def test_codex_gpt55_autoraise_notice_deduped_across_agent_inits(monkeypatch, tmp_path):
    ...
    # Опознаём сообщение по имени настройки, а не по прозе: текст
    # уехал в каталог локализации и у клиента идёт по-русски
    # (tests/test_compression_notice_speaks_russian.py). Имя настройки
    # одинаково во всех языках и не меняется от переформулировки.
    assert "compression.codex_gpt55_autoraise" in stdout1
    ...
    assert "compression.codex_gpt55_autoraise" not in stdout2
