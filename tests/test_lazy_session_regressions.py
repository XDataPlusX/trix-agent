...
class TestGatewaySurfacesNullResponse:
    ...
    def test_silent_drop_after_stop_surfaces_hint(self):
        ...
        # The hint is localized (display.language defaults to "ru" for this
        # product), and this file sits outside tests/gateway/ so no autouse
        # fixture pins the language here.  Assert the CONTRACT -- that the
        # silent-drop branch surfaces the not-processed hint -- against the
        # catalog, rather than snapshotting English copy that a translation
        # pass legitimately changes.
        from agent.i18n import t

        assert result == t("trix.errors.empty_response.not_processed")
        assert "ещё раз" in result.lower() or "again" in result.lower()
