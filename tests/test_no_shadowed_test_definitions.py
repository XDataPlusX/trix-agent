...
def _test_modules() -> list[Path]:
    # walk_files() instead of rglob: tests/ is where the gate's parallel
    # workers churn __pycache__ hardest, and rglob descends into those
    # directories and dies when one vanishes mid-walk (RAF-171).
    return [path for _, path in walk_files(TESTS_ROOT, match="test_*.py")]
