...
# save_config() + migrate_config() write via atomic_roundtrip_yaml_save
# (comment-preserving ruamel round-trip) which produces a fresh inode, so
# stat() sees a new mtime_ns and the next load repopulates automatically —
# no explicit invalidation hook.
...
def format_managed_message(action: str = "modify this installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"

    if managed_system == "NixOS":
        return t(
            "trix.update.managed_nixos",
            action=action,
            default=(
                f"Cannot {action}: this installation is managed by NixOS. "
                "Ask whoever administers it to apply the update through the "
                "system configuration."
            ),
        )

    return t(
        "trix.update.managed_generic",
        action=action,
        managed_system=managed_system,
        default=(
            f"Cannot {action}: this installation is managed by {managed_system}. "
            "Ask whoever administers it to upgrade or reinstall it."
        ),
    )
...
def _explicit_config_paths(config: Dict[str, Any]) -> Set[Tuple[str, ...]]:
    ...
    def _walk(value: Any, path: Tuple[str, ...]) -> None:
        if isinstance(value, dict) and value:
            ...
            return
        # Пустой словарь — тоже написанная пользователем строка (``hooks: {}``).
        # Раньше обход уходил в детей и у пустого словаря не записывал НИЧЕГО,
        # поэтому сохранять было нечего и секция исчезала из файла на первом же
        # save_config: формально значение равно дефолту, но пользователь его
        # написал, и файл — в том числе список того, что вообще настраивается.
        if path:
            paths.add(path)
...
#: Признак «этот ключ выкушен». Раньше эту роль играл ``None`` — и на ключе,
#: чьё ЗНАЧЕНИЕ равно ``None``, роли сталкивались: сохранённый голый ``key:``
#: возвращался как ``None``, вызывающий код читал это как «ребёнок выкушен»,
#: родитель оставался пустым и выкусывался тоже. В поставляемом клиентском
#: шаблоне так исчезала секция ``platforms.telegram.extra.command_menu`` —
#: голый ключ с двадцатью строками объяснения под ним — при любой миграции.
_STRIPPED = object()
...
def _strip_default_values(
    ...
    survive a ``save_config`` round-trip. This holds for EMPTY values too —
    a bare ``key:`` (``None``) and an empty ``key: {}`` are things the user
    wrote, and they survive; see ``_STRIPPED``. When the caller passes a
    MERGED config (``save_config(load_config())``), such a section arrives as
    a full default subtree; it is written back EMPTY rather than dropped
    (which would orphan the comment above it) or materialised (which is the
    "hermes update blows up my config" failure this whole pass exists to
    prevent).
    ...
    def _strip(value: Any, default: Any, path: Tuple[str, ...]) -> Any:
        # Непустой словарь разбирается ПЕРЕД проверкой preserve_keys.
        # Иначе путь самого контейнера («пользователь написал ``agent: {}``»)
        # читался бы как «сохранить всё поддерево целиком» и отключал бы
        # стриппер внутри этой секции. Для прежних входов порядок не значил
        # ничего: ``_explicit_config_paths`` возвращал только пути ЛИСТЬЕВ, и
        # путь непустого словаря в preserve_keys не попадал никогда.
        if isinstance(value, dict) and value:
            ...
            for key, child in value.items():
                ...
                if stripped_child is not _STRIPPED:
                    stripped[key] = stripped_child
            if stripped:
                return stripped
            if path in preserve_keys:
                # Пользователь написал этот раздел (в сыром конфиге он голый
                # `key:` или `key: {}`), а всё его содержимое — дефолты.
                # Удалить раздел нельзя: вместе с ним осиротеет комментарий
                # над ним, а именно комментарии и есть документация продукта.
                # Вернуть поддерево целиком тоже нельзя — это материализация
                # дефолтов, то самое «hermes update blows up my config».
                # Поэтому раздел остаётся ПУСТЫМ, каким его и написали.
                #
                # Разница видна только на СЛИТОМ конфиге
                # (`save_config(load_config())` — agent_import.py, claw.py):
                # там на месте голого `display:` стоит непустой словарь
                # дефолтов, обход уходит внутрь и поддерево пустеет.
                return {}
...
def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    ...
    calling :func:`utils.atomic_roundtrip_yaml_save` directly. It runs
    ...
    Writes through ``utils.atomic_roundtrip_yaml_save`` (comment-preserving
    ruamel round-trip mode), not a plain ``yaml.dump`` — this is the same
    chokepoint the gateway's slash commands write ``config.yaml`` through
    (``/model``, ``/verbose``, ...), so a client's curated template comments
    must survive a Telegram/Discord/etc.-driven setting change too, not only
    the setup wizard's own writes.

    ``extra_content`` (a commented-out example section — see
    ``atomic_roundtrip_yaml_save``) is the only kwarg forwarded. Legacy
    PyYAML-era kwargs from call sites written against the old plain-dump
    writer (``sort_keys``, ``default_flow_style``) are accepted but silently
    ignored: round-trip mode never sorts keys and always writes block style,
    so neither has meaning under the new writer.
    """
    from utils import atomic_roundtrip_yaml_save

    require_readable_config_before_write(config_path)
    atomic_roundtrip_yaml_save(config_path, data, extra_content=kwargs.get("extra_content"))
...
def save_config(
    ...
    *,
    ...
    with _CONFIG_LOCK:
        ...
        def _has_valid_fallback_entry(value: Any) -> bool:
            if isinstance(value, list):
                return any(isinstance(e, dict) and e.get("provider") and e.get("model") for e in value)
            if isinstance(value, dict):
                return bool(value.get("provider") and value.get("model"))
            return False

        # Both the legacy singular key (``fallback_model``) and the current
        # list key (``fallback_providers`` — what the setup wizard and
        # ``hermes model`` write) count as "fallback already configured".
        # Checking only the legacy key left the commented-out "how to set up
        # a fallback" instructions glued onto the end of a config.yaml that
        # already had a real ``fallback_providers`` chain — confusing and
        # redundant.
        fb_is_valid = _has_valid_fallback_entry(
            normalized.get("fallback_model", {})
        ) or _has_valid_fallback_entry(normalized.get("fallback_providers", []))
        if not fb_is_valid:
            parts.append(_FALLBACK_COMMENT)

        # Comment-preserving round-trip write (utils.atomic_roundtrip_yaml_save)
        # instead of the plain-yaml.dump atomic_yaml_write: config.yaml often
        # starts life as a curated template with Russian explanatory comments
        # (assets/config/trix-config.yaml, seeded by doctor --fix /
        # scripts/install.sh) — a full re-serialize from a bare dict would
        # silently wipe every one of them on the very first save_config() call
        # (setup wizard, `hermes model`, dashboard PUT /api/config, ...).
        from utils import atomic_roundtrip_yaml_save

        atomic_roundtrip_yaml_save(
            config_path,
            normalized,
            # Pass the blocks as a list (not "".join(parts)) so
            # atomic_roundtrip_yaml_save can check each one for idempotency
            # independently — a block already preserved from a prior save
            # must not be re-appended (#round-2 fix: unconditional append
            # duplicated _SECURITY_COMMENT/_FALLBACK_COMMENT on every save).
            extra_content=parts or None,
        )
...
def reassert_config_owned_env() -> None:
    """Re-assert env vars whose source of truth is ``config.yaml``, not ``.env``.

    Some env vars Hermes knows about are *mirrors* of a ``config.yaml``
    setting rather than values a user writes into ``.env`` — the whole
    ``terminal.*`` block (``TERMINAL_ENV`` above all) is bridged into the
    environment because ``tools/terminal_tool.py`` runs in child processes
    and reads only ``os.environ``.

    Any code path that re-reads ``.env`` and drops known keys missing from it
    must call this afterwards, or it silently revokes a decision the user
    made in ``config.yaml``.  For ``terminal.backend: docker`` that decision
    is the sandbox itself: dropping ``TERMINAL_ENV`` falls back to the
    historical ``local`` default and the next agent command runs on the host.

    Fail-open by design — a config problem must never break an env reload.
    ``load_hermes_dotenv()`` already does the same thing after its own
    known-key cleanup (``_reapply_terminal_config_bridge``); this is the
    sibling call path that lacked it.
    """
    try:
        apply_terminal_config_to_env(env=None)
    except Exception:
        logger.debug("config.yaml → env re-assert failed", exc_info=True)
...
def reload_env() -> int:
    ...
    Vars that mirror ``config.yaml`` (the ``terminal.*`` bridge) are then
    re-asserted: ``.env`` is not their source of truth, so its silence must
    not delete them.  See ``reassert_config_owned_env``.
    """
    env_vars = load_env()
    known_keys = set(OPTIONAL_ENV_VARS.keys()) | _EXTRA_ENV_KEYS
    before = {key: os.environ.get(key) for key in known_keys | set(env_vars)}
    ...
    for key in known_keys:
        if key not in env_vars and key in os.environ:
            del os.environ[key]
    reassert_config_owned_env()
    # Count net changes, so a key deleted above and immediately restored from
    # config.yaml is not reported to the user as "updated".
    return sum(1 for key, old in before.items() if os.environ.get(key) != old)
