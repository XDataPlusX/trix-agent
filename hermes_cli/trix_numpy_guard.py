...
#
# Снято на живой машине 2026-09-05: когда установщик не смог дочистить
# старый каталог пакета (`uv`: «failed to remove directory
# .../numpy/__pycache__»), на диске остался каталог-обманка — `import
...
#
# Оговорка о происхождении: конкретно ту неудаляемость создал не продукт,
# а диагностика — несколько запусков `python` от root оставили в venv
# root-овые `__pycache__`. Но неудача удаления сама по себе продуктом не
# исключена (права, занятые файлы, кончившееся место), а негодный
# результат у неё один и тот же. Проверять надо состояние, а не гадать о
# причине.
#
...
def ensure_runnable_numpy(python: str | None = None) -> NumpyGuardResult:
    ...
    try:
        ...
        # Код возврата установщика раньше не смотрели вовсе, и любой его
        # провал выглядел как «откат не помог» — то есть как свойство
        # процессора. Найдено на живой машине 2026-09-06: uv падал с
        # `Permission denied` на venv, принадлежащем root, а клиент читал
        # про несовместимость и шёл повторять `hermes doctor --fix`,
        # который упадёт ровно так же. Причина обязана называться своим
        # именем, иначе совет неисполним.
        if proc.returncode != 0:
            return NumpyGuardResult(
                True, False, True, _install_failure_message(proc),
            )
...
def _install_failure_message(proc) -> str:
    """Назвать причину провала установщика словами клиента, а не кодом.

    Права на venv выделены отдельно: это не поломка numpy и не свойство
    процессора, а владелец каталога, и лечится оно совсем другим
    действием. Все остальные случаи сводятся к одной строке — хвост
    вывода установщика уходит в лог, а не в чат.
    """
    ...
    if "permission denied" in text or "read-only file system" in text:
        return (
            ...
            "нет прав на запись в окружение агента. Так бывает на машинах, "
            "установленных до того, как окружение стали отдавать "
            "пользователю."
        )
    return (
        "numpy не запускается на этом процессоре, и установщик не смог "
        f"поставить версию 1.x (код {proc.returncode}). Подробности — в логе."
    )
