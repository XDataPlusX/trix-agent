...
def _format_money(value: Optional[Decimal]) -> str:
    if value is None:
        return t("trix.model_cost.unknown", default="unknown")
    return f"${value:.2f}/M"
...
def expensive_model_warning(
    ...
    *,
    ...
    lines = [
        t(
            "trix.model_cost.banner",
            default="!!! EXPENSIVE MODEL WARNING !!!",
        ),
        "",
        t(
            "trix.model_cost.above_threshold",
            model=model,
            default=f"{model} has known pricing above the safety threshold.",
        ),
        t(
            "trix.model_cost.input_tokens",
            value=_format_money(input_cost),
            default=f"Input tokens: {_format_money(input_cost)}",
        ),
        t(
            "trix.model_cost.output_tokens",
            value=_format_money(output_cost),
            default=f"Output tokens: {_format_money(output_cost)}",
        ),
        t(
            "trix.model_cost.threshold",
            default=(
    ...
    if source:
        lines.append(
            t(
                "trix.model_cost.pricing_source",
                source=source,
                default=f"Pricing source: {source}.",
            )
        )
    if model.lower() == GPT55_PRO_OPENROUTER_ID:
        lines.append(
            t("trix.model_cost.gpt55_suggestion", default=GPT55_SUGGESTION)
        )
    lines.append(
        t(
            "trix.model_cost.confirm_hint",
            default="Confirm only if you intend to use this model.",
        )
    )
