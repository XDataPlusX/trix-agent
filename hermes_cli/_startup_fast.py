...
def print_fast_version_info() -> None:
    from hermes_cli.release_source import RELEASE_TAG_PREFIX, resolve_local_release_tag

    tag = resolve_local_release_tag(project_root_str())
    if tag:
        version = tag[len(RELEASE_TAG_PREFIX):] if tag.startswith(RELEASE_TAG_PREFIX) else tag
        print(f"Trix Agent v{version}")
    else:
        print("Trix Agent (unreleased build)")
