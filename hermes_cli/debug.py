...
Currently supports:
    hermes debug share    Collect a debug report (system info + logs) and
                          print it to your terminal. Never uploaded
                          anywhere. By default, log content is run through
                          ``agent.redact.redact_sensitive_text`` with
                          ``force=True`` before it's printed, so credentials
                          in ``~/.hermes/logs/*.log`` aren't shown even in
                          the local output. Pass ``--no-redact`` to disable.
                          ``--nous`` and ``--expire`` are still accepted for
                          compatibility with older scripts but no longer do
                          anything — nothing is ever uploaded.
"""
...
# Visible in the printed report so the customer knows the content was
# sanitized. Kept short; the trailing newline guarantees the banner sits on
# its own line.
_REDACTION_BANNER = (
    "[hermes debug share: log content redacted at collection time. "
...
# Превью сообщения клиента в строке журнала. `agent/turn_context.py` пишет
# на INFO строку вида
#
#   ... agent.turn_context: conversation turn: session=… platform=telegram
#   history=0 msg='<первые 80 символов сообщения клиента>'
#
# — полезную локально (по ней видно, какой ход разбираем) и недопустимую в
# тексте, который клиент пересылает в поддержку. Превью всегда последнее
# поле строки, поэтому вычищаем от `msg=` до конца строки: если завтра
# появится второе такое место с полем в середине, мы срежем хвост строки —
# сторона ошибки выбрана намеренно, потерять техническую подробность
# дешевле, чем выпустить чужую переписку.
_CONVERSATION_PREVIEW_RE = re.compile(r"""msg=(['"]).*$""", re.MULTILINE)
...
def _save_report_locally(report: str) -> Path:
    """Write a collected debug report to a file under the customer's own
    ``HERMES_HOME``, instead of uploading it anywhere.

    Trix Agent never ships debug data off the VM (see
    docs/product/specs/2026-08-13-trix-agent-persona-design.md). This is the
    on-disk stand-in for what used to be a paste-service URL: a durable,
    local place the customer (or whoever is remoted into the VM on their
    behalf) can retrieve the report from. Returns the path written to.

    The file is written mode 0600 (owner read/write only). Old reports are
    pruned on every write -- see :func:`_prune_old_reports`.
    """
    reports_dir = get_hermes_home() / "debug-reports"
    reports_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    path = reports_dir / f"debug-report-{stamp}.txt"
    tmp = path.with_suffix(path.suffix + ".tmp")
    tmp.write_text(report, encoding="utf-8")
    tmp.chmod(0o600)
    atomic_replace(tmp, path)
    _prune_old_reports(reports_dir, keep=path)
    return path


# Retention for $HERMES_HOME/debug-reports/ (product decision, 2026-09-07).
# Two rules, whichever bites first: keep at most the newest
# _DEBUG_REPORT_KEEP files, and drop anything older than
# _DEBUG_REPORT_MAX_AGE_DAYS even when the directory is nearly empty. A
# report is a few tens of KB, so the cap is not about disk -- it is about
# not keeping a growing pile of machine snapshots around forever for a
# command that is used a handful of times a year. No config knob: a
# retention window nobody asked to tune is one more setting to explain.
_DEBUG_REPORT_KEEP = 10
_DEBUG_REPORT_MAX_AGE_DAYS = 30
...
def _prune_old_reports(
    ...
    *,
    ...
    """Delete stale debug reports; return the paths actually removed.

    Runs inline on each write rather than from a scheduler: reports are
    only ever created here, so there is no drift to catch up on later, and
    a pruning job that needs the cron ticker alive would be dead exactly
    when the machine is unhealthy -- which is when reports get made.

    ``keep`` is never deleted (the report just written, even in the
    pathological case of a clock that jumped). Only files matching the
    generated name shape are considered, so anything a person dropped into
    the directory by hand is left alone. Failures are swallowed: losing a
    prune must never lose the report the client is waiting for.
    """
    cutoff = (now if now is not None else time.time()) - _DEBUG_REPORT_MAX_AGE_DAYS * 86400
    removed: list[Path] = []
    try:
        reports = sorted(
            (p for p in reports_dir.glob("debug-report-*.txt") if p.is_file()),
    ...
    for index, report in enumerate(reports):
        if keep is not None and report == keep:
            continue
        try:
            too_many = index >= _DEBUG_REPORT_KEEP
            too_old = report.stat().st_mtime < cutoff
...
def upload_to_pastebin(content: str, expiry_days: int = 7) -> str:
    """Disabled. Trix Agent never uploads customer logs anywhere.

    This used to upload *content* to a public paste service (paste.rs, then
    dpaste.com). ``run_debug_share`` no longer calls this path -- it forces
    ``local_only`` before any network I/O can happen -- but this function
    stayed fully functional underneath that one guard. This repo absorbs a
    steady stream of upstream merges, and a merge conflict resolved the
    wrong way in ``run_debug_share`` (500 lines from here) would silently
    re-enable public-paste upload of conversation logs with no other line of
    defense. Raising unconditionally here, at the actual network boundary,
    means that can't happen no matter how ``run_debug_share`` is merged.
    """
    raise RuntimeError(
        "Trix Agent never uploads customer logs or debug reports anywhere. "
        "The debug report is collected and printed locally instead -- see "
        "`hermes debug share`."
    )
...
def _redact_log_text(text: str) -> str:
    """Scrub upload-bound log text: secrets, e-mail addresses, chat previews.
    ...
    The third rule (``_CONVERSATION_PREVIEW_RE``) is the product's own, not
    upstream's: a debug report exists to be forwarded to support, so the
    client's own message text must not survive into it. See
    ``tests/hermes_cli/test_debug_report_excludes_conversation.py`` for the
    end-to-end proof over a real conversation turn.

    ``redact_sensitive_text`` alone is NOT enough, and the gap is deliberate
    on its side rather than a bug: the global pass lets web-URL query strings
    and ``user:pass@`` userinfo through on purpose (#34029), because magic
    links and OAuth callbacks are URLs the agent must round-trip through its
    own history — masking them by parameter name broke skills mid-flow.

    A report is the opposite of a round trip. Nobody follows a link out of
    it; it is a snapshot forwarded to a human. So it opts INTO the two URL
    redactors the global pass leaves off, exactly as the CDP endpoint helper
    does (``agent.redact.redact_cdp_endpoint_for_log`` documents the same
    reasoning). Without this, a proxy URL logged anywhere reaches the report
    verbatim — which is how a client's live proxy password ended up in one on
    2026-09-12, with redaction formally enabled the whole time.

    Deliberately NOT fixed by widening the global pass: that would re-break
    the #34029 workflows. ``tests/hermes_cli/test_debug_report_redacts_url_credentials.py``
    pins both halves — the report masks, the global pass still does not.
    """
    if not text:
        return text
    ...
    return _CONVERSATION_PREVIEW_RE.sub("msg='[REDACTED_MESSAGE]'", text)
...
def run_debug_share(args):
    ...
    # Trix Agent: client logs never leave the customer VM. Both upstream
    # upload paths (public paste services and Nous-internal S3) are disabled
    # by product decision — see docs/product/specs/2026-08-13-trix-agent-persona-design.md
    if not local_only:
        print(
            "Trix Agent collects the debug report locally and does not upload "
            "it anywhere. Send the printed output to support yourself if asked.\n"
        )
        local_only = True
        nous = False
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        print()
        print("Commands:")
        print("  share    Collect a debug report and print it to your terminal")
        print("  delete   Delete a paste uploaded by an older build")
        print()
        print("Options (share):")
        print("  --lines N    Number of log lines to include (default: 200)")
        print("  --expire N   Accepted for compatibility; unused (nothing is uploaded)")
        print("  --local      Accepted for compatibility; this is the only behavior now")
        print("  --no-redact  Disable local redaction of secrets (default: redact)")
        print()
