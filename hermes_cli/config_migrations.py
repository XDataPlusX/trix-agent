...
def _migrate_to_35(results: Dict[str, Any], quiet: bool) -> None:
    # ── Version 34 → 35: догнать КОММЕНТАРИИ клиентского шаблона ──
    # Переход на полный config.yaml (решение владельца, сентябрь 2026)
    # поменял не только набор секций, но и тексты, которые клиент читает.
    # Секции доезжают до уже работающих машин досевом
    # (``trix_config_sync``), а комментарии — нет: у досева жёсткий
    # инвариант «ни одной существующей строки не менять». Без этого шага
    # шапка на каждой работающей машине продолжала бы утверждать «здесь
    # только то, что отличается от значений по умолчанию» — над файлом, в
    # который досев только что дописал все 85 секций.
    #
    # Замены дословные: не нашлось точного старого текста — не меняем
    # ничего (клиент правил комментарий сам, или машина на более старой
    # версии шаблона). Значений шаг не касается вовсе и НЕ идёт через
    # ``_persist_migration``: тот пишет со ``strip_defaults`` и выкусил бы
    # из конфига пустые секции.
    from hermes_cli.trix_config_comments import sync_client_comments

    import logging

    _c = _cfg()
    applied, unmatched = sync_client_comments(_c.get_config_path())
    if unmatched:
        # На уже обновлённой машине это норма. А вот если не применилось
        # НИЧЕГО, а файл при этом полный — клиент правил пояснения руками, и
        # у него осталась старая шапка над новым содержимым. Само это не
        # чинится (дословной замены нет — угадывать нельзя), но оператор
        # должен иметь возможность это увидеть.
    ...
    for label in applied:
        results["config_added"].append(f"пояснение в config.yaml: {label}")
    if not quiet:
        print(f"  ✓ Обновлены пояснения в config.yaml ({len(applied)})")
