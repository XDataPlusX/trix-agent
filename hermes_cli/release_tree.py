...
# Ровно то, что исключает установщик (вызов `git sparse-checkout set`
# в scripts/install.sh). Списки обязаны совпадать посимвольно -- это
# проверяет tests/test_client_surface_guards.py; правка одного без
# другого роняет сборку витрины.
...
EXCLUDED_PATHS: tuple[str, ...] = (
    "docs/product",
    # docs/debug -- /debug-отчёты. Здесь основание ДРУГОЕ, чем у соседей:
    # остальное исключено за ненадобностью клиенту (вес, чужая тема), а
    # это -- потому что отчёт несёт хвосты логов с учётными данными, и
    # при этом принадлежит КОНКРЕТНОМУ клиенту. Витрина у всех клиентов
    # одна, поэтому опубликованный отчёт -- это и утечка секрета, и
    # выдача данных одного клиента всем остальным.
    # Каталог отдельно закрыт бы .gitignore, но игнор снят намеренно
    # (владелец работает с двух устройств и возит отчёты через git), так
    # что этот список -- единственное, что стоит между отчётом и
    # публичным репозиторием. Не снимать вместе с игнором.
    # Редактор отчёта тут не помощник: agent/redact.py маскирует
    # `user:pass@` только для БД-схем, а `http://user:pass@host`
    # пропускает насквозь -- проверено запуском 2026-09-12.
    "docs/debug",
    # docs/progress -- логи передачи состояния между сессиями, их пишет
    # скилл логирования. Каталог рабочий и продолжает наполняться сам,
    # поэтому закрыт именно каталог, а не перенесён отдельный файл:
    # иначе следующий лог снова уедет клиенту, и никто этого не заметит.
    "docs/progress",
    "website",
    "apps/desktop",
