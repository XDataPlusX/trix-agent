...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("new", "Start a new conversation with a clean history", "Session",
    ...
    CommandDef("compress", "Compress a long conversation to reduce cost and context usage", "Session",
               aliases=("compact",), args_hint="[here [N] | focus topic | --preview|--dry-run]"),
    CommandDef("rollback", "List or restore files to their state before the agent's edits", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of the agent's config/state", "Session",
    ...
    CommandDef("stop", "Interrupt the agent's current response and any background processes it started", "Session",
    ...
    CommandDef("sethome", "Set this chat as the home channel for scheduled tasks and cross-platform messages", "Session",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "Resume one of your previous sessions", "Session",
               args_hint="[name]"),
    ...
    CommandDef("reasoning", "Manage response thinking depth (cost/accuracy trade-off) and display", "Configuration",
    ...
    CommandDef("fast", "Toggle faster responses at a higher cost (normal/fast)", "Configuration",
    ...
    CommandDef("wake", "Toggle the wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while the agent is working", "Configuration",
    ...
    CommandDef("restart", "Gracefully restart the agent after draining active runs", "Session",
               gateway_only=True, busy_policy="dispatch"),
    CommandDef("setup", "Reopen the setup wizard — the bot sends a fresh address and one-time password",
               "Configuration", gateway_only=True),
    # args_hint intentionally omits "reset [--force]": that subcommand's own
    # reply names "openai-codex" by name (gateway.usage.reset_wrong_provider,
    # both locales) -- a competitor product a Trix client on their own keys
    # will essentially never have configured. Not advertised in /help; still
    # dispatchable if typed, same as any other hidden-but-working command.
    CommandDef("usage", "Show token usage and rate limits", "Info"),
    CommandDef("subscription", "View your provider plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your provider balance and manage billing in the browser", "Info"),
    ...
    CommandDef("update", "Update Trix Agent to the latest version", "Info",
               busy_policy="dispatch"),
    # busy_policy="dispatch": the command touches nothing in the
    # conversation, and free space is asked about precisely when something
    # has ground to a halt -- refusing it mid-run would refuse it exactly
    # when it is needed. (Registering the policy is only half of it: the
    # mid-run handler table in gateway/run.py's
    # _dispatch_busy_slash_command must name it too.)
    CommandDef("disk", "Show disk space; `clean` clears service files", "Info",
               args_hint="[clean]", subcommands=("clean",),
               busy_policy="dispatch", execute="disk"),
    CommandDef("version", "Show Trix Agent version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Collect a diagnostic report as a file (versions, settings and log tails)", "Info"),
...
def command_description(cmd: CommandDef) -> str:
    """Return *cmd*'s description in the active UI language.

    The canonical English lives on ``CommandDef.description``; translations
    live under the registry-backed ``commands.<name>.description`` catalog
    key.  A missing translation falls back to the registry text, so
    ``display.language: en`` renders byte-identically to before i18n and an
    untranslated new command degrades to English rather than a key path.

    Only messaging surfaces (Telegram's command menu, gateway ``/help``,
    Discord's native picker) call this. CLI/TUI/desktop surfaces keep
    reading ``cmd.description`` directly — see the spec, Ruling 4.
    """
    from agent.i18n import t
    return t(f"commands.{cmd.name}.description", default=cmd.description)


# Hints whose brackets hold a LITERAL subcommand the parser compares verbatim,
# but which carry no "|" for the heuristic in command_args_hint to catch.
# Translating one hands the customer a word the parser rejects:
#   deny    -> gateway/slash_commands.py: tokens[0].lower() == "all"
#   context -> gateway/slash_commands.py: args in {"all", "full", "details"}
# /deny is the costly one: a customer with several pending dangerous commands
# who types the translated word gets ONE denial instead of all of them, with
# the rest still blocking and a garbled reason handed to the agent.
_LITERAL_SUBCOMMAND_HINTS = frozenset({"deny", "context"})


def command_args_hint(cmd: CommandDef) -> str:
    """Return *cmd*'s argument hint in the active UI language.

    Translation of hints is enabled per-command by the presence of a
    ``commands.<name>.args_hint`` key; hints listing literal subcommands
    (e.g. ``[on|off]``) are enforced-English (spec, Ruling 2): translating
    them would teach the customer a word the parser does not accept, which
    is worse than leaving it English.
    """
    from agent.i18n import t
    if not cmd.args_hint:
        return ""
    if "|" in cmd.args_hint or cmd.name in _LITERAL_SUBCOMMAND_HINTS:
        # Ruling 2: this hint lists literal subcommands the customer must
        # type verbatim.  Translating it would teach them a word the parser
        # rejects, which is worse than leaving it English.  Enforced here
        # rather than by convention so a stray catalog key cannot override
        # it.
        return cmd.args_hint
    return t(f"commands.{cmd.name}.args_hint", default=cmd.args_hint)
...
def suggest_gateway_command(typed: str | None, *, cutoff: float = 0.75) -> str | None:
    """Closest gateway command name for a typo'd ``typed`` token.

    Fuzzy ``difflib`` match over ``GATEWAY_KNOWN_COMMANDS`` (hyphen-normalized
    like the gateway's dispatch check, so ``/debuf`` suggests ``/debug``).
    Returns the single best candidate or ``None`` — purely advisory; it never
    changes dispatch.  Pure function: the gateway's unknown-command notice
    appends a "Did you mean /x?" hint from it, mirroring the CLI's
    "Did you mean" fallback for typos.
    """
    if not typed:
        return None
    normalized = typed.replace("_", "-").strip()
    if not normalized:
        return None
    from difflib import get_close_matches

    matches = get_close_matches(
        normalized, sorted(GATEWAY_KNOWN_COMMANDS), n=1, cutoff=cutoff
    )
...
def gateway_help_lines(only: Iterable[str] | None = None) -> list[str]:
    """Generate gateway help text lines from the registry.

    With *only* omitted (the default) this stays UNCURATED: every
    gateway-available command in registry order, exactly as before Trix's
    client-command-surface work
    (``docs/product/specs/2026-09-01-trix-agent-client-command-surface-design.md``
    Ruling 2). Five test files call this with no argument and must keep
    seeing the full registry --
    ``tests/hermes_cli/test_approvals_command.py`` in particular asserts
    ``/approvals`` is present even though Trix disables it on the client
    surface; ``tests/hermes_cli/test_commands.py``,
    ``tests/gateway/test_voice_command.py`` and
    ``tests/gateway/test_title_command.py`` make the same no-argument call
    for unrelated commands and would break for an unrelated reason if this
    default curated.

    Pass *only* (an iterable of canonical ``CommandDef.name`` values) to
    restrict the result to just those names, still in registry order and
    still subject to the normal gateway-availability check.
    ``hermes_cli.slash_exec._exec_help`` is the one caller that passes
    Trix's ``hermes_cli.trix_menu.client_surface_commands()``.
    """
    from agent.i18n import t
    ...
    for cmd in COMMAND_REGISTRY:
        if not _is_gateway_available(cmd, overrides):
            continue
        if allowed is not None and cmd.name not in allowed:
            continue
        args = f" {command_args_hint(cmd)}" if cmd.args_hint else ""
        ...
        alias_note = (
            t("trix.commands.alias_note", aliases=", ".join(alias_parts))
            if alias_parts else ""
        )
...
def telegram_bot_commands() -> list[tuple[str, str]]:
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        if tg_name:
            result.append((tg_name, command_description(cmd)))
...
# Telegram allows up to 100 BotCommands. This default must stay >=
# len(telegram_bot_commands()) (see
# TestTelegramBotCommands::test_default_cap_covers_every_gateway_command
# in tests/hermes_cli/test_commands.py) -- otherwise the clamp in
# telegram_menu_commands() silently drops whichever command sorts last
# out of _TELEGRAM_MENU_PRIORITY, off the visible menu, with no error
# anywhere (review fix round, Task 11: adding /setup pushed the registry
# to 61 and this constant, still at the old 60, silently dropped
# /version). A 71-slot default (registry size + ~10) keeps every
# built-in plus common skill commands visible in the `/` menu while
# staying comfortably under Telegram's ~4KB payload limit. Users can
# tune this via platforms.telegram.extra.command_menu.max_commands.
_DEFAULT_TELEGRAM_MENU_MAX_COMMANDS = 71
...
def _telegram_command_menu_config() -> dict[str, Any]:
    ...
    # No explicit max_commands in config: when curation is on, derive the
    # cap from the client menu's own length instead of the built-in
    # registry-sized default (spec 12 §4 / plan Task 1b). Curation already
    # trims the visible menu down to CLIENT_MENU_COMMANDS; a cap that
    # doesn't track that list either truncates it silently once the list
    # grows past a stale hand-copied number, or -- if someone "fixes" the
    # mismatch by widening the cap back to the built-in default -- opens
    # slots that skill commands fill instead, alphabetically, with no
    # curation of their own (see hermes_cli/trix_menu.py and
    # assets/config/trix-config.yaml). An explicit max_commands in config
    # remains a respected override -- our own way to debug on a client
    # machine -- so this derivation only applies when the key is absent.
    if "max_commands" in menu_cfg:
        default_max_commands = _DEFAULT_TELEGRAM_MENU_MAX_COMMANDS
    else:
        from hermes_cli.trix_menu import CLIENT_MENU_COMMANDS, menu_curation_enabled
        if menu_curation_enabled(raw_cfg):
            default_max_commands = len(CLIENT_MENU_COMMANDS)
        else:
            default_max_commands = _DEFAULT_TELEGRAM_MENU_MAX_COMMANDS

    max_commands = menu_cfg.get("max_commands", default_max_commands)
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    """
    raw_core_commands = list(telegram_bot_commands())

    # Trix curation: fold all the built-ins (see raw_core_commands above —
    # every telegram_bot_commands() entry) down to the CLIENT_MENU_COMMANDS
    # a non-technical client actually needs. The list is named rather than
    # counted here on purpose: a literal count in this comment went stale the
    # first time the menu changed (it still said "~27" after /disk made it
    # 28), and a stale count is worse than none — it reads as a second,
    # disagreeing source of truth. Gated by
    # ``platforms.telegram.extra.command_menu.curated`` (default True). This
    # is a pure discovery-time filter -- ``telegram_bot_commands()``,
    # ``gateway_help_lines()``, and ``GATEWAY_KNOWN_COMMANDS`` are untouched,
    # so a hidden command still dispatches when a client types it and still
    # appears in ``/help``. See hermes_cli/trix_menu.py.
    from hermes_cli.trix_menu import filter_client_menu, menu_curation_enabled
    try:
        from hermes_cli.config import read_raw_config
        raw_cfg = read_raw_config() or {}
    except Exception:
        raw_cfg = {}
    if menu_curation_enabled(raw_cfg):
        core_commands = filter_client_menu(raw_core_commands)
    else:
        core_commands = _prioritize_telegram_menu_commands(raw_core_commands)
...
#   - setup: reopen the Telegram-first setup wizard (address + one-time
#     password, reply in Russian for a non-technical client) — a
#     Telegram-onboarding surface, not something a Slack operator needs
#     natively; reached via /hermes setup on Slack. Added at the 50-cap —
#     a native slot would clamp /platform (the same slot /pause above was
#     already demoted to protect).
#   - disk: free space on the machine hosting the agent, plus the cleanup
#     of its own service files — a host-maintenance lookup an operator
#     runs occasionally, in the same family as /debug and /update above;
#     reached via /hermes disk on Slack. The registry was already exactly
#     at the 50-cap when it was added, so a native slot would have clamped
#     /platform without this entry.
_SLACK_VIA_HERMES_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "setup", "disk"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to the agent or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
