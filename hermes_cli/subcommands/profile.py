...
def build_profile_parser(subparsers, *, cmd_profile: Callable) -> None:
    ...
    # ---------- Раскладка профиля (contained) ----------
    # Без флагов — разведка. Так задумано: команда меняет место, где профиль
    # ищет свои логины, и «сделать» не может быть поведением по умолчанию.
    profile_migrate_layout = profile_subparsers.add_parser(
        "migrate-layout",
        help="Показать или переключить раскладку профиля (contained)",
        description=(
            "Показывает, что изменится при переходе профиля в contained, и по "
            "явному --apply переключает. Данные не перемещаются: миграция — это "
            "маркер, ветви home/tmp/workspace и честный список входов, которые "
            "профиль перестанет видеть. Без флагов и с --dry-run диск не "
            "трогается; --apply печатает тот же отчёт ПЕРЕД первой записью и "
            "ничего не спрашивает — команда рассчитана на автоматизацию. "
            "--rollback снимает маркер."
        ),
    )
    profile_migrate_layout.add_argument(
        "profile_name",
        nargs="?",
        default=None,
        help="Профиль (по умолчанию — активный корень)",
    )
    profile_migrate_layout.add_argument(
        "--apply", action="store_true", help="Переключить (по умолчанию — только отчёт)"
    )
    profile_migrate_layout.add_argument(
        "--dry-run",
        action="store_true",
        help=(
            "Явная форма поведения по умолчанию: только отчёт. "
            "Вместе с --apply или --rollback — ошибка"
        ),
    )
    profile_migrate_layout.add_argument(
        "--rollback", action="store_true", help="Снять маркер и пустые созданные ветви"
    )
