...
# Раннер склоняет существительное по числу: "1 FLAKY file", но
# "2 FLAKY files" (см. run_tests_parallel.py). Поэтому "s" здесь
# ОБЯЗАТЕЛЬНО необязательная: если требовать только множественное, то
# прогон, где сфлакал ровно ОДИН файл, вообще не найдёт секцию, и
# падения его первой (отменённой повтором) попытки уедут в настоящие.
...
_NO_TESTS_HEADER_RE = re.compile(r"^=== \d+ files? where no tests ran\b")


def parse_uncollected(output: str) -> list[str]:
    """Файлы, которые не отработали ни одним тестом.

    Раннер сводит сюда два разных случая: файл не собрался (импорт не
    удался — обычно нехватка зависимости в venv этой машины) и файл был
    убит файловым таймаутом. Общее у них одно и то же и оно опасное:
    **ни одной строки ``FAILED`` такой файл не даёт**, поэтому сверка с
    базовой линией показывает совпадение, хотя целый файл тестов не
    выполнился.

    Разбор идёт по секции сводки, а не по строкам прогресса: строка
    прогресса у таймаута выглядит как обычный результат файла
    (``✗ tests/x.py (56 tests, 600.1s)``) и от настоящего падения
    неотличима одним шаблоном.
    """
    lines = output.splitlines()
    out: list[str] = []
    inside = False
    for line in lines:
        if _NO_TESTS_HEADER_RE.match(line):
            inside = True
            continue
        if inside:
            stripped = line.strip()
...
def read_baseline(text: str) -> set[str]:
    """Node-id из файла базовой линии: без комментариев и пустых строк."""
    return {
        line.strip()
...
def compare_to_baseline(
    ...
    """Что появилось сверх базовой линии и что из неё теперь проходит.

    Сравнение по МНОЖЕСТВАМ, а не построчной сверкой двух отсортированных
    файлов. Прежняя редакция гейта делала это в bash: ``sort -u`` по
    базовой линии и ``comm`` против списка, отсортированного здесь
    питоновским ``sorted()``. Две разные сортировки — и ``comm``,
    которому обязателен ОДИН порядок на обоих входах, начинает врать.

    Это не теория: релиз 0.1.38 был остановлен гейтом с вердиктом «новые
    падения» на двух тестах, которые в базовой линии ЕСТЬ. Под
    ``LANG=en_US.UTF-8`` системный ``sort`` игнорирует пунктуацию и ставит
    ``test_wecom_callback.py`` ПЕРЕД ``test_wecom.py``, а ``sorted()``
    сравнивает кодовые точки и ставит наоборот (``.`` = 0x2E < ``_`` =
    0x5F). Расхождение зависело от локали машины, на которой режут релиз.

    Опаснее обратная сторона: в том же рассогласованном участке ``comm``
    так же молча СЪЕДАЕТ настоящее новое падение — гейт объявляет «нет
    новых падений» и выпускает непроверенное дерево. Поэтому порядок
    убран из сравнения совсем; сортируется только то, что печатается
    человеку.
    """
    current_set = set(current)
    return sorted(current_set - baseline), sorted(baseline - current_set)
...
def _cli(argv: list[str] | None = None) -> int:
    ...
    ``python3 -m hermes_cli.release_gate --compare-baseline <файл>``
    читает со stdin список настоящих падений (вывод --parse-failures) и
    печатает строки ``new\t<node-id>`` и ``fixed\t<node-id>``. Сверка
    живёт здесь, а не в bash, потому что в bash она делалась ``comm``
    по двум по-разному отсортированным спискам — см. compare_to_baseline.
    """
    import argparse
    ...
    group.add_argument("--parse-uncollected", action="store_true")
    group.add_argument("--compare-baseline", metavar="FILE")
    ...
    if args.parse_uncollected:
        for f in parse_uncollected(sys.stdin.read()):
            print(f)
        return 0
