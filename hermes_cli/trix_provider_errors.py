...
# hermes_cli.auth.resolve_runtime_provider raises AuthError(code=
# "no_provider_configured") with a raw upstream message that names a shell
# command ("Run 'hermes model' ...") and a file path ("~/.hermes/.env") —
# both meaningless to a Telegram-only client with no shell (spec 18 class:
# "имя Hermes не протекает" + spec 1's client-guide promise that the client
# is never told to run a command or edit a file). See
# gateway/run.py's `_client_provider_resolution_error_message` -- the one
# call site (inside `TurnRunner.run_sync`) that renders this -- it detects
# the AuthError via `exc.__cause__` (set by `raise ... from auth_exc` in
# `_resolve_runtime_agent_kwargs`) rather than string-matching the upstream
# text.
_DEFAULT_PROVIDER_NOT_CONFIGURED = (
    "⚙️ The model provider isn't configured yet on this machine. Open the "
    "setup wizard — its address and password arrived by email when the "
    "machine was created — and pick a provider there."
)


def client_provider_not_configured_message() -> str:
    """Клиентский текст, когда провайдер модели вообще не настроен.

    Заменяет собой сырое исключение ``hermes_cli.auth`` ("No inference
    provider configured. Run 'hermes model' ... ~/.hermes/.env.") — у
    клиента нет ни шелла, ни доступа к файлам машины, только мастер
    настройки (спека 18/1).
    """
    return t(
        "trix.errors.provider.not_configured",
        default=_DEFAULT_PROVIDER_NOT_CONFIGURED,
    )
