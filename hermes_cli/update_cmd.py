...
from hermes_cli.release_source import RELEASE_ARCHIVE_URL, RELEASE_BRANCH, is_release_remote
...
def _sync_trix_config_sections(*, quiet: bool = False) -> bool:
    """Splice missing client-template sections into ``config.yaml``.

    Возвращает True, если файл (или ``.env``) действительно изменился —
    нужно вызывающему, который по этому решает, стоит ли перезапускать шлюз
    (см. :func:`_finish_interrupted_update`).

    MUST be called unconditionally on every ``hermes update`` run — NOT
    from inside ``_run_migrate_config_fresh`` / gated behind
    ``needs_migration``. Spec 9 deliberately did not bump
    ``_config_version`` when it added new template sections (display,
    approvals, platform_hints, gateway, terminal.docker_extra_args, ...),
    so ``check_config_version()`` reports the config as up to date and the
    migration branch never runs at all. An earlier revision of this fix
    called this from inside ``_run_migrate_config_fresh``, which meant the
    splice never fired for exactly the already-installed machines it
    exists for — confirmed by running ``check_config_version()`` on a
    previous-version config and observing ``(34, 34)``: no migration, no
    call.

    Uses ONLY the curated ``assets/config/trix-config.yaml`` — never
    upstream's 1700-line kitchen-sink example that
    ``resolve_config_template()``'s fallback would offer for a from-empty
    install. Grafting that fallback's English prose and sandbox-defeating
    keys (``docker_mount_cwd_to_workspace``, ``home_mode``, ...) into a
    live client file would undo the exact sandboxing the curated template
    exists to pin down.

    Best-effort: must never fail ``hermes update``.
    """
    try:
        from hermes_cli.config import get_config_path, get_project_root
        ...
        template = resolve_trix_config_template_only(get_project_root())
        if template is None:
            return False
        changed = False
        added, skipped = sync_missing_client_sections(get_config_path(), template)
        changed = changed or bool(added)
        if added:
            # Полный список — в лог, наружу только счёт и разделы: одна
            # строка из 238 путей не влезает в сообщение Телеграма.
            logger.info("trix_config_sync: дописаны пути: %s", ", ".join(added))
        if added and not quiet:
            print(f"  ✓ Дописаны новые настройки — {added_summary(added)}")
            # Порты — единственная досеянная настройка, которой одного
            # config.yaml мало: докер публикует их только при СОЗДАНИИ
            # контейнера, а подсказка агенту («назови собеседнику публичный
            # адрес») уезжает в промпт уже со следующего сообщения.
            _notice = sandbox_recreate_notice(added)
            if _notice:
                print(f"  ⚠ {_notice}")
        if skipped:
            logger.info(
        ...
        # проходом и обязательно ПОСЛЕ общего досева: тот сравнивает по
        # наличию ключа, а список `telegram` у клиента есть, поэтому нового
        # тулсета внутри него он не увидит никогда.
        entries_added, entries_skipped = sync_toolset_list_entries(get_config_path())
        changed = changed or bool(entries_added)
        if entries_added:
            logger.info(
                "trix_config_sync: дописаны тулсеты: %s", ", ".join(entries_added)
            )
        if entries_added and not quiet:
            print(f"  ✓ Дописаны новые тулсеты — {added_summary(entries_added)}")
        if entries_skipped:
            logger.info(
                "trix_config_sync: пропущены тулсеты (не тронуты): %s",
                ", ".join(f"{p} ({reason})" for p, reason in entries_skipped),
            )

        # Справочная часть .env — тем же проходом. Шаблон .env кладётся только
        # когда файла нет вовсе, поэтому полный список переменных доезжал
        # исключительно до новых установок; найдено живой проверкой машины
        # после 0.1.30. Все дописанные строки — комментарии, поведение
        # измениться не может.
        ...
        env_template = resolve_trix_env_template_only(get_project_root())
        if env_template is not None:
            env_added = sync_missing_env_documentation(get_env_path(), env_template)
            changed = changed or bool(env_added)
            if env_added and not quiet:
                print(
...
def _restart_gateways_via_cli(*, quiet: bool = False) -> bool:
    """Перезапустить шлюзы штатной командой ``hermes gateway restart``.

    Почему подпроцессом, а не переиспользованием кода: вся логика перезапуска
    (systemd user/system со sudo-фолбэком, launchd, профили, SIGUSR1 с
    ожиданием слива, «внешне надзираемые» шлюзы) живёт внутри
    ``_cmd_update_impl`` замыканиями на локальных переменных — вытащить её
    отсюда значило бы переписать 450 строк самого опасного кода в продукте на
    пути, который трогает работающий шлюз клиента. Подпроцесс даёт ровно то же
    поведение, что у команды, которую мы и советуем оператору, и его
    ``sys.exit`` не может уронить обновление.
    """
    import sys as _sys

    argv = [_sys.executable, str(_m().PROJECT_ROOT / "hermes"), "gateway", "restart"]
    try:
        ...
        logger.debug("gateway restart во время доделки не запустился: %s", exc)
        return False
    if result.returncode == 0:
        if not quiet:
            print("  ✓ Шлюз перезапущен на новом коде")
        return True
    if not quiet:
        print("  ⚠ Не удалось перезапустить шлюз автоматически.")
        print("    Сделайте вручную: hermes gateway restart")
        if (result.stderr or "").strip():
            print(f"    {result.stderr.strip().splitlines()[0]}")
    return False


def _finish_interrupted_update(*, quiet: bool = False) -> bool:
    """Доделать хвост обновления, который прошлый запуск не довёл до конца.

    Возвращает True, если доделка вообще понадобилась.

    **Зачем.** ``hermes update`` ставит новый код первым делом, а миграцию
    конфига, досев секций и перезапуск шлюза делает ПОСЛЕ. Процесс, умерший в
    этом хвосте — на машине клиента это обычная закрывшаяся SSH-сессия, —
    оставлял установку с новым кодом, старым конфигом и шлюзом на прежних
    модулях. Повторный ``hermes update`` положение не исправлял: видя свежий
    checkout, он выходил на «Already up to date» раньше хвоста. То есть
    единственное действие, которое оператор предпримет первым, не работало.

    Все шаги хвоста идемпотентны, поэтому доделка — это просто вызов их
    заново; новизна здесь только в том, что есть признак «надо» и повторный
    запуск его видит.
    """
    tail_unfinished = _post_update_tail_unfinished()

    # Досев секций делается ВСЕГДА, а не только по крошке, и вот почему:
    # настройки могут не доехать и без всякого обрыва. Машина на свежем коде,
    # которой досев ни разу не делали (обновлялась старым бинарником; ставили
    # шаблон руками), через `hermes update` не лечилась вообще — штатный досев
    # висит на пути «есть новые коммиты», а здесь коммитов нет.
    #
    # Это дешёвый признак ИЗ СОСТОЯНИЯ: прочитать конфиг и шаблон, сравнить
    # наборы ключей. Когда досевать нечего, функция молчит и ничего не пишет.
    config_changed = _sync_trix_config_sections(quiet=quiet)

    if not (tail_unfinished or config_changed):
        return False

    if not quiet and tail_unfinished:
        print()
        print("→ Прошлое обновление не довело дело до конца — доделываю:")

    if tail_unfinished:
        # Остальные шаги хвоста имеют смысл только после настоящего обрыва:
        # миграция версии схемы, разовые переезды песочницы и прокси.
        from hermes_cli.config import check_config_version

        try:
            current_ver, latest_ver = check_config_version()
        except Exception:
            current_ver, latest_ver = None, None
        if (
            current_ver is not None
            and latest_ver is not None
            and current_ver < latest_ver
        ):
            _run_migrate_config_fresh(interactive=False, quiet=quiet)

        _migrate_sandbox_hermes_base(quiet=quiet)
        _backfill_proxy_env(quiet=quiet)

        # Шлюз держит модули той версии, при которой его запустили, поэтому
        # перезапуск — часть хвоста, а не косметика: без него машина работает
        # на старом коде при новом checkout.
        #
        # Только по крошке: досеянные секции стоят на своих же дефолтах и
        # поведения не меняют, так что из-за них дёргать работающий шлюз
        # (а с ним и живой разговор клиента) нельзя.
        _restart_gateways_via_cli(quiet=quiet)

        _m()._clear_post_update_marker()
        if not quiet:
            print("  ✓ Хвост обновления доделан")
    return True
...
def _update_via_zip(args):
    ...
    # The channel is pinned (see hermes_cli.release_source.RELEASE_BRANCH);
    # _resolve_update_branch() always returns that one branch regardless of
    # args, so there is no other channel left to silently diverge onto here.
    branch = _m()._resolve_update_branch(args)
    zip_url = RELEASE_ARCHIVE_URL
...
def _assert_release_source(git_cmd: list[str], cwd: Path) -> None:
    """Refuse to update from anything but our own release repo.

    Cheap, local-only check that runs before any network access. It does
    not make switching sources impossible — the client is root on their
    own machine and can rewrite the remote — but it removes the last
    unannounced way to drift onto a different source: a mismatch now
    stops the update and says so, instead of silently fetching from
    whatever ``origin`` happens to be.

    Only the remote is checked here, not the current local branch. The
    update flow itself (``_cmd_update_impl``'s checkout step, further down)
    already force-migrates any local branch onto ``RELEASE_BRANCH`` and
    fails loudly ("does not exist locally or on origin") if that migration
    can't complete — that failure IS the branch postcondition. A pre-flight
    branch refusal here bought no security (with origin verified, the
    update only ever fetches and merges ``origin/<RELEASE_BRANCH>``, so a
    local branch name cannot introduce foreign code) while breaking the
    fix-forward path for every checkout made before the channel was pinned
    to ``RELEASE_BRANCH`` — controller ruling, fix round 1.
    """
    origin = _get_origin_url(git_cmd, cwd)
    if not is_release_remote(origin):
        print("✗ Update source not recognized.")
        print(f"  origin: {origin or '(not set)'}")
        print("  This installation only updates from the Trix Agent repository.")
        print("  Contact XDataPlus support.")
        sys.exit(1)
...
def _write_post_update_marker() -> None:
    """Drop the unfinished-tail breadcrumb. Never raises.

    Ставится сразу после успешного pull и снимается только когда хвост
    обновления дошёл до конца, включая перезапуск шлюзов. См.
    ``hermes_cli.main._post_update_marker_path``.
    """
    _write_marker_file(_m()._post_update_marker_path(), label="post-update-incomplete")

def _post_update_tail_unfinished() -> bool:
    """True, если прошлый ``hermes update`` не довёл хвост до конца."""
    try:
        return _m()._post_update_marker_path().exists()
    except Exception:
        return False
...
def _cmd_update_check(branch: str = RELEASE_BRANCH, *, branch_explicit: bool = False):
    """Implement ``hermes update --check``: fetch and report without installing.

    ``branch`` selects which branch the check compares against. Default is
    ``RELEASE_BRANCH`` — the one branch a client checkout can ever be on.
    Naming any other branch here as the default would be a loaded gun: a
    stray direct call or a future edit could silently re-enable comparing
    against a branch we don't ship.
    ...
    # pulls every ref, and this repo has thousands of auto-generated
    # branches, so scope the fetch to <branch>.
    ...
    # Always compare against origin/<branch>. There is no 'upstream' remote
    # on a client install, and even a leftover one from a migrated install
    # must never be consulted here — see _assert_release_source for the
    # enforced origin/branch guard on the apply path; --check has no
    # equivalent guard of its own, so it must not read an 'upstream' remote
    # under any branch value, explicit or default.
...
def _ensure_acp_launcher() -> None:
    ...
    if _m().sys.platform == "win32":
        return
    # OS_RUNTIME: PATH пользователя ОС — скоуп установки, не профиля.
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Trix Agent — ACP launcher (written by `hermes update`).\n"
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # ── Install scope, not profile scope ────────────────────────────────
    # Updating touches the code, the venv and their package caches — shared
    # machine assets owned by the OS user, not data belonging to a profile.
    # `/update` from chat spawns this as a separate process *from the
    # gateway*, so under contained isolation it arrives with
    # HOME=<root>/home. Left alone, every git/uv/npm below would rebuild the
    # pre-seeded ~/.cache/uv and ~/.npm/_cacache (hundreds of MB the install
    # recipe puts there on purpose) inside the profile root — on every
    # machine, on every update. Repair HOME from HERMES_REAL_HOME before the
    # first subprocess. No-op in shared-host.
    try:
        from hermes_constants import restore_install_scope_home

        _restored_home = restore_install_scope_home()
        if _restored_home:
            logger.info(
                "Обновление идёт в скоупе установки: HOME восстановлен в %s",
                _restored_home,
            )
    except Exception as exc:
        logger.debug("Не удалось восстановить HOME скоупа установки: %s", exc)
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("✗ Not a git repository. Please reinstall using your original")
            print("  Trix Agent installer, or contact XDataPlus support.")
            sys.exit(1)
    ...
    # Build git command once — reused for the release-source guard and the
    # update itself.
    git_cmd = ["git"]
    if sys.platform == "win32":
        git_cmd = ["git", "-c", "windows.appendAtomically=false"]

    # Refuse before any network access if origin isn't ours. Skipped when
    # there's no .git at all — the only way to reach here without one is
    # the Windows ZIP fallback (use_zip_update, POSIX already exited above
    # with "Not a git repository..."), and there is no origin to check on
    # a non-git checkout; running the guard there produced a misdiagnosed
    # "origin: (not set)" refusal instead of just letting the ZIP path run.
    if git_dir.exists():
        _assert_release_source(git_cmd, _m().PROJECT_ROOT)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                # Свежий checkout не значит, что ХВОСТ прошлого обновления
                # отработал. Ровно так же, как выше «свежий checkout не значит
                # здоровый venv»: обновление ставит код первым делом, а
                # миграцию конфига, досев секций и перезапуск шлюза делает
                # после — и процесс, умерший в этом хвосте, оставлял машину с
                # новым кодом, старым конфигом и шлюзом на прежних модулях.
                # Раньше этот ранний выход такую машину не лечил вовсе.
                if _finish_interrupted_update(quiet=False):
                    _print_update_completion("✓ Update complete!")
                else:
        ...
        try:
            ...
            # Новый код встал. Всё, что идёт дальше — установка зависимостей,
            # миграция конфига, досев секций, перезапуск шлюзов — это хвост,
            # и именно в нём обрывается обновление на машине клиента
            # (закрывшаяся SSH-сессия). Крошка снимается только в самом конце.
            _write_post_update_marker()
        finally:
            if auto_stash_ref is not None:
        ...
        if not import_ok:
            ...
            print("    Run `hermes update` again — if it persists, contact")
            print("    XDataPlus support.")
        ...
        # assets/api/model-catalog.json, and `git pull` just made it
        # current — so copy it straight over ~/.hermes/cache/model_catalog.json
        # instead of waiting on a network fetch (which can be slow or hit a
        ...
        # Unconditional — runs on EVERY `hermes update`, independent of the
        # version_bump_only / needs_migration branching above. Spec 9 did
        # not bump _config_version, so "Configuration is up to date" above
        # is exactly the branch a machine that's missing the new sections
        ...
        # Хвост дошёл до конца — крошку можно снимать. Исключение: если
        # перезапустились не все шлюзы, часть из них осталась на прежних
        # модулях, то есть обновление НЕ закончено, и повторный запуск должен
        # это увидеть и доделать.
