...
def _pid_alive(pid: Optional[int]) -> bool:
    ...
    **Zombie handling:** the existence check succeeds against zombie
    ...
    Popen handle abandonment — but note ``setsid()`` does NOT reparent
    them: an exited worker stays a child of the dispatcher process
    until ``reap_worker_zombies()`` waitpids it (once per dispatcher
    tick), and during that window we'd otherwise see stale "alive"
