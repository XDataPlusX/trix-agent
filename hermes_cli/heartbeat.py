...
class HeartbeatManager:
    ...
    def status_line(self) -> str:
        s = self._state
        if s is None:
            return t("trix.cmd.heartbeat.status.none")
        every = format_interval(s.interval_seconds)
        fired = ""
        if s.fire_count:
            key = _ru_plural(
                s.fire_count,
                "trix.cmd.heartbeat.status.fired_one",
                "trix.cmd.heartbeat.status.fired_few",
                "trix.cmd.heartbeat.status.fired_many",
            )
            fired = t(key, count=s.fire_count)
        if s.status == "active":
            anchor = s.last_fired_at or s.created_at
            next_in = max(0, int(anchor + s.interval_seconds - time.time()))
            return t(
                "trix.cmd.heartbeat.status.active",
                every=every, next_in=next_in, fired=fired, prompt=s.prompt,
            )
        if s.status == "paused":
            return t(
                "trix.cmd.heartbeat.status.paused",
                every=every, fired=fired, prompt=s.prompt,
            )
        return t(
            "trix.cmd.heartbeat.status.other",
            status=s.status, every=every, fired=fired, prompt=s.prompt,
        )
