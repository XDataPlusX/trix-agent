...
def _fresh_profile_env_defaults() -> dict:
    ...
    ``resolve_env_template``). Пустые заготовки шаблона
    (``OPENROUTER_API_KEY=``) при посеве закомментированы и сюда не попадают
    — ключа в свежем профиле просто нет, и ``admin_has`` ниже и так выходит
    ложным. Но не всё в шаблоне пусто: ``NO_PROXY``/``no_proxy`` несут
    ...
    Тот же ``comment_out_empty_assignments``, что и в посеве: функция обязана
    возвращать то, что реально лежит в свежем профиле, а не то, что лежит в
    шаблоне.
    """
    try:
