...
from hermes_cli.trix_config_pins import set_unless_client_pinned  # noqa: E402
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Trix Agent runs shell commands and code.")
...
def _apply_default_agent_settings(config: dict):
    ...
    #
    # Trix: unless the client's own config.yaml already answers this. Its
    # answer comes with eight lines of Russian comment explaining it, and
    # this write does not touch comments -- flipping the value alone leaves
    # the file contradicting its own documentation. See
    # hermes_cli/trix_config_pins.py.
    reset_applied = set_unless_client_pinned(config, "session_reset", "mode", "none")
    ...
    if reset_applied:
        ...
        print_info(
            f"  Session reset: kept your setting "
            f"({config.get('session_reset', {}).get('mode')})"
        )
...
def _setup_telegram():
    ...
    print_info("Create a bot via @BotFather on Telegram, then paste its token here.")
    print_info("The token never leaves this machine — this setup talks only to")
    print_info("Telegram, once you save the config.")
    print()
    ...
    print()
    print_info("📬 Home Channel: where Trix Agent delivers cron job results,")
...
def _blank_slate_minimize_config(config: dict):
    ...
    # Same client pin as _apply_default_agent_settings above: a Blank Slate
    # install still must not flip a documented value out from under the
    # comment that explains it (hermes_cli/trix_config_pins.py).
    set_unless_client_pinned(config, "session_reset", "mode", "none")
