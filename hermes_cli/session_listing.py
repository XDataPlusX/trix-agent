...
def _format_unnamed_session_label(row: dict[str, Any]) -> str:
    """A readable stand-in for an unnamed session's title: its start date
    plus how far in it already got, so the client can tell rows apart
    instead of seeing an unlabeled placeholder for every one (Task 9f —
    the client now sees unnamed sessions by default, so a bare "—" for
    every row would make the newly-visible half of the list unreadable).
    """
    ...
    if started_at:
        try:
            ...
            date_text = datetime.datetime.fromtimestamp(float(started_at)).strftime(
                "%Y-%m-%d %H:%M"
            )
        except (TypeError, ValueError, OSError):
            date_text = ""
    return date_text or t("trix.cmd.sessions.unnamed_fallback")
...
def format_gateway_session_listing(
    ...
    *,
    ...
    if not rows:
        return t("trix.cmd.sessions.empty")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        title_text = str(raw_title) if raw_title else _format_unnamed_session_label(row)
        preview_part = f" — _{preview}_" if preview else ""
        lines.append(f"{idx}. **{title_text}**{source_part} — `{session_id}`{preview_part}")
    lines.append("")
    lines.append(t("trix.cmd.sessions.footer_resume"))
    lines.append(t("trix.cmd.sessions.footer_more"))
    return "\n".join(lines)
