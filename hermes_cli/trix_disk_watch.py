...
def sweep_profile_tmp(logger) -> None:
    """Убрать временный каталог профиля, если пора, и рассказать об этом в лог.

    Своего расписания не заводим — тот же почасовой тик, что уже ходит за
    местом на диске; сама уборка сверяется с отметкой внутри каталога и
    работает не чаще раза в сутки. В ``shared-host`` это ничего не стоит:
    уборка сразу возвращает ``None``, каталога там нет.

    Всё обёрнуто в ``except``, и это намеренно: падение уборки не имеет
    права снять предупреждение о кончающемся месте — ради него этот тик и
    существует.
    """
    try:
        from hermes_cli.trix_profile_tmp import sweep_profile_tmp_if_due

        try:
            # Readonly: тик ходит каждый час, и досев дефолтов из фонового
            # потока — не его дело.
        ...
        swept = sweep_profile_tmp_if_due(config)
        if swept is None:
            return
        if swept.removed:
            logger.info(
                "Временный каталог профиля убран: удалено записей — %d, "
                "освобождено %.1f МБ",
                swept.removed,
                swept.freed_bytes / (1024 * 1024),
            )
        if swept.skipped_outside:
            logger.warning(
                "Уборка временного каталога пропустила %d путей вне корня: %s",
                len(swept.skipped_outside),
                ", ".join(swept.skipped_outside[:5]),
            )
    except Exception as exc:
        logger.warning("Уборка временного каталога профиля не удалась (%s)", exc)
