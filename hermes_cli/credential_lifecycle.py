...
def _scrub_config_yaml_mirrors(old_value: str, new_value: str | None) -> List[str]:
    ...
    if touched:
        ...
        # Comment-preserving round-trip write (round-2 fix): this path is
        # reachable from the setup wizard's own apply_settings() step 1 —
        # save_provider_env_credential() -> this scrub — so a key rotation
        # during the wizard's own run could otherwise wipe the client
        # template's comments via this second write, even after the
        # wizard's other writers were already fixed to preserve them.
