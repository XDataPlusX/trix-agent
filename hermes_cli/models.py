...
# the labeled entry in assets/api/model-catalog.json. Deliberately a
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    ``assets/api/model-catalog.json`` in the release repo); falls back to
    the in-repo snapshot in ``_PROVIDER_MODELS["nous"]`` when the manifest is
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # External well-known credential file locations. These follow the PROFILE
    # home, not the OS user home: under contained isolation each profile keeps
    # its own provider logins, and a cache key built from another profile's
    # files would go stale — or stay fresh — for the wrong reason.
    from hermes_constants import user_home_path as _user_home_path

    for path in (
        str(_user_home_path(".codex", "auth.json")),
        str(_user_home_path(".claude", ".credentials.json")),
        str(_user_home_path(".config", "github-copilot", "hosts.json")),
        str(_user_home_path(".minimax", "credentials.json")),
    ):
        try:
