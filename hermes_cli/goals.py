...
def _ru_plural(count: int, one: str, few: str, many: str) -> str:
    """Russian numeral agreement (1 -> one, 2-4 -> few, 5+/11-14 -> many)."""
...
class GoalManager:
    ...
    def status_line(self) -> str:
        ...
        if s is None or s.status in {"cleared",}:
            return t("trix.cmd.goal.status.none")
        meta = t("trix.cmd.goal.status.meta_turns", used=s.turns_used, total=s.max_turns)
        if s.subgoals:
            n = len(s.subgoals)
            key = _ru_plural(
                n,
                "trix.cmd.goal.status.subgoal_one",
                "trix.cmd.goal.status.subgoal_few",
                "trix.cmd.goal.status.subgoal_many",
            )
            meta += ", " + t(key, count=n)
        if self.has_contract():
            meta += ", " + t("trix.cmd.goal.status.contract")
        if s.gates:
            n = len(s.gates)
            key = _ru_plural(
                n,
                "trix.cmd.goal.status.gate_one",
                "trix.cmd.goal.status.gate_few",
                "trix.cmd.goal.status.gate_many",
            )
            meta += ", " + t(key, count=n)
        if s.status == "active":
            if s.waiting_on_session and _session_waiting(s.waiting_on_session):
                wr = s.waiting_reason or t(
                    "trix.cmd.goal.status.default_reason_session", session=s.waiting_on_session
                )
                return t("trix.cmd.goal.status.parked_on", reason=wr, meta=meta, goal=s.goal)
            if s.waiting_on_pid and _pid_alive(s.waiting_on_pid):
                wr = s.waiting_reason or t(
                    "trix.cmd.goal.status.default_reason_pid", pid=s.waiting_on_pid
                )
                return t("trix.cmd.goal.status.parked_on", reason=wr, meta=meta, goal=s.goal)
            if s.waiting_until and time.time() < s.waiting_until:
                remaining = int(s.waiting_until - time.time())
                wr = s.waiting_reason or t(
                    "trix.cmd.goal.status.default_reason_timer", remaining=remaining
                )
                return t(
                    "trix.cmd.goal.status.parked_timer",
                    remaining=remaining, reason=wr, meta=meta, goal=s.goal,
                )
            return t("trix.cmd.goal.status.active", meta=meta, goal=s.goal)
        if s.status == "paused":
            extra = t("trix.cmd.goal.status.paused_extra", reason=s.paused_reason) if s.paused_reason else ""
            return t("trix.cmd.goal.status.paused", meta=meta, extra=extra, goal=s.goal)
        if s.status == "done":
            return t("trix.cmd.goal.status.done", meta=meta, goal=s.goal)
        return t("trix.cmd.goal.status.other", status=s.status, meta=meta, goal=s.goal)
