...
def migrate(
    ...
    *,
    ...
    if codex_home is None:
        # Тот же шов, что читает ``codex_models``: раньше чтение шло по
        # ``user_home_path(".codex")``, а запись — по
        # ``Path.home() / ".codex"``. Под containment профиль правил один
        # файл, а читал другой. В shared-host обе формы совпадают.
        from hermes_constants import user_home_path

        codex_home = user_home_path(".codex")
