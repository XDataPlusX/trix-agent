...
_PROFILE_DIRS = [
    ...
    "home",
    # TMPDIR профиля в contained. Создаётся всегда, потому что новый профиль
    # переключается маркером при создании (см. create_profile), а резолвер
    # ветвей каталогов не создаёт — он чистая функция.
    "tmp",
]
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    ...
    "sandboxes",
    "logs",                 # gateway logs
    # Временные файлы профиля в contained. В shared-host это `/tmp` и в
    # экспорт попасть не могло в принципе; под корнем — может, и не должно:
    # содержимое эфемерно по определению, а весить умеет гигабайты.
    "tmp",
    ".layout-migration.json",  # журнал миграции — свойство ЭТОЙ машины
})
...
_DEFAULT_EXPORT_INCLUDE_ROOT = frozenset({
    ...
    # Маркер раскладки: режим изоляции — свойство КОРНЯ, и он обязан уехать
    # вместе с ним. Иначе профиль, экспортированный с одной машины и
    # развёрнутый на другой, молча меняет место хранения логинов.
    ".layout-version",
})
...
def _get_wrapper_dir() -> Path:
    ...
    # OS_RUNTIME: реестр команд пользователя ОС. Принадлежит УСТАНОВКЕ, не
    # профилю, и в contained не переезжает — см. allowlist §6 в
...
def create_profile(
    ...
    # Seed config.yaml from the SAME resolver install.sh / doctor --fix /
    # docker/stage2-hook.sh use. Without this a fresh profile has no
    # config.yaml at all and resolves everything from DEFAULT_CONFIG, where
    # terminal.backend is "local" — the agent runs commands on the host with
    # access to the profile's .env. Skipped when --clone/--clone-all already
    # copied one.
    config_path = profile_dir / "config.yaml"
    if not config_path.exists():
        try:
            ...
            template_path = resolve_config_template(get_project_root())
            if template_path is not None:
                shutil.copy2(str(template_path), str(config_path))
        except Exception:
            # Best-effort — an incomplete profile is worse than one without
            # the template, so profile creation must still succeed. But
            # swallowing this silently reproduces exactly the defect this
            # seeding closes: the profile looks created, yet has no
            # config.yaml and resolves from DEFAULT_CONFIG (terminal.backend
            # "local", .env readable by the agent). Log it so the regression
            # is visible.
            logger.warning(
                "Could not seed config.yaml for profile '%s' from the client "
                "template — profile has no config.yaml and will resolve "
                "settings from DEFAULT_CONFIG (terminal.backend: local) "
                "until one is created.",
                canon,
                exc_info=True,
            )

    # Seed .env from the same resolver, so a fresh profile gets the curated
    # template (three required variables, clearly marked) instead of a bare
    # comment-only stub. Falls back to the stub when neither template exists
    # on disk (matches doctor.py's _seed_missing_env corner case). Without
    # this, profile-scoped env writes (dashboard Channels / Keys pages,
    # `hermes -p <name> auth add`) had no file until first write, and the
    # profile silently inherited API keys from the shell environment — users
    # reasonably read that as "the new profile reads the root .env". Skipped
    # when --clone/--clone-all already copied one.
    #
    # The template's bare ``KEY=`` blanks are commented out on the way in
    # (see ``comment_out_empty_assignments``): under
    # ``gateway.multiplex_profiles`` this file becomes the profile's
    # authoritative secret scope, where an empty value is a configured value,
    # not an unfilled blank.
    #
    # Authorization allowlists are then inherited from the default install
    # (``_inherit_authz_gate_keys``) — and ONLY those. Copying the root's
    # TELEGRAM_BOT_TOKEN into a second profile would collide with it on the
    # very first `hermes gateway` (the runner refuses duplicate credential
    # claims), which is why `backfill_profile_envs`' wholesale copy is right
    # for profiles that were ALREADY running on the root .env and wrong here.
    ...
    if not env_path.exists():
        try:
            ...
            # Best-effort, same reasoning as the config.yaml seed above —
            # save_env_value() will create the file on first write regardless,
            # but a silent failure here would hide that the profile's .env
            # never got the curated template. Log it.
            logger.warning(
                "Could not seed .env for profile '%s' from the client "
                "template — profile has no .env until first credential "
                "write.",
                canon,
                exc_info=True,
            )

    # Новый профиль — contained по построению. Маркер ставится здесь, а не
    # догадкой по форме пути: догадка меняла бы поведение УЖЕ существующих
    # многопрофильных установок, а это первый запрет плана. Существующие
    # корни маркера не получают и остаются shared-host, пока оператор не
    # позовёт `hermes profile migrate-layout --apply`.
    #
    # --clone-all копирует исходный корень целиком, вместе с его маркером
    # (или его отсутствием): клон обязан вести себя как оригинал, иначе
    # «клонировал профиль» тихо меняло бы место хранения логинов.
    if not clone_all:
        try:
            from hermes_constants import PROFILE_LAYOUT_MARKER, write_layout_marker

            marker_path = profile_dir / PROFILE_LAYOUT_MARKER
            if not marker_path.exists():
                # Атомарно, как и в migrate-layout: оборванная запись
                # оставила бы пустой маркер, а пустой маркер — это корень
                # в неопределённом состоянии.
                write_layout_marker(profile_dir)
        except OSError:
            # Best-effort: без маркера профиль просто останется shared-host.
            # Это хуже, чем задумано, но не ломает ничего — в отличие от
            # падения создания профиля на ровном месте.
            logger.warning(
                "Не удалось поставить маркер раскладки профилю '%s' — он "
                "останется shared-host; переключить можно командой "
                "`hermes profile migrate-layout --apply`.",
                canon,
                exc_info=True,
            )
...
# Authorization allowlists, by the shape every one of them shares:
# TELEGRAM_ALLOWED_USERS, DISCORD_ALLOWED_USERS, GATEWAY_ALLOWED_USERS,
# TELEGRAM_GROUP_ALLOWED_USERS, TELEGRAM_GROUP_ALLOWED_CHATS, QQ_GROUP_ALLOWED_USERS…
# (see the platform maps in ``gateway.authz_mixin._is_user_authorized``).
# Matching on the suffix rather than an enumerated list means a newly added
# platform is covered the day its var is introduced. ``*_ALLOW_ALL_USERS``
# does NOT match and is deliberately not inherited: an allowlist names
# specific principals, that flag widens access to everyone, and only one of
# those is safe to hand a new profile without being asked.
_AUTHZ_GATE_ENV_SUFFIXES = ("_ALLOWED_USERS", "_ALLOWED_CHATS")


def _inherit_authz_gate_keys(env_path: Path) -> List[str]:
    """Copy the default install's allowlists into a freshly seeded profile ``.env``.

    Under ``gateway.multiplex_profiles`` a profile's ``.env`` IS its
    authorization boundary: ``build_profile_secret_scope`` reads that file and
    nothing else, and ``_platform_gate_env`` then treats the result as
    authoritative — an allowlist key the profile does not carry reads as "no
    one is allowed", whatever the root install says. A profile created from
    the template alone is therefore born denying every user, and the operator
    finds out only when something stops responding.

    This is a SEPARATE defect from the one RAF-176 was filed for. There, the
    owner's dead ``/update`` buttons were in his MAIN (``default``) chat, whose
    allowlist was filled all along; the empty
    ``profiles/system_admin/.env`` reached them only because the callback path
    was reading another profile's secret scope (fixed in
    ``GatewayRunner._authorize_in_source_profile_scope``). The generator bug
    below is what that investigation walked into on the way: it did not cause
    the owner's symptom, and it would have bitten the first user actually
    served BY a freshly created profile.

    So a new profile starts with the same people allowed as the install it was
    created from. That is not a widening: the values come from the root
    ``.env`` on this same host, written by this same operator, and only
    allowlists are copied — never a token, key, or ``*_ALLOW_ALL_USERS`` flag.
    An operator who wants a narrower profile edits the seeded lines, which are
    at least visible in the file instead of being an absence.

    Keys the seeded file already sets are left alone, and empty values in the
    root ``.env`` are skipped (nothing to inherit). Returns the names copied.
    """
    default_env = _get_default_hermes_home() / ".env"
    if not default_env.is_file() or default_env.resolve() == env_path.resolve():
        return []

    from agent.secret_scope import load_env_file

    parent = load_env_file(default_env)
    existing = load_env_file(env_path)

    inherited = {
        name: value
        for name, value in parent.items()
        if name.endswith(_AUTHZ_GATE_ENV_SUFFIXES)
        and str(value).strip()
    ...
    lines = [
        "",
        "# Списки доступа, унаследованные от установки, из которой создан",
        "# профиль. При мультиплексе профиль авторизует пользователей ТОЛЬКО",
        "# по своему .env, поэтому без этих строк профиль не пускает никого.",
        "# Сузить — отредактируйте строки ниже.",
    ]
    # Round-trip through the canonical quoter: ``load_env_file`` returns the
    # PARSED value (quotes stripped, escapes reversed), so re-emitting it raw
    # would mangle anything the root ``.env`` had quoted for a reason.
    from hermes_cli.config import _quote_env_value

    lines += [
        f"{name}={_quote_env_value(str(value))}"
...
def _remove_profile_containers(profile: str, *, docker_exe: str | None = None) -> int:
    """Снести docker-контейнеры, помеченные этим профилем. Вернуть их число.

    Лучшее-из-возможного: докер может быть не установлен, демон — лежать, а
    бэкенд терминала вообще быть `local`. Ни один из этих случаев не повод
    отменить удаление профиля, поэтому все ошибки глушатся в debug-лог.

    Почему `-f` и почему без фильтра по статусу: в отличие от сборщика сирот
    (`reap_orphan_containers`), который обязан щадить контейнеры соседних
    процессов, здесь профиль удаляют целиком — работающему контейнеру этого
    профиля принадлежать больше нечему.
    """
    try:
        from tools.environments.docker import _sanitize_label_value, find_docker
    except Exception as exc:  # pragma: no cover — docker backend отсутствует
        logger.debug("Уборка контейнеров профиля пропущена: %s", exc)
        return 0
    ...
    label = f"hermes-profile={_sanitize_label_value(profile)}"
    try:
        listing = subprocess.run(
            [docker, "ps", "-a", "--filter", "label=hermes-agent=1",
             "--filter", f"label={label}", "--format", "{{.ID}}"],
            capture_output=True, text=True, encoding="utf-8", errors="replace",
            timeout=20, check=False, stdin=subprocess.DEVNULL,
        )
    except (subprocess.SubprocessError, OSError) as exc:
        logger.debug("Не удалось перечислить контейнеры профиля: %s", exc)
        return 0
    if listing.returncode != 0:
        logger.debug(
            "docker ps вернул %d: %s", listing.returncode, (listing.stderr or "").strip()
        )
        return 0
    ...
    for cid in ids:
        try:
            ...
            logger.debug("Не удалось удалить контейнер %s: %s", cid, exc)
            continue
        if result.returncode == 0:
            removed += 1
        else:
            logger.debug(
                "docker rm %s вернул %d: %s",
                cid, result.returncode, (result.stderr or "").strip(),
            )

    if removed:
        print(f"✓ Удалено контейнеров песочницы: {removed}")
    return removed
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # 3b. Снести контейнеры песочницы этого профиля.
    #
    # Зачем это здесь. Долгоживущая песочница (`container_persistent: true`)
    # держит writable-слой в `/var/lib/docker` на хосте — ВНЕ корня профиля.
    # Туда попадает всё, что агент поставил в систему контейнера: `apt
    # install`, `pip install`, файлы в `/opt`. Это след работы профиля,
    # переживающий `rmtree` корня. Без этой уборки обещание «удаление корня
    # не оставляет данных профиля» было бы просто неправдой.
    #
    # Удаляем по метке `hermes-profile`, которую docker-бэкенд ставит при
    # создании контейнера. В отличие от штатного сборщика сирот, здесь
    # сносим и РАБОТАЮЩИЕ контейнеры (`-f`): профиль удаляют, его
    # контейнеру принадлежать больше нечему.
    _remove_profile_containers(canon)
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        if _platform.system() == "Linux":
            ...
            # OS_RUNTIME: юнит службы — интерфейс ОС к процессу. Изоляция
            # профилей здесь достигается ИМЕНЕМ юнита, а не переносом пути.
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    Credential files (``auth.json``, ``.env``) are excluded, the contained
    private home (``home/``) and the ephemeral ``tmp/`` branch are dropped
    whole, and secret-shaped strings in the remaining staged text files are
    force-redacted before the archive is written. Returns the output file
    path.
    """
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Корневые исключения: до contained временные файлы профиля жили в
        # `/tmp` и в архив попасть не могли. Теперь `tmp/` лежит под корнем,
        # и без явного исключения экспорт утащил бы с собой распаковки
        # браузера — эфемерный мусор размером с сам профиль. Журнал миграции
        # описывает ЭТУ машину и на другой смысла не имеет.
        #
        # `home` — та же логика, но с ценой на порядок выше: в contained это
        # ДОМ подпроцессов, куда внешние CLI кладут готовые к использованию
        # логины (`.claude/.credentials.json`, `.config/github-copilot/
        # hosts.json`, `.qwen/oauth_creds.json`, `.modal.toml`, …). До
        # containment они жили в доме ОС и попасть в архив профиля не могли
        # в принципе. Исключение ПО ИМЕНИ ВЕТВИ, а не опора на
        # `_scrub_export_secrets`: редактор регулярками — фильтр по форме
        # строки, и `oauth_token` из hosts.json проходил его насквозь. Здесь
        # нужна граница, а не эвристика.
        _ROOT_EXCLUDE = {"home", "tmp", ".layout-migration.json"}

        def _ignore(directory: str, contents: list) -> set:
            ignored = _CREDENTIAL_FILES & set(contents)
            if Path(directory) == profile_dir:
                ignored = ignored | (_ROOT_EXCLUDE & set(contents))
            return ignored
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        ...
        user_home_path(".honcho", "config.json"),
    ]
