...
def _parse_dotenv_lines(content: str) -> dict:
    """Minimal, dependency-free ``.env`` line parser: ``KEY=VALUE`` per
    line, blank/comment lines skipped, matching surrounding quotes
    stripped, ``export `` prefix tolerated.

    Used as ``_has_provider_env_config``'s fallback when the real
    ``python-dotenv`` package isn't usable (see there for why). Doesn't
    need multiline values or variable expansion for that diagnostic
    check -- it only has to tell "real assignment" apart from "comment"
    or "name never assigned", same as ``hermes_cli.env_loader.
    _env_keys_defined_in_dotenv`` does for key names alone.
    """
    values: dict = {}
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.hermes/.env contains a USABLE provider auth/base
    URL setting.

    Parses the content as real ``KEY=VALUE`` assignments and requires
    ``has_usable_secret()`` on the value, rather than a substring search
    for the variable NAME anywhere in the raw text. A naive substring
    search treats ``OPENROUTER_API_KEY=`` (present, empty -- exactly what
    Trix's own curated .env template ships before the customer fills it
    in) as "configured", and also true-positives on upstream's
    ``.env.example`` where every key is commented out
    (``# OPENROUTER_API_KEY=``) -- the substring still matches inside the
    comment. Both false positives make ``doctor`` report a green
    checkmark on a machine that cannot actually authenticate.

    Prefers ``python-dotenv``'s own ``dotenv_values()`` when it's
    importable, falling back to ``_parse_dotenv_lines()`` otherwise: several
    test modules replace ``sys.modules["dotenv"]`` with a bare stub that
    only exposes ``load_dotenv`` (to keep .env auto-loading a no-op during
    collection), which breaks ``from dotenv import dotenv_values`` for
    every other caller sharing that process -- not just this one. The
    fallback keeps this function correct in that situation instead of
    crashing `doctor` entirely.
    """
    ...
    try:
        ...
        values = _parse_dotenv_lines(content)

    from hermes_cli.trix_provider_env_names import provider_env_names

    return any(
        has_usable_secret(values.get(key))
        for key in provider_env_names(_PROVIDER_ENV_HINTS)
    )
...
def _report_telegram_link_health() -> None:
    """Show how often the Telegram link dropped, if it dropped at all.

    Retries and pool drains are survived silently by the adapter, so the
    only trace they leave is this counter (see
    ``plugins/platforms/telegram/link_health.py``). Support cannot ask the
    client to read a log, and telling them "the link is down" *through* the
    link is impossible — so the number has to be readable from here.

    Stays quiet when nothing was recorded: a healthy machine should not
    grow a line that only ever says "zero".
    """
    try:
    ...
    _section("Telegram Link")
    detail = ", ".join(
        f"{kind}: {count}" for kind, count in sorted(day["by_kind"].items())
    )
    if day["total"]:
        check_warn(
            f"Link dropped {day['total']}x in the last 24h",
            f"({detail})" if detail else "",
        )
    else:
        check_ok("No link drops in the last 24h")
    check_info(
        f"{week['total']} in the last 7 days — "
        f"{link_health.state_path()}"
    )
...
def _seed_missing_config_yaml(config_path: Path, project_root: Path) -> str:
    """Create a missing ``config.yaml`` from the best available template.

    Used by ``doctor --fix`` when ``~/.hermes/config.yaml`` is absent.
    Goes through the same resolver ``scripts/install.sh`` uses
    (:func:`hermes_cli.config_template.resolve_config_template`) so a
    machine repaired via ``doctor --fix`` gets Trix's curated template
    (sandboxed ``terminal.backend: docker``, etc.) instead of silently
    reverting to upstream's ``cli-config.yaml.example`` (``terminal.backend:
    local`` — the agent running on the host with access to ``.env``, which
    Ruling 1 of the standard-build spec rules out).

    Falls back to writing ``DEFAULT_CONFIG`` verbatim only when neither
    template exists on disk at all.

    Returns a short human-readable description of the source used, for the
    caller's ``check_ok()`` message.
    """
    from hermes_cli.config_template import resolve_config_template

    config_path.parent.mkdir(parents=True, exist_ok=True)
    template_path = resolve_config_template(project_root)
...
def _seed_missing_env(env_path: Path, project_root: Path) -> str:
    """Create a missing ``.env`` from the best available template.

    Used by ``doctor --fix`` when ``~/.hermes/.env`` is absent. Goes through
    the same resolver ``scripts/install.sh`` uses
    (:func:`hermes_cli.config_template.resolve_env_template`) so a machine
    repaired via ``doctor --fix`` gets Trix's curated, ~45-line template
    (three required variables, clearly marked) instead of a bare empty
    file with no guidance on what to fill in — the same class of gap
    :func:`_seed_missing_config_yaml` closed for ``config.yaml``.

    Falls back to an empty file only when neither template exists on disk
    at all (matches the pre-existing behavior for that corner case).
    ...
    template_path = resolve_env_template(project_root)
    if template_path is not None:
        shutil.copy2(str(template_path), str(env_path))
        return template_path.name
...
def _sync_trix_config_sections(config_path: Path, project_root: Path) -> int:
    """Досеять недостающие секции клиентского шаблона в существующий config.yaml.

    Второй канал доставки (наряду с ``hermes update``) для машин, которые
    давно не обновлялись, но которым саппорт советует ``doctor --fix`` первым
    делом. ``install.sh`` / ``doctor --fix`` кладут
    ``assets/config/trix-config.yaml`` только когда ``config.yaml``
    ОТСУТСТВУЕТ — уже существующий файл никто и никогда не трогает, поэтому
    всё, что спека 9 добавила в шаблон (display, approvals, platform_hints,
    gateway, terminal.docker_extra_args, ...) без этого досева не доезжает ни
    до одной уже установленной машины.

    Отдельная функция, а не блок внутри ``run_doctor()``: ветка
    «предупредить про порты» иначе достижима только прогоном всего доктора и
    поэтому не была покрыта ничем (её удаление оставляло сьют зелёным).
    Зеркалит ``hermes_cli/update_cmd.py::_sync_trix_config_sections`` —
    расхождение формулировок между двумя каналами доставки было бы
    расхождением в том, что клиенту сказали про одно и то же состояние.

    Best-effort: не имеет права уронить ``doctor``. Возвращает число правок
    для счётчика ``fixed_count``.
    """
    try:
        # Только наш курированный шаблон — НЕ resolve_config_template()
        # (его фолбэк — upstream-пример на 1700+ строк, годный для
        # копирования в пустое место при установке, но не для прививки
        # английского текста и sandbox-defeating ключей вроде
        # docker_mount_cwd_to_workspace/home_mode в живой клиентский файл).
        from hermes_cli.config_template import resolve_trix_config_template_only
        from hermes_cli.trix_config_sync import (
            added_summary,
            sandbox_recreate_notice,
            sync_missing_client_sections,
            sync_toolset_list_entries,
        )

        template_path = resolve_trix_config_template_only(project_root)
        if template_path is None:
            return 0
        added, skipped = sync_missing_client_sections(config_path, template_path)
        fixed = 0
        if added:
            logger.info("trix_config_sync: дописаны пути: %s", ", ".join(added))
            check_ok(f"Дописаны недостающие настройки — {added_summary(added)}")
            fixed = 1
            # Порты доезжают конфигом, но не контейнером: докер публикует их
            # только при СОЗДАНИИ песочницы, а подсказка агенту про публичный
            # адрес действует уже со следующего сообщения. Один текст на оба
            # канала доставки — см. hermes_cli/trix_config_sync.py.
            notice = sandbox_recreate_notice(added)
            if notice:
                check_warn(notice)
        if skipped:
            logger.info(
                "trix_config_sync: пропущены пути (не тронуты): %s",
                ", ".join(f"{_p} ({_reason})" for _p, _reason in skipped),
            )

        # Тулсеты внутри УЖЕ существующего platform_toolsets — отдельным
        # проходом и обязательно ПОСЛЕ общего досева. Общий досев сравнивает
        # по наличию ключа: список `telegram` у клиента есть, значит новый
        # тулсет внутри него он не увидит никогда. Порядок важен: если блок
        # platform_toolsets отсутствовал целиком, его только что довёз общий
        # досев — уже с полным содержимым шаблона, и этому проходу останется
        # нечего делать.
        entries_added, entries_skipped = sync_toolset_list_entries(config_path)
        if entries_added:
            logger.info(
                "trix_config_sync: дописаны тулсеты: %s", ", ".join(entries_added)
            )
            check_ok(f"Дописаны недостающие тулсеты — {added_summary(entries_added)}")
            fixed = 1
        if entries_skipped:
            logger.info(
                "trix_config_sync: пропущены тулсеты (не тронуты): %s",
                ", ".join(f"{_p} ({_reason})" for _p, _reason in entries_skipped),
            )

        # Смена НАШЕГО умолчания — вторым проходом, ПОСЛЕ досева.
        #
        # Порядок обязателен: только что дописанный досевом ключ приходит со
        # значением шаблона, и этот проход обязан увидеть его совпадающим,
        # чтобы записать «отгружено нами». Пойди он первым — ключа бы ещё не
        # было, и первая же смена умолчания после этого объявила бы его
        # клиентским навсегда.
        #
        # Досев отвечает на «ключа нет», этот проход — на «ключ есть, но
        # умолчание с тех пор изменилось». Правило владельца 2026-09-10:
        # изменение клиента превыше всего, но если он ничего не менял, мы
        # меняем за него. Разбор — hermes_cli/trix_config_defaults.py.
        from hermes_cli.trix_config_defaults import (
            apply_default_changes,
            updated_summary,
        )

        migrated, migrate_skipped = apply_default_changes(config_path, template_path)
        if migrated:
            logger.info(
                "trix_config_defaults: приведены к новым умолчаниям: %s",
                ", ".join(migrated),
            )
            check_ok(updated_summary(migrated))
            fixed = 1
        if migrate_skipped:
            logger.info(
                "trix_config_defaults: пропущены пути (не тронуты): %s",
                ", ".join(f"{_p} ({_reason})" for _p, _reason in migrate_skipped),
            )

        # Правки ПОЯСНЕНИЙ идут тем же проходом, что и секции.
        #
        # Штатно они доезжают шагом миграции (`_migrate_to_35`), то есть
        # через `hermes update`. Но досев секций живёт и здесь, а миграций
        # здесь нет — значит машина, на которой обновили каталог установки и
        # позвали только `doctor --fix`, получила бы полный конфиг со СТАРОЙ
        # шапкой: «здесь только то, что отличается от значений по
        # умолчанию» — над файлом, в который этот же вызов дописал все
        # секции. Замена дословная и идемпотентная, поэтому лишний прогон
        # ничего не стоит: после первого старого текста в файле уже нет.
        from hermes_cli.trix_config_comments import sync_client_comments

        # Справочная часть .env — см. тот же блок в update_cmd.
        ...
        env_template = resolve_trix_env_template_only(project_root)
        if env_template is not None:
            env_added = sync_missing_env_documentation(get_env_path(), env_template)
            if env_added:
                check_ok(
                    f"В .env дописан полный список переменных "
                    f"({env_added} строк, все закомментированы)"
                )
                fixed = 1

        rewritten, unmatched = sync_client_comments(config_path)
        if unmatched:
            logger.info(
                "trix_config_comments: старый текст не найден дословно (%d): %s",
                len(unmatched),
                ", ".join(unmatched),
            )
        if rewritten:
            check_ok(f"Обновлены пояснения в config.yaml: {len(rewritten)}")
            fixed = 1

        return fixed
    except Exception:
        logger.warning(
            "Не удалось досеять недостающие секции config.yaml из "
            "клиентского шаблона",
            exc_info=True,
        )
        return 0
...
def _check_soul_md(hermes_home: Path, should_fix: bool) -> int:
    """Report on ~/.hermes/SOUL.md and, with --fix, seed it if missing.

    Extracted from run_doctor() so it's independently testable: the seeded
    content MUST match DEFAULT_SOUL_MD in hermes_cli/default_soul.py, the same
    text every other seeding path (first-run, install.sh, install.ps1,
    docker/SOUL.md, `hermes profile create`) writes. This path used to carry
    its own hardcoded upstream-branded scaffold that is_legacy_template_soul()
    didn't recognize, which made the self-heal in
    hermes_cli.config._ensure_default_soul_md treat the file as
    user-customized and never upgrade it -- one `doctor --fix` run silently
    reverted the product's identity forever. Importing the shared constant
    instead of duplicating it means this can't drift again.

    Returns 1 if a fix was applied, 0 otherwise (for the caller's
    fixed_count accumulator).
    """
    ...
    if soul_path.exists():
        ...
        return 0
    check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Trix Agent a custom personality)")
    if not should_fix:
        return 0
    soul_path.parent.mkdir(parents=True, exist_ok=True)
    soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
    check_ok(f"Created {_DHH}/SOUL.md with the default persona")
    return 1
...
def run_doctor(args):
    ...
    if env_path.exists():
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                env_source = _seed_missing_env(env_path, PROJECT_ROOT)
                # .env holds API keys — restrict to owner-only access from
                # creation. touch()/copy2() obey umask which is commonly
                # 0o022, leaving the file world-readable; tighten explicitly.
                try:
                    os.chmod(str(env_path), 0o600)
                except OSError:
                    pass
                check_ok(f"Created {_DHH}/.env from {env_source}")
    ...
    if config_path.exists():
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                check_ok(f"Created {_DHH}/config.yaml from {source}")
                fixed_count += 1
            else:
    ...
    if config_path.exists():
        ...
        # Досеять недостающие секции клиентского шаблона (спека 9, задача 11).
        if should_fix:
            fixed_count += _sync_trix_config_sections(config_path, PROJECT_ROOT)

        # Discoverability: this sidecar is the ONLY way to un-do the "already
        # seeded, so it's never re-added" protection above — if a client
        # deliberately removed a setting and support/a future maintainer
        # can't find why doctor/update won't restore it, deleting this file
        # is the answer. Surfaced unconditionally (not just under --fix) so
        # a plain `hermes doctor` run reveals it too. Gated on the sidecar
        # actually EXISTING — doctor must not claim a fact it never checked;
        # on a machine where trix_config_sync has never run (or added
        # nothing yet), there is nothing to un-do and the file isn't there.
        from hermes_cli.trix_config_sync import SEEDED_STATE_FILENAME
        _sync_state_path = HERMES_HOME / SEEDED_STATE_FILENAME
        if _sync_state_path.exists():
            check_info(
                f"Отметка о ранее дописанных настройках шаблона хранится в "
                f"{_DHH}/{SEEDED_STATE_FILENAME} — удаление этого файла "
                f"вернёт досев к чистому листу (настройки, которые вы однажды "
                f"удалили, снова смогут быть дописаны)."
            )
    ...
    fixed_count += _check_soul_md(hermes_home, should_fix)
    ...
    if check_numpy_runnable(issues, should_fix=should_fix):
        fixed_count += 1
    from hermes_cli.trix_layout_check import check_layout_containment
    fixed_count += check_layout_containment(issues, should_fix=should_fix)
    ...
    if sys.platform != "win32":
        ...
        # Determine the expected command link directory. This must mirror
        # install.sh's get_command_link_dir() — including its FHS branch,
        # which this code used to omit: a root install on Linux puts the
        # launcher in /usr/local/bin, and the client VM recipe deliberately
        # deletes ~/.local/bin/hermes as a leftover of the user-install
        # layout. Asking for ~/.local/bin there reported a permanent
        # "missing symlink" issue on a machine where the command works,
        # and offered a --fix that would recreate what the recipe removed.
        # The mapping lives in hermes_cli/command_link.py so the two ends
        # can't drift again.
        from hermes_cli.command_link import expected_command_link_dir

        _prefix = os.environ.get("PREFIX", "")
        _is_termux_env = bool(os.environ.get("TERMUX_VERSION")) or "com.termux/files/usr" in _prefix
        _cmd_link_dir, _cmd_link_display = expected_command_link_dir(
            PROJECT_ROOT,
            home=Path.home(),
            termux_prefix=_prefix if _is_termux_env else None,
        )
        ...
        if _venv_bin is None:
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        # The FHS layout puts this link in /usr/local/bin,
                        # which the agent's own user cannot write. A repair
                        # that can't be performed must say so, not raise out
                        # of the middle of `doctor --fix`.
                        try:
                            _cmd_link.unlink()
                            _cmd_link.symlink_to(_venv_bin)
                        except OSError as exc:
                            check_warn(
                                f"Could not repair {_cmd_link_display}/hermes",
                                f"({exc}) — needs the owner of {_cmd_link_dir}",
                            )
                            manual_issues.append(
                                f"Re-link {_cmd_link_display}/hermes → {_venv_bin} as the owner of {_cmd_link_dir}"
                            )
                        else:
                ...
                if should_fix:
                    # See the same guard above: /usr/local/bin is root's.
                    try:
                        _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                        _cmd_link.symlink_to(_venv_bin)
                    except OSError as exc:
                        check_warn(
                            f"Could not create {_cmd_link_display}/hermes",
                            f"({exc}) — needs the owner of {_cmd_link_dir}",
                        )
                        manual_issues.append(
                            f"Link {_cmd_link_display}/hermes → {_venv_bin} as the owner of {_cmd_link_dir}"
                        )
                    else:
