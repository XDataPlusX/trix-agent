...
def active_session_limit_message(
    ...
    detail = (
        t("trix.session_limit.held_by", held=held, default=f" Held by: {held}.")
        if held
        else ""
    )
    return t(
        "trix.session_limit.message",
        active_count=active_count,
        max_sessions=max_sessions,
        detail=detail,
        default=(
            f"The agent is at the active session limit ({active_count}/{max_sessions})."
