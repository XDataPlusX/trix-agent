...
# The exact PATH-guard comment text scripts/install.sh, hermes_cli/update_cmd.py,
# and setup-hermes.sh write into a customer's shell rc -- in both brandings,
# since an install from before the Trix rebrand wrote the "Hermes Agent"
# form and one from after it writes "Trix Agent", and a single machine can
# carry either (or, after an upgrade, both). Keep this in sync with those
# three writers if the wording ever changes again.
_MANAGED_PATH_COMMENTS = frozenset({
...
# The exact `export PATH=...` line the installer writes immediately after one
# of the comments above. Only ever removed when it directly follows a
# recognized comment (see remove_path_from_shell_configs) -- never removed on
# its own, so a customer's own line that happens to read identically but
# wasn't written as part of a managed block is left alone.
_MANAGED_PATH_LINES = frozenset({
...
def remove_path_from_shell_configs():
    """Remove the PATH-guard comment+export pair the installer wrote.

    Anchored on the exact managed markers above, not a substring heuristic.
    An earlier version matched any line containing 'hermes' (case-
    insensitive) together with 'PATH=' or '#' — which meant a customer's own
    `export MY_HERMES_PATH=...`, `export MYPATH=/opt/hermes-tools/bin`, or
    even a comment like `# Trix Agent notes: renew the license` got deleted
    from their shell rc during uninstall, along with collapsing every blank-
    line run in the file down to one blank line. This deletes only the exact
    two-line pair this codebase itself writes, and touches nothing else in
    the file -- including blank lines, which are left exactly as found.
    """
    ...
    for config_path in configs:
        try:
            ...
            while i < n:
                line = lines[i]
                if (
                    line in _MANAGED_PATH_COMMENTS
                    and i + 1 < n
                    and lines[i + 1] in _MANAGED_PATH_LINES
                ):
                    # The pair this codebase wrote together — drop both,
                    # keep everything else, including surrounding blank
                    # lines, untouched.
                    i += 2
                    continue
...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        # OS_RUNTIME: реестр команд пользователя ОС. Принадлежит УСТАНОВКЕ,
        # не профилю, поэтому в contained не переезжает.
...
def _perform_uninstall(
    *,
    ...
    if not full_uninstall:
        ...
        print(f"  {hermes_home}/")
        print()
        print("To reinstall later with your existing settings, re-run the installer")
        print("you originally received from XDataPlus.")
        print()
