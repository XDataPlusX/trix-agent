...
The one sanctioned import below is ``hermes_cli.release_source`` -- a
dependency-free leaf module in its own right (no imports of its own), kept
separate from ``hermes_cli.config`` precisely so data files like this one
can derive the release repo's address without risking a cycle.
"""
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # Example: ["/home/user/projects:/workspace/projects"]
        # Not needed for gateway MEDIA delivery: a persistent container already
        # binds /workspace and /root to host directories, and the gateway
        # translates those (plus the auto-mounted cache dirs and anything listed
        # here) back to host paths before uploading — see
        # gateway/platforms/base.py::_translate_docker_container_media_path.
        # So the agent emits the path it sees; do NOT invent an /output mount.
        "docker_volumes": [],
    ...
    "display": {
        ...
        # handful of gateway slash-command replies, and -- via the
        # registry-backed commands.* catalog namespace -- the slash command
        # descriptions shown in messaging clients).  Does NOT affect agent
        # responses, log lines, tool outputs, or error tracebacks.
        # Supported languages: see agent.i18n.SUPPORTED_LANGUAGES (the single
        # authoritative list -- do not re-enumerate them here, it will drift).
    ...
    "stt": {
        ...
        # language overrides it. Defaults to the product's UI language ("ru") —
        # Whisper auto-detection frequently misidentifies short/accented clips,
        # which reads as "STT transcribed the wrong language". Set to "" to
        # restore auto-detect, or to your language code ("en", "es", ...).
        "language": "ru",
    ...
    "context": {
        ...
        "memory_trim": {
            ...
            # 0 reports every successful configured trim.  The default of 1 MB
            # keeps steady-state housekeeping trims (~0.5-0.8 MB per minute on
            # an idle gateway) silent while meaningful releases (post-
            # compression, post-delegation, startup) still log once.
            "info_log_min_delta_mb": 1.0,
        },
    },
    ...
    "telegram": {
        ...
        # HTTP client tuning: how patiently the bot waits on a flaky link.
        # Seconds, except pool_size (number of connections). These are the
        # numbers the adapter uses when the section is absent — the section
        # exists so a user without a shell can change them. See
        # plugins/platforms/telegram/telegram_network.py
        # (resolve_http_request_kwargs) for the resolution order; the
        # HERMES_TELEGRAM_HTTP_* env vars remain an emergency override and
        # lose to anything written here.
        "network": {
            "pool_size": 512,           # Max concurrent connections to the Bot API
            "pool_timeout": 8,          # Wait for a free connection from the pool
            "connect_timeout": 10,      # Wait for the TCP connection
            "read_timeout": 20,         # Wait for Telegram's answer
            "write_timeout": 20,        # Wait for a non-file request to go out
            "media_write_timeout": 60,  # Per-socket-write budget for uploads (not a duplicate of write_timeout)
        },
        "extra": {
    ...
    # The default URL is the raw-content address of the release repo's
    # ``assets/api/model-catalog.json`` (see hermes_cli/release_source.py) --
    # freshness is tied to the release cadence rather than a separate docs
    # site, which this fork doesn't publish.
    "model_catalog": {
        "enabled": True,
        "url": f"{RELEASE_RAW_BASE_URL}/assets/api/model-catalog.json",
    ...
    "gateway": {
        ...
        # How many hours the periodic housekeeping thread keeps files in the
        # per-kind media caches (gateway/platforms/base.py cleanup_*_cache
        # helpers) before deleting them. ``default`` applies to any cache
        # kind without its own entry; a per-kind key (``images``,
        # ``documents``, ``audio``, ``videos``, ``screenshots``) overrides
        # it. ``0`` means "never delete". Read by
        # ``gateway.run.resolve_media_retention_hours`` on every housekeeping
        # pass, so a change here takes effect on the next hourly tick without
        # a gateway restart. These defaults (24h for every kind) match the
        # historical hardcoded behavior — adding this section does not change
        # upstream retention by itself.
    ...
    "session_search": {
        # Whether the `session_search` tool may open ANOTHER profile's
        # state.db (via the `profile` arg or a bare session_id fallback
        # scan). Each profile is meant to be a fully isolated instance —
        # separate memory, separate history — so a messaging-gateway agent
        # running as profile A must never be able to read profile B's
        # conversations just because it was asked to. Default false: this
        # closes that cross-tenant leak for every host-side deployment.
        # Legitimate use is narrow (the desktop app resolving an
        # `@session:<profile>/<id>` link the user dragged into the chat) —
        # enable it there, per profile, not fleet-wide.
    ...
    # Место на диске клиентской машины: когда предупредить и что считать
    # уборкой, которая стоит упоминания. Проверку ведёт почасовая уборка
    # шлюза (hermes_cli/trix_disk_watch.py::disk_watch_tick); те же пороги
    # красят отчёт команды /disk.
    "disk": {
        # Занято столько процентов раздела — сказать клиенту один раз.
        # Повторно — только если диск разгрузили ниже порога и он снова
        # поднялся, или если перейдён срочный порог.
        "warn_percent": 80,
        # Второй, срочный порог: говорится ещё раз даже после мягкого.
        # Ниже warn_percent не опускается (см. disk_thresholds).
        "urgent_percent": 90,
        # Гистерезис: насколько ниже порога надо упасть, чтобы то же самое
        # предупреждение снова имело право прозвучать. Одна десятая
        # процентного пункта — это двадцать мегабайт, один присланный
        # документ: без запаса обычная работа агента качала бы занятость
        # через порог весь день, и клиент перестал бы читать жёлтые
        # сообщения раньше, чем придёт красное.
        "rearm_percent": 3,
        # Потолок на повтор ОДНОГО И ТОГО ЖЕ предупреждения, даже если
        # занятость честно ходит через порог с большим размахом. Он же —
        # пауза перед повтором недоставленной месячной сводки (она не
        # срочная, а её текст стоит полного обхода дерева).
        "repeat_after_hours": 24,
        # Ниже этого объёма уборку не предлагаем: обещание «освободить
        # 0 МБ» клиент проверит один раз, ничего не увидит и перестанет
        # верить отчёту.
        "min_cleanup_mb": 100,
    },

    # Изоляция профиля. Корень верхнего уровня: изоляция шире терминала —
    # она про все данные профиля сразу.
    #
    # Здесь объявлены ТОЛЬКО ручки уборки временного каталога. В режиме
    # ``contained`` этот каталог лежит внутри корня профиля, и ОС его не
    # чистит — ни systemd-tmpfiles, ни перезагрузка; убираем сами.
    # Значения обязаны совпадать с ``DEFAULT_MAX_AGE_DAYS`` /
    # ``DEFAULT_MAX_SIZE_MB`` в ``hermes_cli/trix_profile_tmp.py``:
    # расхождение дало бы два разных ответа на вопрос «сколько по умолчанию».
    #
    # ``isolation.mode`` здесь СОЗНАТЕЛЬНО отсутствует. Досев клиентского
    # конфига безусловен — любой ключ из этой карты дописывается в config.yaml
    # на каждом ``hermes update``, — так что объявить здесь режим значило бы
    # разом переключить все работающие машины. Режим приходит маркером
    # ``.layout-version`` в корне профиля либо переменной ``HERMES_ISOLATION``
    # (см. ``hermes_constants.bridge_isolation_mode_from_config``), а конфиг
    # его лишь читает, если владелец вписал ключ руками.
    "isolation": {
        # Удалять записи старше стольких дней. 0 — выключить уборку по
        # возрасту (аварийный выключатель без отката кода).
        "tmp_max_age_days": 7,
        # Потолок суммарного размера в МБ: сверх него удаляются самые старые
        # записи. 0 — без потолка.
        "tmp_max_size_mb": 2048,
    },
