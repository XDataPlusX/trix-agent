...
_LEGACY_TEMPLATE_SOULS = (
    ...
    # hermes doctor --fix (pre-fix) wrote this short, non-comment template
    # when SOUL.md was missing -- unlike the two scaffolds above, this one
    # DOES look like content (no HTML comment wrapper), but "You are Hermes,
    # a helpful AI assistant." is doctor's own generic placeholder, not
    # anything a user typed. Recognizing it lets already-affected installs
    # (doctor ran before this fix shipped) self-heal on next run instead of
    # being permanently stuck on upstream identity.
    (
        "# Hermes Agent Persona\n"
        "\n"
    ...
    # The Trix persona as it shipped before the Russian-by-default sentence
    # was added. An install seeded from that build carries this text verbatim,
    # and without an entry here it would never pick up the language
    # instruction: the file isn't a legacy scaffold, so _ensure_default_soul_md
    # leaves it alone forever and the agent keeps answering in whatever
    # language the first message happens to arrive in. Byte-exact and
    # machine-written, so it carries no user intent -- a customer who edited
    # even one character no longer matches and is left untouched.
    (
        "You are Trix Agent, an intelligent AI assistant provided by XDataPlus. "
        "You are helpful, knowledgeable, and direct. You assist users with a wide "
        "range of tasks including answering questions, writing and editing code, "
