...
def _translated_title(blueprint) -> str:
    """The catalog title in the active language.

    The English original stays a plain string literal on ``AutomationBlueprint``
    in ``cron/blueprint_catalog.py`` (an upstream file we don't edit for
    wording) so merges never conflict; the translation lives here as a
    ``trix.blueprint.<key>.title`` key with the English literal as the
    ``t()`` default, so an untranslated future blueprint still renders its
    real English title instead of a raw key path.
    """
    from agent.i18n import t

    return t(f"trix.blueprint.{blueprint.key}.title", default=blueprint.title)


def _translated_description(blueprint) -> str:
    from agent.i18n import t

    return t(f"trix.blueprint.{blueprint.key}.description", default=blueprint.description)
...
def _fmt_catalog() -> str:
    ...
    lines = [
        t(
            "trix.blueprint.catalog_header",
            default="Automation Blueprints — `/blueprint <name>` and I'll ask you what I need:\n",
        )
    ]
    for r in CATALOG:
        lines.append(f"  • {r.key} — {_translated_title(r)}")
        lines.append(f"    {_translated_description(r)}")
    lines.append(
        "\n"
        + t(
            "trix.blueprint.catalog_tip",
            default="Tip: `/blueprint <name>` walks you through it. Power users can "
...
def _translated_slot_label(blueprint, slot) -> str:
    from agent.i18n import t

    return t(f"trix.blueprint.{blueprint.key}.slot.{slot.name}.label", default=slot.label)


def _translated_slot_help(blueprint, slot) -> Optional[str]:
    if not slot.help:
        return None
    from agent.i18n import t

    return t(f"trix.blueprint.{blueprint.key}.slot.{slot.name}.help", default=slot.help)


def _fmt_missing_slots(blueprint, missing: List[Any]) -> str:
    """Render the slots a direct-fill shortcut (``/blueprint <name> slot=val``)
    is missing, with each slot's label (and help, when it has one) in the
    active language — the one place ``BlueprintSlot.label``/``.help`` reach
    the client raw rather than through the agent's own Russian phrasing of
    ``build_blueprint_seed()``."""
    parts = []
    for s in missing:
        label = _translated_slot_label(blueprint, s)
        help_text = _translated_slot_help(blueprint, s)
        parts.append(f"{label} ({help_text})" if help_text else label)
    return "; ".join(parts)
...
def handle_blueprint_command(
    ...
    *,
    ...
    if not values:
        ...
        text = (
            f"Setting up '{_translated_title(blueprint)}' ({_humanize_schedule(blueprint)}). "
    ...
    # Pre-check required slots ourselves (mirroring fill_blueprint's own
    # `raw in (None, "")` rule) so a missing value can be reported with the
    # slot's translated label/help instead of the raw English text
    # `fill_blueprint`'s BlueprintFillError embeds (cron/blueprint_catalog.py
    # is upstream code we don't edit for wording).
    missing = [
        s for s in blueprint.slots
    ...
    if missing:
        ...
        return BlueprintCommandResult(
            t(
                "trix.blueprint.missing_values",
                default=(
                    "Can't set up '{title}' — missing: {fields}.\n"
                    "Or just run /blueprint {bp_key} and I'll ask you for the values."
                ),
                title=_translated_title(blueprint),
                fields=_fmt_missing_slots(blueprint, missing),
                bp_key=blueprint.key,
            )
        )
    ...
    try:
        ...
        return BlueprintCommandResult(
            f"Can't set up '{_translated_title(blueprint)}': {e}\n"
