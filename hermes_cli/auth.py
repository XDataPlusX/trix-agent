...
from hermes_constants import OPENROUTER_BASE_URL, secure_parent_dir, user_home_path
...
def _qwen_cli_auth_path() -> Path:
    return user_home_path(".qwen", "oauth_creds.json")
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    if not codex_home:
        codex_home = str(user_home_path(".codex"))
...
def _update_config_for_provider(
    ...
    # Comment-preserving round-trip write: config.yaml frequently starts as
    # a curated template with explanatory comments (e.g. Trix's
    # assets/config/trix-config.yaml, seeded by doctor --fix /
    # scripts/install.sh). A plain atomic_yaml_write() here would
    # re-serialize the whole file from a bare dict and silently drop every
    # comment on the very first provider switch (setup wizard, `hermes
    # model`, `hermes auth`).
...
def _reset_config_provider() -> Path:
    ...
    # Comment-preserving round-trip write (round-2 fix: `hermes logout`
    # shared the same plain-yaml.dump defect as every other config.yaml
    # writer — logging out would silently wipe a client's curated template
    # comments same as a provider switch used to).
    from utils import atomic_roundtrip_yaml_save
