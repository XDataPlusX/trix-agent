...
#
# Ни одно из этих правил НЕ доказывает удаления, и текст не имеет права его
# заявлять. Снято с клиентской установки 2026-09-10: агент позвал вебхук
# однострочником на питоне -- обычный HTTP-запрос, -- а клиент прочитал
# «запуск однострочной программы, которая удаляет файлы». Правило апстрима
# `script execution via -e/-c flag` срабатывает на ЛЮБОЙ `python -c`, не
# заглядывая внутрь строки; `parser limit` -- на любой слишком длинной
# команде; `execute_code` апстрим описывает словом «can», а не «does».
# Перевод превращал «не смог проверить» в «удаляет файлы» -- и приучал
# клиента, что «Разрешить» здесь про удаление.
#
# Сигнал безопасности от этого не теряется: содержимое `-c` мы разбираем
# САМИ (`detect_delete_in_code`), и когда там настоящее удаление, рядом в
# той же склейке стоит описание из SANDBOX_DELETE_RU -- со словом
# «удаление», потому что оно правда.
#
# Формулировки по-прежнему про последствие для клиента, а не про механику:
# ему нечего делать со словами «parser limit» и «-c flag». Но последствие
# называется то, которое есть: «из команды не видно, что она сделает».
SANDBOX_VERDICT_RU: dict[str, str] = {
    "script execution via -e/-c flag": (
        "запуск программы, переданной одной строкой: что она сделает, из самой "
        "команды не видно"
    ),
    # Тот же класс, и до 2026-09-10 перевода не имел вовсе -- клиент читал
    # английское «script execution via heredoc».
    "script execution via heredoc": (
        "запуск скрипта, вставленного прямо в команду: что он сделает, из самой "
        "команды не видно"
    ),
}
...
def _verdict_ru() -> dict[str, str]:
    ...
    long_delete = (
        "команда слишком длинная, чтобы разобрать её целиком — что внутри, "
    ...
    return {
        **SANDBOX_VERDICT_RU,
        ...
        approval._MALFORMED_EXEC_DESCRIPTION: (
            "команду не удалось разобрать целиком — что внутри, проверить не удалось"
        ),
        approval._EXECUTE_CODE_DESCRIPTION: (
            "запуск скрипта: он может менять файлы и запускать другие программы "
            "в обход обычной проверки команд"
        ),
    }
