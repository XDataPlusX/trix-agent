    ...
    if provider_changed or explicit_provider:
        ...
        if _user_pdef is not None and _user_pdef.base_url:
            ...
            try:
                ...
                # The raw exception (from resolve_runtime_provider, e.g. a
                # keyring/subprocess/network error) can carry Python
                # internals -- file paths, provider-SDK stack frames -- that
                # never belongs in a chat message to a non-technical client.
                # It reaches this function's caller unlocalized either way
                # (this module has no gateway/CLI context of its own), so it
                # goes to the log here and a short, localized, actionable
                # message goes to error_message instead (client-command-
                # surface plan, Task 8).
                logger.warning(
                    "model switch: could not resolve credentials for provider %r: %s",
                    provider_label, e,
                )
                return ModelSwitchResult(
                    success=False,
                    ...
                    is_global=is_global,
                    error_message=t(
                        "trix.cmd.model.credentials_failed",
                        provider=provider_label,
                        default=(
                            f"Could not resolve credentials for provider "
                            f"'{provider_label}'. Check its access settings "
                            f"or try a different model."
                        ),
                    ),
                )
    else:
...
def list_authenticated_providers(
    ...
    *,
    ...
    # "nous" pulls from the remote model-catalog manifest published at the
    # release repo's assets/api/model-catalog.json (see
    # hermes_cli/release_source.py) so newly added Portal models surface in
    # the /model picker without requiring a Hermes release. Falls back to
    # the in-repo _PROVIDER_MODELS["nous"] snapshot when the manifest is
    # unreachable.
