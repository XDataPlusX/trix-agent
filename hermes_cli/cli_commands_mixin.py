...
class CLICommandsMixin:
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        """Handle /debug — collect a debug report and print it locally.

        Accepts optional destination words after the command, kept for
        backward compatibility with older scripts/muscle memory. They no
        longer change the outcome: ``run_debug_share`` always collects and
        prints the report locally and never uploads it anywhere.

        - ``/debug``        → collect and print locally (the only behavior)
        - ``/debug nous``   → same; accepted but has no effect
        - ``/debug local``  → same; this is already what always happens
        """
        ...
        # run_debug_share always takes the local-print path regardless of
        # these flags — there's no upload left to consent to. yes=True just
        # skips a confirmation prompt that's dead code for the same reason
        # (and would otherwise hang: input() doesn't work inside
    ...
    def _handle_update_command(self) -> bool:
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Trix Agent')}")
            return False
