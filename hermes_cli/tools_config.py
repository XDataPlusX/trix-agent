...
CONFIGURABLE_TOOLSETS = [
    ...
    ("secrets",          "🔑 Secret Requests (messaging)", "ask the current chat for a missing API key by name/purpose"),
]
...
_TOOLSET_PLATFORM_RESTRICTIONS: Dict[str, Set[str]] = {
    ...
    # Spec 19 — secret_request only makes sense where the client has no other
    # way to hand the agent a credential (no shell, no host filesystem from
    # the sandbox). Scoped to Telegram, the one messaging platform this
    # product actually ships to clients on; CLI/desktop/TUI already have a
    # local masked-input secret prompt (hermes_cli/callbacks.py).
    "secrets": {"telegram"},
    # Спека 20 — mcp_connect нужен там же и по той же причине: у клиента в
    # мессенджере нет ни шелла, ни хостовой файловой системы из песочницы,
    # поэтому подключить MCP-сервер он может только разговором. У CLI,
    # десктопа и TUI для этого есть `hermes mcp add` и свой терминал.
    "mcp": {"telegram"},
}
...
def _cua_install_home() -> "Path":
    ...
    return Path(
        ...
        or str(user_home_path(".cua-driver"))
    )
...
def _run_post_setup_impl(post_setup_key: str):
    ...
    # npm и npx — не бинарники, а скрипты с шапкой `#!/usr/bin/env node`.
    # Найти их абсолютный путь мало: без `node` в PATH запуск падает
    # мгновенно с «/usr/bin/env: 'node': No such file or directory», то
    # есть ровно от той причины, ради которой ниже и зовётся
    # find_node_executable ($HERMES_HOME/node в PATH не лежит).
    #
    # Снято с клиентской машины 2026-09-10: мастер настройки три раза
    # подряд не смог поставить браузер, каждая попытка отваливалась за
    # доли секунды, в логе — ни одной причины. На машине, где Node только
    # наш (а рецепт кладёт его именно туда), браузер не устанавливался
    # никогда.
    #
    # Рантайм браузера этот урок уже усвоил (`_merge_browser_path` в
    # tools/browser_tool.py), установочные хуки — нет.
    node_env = with_hermes_node_path()
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            ...
            # Routed through tools.lazy_deps.ensure() (feature "search.ddgs")
            # instead of an ad hoc `pip install -U ddgs` so the version pin
            # has a single source of truth — LAZY_DEPS mirrors the `ddgs`
            # extra in pyproject.toml, which scripts/install.sh also
            # installs from (`uv pip install -e ".[ddgs]"`). Before this,
            # the wizard and the installer could each pick a different ddgs
            # version with no relation to each other.
            try:
...
#: "secrets" (spec 19) added for the same reason: a machine whose
#: `platform_toolsets.telegram` was saved by a prior `hermes tools` run
#: (`_save_platform_tools` freezes an explicit list of configurable keys)
#: would otherwise never see the new toolset even though it was added to
#: `hermes-telegram`'s default composite for fresh installs. Remove this
#: entry in the release after the one that ships it.
_RECENTLY_SHIPPED_TOOLSETS = frozenset({"bfl", "secrets"})
