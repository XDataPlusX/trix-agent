...
The release repo's raw content (``assets/api/model-catalog.json`` on the
``release`` branch — see ``hermes_cli/release_source.py``) hosts a JSON
manifest of curated models for providers we want to update without shipping
a release (currently OpenRouter and Nous Portal). This module fetches,
...
DEFAULT_CATALOG_URL = f"{RELEASE_RAW_BASE_URL}/assets/api/model-catalog.json"
# Historically this walked a second host: the Nous docs site (served
# through Vercel, which occasionally returned HTTP 403 + x-vercel-mitigated:
# challenge for non-browser clients) with raw.githubusercontent.com as the
# fallback when that host was bot-gated. There is no product docs site for
# this fork, so the primary URL above IS already the raw-content address --
# a fallback list containing that same URL would just double-fetch the one
# host that's already down, not add resilience. Offline degradation is
# handled separately: seed_cache_from_checkout() seeds the disk cache from
# the local checkout, and get_catalog() below falls back to a stale disk
# copy when every network attempt fails.
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``assets/api/model-catalog.json`` IS the newest catalog — no network
    round-trip needed. Copying it straight over the disk cache keeps the
    model picker current even when the remote manifest fetch fails or is
    slow to reach.

    The file lives under ``assets/`` rather than ``website/`` on purpose:
    ``website/`` is the docs-site source and is excluded from the client
    checkout (sparse-checkout in ``scripts/install.sh``), so a fallback
    seeded from there would silently never resolve on a real client
    machine — offline model resolution needs a path that actually ships.
    ...
    missing, empty, malformed, or fails schema validation (caller should
    treat a ``False`` as non-fatal — the network fetch path still applies on
    the next picker open).
    """
    src = Path(project_root) / "assets" / "api" / "model-catalog.json"
    try:
