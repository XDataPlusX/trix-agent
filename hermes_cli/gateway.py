...
def get_systemd_unit_path(system: bool = False) -> Path:
    ...
    # OS_RUNTIME: юнит systemd — интерфейс ОС к процессу. Изоляция профилей
    # здесь достигается ИМЕНЕМ юнита, а не переносом пути под корень.
...
def _legacy_unit_search_paths() -> list[tuple[bool, Path]]:
    ...
    """
    return [
        # OS_RUNTIME: каталог пользовательских юнитов ОС — см. allowlist §6.
...
# Стандартные каталоги, которые попадают в PATH любого сгенерированного
# юнита. Вынесены сюда, потому что по этому же перечню ищется node: иначе
# юнит и поиск опирались бы на разные списки и могли разойтись.
_SERVICE_COMMON_BIN_PATHS = (
    "/usr/local/sbin",
    "/usr/local/bin",
    "/usr/sbin",
    "/usr/bin",
...
def _append_node_dir_for_service(
    ...
    *hermes_root* is the Hermes home the unit will run against. System units
    ...
    # ``search_path`` делает результат воспроизводимым. Без него поиск идёт
    # по PATH ТОГО, КТО В ЭТУ СЕКУНДУ запускает генератор, и юнит выходит
    # разным у одной и той же машины: написанный из-под `runuser` с урезанным
    # PATH он не содержит каталога node, а сверяется потом в обычном сеансе,
    # где node находится. Отсюда вечное «Installed gateway service definition
    # is outdated» и переписывание юнита на каждом `restart` — тот же дефект,
    # что комментарий ниже уже описывает для симлинков, только со стороны
    # окружения, а не цели ссылки.
    #
    # Хуже видимого: вместе со строкой PATH так же случайно решается, КАКОЙ
    # node достанется шлюзу навсегда.
    #
    # Ищем по тому же перечню, который и попадёт в юнит, — тогда найденное
    # совпадает с тем, что служба реально увидит в бою. macOS-путь (launchd)
    # намеренно не трогаем: он захватывает пользовательский PATH по другому
    # замыслу.
    resolved_node = shutil.which("node", path=search_path)
    ...
    if not system:
        ...
        _append_node_dir_for_service(
            ...
            search_path=os.pathsep.join(
                list(path_entries) + list(_SERVICE_COMMON_BIN_PATHS)
            ),
        )

    common_bin_paths = list(_SERVICE_COMMON_BIN_PATHS)
...
def _repair_install_scope_home() -> None:
    """Вернуть процессу настоящий HOME перед работой над УСТАНОВКОЙ.

    Юнит systemd и plist launchd живут в ``~/.config/systemd/user`` и
    ``~/Library/LaunchAgents`` — это интерфейсы ОС (класс OS_RUNTIME), и в
    корне профиля они бесполезны: их там никто не прочитает. Процесс,
    пришедший от шлюза с contained-окружением (``/update`` из чата,
    десктопное приложение), положил бы их именно туда.

    No-op в shared-host и no-op, когда HOME уже настоящий.
    """
    try:
        ...
        # Починка окружения не имеет права ронять установку службы.
        pass
...
def _setup_standard_platform(platform: dict):
    ...
    # Telegram bot tokens are always created manually via @BotFather and
    # pasted here — there is no automatic/managed path. Nothing in this
    # setup flow talks to anything other than what the user configures.
