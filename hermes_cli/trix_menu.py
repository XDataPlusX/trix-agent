...
- ``SERVICE_COMMANDS`` -- dispatchable, but never shown in the menu or in
  ...
  never message the client first), ``/sethome`` (an escape hatch for a
  misconfigured home chat) and ``/debug`` (the support channel: typed on
  request, never advertised) live here.
...
# The 32 commands shown in Telegram's `/` menu, IN THE ORDER Telegram lists
...
#
# Three, none of them a client-facing feature:
...
# - "debug": the support channel. Owner decision 2026-09-04 moved it here
#   from the menu, cancelling Ruling 6 of the standard-build spec (which
#   had kept it visible). A menu entry advertised an action whose whole
#   payload is the client's own conversation: the report inlines log tails
#   that hermes_cli/debug.py's own docstring calls "plaintext conversation
#   content", and when it exceeds 3500 chars the reply tells the client to
#   ask "whoever administers this machine" -- who is the client, with no
#   shell. Disabling it outright was rejected: it is the only way a client
#   can hand us anything at all when the product breaks, so support asks
#   them to type it. Not shown, still runs.
SERVICE_COMMANDS: dict[str, str] = {
    ...
    "debug": (
        "решение владельца 2026-09-04: пункт меню предлагал клиенту выгрузить "
        "собственную переписку — в отчёт входят хвосты логов с фрагментами "
        "разговора, а длинный отчёт вообще отвечает «попросите администратора "
        "машины», которым клиент и является. Команда остаётся набираемой: это "
        "единственный способ прислать нам диагностику, пока нет автоматического "
        "канала ошибок"
    ),
}
