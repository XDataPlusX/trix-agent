...
_EXCLUDED_DIRS = {
    ...
    ".mypy_cache",
    ".ruff_cache",
    # Contained layout branches. ``tmp/`` is by definition ephemeral —
    # browser unpacks and half-written scratch files the size of the profile
    # itself. ``workspace/`` is the terminal's default cwd: somebody's
    # checkout, not the profile's state. Both are excluded ONLY at the root
    # (see ``_ROOT_ONLY_EXCLUDED_DIRS``) — ``skills/mine/workspace/`` is a
    # skill's own data and stays. ``home/`` deliberately does NOT join them:
    # a backup is restored onto the same machine, so the profile's logins
    # have to survive it. That is the honest difference from ``profile
    # export``, which is handed to somebody else.
    "tmp",
    "workspace",
}

# Names excluded at the root of the profile only. Elsewhere in the tree the
# same name means something else and must be preserved.
_ROOT_ONLY_EXCLUDED_DIRS = {"hermes-agent", "tmp", "workspace"}
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # Root-only names match at the first component only. Nested
        # directories with the same name — ``skills/autonomous-ai-agents/
        # hermes-agent/``, ``skills/mine/workspace/`` — are somebody's data
...
def _default_backup_dir(hermes_root: Path) -> Path:
    """Where ``hermes backup`` puts the zip when ``--output`` is absent.

    shared-host keeps the upstream answer — the OS home directory, which is
    where users look for it.

    Contained does not, and for a reason that is easy to miss: the engine's
    own ``HOME`` is deliberately never redirected (systemd units and
    ``~/.local/bin`` have to stay in the real home), so ``Path.home()`` there
    means "outside the profile root". A backup is the single most
    concentrated copy of a profile that exists — config, .env, the session
    DB, and under containment the provider logins too. Writing it outside
    the root by default undoes containment in one step, silently.

    ``backups/`` is already in ``_EXCLUDED_DIRS``, so backups do not nest.

    ``hermes disk clean`` knows about this default: in contained it clears
    only the engine's own restore points from that directory (the
    ``pre-update-`` / ``pre-migration-`` generations pruned below) and leaves
    operator archives alone. See ``trix_disk._clear_candidate``.
    """
    try:
        from hermes_constants import is_contained
        if is_contained():
            return hermes_root / "backups"
    except Exception:
        pass
    return Path.home()
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    if args.output:
        ...
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = _default_backup_dir(hermes_root) / f"hermes-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``hermes-agent``, ``tmp`` and ``workspace`` are pruned at the root
        # level only; nested dirs with the same name (e.g. in skills/) are
        # somebody's data and must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS
            or (d in _ROOT_ONLY_EXCLUDED_DIRS and not is_root)
        ]
    ...
    # ``_external/`` arc prefix, encoded relative to the PROFILE's home dir.
    #
    # ``user_home_path()`` and not ``Path.home()``: under containment the
    # providers moved to ``<root>/home/...``, and encoding those against the
    # OS home produced one of two wrong answers — the same file twice (once
    # from the walk, once under ``_external/``), or a "skipped as external"
    # note about a path that sits inside the root. In shared-host the two
    # are the same directory, so nothing changes there.
    home_dir = user_home_path().resolve()
    external_to_add: list[tuple[Path, str]] = []  # (absolute, arcname)
    skipped_external: list[str] = []
    root_resolved = hermes_root.resolve()
    for base in _collect_memory_provider_external_paths():
        try:
            ...
            continue
        # Already inside the root — the walk above took it. Not external,
        # not skipped, not a duplicate: simply already done.
        try:
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        # Same anchor as the capture side — otherwise a contained backup
        # restores provider state into the OS home it came from.
        home_dir = user_home_path().resolve()
        t0 = time.monotonic()

        # Restored files are written via a temp-file + atomic-replace so a
        # crash/interrupt/full-disk mid-write can never leave a truncated or
        # partially-overwritten file behind.
        #
        # STATUS.md filed this as "writes config.yaml past the comment-safe
        # writer". That reading is wrong: this is a byte-for-byte copy out of
        # the archive, so comments survive it trivially, and routing a restore
        # through ``atomic_roundtrip_yaml_save`` would be actively worse --
        # its merge semantics take comment placement from whatever is already
        # on disk, so "restore" would stop meaning "exactly what was backed
        # up". The defect was never the writer; it was that ``open(target,
        # "wb")`` truncates the live file in place. Which is why the fix
        # covers EVERY restored file, not just config.yaml: the same window
        # sat over .env and the session database.
        # New (not-previously-existing) files should land at the same
        # permissions a plain ``open(path, "wb")`` would have produced, not
        # mkstemp's restrictive 0600 default -- peek the umask to reproduce
        # that.
        _saved_umask = os.umask(0)
        os.umask(_saved_umask)
        _default_new_file_mode = 0o666 & ~_saved_umask
            ...
            if member.startswith(_EXTERNAL_PREFIX):
                ...
                try:
                    ...
                    if target.suffix in {".json", ".env", ".conf"} or target.name in _SECRET_FILE_NAMES:
                        atomic_write_bytes(target, data, force_mode=0o600)
                    else:
                        atomic_write_bytes(
                            target, data,
                            preserve_mode=True,
                            create_mode=_default_new_file_mode,
                        )
                    restored += 1
                    restored_external += 1
...
def restore_cron_jobs_if_emptied(
    ...
    #
    # But say so, because declining silently here is how this safety net dies
    # for good. Verified by executing the chain: (1) an unreadable live file
    # makes this branch decline; (2) the NEXT update's pre-update snapshot
    # copies that same unreadable file, so ``snap_count`` is ``None`` too and
    # the guard below declines forever after — there is no longer any good
    # snapshot to restore from, and nothing ever mentions it.
    #
    # One line of log turns "permanently dead" into "dead, and it told you
    # once, naming the snapshot that still had the jobs". Left as a warning
    # rather than a restore on purpose: papering over a corrupt file is the
    # thing this branch exists to refuse.
    if live_count is None:
        if live_path.exists():
            snap_probe = _count_cron_jobs(
                _quick_snapshot_root(home) / snapshot_id / _CRON_JOBS_REL
            )
            logger.warning(
                "cron/jobs.json is unreadable, so the post-update job-loss "
                "safety net declined to act (it must not overwrite a file it "
                "cannot parse). Pre-update snapshot %s holds %s job(s). Repair "
                "or replace the live file: while it stays unreadable this net "
                "is disabled, and the next update's snapshot will copy the "
                "unreadable file too.",
                snapshot_id,
                "an unknown number of" if snap_probe is None else snap_probe,
            )
        return None
