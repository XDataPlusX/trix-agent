...
# ── Contained profile root: isolation mode + TMPDIR of THIS process ─────
# Must run before anything calls ``tempfile.gettempdir()``, which caches its
# answer for the lifetime of the process — after that no amount of env
# fixing moves a single temp file, and ``tempfile.*`` appears in the engine
# ~190 times. This entry point knows its profile unambiguously, so this is
# both the right place and the earliest one.
#
# ``bootstrap_process_isolation`` reads ``isolation.mode`` from config.yaml
# BEFORE the redirect: the full config load happens hundreds of lines later,
# and a mode that lands after the cache is a mode that never applied.
#
# ``select_profile_root=True`` answers the question that comes even earlier:
# the mode of WHICH root.  This entry point picks its own profile — by
# ``-p``/``--profile`` or the sticky ``active_profile`` — and that choice used
# to land in ``_apply_profile_override()`` six hundred lines below, long past
# the ``tempfile`` cache.  A named contained profile then wrote its temp files
# into the real ``/tmp`` while ``is_contained()`` answered ``True``, and the
# config bridge read ``~/.hermes/config.yaml`` instead of the profile's.  The
# bootstrap does that resolution on stdlib only, and
# ``_apply_profile_override()`` below reuses the same scanner, so the two
# cannot drift.
#
# No-op in shared-host. HOME of this process is never touched — systemd
# units, ``~/.local/bin`` and the desktop entry must stay in the real home.
try:
    from hermes_constants import bootstrap_process_isolation as _bootstrap_isolation  # noqa: E402

    _bootstrap_isolation(select_profile_root=True)
except Exception:
    # Изоляция не имеет права мешать процессу стартовать: в худшем случае
    # временные файлы останутся там же, где были до этой работы.
    pass
...
#
# The argv scan itself lives in ``hermes_constants.scan_profile_flag`` — the
# isolation bootstrap at the top of this file has to answer the same question
# hundreds of lines earlier (before ``tempfile`` caches its temp dir) and on
# stdlib only.  Two parsers for one flag would mean two different answers for
# one invocation; this function keeps the parts that need ``hermes_cli`` —
# argv stripping, the sudo fallback and the operator-facing errors.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HERMES_HOME before imports."""
    argv = sys.argv[1:]

    # `sudo hermes -p <name>` resolves against the invoking user's home, not
    # root's — see hermes_constants.sudo_user_profile_root. It lives there
    # because the isolation bootstrap needs the same answer before any
    # hermes_cli import exists.
    from hermes_constants import sudo_user_profile_root as _resolve_sudo_user_profile_env

    # 1. Explicit -p / --profile flag. Historically this worked even after the
    # subcommand (`hermes chat -p coder`), so the scan is broad; it skips
    # command-argv passthrough regions such as `mcp add --args` and rejects
    # values that can't be profile names (pytest's `-p no:xdist`).  Same
    # scanner the isolation bootstrap ran at import time — see the note above
    # this function.
    from hermes_constants import scan_profile_flag

    profile_name, consume, profile_index = scan_profile_flag(argv)
    ...
    #
    # The read itself is ``hermes_constants.sticky_profile_name`` — the same
    # one the isolation bootstrap used, s6 exception included.
    if profile_name is None:
        from hermes_constants import sticky_profile_name
...
def _ensure_tui_node() -> None:
    ...
    # OS_RUNTIME: ~/.local/bin — PATH пользователя ОС, принадлежит установке.
...
def cmd_doctor(args):
    ...
    from hermes_cli.trix_doctor_verdict import run_doctor_with_verdict

    return run_doctor_with_verdict(run_doctor, args)
...
def cmd_uninstall(args):
    ...
    # Скоуп УСТАНОВКИ, как и обновление: удалять надо юнит, ``~/.local/bin``
    # и desktop entry в настоящем доме ОС. Десктопное приложение запускает
    # это отдельным процессом, и под containment он пришёл бы с
    # HOME=<root>/home — то есть искал бы установку там, где её нет, и
    # «удалил» бы пустоту. No-op в shared-host.
    try:
        from hermes_constants import restore_install_scope_home

        restore_install_scope_home()
    except Exception:
        pass
...
def _run_npm_watching_for_engine_failure(
    ...
    *,
    ...
    if capture_output:
        try:
            ...
            return subprocess.CompletedProcess(
                ...
                124,
                exc.stdout or "",
                _npm_timeout_message(
                    timeout, exc.stderr, _missing_declared_node_deps(cwd)
                ),
            )
    ...
    if timed_out:
        returncode = 124
        captured.append(
            _npm_timeout_message(timeout, None, _missing_declared_node_deps(cwd))
        )
        sys.stderr.write(captured[-1])
    return subprocess.CompletedProcess(cmd, returncode, None, "".join(captured))


def _missing_declared_node_deps(project_root: Path | str) -> list[str]:
    """Объявленные в package.json пакеты, которых нет на диске.

    `npm install` правит `node_modules` НА МЕСТЕ. Остановленный на
    полпути, он оставляет дерево неполным — не «как было», а хуже, чем
    было. Снято с клиентской машины 2026-09-10: потолок в 15 минут убил
    npm, из РАБОЧЕЙ установки пропал `agent-browser`, агент лишился
    браузера, а клиенту сообщение об остановке сказало, что зависимости
    остались прежними.

    Спрашиваем у диска, а не у команды — тот же приём, которым ловится
    «Camofox installed» без Camofox.

    Присутствие считается по правилам самого Node: пакет виден, если он
    лежит в `node_modules` рядом ЛИБО в `node_modules` любого каталога
    выше. Без этого проверка рабочего пространства (`ui-tui`, `web`)
    объявила бы пропавшим всё: npm поднимает их зависимости в корневой
    `node_modules`, и рядом с самим пакетом их не бывает.
    """
    ...
    return [name for name in declared if not _present(name)]


def _npm_timeout_message(
    timeout: float | None, tail: str | None, missing: list[str] | None = None
) -> str:
    ...
    text = (
        ...
        "код уже обновлён. Но npm правит node_modules НА МЕСТЕ, и "
        "остановленный на полпути мог оставить зависимости неполными. "
    )
    if missing:
        # Называем поимённо. Прежняя редакция обещала, что зависимости
        # остались прежними, и увести из рабочей установки пакет она этим
        # не мешала — она мешала это ЗАМЕТИТЬ.
        text += (
            "Так и вышло: на диске нет — " + ", ".join(missing) + ". "
            "Повторите обновление; если не поможет, дерево восстановит "
            "`npm install --workspaces=false` в каталоге установки.\n"
        )
    else:
        text += "Повторите обновление позже.\n"
    if tail:
        text = f"{tail}{text}"
    return text
...
def _post_update_marker_path() -> Path:
    """Крошка «новый код встал, но хвост обновления не дошёл до конца».

    Третья того же вида, что ``.update-incomplete`` и
    ``.lazy-refresh-incomplete``, и нужна по отдельной причине. Те две
    охраняют установку зависимостей и снимаются сразу после неё — а после
    них у ``hermes update`` остаётся хвост: миграция конфига, досев секций и
    перезапуск шлюза. Процесс, умерший в этом хвосте (оборванная SSH-сессия
    — обычный случай на машине клиента), оставлял машину с новым кодом,
    старым конфигом и шлюзом, работающим на прежних модулях. Повторный
    ``hermes update`` положение не исправлял: на «Already up to date» он
    выходит раньше хвоста.

    Почему крошка, а не вывод из состояния: «настройки не доехали» из
    состояния выводится (сравнить файл с шаблоном), а «запущенный процесс
    работает на старом коде» — нет, это факт о прошлом событии.
    """
    return PROJECT_ROOT / ".post-update-incomplete"
...
def _clear_post_update_marker() -> None:
    """Remove the unfinished-tail breadcrumb. Never raises."""
    _clear_marker_file(_post_update_marker_path(), label="post-update-incomplete")
...
def _resolve_update_branch(args) -> str:
    """Ветка, за которой ходит обновление — всегда релизная.

    Раньше здесь разбирался ``--branch`` с дефолтом "main". Флаг убран:
    клиентская установка обновляется только с релизной ветки нашего
    репозитория, а любой другой канал недостижим штатным путём. Аргумент
    ``args`` сохранён в сигнатуре — три вызывающих (путь проверки,
    git-путь, ZIP-путь) передают его, и все должны получать один ответ.
    """
    del args  # канал не зависит от ввода
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Trix Agent")
        return
    ...
    if getattr(args, "check", False):
        # --check compares against the pinned release channel, same as the
        # apply path — args is ignored (see _resolve_update_branch).
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from hermes_cli.trix_layout import cmd_migrate_layout

        rc = cmd_migrate_layout(args)
        if rc:
            sys.exit(rc)
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        "acp", "approvals", "auth", "backup", "bundles", "business", "checkpoints", "claw", "completion",
        "computer-use",
        "config", "console", "contour", "cron", "curator", "dashboard", "serve", "debug", "doctor",
...
def cmd_setup_wizard(args):
    """Web setup wizard: bootstrap TLS/password, serve, open, status, close."""
    from hermes_cli.setup_wizard.cli import cmd_setup_wizard as _cmd_setup_wizard

    return _cmd_setup_wizard(args)
...
def main():
    ...
    # =========================================================================
    # setup-wizard command — bootstrap/serve/open/status/close/set-password/
    # install-service for the web setup wizard (all logic lives in
    # hermes_cli/setup_wizard/cli.py). `cmd_setup_wizard` (defined near the
    # other lazy-import wrappers, e.g. `cmd_kanban`/`cmd_mcp`) imports that
    # module only when the subcommand is actually dispatched — building this
    # parser must not pull in setup_wizard's transitive deps (gateway_ctl ->
    # gateway.status, etc.) on every single `hermes` invocation.
    # =========================================================================
    wizard_parser = subparsers.add_parser(
        "setup-wizard",
        help="Web setup wizard: bootstrap, serve, open, status, close, set-password",
        description=(
            "Manage the web-based setup wizard: write the primary "
            "credential slot from provisioning-generated values and "
            "generate the TLS cert (bootstrap), run the wizard's web "
            "server (serve), issue an emergency temporary password and "
            "restart the service (open), check its state (status), "
            "disable it (close), rotate the permanent login/password "
            "(set-password), and install the systemd user service that "
            "hosts it (install-service)."
        ),
    )
    wizard_subparsers = wizard_parser.add_subparsers(dest="wizard_command")

    wizard_bootstrap = wizard_subparsers.add_parser(
        "bootstrap",
        help="Write the primary credential slot (login/password from provisioning) + generate the TLS cert",
    )
    wizard_bootstrap.add_argument("--ip", required=True, help="Bare IP address (v4 or v6) of this machine")
    wizard_bootstrap.add_argument(
        "--login", required=True, help="Primary login, already generated by the provisioning script"
    )
    wizard_bootstrap.add_argument(
        "--password-file",
        required=True,
        dest="password_file",
        help="Path to a 0600 file holding the primary password; deleted immediately after reading",
    )

    wizard_serve = wizard_subparsers.add_parser(
        "serve",
        help="Run the wizard's web server (used by the systemd service)",
    )
    wizard_serve.add_argument("--port", type=int, default=8443)

    wizard_open = wizard_subparsers.add_parser(
        "open",
        help="Emergency: issue a temporary password (does not touch the primary login/password) and (re)start the service",
    )
    wizard_open.add_argument("--ttl-minutes", type=int, default=60, dest="ttl_minutes")

    wizard_subparsers.add_parser("status", help="Show whether the wizard is open/closed/completed")
    wizard_subparsers.add_parser("close", help="Disable the wizard and stop its service")

    wizard_set_password = wizard_subparsers.add_parser(
        "set-password",
        help="Emergency: rotate the primary login/password (e.g. the provisioning email leaked)",
    )
    wizard_set_password.add_argument("--login", help="Custom login (default: generated)")
    wizard_set_password.add_argument(
        "--print",
        dest="print_credentials",
        action="store_true",
        help="Print the new login/password to stdout",
    )

    wizard_subparsers.add_parser(
        "install-service",
        help="Write the systemd user unit that hosts the wizard's web server",
    )
    ...
    # =========================================================================
    # contour command — company file contour (host folders + profile mounts)
    # =========================================================================
    contour_parser = subparsers.add_parser(
        "contour",
        help="Trix file contour — company folders and per-profile mounts (status, check, apply-request, push)",
        description=(
            "Administrative layer for the shared-company-base file contour: "
            "host folders, per-profile Docker mounts, and the shared skills "
            "directory. Consumes only closed-grammar JSON requests validated "
            "against real machine state — see "
            "optional-skills/autonomous-ai-agents/trix-file-contour/."
        ),
    )
    try:
        from hermes_cli.trix_contour_cli import register_cli as _register_contour_cli

        _register_contour_cli(contour_parser)
    except Exception as _exc:
        logging.getLogger(__name__).debug("contour CLI wiring failed: %s", _exc)

    # =========================================================================
    # business command — one-shot business-mode deployment (spec 21 §8)
    # =========================================================================
    business_parser = subparsers.add_parser(
        "business",
        help="Business-mode deployment — system_admin profile, topic route, shared skills (spec 21)",
        description=(
            "Turns an already-installed standard machine into a business "
            "machine: creates the system_admin profile, restricts its "
            "toolset, routes the admin Telegram topic to it in the default "
            "profile's config, pins the administrator by Telegram id, and "
            "wires the shared company skills directory. Idempotent — safe "
            "to re-run with the same arguments."
        ),
    )
    try:
        from hermes_cli.trix_business import register_cli as _register_business_cli

        _register_business_cli(business_parser)
    except Exception as _exc:
        logging.getLogger(__name__).debug("business CLI wiring failed: %s", _exc)
