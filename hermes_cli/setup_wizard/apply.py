...
from hermes_cli.setup_wizard.providers_view import CLIENT_SUPPLIED_ENDPOINT_PROVIDERS
...
def apply_settings(form: dict) -> dict:
    ...
    # Свой эндпоинт клиента — единственный случай, когда ключ провайдера
    # уезжает НЕ в .env. У провайдера ``custom`` нет своей переменной
    # окружения (``env_vars=()``), и рантайм читает его ключ из
    # config.yaml — ``model.api_key``, источник ``model_config`` в
    # agent/credential_sources.py. Решение владельца 2026-09-12: принять
    # апстримный способ, а не строить свой мост в резолве ключей. Права у
    # config.yaml те же 600, что и у .env, так что в доступе разницы нет.
    # Пишется ниже, вместе с остальным config.yaml (шаг 3), чтобы не
    # делать отдельного сохранения.
    custom_endpoint_key = ""
    if provider_name in CLIENT_SUPPLIED_ENDPOINT_PROVIDERS:
        custom_endpoint_key = (provider.get("api_key") or "").strip()
    ...
    try:
        ...
        # Ключ своего эндпоинта — сюда же, в одно сохранение конфига (см.
        # его сбор выше). Пустая строка ничего не трогает: у возвратного
        # клиента поле ключа не заполняется значением, и пустота означает
        # «оставить как было», а не «стереть» — тот же контракт, что у
        # каждого секретного поля формы.
        if custom_endpoint_key and provider_step_ok:
            cfg.setdefault("model", {})["api_key"] = custom_endpoint_key
            pending.append("model.api_key")
