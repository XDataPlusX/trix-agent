...
# Провайдеры, у которых АДРЕС ЭНДПОИНТА задаёт клиент, а не каталог.
# Таких сегодня ровно один — ``custom`` (свой OpenAI-совместимый сервер:
# развёрнутая модель, корпоративный шлюз, локальная Ollama, vLLM). У него
# в профиле пустые ``base_url`` и ``env_vars``: подставлять нечего и ключ
# писать некуда — он единственный, чей ключ уезжает не в ``.env``, а в
# ``config.yaml`` (``model.api_key``, источник ``model_config`` в
# agent/credential_sources.py). Так устроен апстрим; решение владельца
# 2026-09-12 — принять это, а не городить свой мост в резолве ключей.
#
# Мастер читает этот набор дважды: страница раскрывает поле адреса сразу
# (а не прячет за «изменить»), а серверная валидация требует адрес и ключ.
CLIENT_SUPPLIED_ENDPOINT_PROVIDERS: frozenset[str] = frozenset({"custom"})

# Слоты, в которых ``custom`` пока НЕ предлагается, с причиной. Запасной
# слот пишет выбор в цепочку ``fallback_providers``, а ключ своего
# эндпоинта живёт в ``model.api_key`` — одном на конфиг. Второму адресу
# там места нет: для этого в апстриме есть отдельный список
# ``custom_providers`` с идентификаторами ``custom:<имя>``, и это
# отдельная задача. Полурабочую строку клиенту не показываем.
CUSTOM_ENDPOINT_UNSUPPORTED_SLOTS: dict[str, str] = {
    "fallback": (
        "ключ своего эндпоинта хранится в model.api_key — он один на "
        "конфиг, второму адресу места нет; нужен список custom_providers"
    ),
}
...
RU_PROVIDER_DESCRIPTIONS: dict[str, str] = {
    "custom": (
        "Любой сервер с API как у OpenAI: своя развёрнутая модель, "
        "корпоративный шлюз, Ollama или vLLM. Нужны адрес и ключ — "
        "каталог моделей и тарифы здесь не наши, а ваши."
    ),
    "actual": (
    ...
    "kimi-code": (
        "Kimi Code — подписка kimi.com на код-ассистента, отдельный адрес и "
        "ключ от Moonshot API выше. Фиксированная плата вместо оплаты за "
        "токены."
    ),
    ...
    "xiaomi-token-plan": (
        "Тот же Xiaomi MiMo, но по подписке Token Plan — отдельный адрес и "
        "ключ, фиксированная плата вместо оплаты за токены."
    ),
    "zai": "Тоже напрямую из России, сильнее в длинных задачах, дороже DeepSeek.",
    "zai-coding-plan": (
        "Тот же Z.AI / GLM, но по подписке Coding Plan — отдельный адрес и "
        "ключ, фиксированная плата вместо оплаты за токены."
    ),
}
        ...
        rows.append(
            {
                ...
                # Клиент задаёт адрес сам — страница раскрывает поле сразу,
                # сервер требует его непустым. См.
                # CLIENT_SUPPLIED_ENDPOINT_PROVIDERS.
                "endpoint_from_client": p.name in CLIENT_SUPPLIED_ENDPOINT_PROVIDERS,
