...
def _rail_support_entry_html() -> str:
    """The support entry (spec 15) — a distinct block under the step list,
    not a line in the rail's foot.

    An earlier pass put this as a small underlined line inside
    ``.rail-foot`` (next to the machine address), where the owner
    literally could not find it on a screenshot. It now lives as its own
    element between ``#progress-bar`` and ``.rail-foot`` — outside both,
    so it reads as neither "one more step" nor "footer trivia" — styled
    with ``--gold`` (see ``.rail-support`` in ``_CSS``) so it visually
    pops the way the owner asked ("выделить, например цветом"), the same
    accent already used for the CURRENT step number, never for a plain
    step or for the primary call-to-action buttons (those are blue) —
    borrowing that color keeps this from reading as "the next thing to
    do" while still being unmistakably highlighted.
    """
    return (
        '<div class="rail-support" id="rail-support">'
        '<a href="/support">Что-то не работает?</a>'
        "</div>"
    )


def _rail_update_entry_html() -> str:
    """Вход на страницу обновления — отдельный блок рядом с «Что-то не
    работает?».

    Стоит именно в колонке, а не внутри формы, по той же причине, что и
    вход в поддержку: клиент приходит сюда не в середине настройки, а
    отдельным заходом, часто спустя недели — и обычно потому, что бот
    молчит и обновление это единственная починка, которую он может
    запустить сам. Путь к ней не должен лежать через шаги формы, которые
    заново применяют настройки и перезапускают шлюз.
    """
    return (
        '<div class="rail-support" id="rail-update">'
        '<a href="/update">Обновить продукт</a>'
        "</div>"
    )


def _rail_wizard_entry_html() -> str:
    """The support page's own way back into the wizard (owner feedback,
    2026-09-03: "со страницы поддержки нет выхода") -- the exact symmetric
    counterpart of :func:`_rail_support_entry_html`: same slot in the rail
    (between the step list and the rail foot), same visual treatment
    (``--gold``, the "not a step, not the primary action" accent), just
    pointing the other way. Reusing that slot/style rather than inventing
    a second affordance is deliberate -- a client who has just landed here
    from the wizard's own "Что-то не работает?" link already knows what
    that highlighted block in that position means.

    Fixing the actual cause is very often a setup change (a rotated proxy,
    a swapped provider key) -- the repair tool that cannot reach the form
    that makes those changes is a dead end, not a repair tool.
    """
    return (
        '<div class="rail-support" id="rail-wizard-entry">'
        '<a href="/">← Вернуться к настройке</a>'
        "</div>"
    )


def _rail_html(rail_address: str, header_intro_html: str, *, support_visible: bool = False,
               subtitle: str = "Настройка", wizard_entry_visible: bool = False,
               update_visible: bool = False) -> str:
    ...
    ``support_visible`` (spec 15) gates the "Что-то не работает?" entry
    (``_rail_support_entry_html``) — see that helper's own docstring for
    why it is a distinct block, not a foot line. It defaults to ``False``
    so every existing caller (the pure ``render_page()`` default, and
    every test that doesn't pass a host/state) keeps rendering the rail
    exactly as before. The wizard's own ``/`` route is the one caller
    that computes a real value, from ``WizardState.load().is_completed()``
    (``app.py``): before a client's very first successful "Готово" there
    is no saved bot token or provider key yet, so every one of
    ``run_support_pass()``'s checks would just fail on missing
    credentials — offering "почини" before there is anything configured
    to check is not a real affordance, it is a guaranteed dead end for a
    client mid-setup. Once ``is_completed()`` flips true it stays true
    (see ``WizardState.is_completed``'s own docstring — nothing ever
    clears it), so the entry, once it appears for a given machine, never
    disappears again on a later visit.

    ``wizard_entry_visible`` (owner feedback 2026-09-03: "со страницы
    поддержки нет выхода") gates the reverse trip —
    ``_rail_wizard_entry_html()``, the support page's own "Вернуться к
    настройке" link — see that helper's own docstring. Mutually exclusive
    with ``support_visible`` in every real caller (the wizard never needs
    a way back into itself; the support page never needs a second link to
    the page it is already on), so ``support_visible`` wins if a future
    caller ever passed both.
    """
    if support_visible:
        entry_html = _rail_support_entry_html()
    elif wizard_entry_visible:
        entry_html = _rail_wizard_entry_html()
    else:
        entry_html = ""
    # Обновление — САМОСТОЯТЕЛЬНЫЙ блок, а не третья ветка этого выбора:
    # оно нужно и в мастере, и на странице поддержки, то есть рядом с
    # каждым из двух входов выше, а не вместо них.
    if update_visible:
        entry_html += _rail_update_entry_html()
    return (
        '<aside class="rail" id="rail">'
        f'<div id="rail-logo">{_LOGO_SVG}</div>'
        '<div class="product">Trix Agent</div>'
        f'<p class="product-sub" id="rail-sub">{subtitle}</p>'
...
/* Вход в поддержку (спека 15) и обратный вход в мастер (owner, 2026-09-03:
   "со страницы поддержки нет выхода") — один и тот же блок/класс, только
   один из двух рендерится за раз (см. _rail_html), не строка в подвале
   рейла (там владелец физически не смог найти вход в поддержку на
   скриншоте). --gold — тот же акцент, что у номера ТЕКУЩЕГО шага, а не
   синий цвет основных кнопок-действий: это заметно, но не читается как
   «ещё один шаг мастера» или «следующее действие». Вход в поддержку
   рендерится сервером только когда WizardState.is_completed() истинно
   (см. render_page()/_rail_html) — до первой пройденной настройки ссылки
   нет вовсе; вход в мастер рендерится всегда на /support. */
.rail-support { margin: 1rem 0 0; }
.rail-support a {
  display: block;
...
.rail-support a:hover { background: rgba(245, 226, 158, 0.12); }
.rail-foot {
  margin-top: auto;
  padding-top: 1.4rem;
...
/* Четвёртый экземпляр той же схватки UA против автора, что у field-row,
   botlink и verdict выше (селекторы тут намеренно НЕ повторены дословно:
   соседние тесты ищут своё правило подстрокой от начала файла, и
   упоминание в комментарии уводило их на комментарий вместо правила —
   поймано прогоном сразу после этой правки).
   `.stages li { display: flex }` — правило автора, а `[hidden]{display:none}`
   приходит из таблицы браузера, и автор её всегда перебивает. Следствие
   наблюдалось клиентом на живой машине 2026-09-04: setStageOrder(false)
   честно ставит `installLi.hidden = true`, когда ставить нечего, но строка
   «Устанавливаем инструменты» всё равно оставалась на экране — серая,
   между выполненным «Настройки сохранены» и идущим «Перезапускаем
   агента…». Читалось это как «шаг пропустили и всё зависло», хотя шаги шли
   нормально: renderProgressStages() до этой строки просто не доходит,
   потому что её нет в currentStageOrder, — и она навсегда застывала в
   своём статическом виде из HTML.
   Правило нарочно на `.stages li[hidden]`, а не на `#stage-install`: прячут
   тут по атрибуту, и следующий скрываемый шаг не должен снова наступить на
   те же грабли. */
.stages li[hidden] { display: none; }
...
#fallback-auth-choice, #fallback-api-key-block, #fallback-device-code-block {
  margin-top: 1.75rem;
}
#provider-api-key-block .field-row + .field-row, #fallback-api-key-block .field-row + .field-row {
  margin-top: 0.85rem;
}
...
/* Раздел поддержки (спека 15, support_view.py) — рендерится через
   render_shell() этого же файла, поэтому уже получает все правила выше
   (шрифты, .screen-title/.screen-sub, кнопки, .rail и т.д.) бесплатно.
   Ниже — только то, чего в самом мастере нет: чат и промежутки между
   блоками результата/обратной связи, которые в мастере сидят внутри
   шагов формы (и там расстояние даёт .step-nav/.field-group), а на
   странице поддержки эти блоки — верхнеуровневые соседи. */
#wait, #report, #feedback, #noChatNotice, #thanks, #chat { margin-top: 1.1rem; }
/* Лента переписки (владелец, 2026-09-03: "чат — это не поле ввода"):
   своя высота со скроллом, чтобы длинный разговор не растягивал всю
   страницу, и собеседники различимы подписью-меткой (.who), а не только
   начертанием — тот же .hint-приглушённый серый для "Поддержка", акцент
   --blue-d (уже используемый для ссылок/hover кнопок выше) для "Вы",
...
#chatLog p { margin: 0 0 0.55rem; font-size: 0.92rem; line-height: 1.45; }
#chatLog p:last-child { margin-bottom: 0; }
#chatLog p .who { font-weight: 700; }
#chatLog p.user .who { color: var(--blue-d); }
#chatLog p.bot .who { color: var(--ink-2); }
/* "Бот думает" (owner: "видно, что бот думает") переиспользует ту же
   строку и тот же поминутный тикер, что и #wait выше — build_heartbeat_text
   для check-прогона; см. showChatThinking() в скрипте. */
#chatThinking { font-style: italic; }
#chat input[type="text"] { margin-right: 0.5rem; width: auto; flex: 1; }
#chat .chat-row { display: flex; gap: 0.5rem; align-items: center; }
"""

# ---------------------------------------------------------------------------
# Блок провайдера — ОДИН шаблон на два слота (основной и запасной).
#
# Почему шаблон, а не две копии разметки. Владелец 2026-09-10, упершись в
# живом мастере: «выбираешь запасного провайдера, и там всё так же
# раскрывается, как и в первом разе». До этого запасной слот был голым
# списком провайдеров плюс полем ключа — ни выбора модели, ни адреса API,
# ни автопроверки ключа, ни описаний, ни входа по аккаунту. Конкретный
# упор: основным выбран ChatGPT, запасным нужен GLM, а у GLM два варианта
# с разными адресами — и поменять адрес было негде.
#
# Две копии одной разметки разъезжаются на первой же правке, поэтому
# слот описан один раз, а идентификаторы подставляются по карте.
# Идентификаторы ОСНОВНОГО слота — ровно те, что были до появления
# шаблона, байт в байт (закреплено тестом
# ``test_primary_provider_block_markup_is_unchanged``): вся разметка
# первого слота, весь его CSS и весь внешний код, который держится за эти
# имена, продолжают работать без единой правки.
#
# Идентификаторы не выводятся из префикса формулой: у основного слота они
# исторически разношёрстные (``key-verdict``, ``models-fetch-error``,
# ``device-login-start`` — без всякого префикса), и попытка их
# «причесать» была бы переименованием ради красоты с риском отвалиться
# в местах, до которых тесты не дотягиваются. Карта честнее.
_PROVIDER_BLOCK_TMPL = """\
    <div id="@@block@@">
      <label id="@@group_label@@" class="section-title" for="@@group@@">@@section_title@@</label>
      ...
      <div id="@@group@@" class="prov" role="group" aria-labelledby="@@group_label@@"></div>
      <div id="@@current_hint@@" class="hint" style="padding-top:0.35rem"></div>
      <div id="@@err_name@@" class="field-error" hidden></div>
      @@intro@@
      <p class="hint" id="@@signup_hint@@">@@signup_default@@</p>
      ...
      <div id="@@auth_choice@@" hidden>
        <label id="@@auth_choice_label@@">Способ подключения</label>
        <div id="@@auth_options@@" class="prov" role="group" aria-labelledby="@@auth_choice_label@@"></div>
        <p class="hint">У этого провайдера несколько способов подключения — выберите один.</p>
      </div>

      <div id="@@api_key_block@@" hidden>
        <div class="field-row">
          <div class="field">
            <label for="@@api_key@@">Ключ API провайдера</label>
            <input id="@@api_key@@" name="@@api_key@@" type="password" autocomplete="off">
            <div id="@@api_key_saved_note@@" class="field-note" hidden></div>
            ...
            <div class="verdict-slot"><div id="@@key_verdict@@" class="verdict" hidden></div></div>
            <div id="@@err_api_key@@" class="field-error" hidden></div>
          </div>
          <div class="hint" id="@@key_hint@@">Ключ выдаёт сам провайдер после регистрации по ссылке слева.</div>
        </div>
        <div class="field-row">
          <div class="field">
            ...
            <!-- Spec B3: "получить список моделей" button is gone — the
                 ...
                 <option> that reveals @@model_custom@@ instead of the
                 field sitting there permanently. -->
            <select id="@@model@@" name="@@model@@">
              <option value="">по умолчанию</option>
              <option value="__custom_model__">Ввести вручную…</option>
            </select>
            <input id="@@model_custom@@" name="@@model_custom@@" type="text" autocomplete="off" placeholder="имя модели" hidden>
            <div id="@@models_error@@" class="verdict bad" hidden></div>
          </div>
        ...
        <details id="@@base_url_details@@" class="reveal">
          <summary>Адрес API — изменить</summary>
          <div class="field-row">
            <div class="field">
              <label for="@@base_url@@">Адрес API</label>
              <input id="@@base_url@@" name="@@base_url@@" type="text" autocomplete="off">
            </div>
            <div class="hint" id="@@base_url_hint@@">Обычно менять не нужно — подставляется автоматически для выбранного провайдера.</div>
          </div>
        </details>
      </div>
      ...
      <!-- Owner feedback (live walkthrough): a .field-row split the button
           ...
           label+input pair. -->
      <div id="@@device_block@@" hidden>
        <button type="button" id="@@device_start@@">Войти по аккаунту</button>
        ...
        <div id="@@device_info@@" hidden>
          <p class="hint">Откройте <a id="@@device_url@@" href="#" target="_blank" rel="noopener"></a> и введите код:</p>
          <p id="@@device_code@@" class="@@device_code@@"></p>
          <p id="@@device_status@@" class="@@device_status@@ hint"></p>
        </div>
        <div id="@@err_device_login@@" class="field-error" hidden></div>
        <div id="@@device_model_block@@" hidden>
          <div class="field-row">
            <div class="field">
              <label for="@@model_device@@">Модель</label>
              <select id="@@model_device@@" name="@@model_device@@">
                <option value="">по умолчанию</option>
                <option value="__custom_model__">Ввести вручную…</option>
              </select>
              <input id="@@model_device_custom@@" name="@@model_device_custom@@" type="text" autocomplete="off" placeholder="имя модели" hidden>
            </div>
...
# Имена элементов слота -> идентификаторы в DOM. Ключи обоих словарей
# обязаны совпадать (проверяется при сборке ниже), иначе один слот тихо
# останется без элемента.
_PRIMARY_SLOT_IDS: dict[str, str] = {
    "block": "provider-block",
    "group_label": "provider_group_label",
    "group": "provider_group",
    "current_hint": "provider-current-hint",
    "err_name": "err_provider_name",
    "signup_hint": "provider-signup-hint",
    "auth_choice": "provider-auth-choice",
    "auth_choice_label": "provider-auth-choice-label",
    "auth_options": "provider-auth-options",
    "api_key_block": "provider-api-key-block",
    "api_key": "provider_api_key",
    "api_key_saved_note": "provider_api_key_saved_note",
    "key_verdict": "key-verdict",
    "err_api_key": "err_provider_api_key",
    "key_hint": "provider-key-hint",
    "model": "provider_model",
    "model_custom": "provider_model_custom",
    "models_error": "models-fetch-error",
    "base_url_details": "provider-base-url-details",
    "base_url": "provider_base_url",
    "base_url_hint": "provider-base-url-hint",
    "device_block": "provider-device-code-block",
    "device_start": "device-login-start",
    "device_info": "device-login-info",
    "device_url": "device-login-url",
    "device_code": "device-login-code",
    "device_status": "device-login-status",
    "err_device_login": "err_device_login",
    "device_model_block": "device-model-block",
    "model_device": "provider_model_device",
    "model_device_custom": "provider_model_device_custom",
}

# Запасной слот: те же имена, префикс ``fallback``. Два имени обязаны
# остаться ровно такими — ``err_fallback_name`` и ``err_fallback_api_key``:
# на них завязан FIELD_MAP на клиенте, который сопоставляет ответ сервера
# (``fallback.name`` / ``fallback.api_key``) с полем, куда подсветить
# ошибку.
_FALLBACK_SLOT_IDS: dict[str, str] = {
    "block": "fallback-block",
    "group_label": "fallback_group_label",
    "group": "fallback_group",
    "current_hint": "fallback-current-hint",
    "err_name": "err_fallback_name",
    "signup_hint": "fallback-signup-hint",
    "auth_choice": "fallback-auth-choice",
    "auth_choice_label": "fallback-auth-choice-label",
    "auth_options": "fallback-auth-options",
    "api_key_block": "fallback-api-key-block",
    "api_key": "fallback_api_key",
    "api_key_saved_note": "fallback_api_key_saved_note",
    "key_verdict": "fallback-key-verdict",
    "err_api_key": "err_fallback_api_key",
    "key_hint": "fallback-key-hint",
    "model": "fallback_model",
    "model_custom": "fallback_model_custom",
    "models_error": "fallback-models-fetch-error",
    "base_url_details": "fallback-base-url-details",
    "base_url": "fallback_base_url",
    "base_url_hint": "fallback-base-url-hint",
    "device_block": "fallback-device-code-block",
    "device_start": "fallback-device-login-start",
    "device_info": "fallback-device-login-info",
    "device_url": "fallback-device-login-url",
    "device_code": "fallback-device-login-code",
    "device_status": "fallback-device-login-status",
    "err_device_login": "err_fallback_device_login",
    "device_model_block": "fallback-device-model-block",
    "model_device": "fallback_model_device",
    "model_device_custom": "fallback_model_device_custom",
}

assert set(_PRIMARY_SLOT_IDS) == set(_FALLBACK_SLOT_IDS)

# Текст, который у слотов разный. Вводный абзац запасного слота называет
# последствие прямо: отказ основного провайдера — это молчащий бот до
# ручного вмешательства, а вмешаться клиент может только через мастер.
_FALLBACK_INTRO_HTML = (
    '<p class="hint">Если основной провайдер откажет — закончатся средства '
    "или он станет недоступен, — бот перестанет отвечать, пока вы не "
    "вмешаетесь. Запасной провайдер с отдельным ключом снимает этот риск. "
    "Настраивать необязательно.</p>"
)


def _provider_block_html(ids: dict, *, section_title: str, intro: str, signup_default: str) -> str:
    """Разметка одного слота провайдера по карте идентификаторов."""
    out = _PROVIDER_BLOCK_TMPL
    for token, value in ids.items():
        out = out.replace("@@%s@@" % token, value)
    out = out.replace("@@section_title@@", section_title)
    # Пустой вводный абзац убирается ВМЕСТЕ со своей строкой: иначе у
    # основного слота остаётся строка из одних пробелов, и разметка
    # перестаёт совпадать с прежней байт в байт.
    out = out.replace("      @@intro@@\n", (intro + "\n") if intro else "")
    out = out.replace("@@signup_default@@", signup_default)
    if "@@" in out:
        raise AssertionError("незаполненный токен в шаблоне слота: %r" % out[out.index("@@"):][:40])
    return out
...
<section id="success" hidden>
  ...
  <!-- Спека 15: тот же прогон проверок, что делает кнопка на /support,
       выполняется ещё раз в конце "Готово" (app.py's _run_submit) —
       результат (одна из готовых фраз build_client_report()) дописывается
       сюда, а не проваливает отправку. См. #support-check-notice в
       doSubmit()'s own script. -->
  <p id="support-check-notice" class="hint" hidden></p>
</section>
"""
...
# Основной слот собирается первым и обязан совпасть с прежней разметкой
# байт в байт; запасной — тот же шаблон с другой картой имён.
_MAIN_FORM_HTML = _MAIN_FORM_HTML.replace(
    "@@PROVIDER_BLOCK@@",
    _provider_block_html(
        _PRIMARY_SLOT_IDS,
        section_title="Провайдер модели",
        intro="",
        signup_default="Чей ключ/аккаунт будете использовать для модели.",
    ),
).replace(
    "@@FALLBACK_BLOCK@@",
    _provider_block_html(
        _FALLBACK_SLOT_IDS,
        section_title="Запасной провайдер",
        intro=_FALLBACK_INTRO_HTML,
        signup_default="Чей ключ/аккаунт использовать, если основной откажет.",
    ),
)
...
(function () {
  ...
  var state = {
    ...
    // Выбранная группа/строка, раскрытость списка и «показать остальные»
    // живут ТЕПЕРЬ В СЛОТЕ (makeProviderSlot), а не здесь: блоков
    // провайдера на странице два — основной и запасной, — и одно общее
    // поле означало бы, что выбор в одном затирает выбор в другом.
  ...
  function handleAuthLost() {
    PROVIDER_SLOTS.forEach(stopDevicePoll);
  ...
  byId("proxy").addEventListener("input", function () {
    ...
    // Сброс касается ОБОИХ слотов провайдера: ключи основного и
    // запасного проверяются через один и тот же прокси, и вердикт,
    // заработанный через старый, одинаково перестаёт быть правдой.
    PROVIDER_SLOTS.forEach(function (slot) {
      setHidden(slot.ids.key_verdict, true);
      slot.keyCheckSeq++;
      slot.keyCheckSettled = false;
    });
  ...
  // ---- Слоты блока провайдера ----------------------------------------
  //
  // Основной и запасной провайдер — это ОДИН И ТОТ ЖЕ блок, отрисованный
  // дважды. Разметка приезжает из общего шаблона (см. python-сторону,
  // _provider_block_html), код ниже параметризован слотом: `slot.ids`
  // говорит, в какие элементы писать, `slot.chosenRow`/`slot.keyCheckSeq`
  // и прочее — состояние ровно этого слота.
  //
  // Зачем. Владелец 2026-09-10, на живой машине: «выбираешь запасного
  // провайдера, и там всё так же раскрывается, как и в первом разе». До
  // этого запасной слот был одиноким <select> плюс полем ключа: ни
  // модели, ни адреса API, ни автопроверки ключа, ни описаний, ни входа
  // по аккаунту. Упор был конкретный — основным выбран ChatGPT, запасным
  // нужен GLM, у GLM два варианта с разными адресами, и поменять адрес
  // негде.
  //
  // Каждая переменная последовательности (keyCheckSeq, modelsFetchSeq) и
  // таймер опроса живут В СЛОТЕ, а не в модуле: два блока проверяют ключи
  // и тянут списки моделей независимо, и общий счётчик означал бы, что
  // ответ одного слота гасит ответ другого.
  var PROVIDER_SLOT_IDS = @@SLOT_IDS@@;

  // Псевдогруппа необязательного слота. Живёт только на клиенте и
  // наружу не уезжает: выбранной строки у неё нет, значит slotPayload()
  // вернёт null, а сервер трактует отсутствующий fallback как «не
  // трогать» — ровно то, что обещает описание строки.
  var NO_SLOT_GROUP_ID = "__none__";
  var NO_SLOT_GROUP = {
    group_id: NO_SLOT_GROUP_ID,
    display_name: "Не настраивать",
    description_ru: "Запасной провайдер не изменится: если он уже настроен — останется как есть.",
    variants: [],
    recommended: false,
  };

  function makeProviderSlot(key, ids) {
    var optional = key === "fallback";
    return {
      key: key,
      ids: ids,
      // Необязательный слот стартует на своей строке «не настраивать» и
      // СВЁРНУТЫМ: иначе шаг 4 открывался бы двумя полноразмерными
      // списками провайдеров подряд.
      chosenGroupId: optional ? NO_SLOT_GROUP_ID : null,
      chosenRow: null,
      // Picker стартует раскрытым (ничего не выбрано); pick() схлопывает
      // его до одной строки, ссылка «выбрать другого» раскрывает обратно.
      pickerOpen: !optional,
      listExpanded: false,
      keyCheckSeq: 0,
      keyCheckSeqInFlight: -1,
  ...
  var providerSlot = makeProviderSlot("provider", PROVIDER_SLOT_IDS.provider);
  var fallbackSlot = makeProviderSlot("fallback", PROVIDER_SLOT_IDS.fallback);
  var PROVIDER_SLOTS = [providerSlot, fallbackSlot];

  // Сохранённое состояние ИМЕННО ЭТОГО слота: основной читает
  // current.provider, запасной — current.fallback (первый элемент цепочки
  // fallback_providers, см. _current_fallback() на сервере). Раньше
  // сервер состояние запасного не отдавал вовсе, и возвращавшийся клиент
  // не видел, что у него настроено.
  function slotCurrent(slot) {
    var current = state.current || {};
    return (slot.key === "fallback" ? current.fallback : current.provider) || {};
  }

  // Запасной слот — необязательный: у него в списке провайдеров есть
  // строка «не использовать», у основного её нет.
  function slotIsOptional(slot) { return slot.key === "fallback"; }

  function renderProviderSlots() {
    PROVIDER_SLOTS.forEach(function (slot) { renderProviderGroupOptions(slot); });
  }
  ...
  // (slot.chosenRow) is what api_key/device_code rendering and
  ...
  function providerGroupFor(groupId) {
    if (groupId === NO_SLOT_GROUP_ID) return NO_SLOT_GROUP;
  ...
  function renderProviderRow(slot, group) {
    var row = document.createElement("div");
    row.className = "p" + (slot.chosenGroupId === group.group_id ? " sel" : "");
    row.tabIndex = 0;
    ...
    function pick() {
      slot.chosenGroupId = group.group_id;
      ...
      // renderProviderGroupOptions(slot) switch into the collapsed
      // "one chosen row + изменить" view below.
      slot.pickerOpen = false;
      renderProviderGroupOptions(slot);
      onProviderGroupChange(slot, group.group_id);
    }
  ...
  function updateProviderCurrentHint(slot) {
    var currentName = (slotCurrent(slot) || {}).name || "";
    var hint = byId(slot.ids.current_hint);
  ...
  // (slot.chosenGroupId set) and the client hasn't explicitly asked to
  // change it (slot.pickerOpen), the picker collapses to just
  ...
  function renderProviderGroupOptions(slot) {
    var list = byId(slot.ids.group);
    clearContainer(list);

    var chosenGroup = slot.chosenGroupId ? providerGroupFor(slot.chosenGroupId) : null;
    if (chosenGroup && !slot.pickerOpen) {
      list.appendChild(renderProviderRow(slot, chosenGroup));
      var change = document.createElement("div");
      change.className = "more";
      var changeLink = document.createElement("a");
      changeLink.href = "#";
      changeLink.textContent = slot.chosenGroupId === NO_SLOT_GROUP_ID
        ? "Выбрать запасного провайдера"
        : "Выбрать другого провайдера";
      changeLink.addEventListener("click", function (e) {
        e.preventDefault();
        slot.pickerOpen = true;
        renderProviderGroupOptions(slot);
      });
      change.appendChild(changeLink);
      list.appendChild(change);
      updateProviderCurrentHint(slot);
      return;
    }

    // Свой эндпоинт клиента показывается только в ОСНОВНОМ слоте: его
    // ключ живёт в model.api_key — одном на конфиг, и второму адресу там
    // места нет (для этого в апстриме есть список custom_providers, это
    // отдельная задача). Полурабочую строку клиенту не показываем.
    var groups = state.providerGroups.filter(function (g) {
      if (!slotIsOptional(slot)) return true;
      return !g.variants.some(function (v) { return v.endpoint_from_client; });
    });
    var recommended = groups.filter(function (g) { return g.recommended; });
    var rest = groups.filter(function (g) { return !g.recommended; });

    // Необязательный слот: первой строкой — отказ. Он такой же .p-строкой,
    // а не отдельной ссылкой сбоку, потому что это такой же выбор, как и
    // любой провайдер в списке, и вести себя должен так же.
    if (slotIsOptional(slot)) list.appendChild(renderProviderRow(slot, NO_SLOT_GROUP));
    recommended.forEach(function (g) { list.appendChild(renderProviderRow(slot, g)); });

    if (slot.listExpanded) {
      rest.forEach(function (g) { list.appendChild(renderProviderRow(slot, g)); });
    } else if (rest.length) {
  ...
  function onProviderGroupChange(slot, groupId) {
    var group = providerGroupFor(groupId);
    var authChoice = byId(slot.ids.auth_choice);
    var authOptions = byId(slot.ids.auth_options);
    clearContainer(authOptions);

    if (!group) {
      authChoice.hidden = true;
      onProviderChange(slot, null);
      return;
    }

    if (group.variants.length <= 1) {
      authChoice.hidden = true;
      onProviderChange(slot, group.variants[0] || null);
      return;
    }

    authChoice.hidden = false;
    // Same clickable-card pattern as renderProviderRow(slot) above — a native
  ...
  // IS — reuse the exact same source renderKeyVerdict(slot)'s own "Модель по
  // умолчанию: …" already trusts (row.fallback_models[0]), so the two
  // never disagree.
  function defaultModelOptionLabel(row) {
    // У своего эндпоинта клиента каталога моделей нет вовсе
    // (fallback_models пуст) и подставить по умолчанию НЕЧЕГО — сервер
    // такую отправку и не примет. Поэтому первая опция здесь не «по
    // умолчанию», а прямое требование выбрать.
    if (row && row.endpoint_from_client) return "— выберите модель —";
    var defaultModel = (row && row.fallback_models && row.fallback_models[0]) || "";
    return defaultModel ? "по умолчанию (" + defaultModel + ")" : "по умолчанию";
  }

  function onProviderChange(slot, row) {
    ...
    // they never touched and can submit it blind (see slot.keyCheckSeq/
    // slot.modelsFetchSeq's own declarations below for the invariant this
    ...
    slot.modelsFetchSeq++;
    slot.chosenRow = row;
    var apiBlock = byId(slot.ids.api_key_block);
    var deviceBlock = byId(slot.ids.device_block);
    var signupHint = byId(slot.ids.signup_hint);
    ...
    setHidden(slot.ids.key_verdict, true);
    setHidden(slot.ids.models_error, true);
    ...
    byId(slot.ids.api_key).value = "";

    if (!row) {
      apiBlock.hidden = true;
      deviceBlock.hidden = true;
      resetDeviceLoginUI(slot);
    ...
    if (row.kind === "device_code") {
      ...
      // this branch runs — loadDeviceModels(slot) (below, or the return-mode
      // branch right under this) repopulates it fresh whenever a login is
      // already known-good.
      var deviceModelSelect = byId(slot.ids.model_device);
      ...
      var deviceModelCustom = byId(slot.ids.model_device_custom);
      ...
      var currentProviderDC = slotCurrent(slot) || {};
      if (currentProviderDC.name === row.name && currentProviderDC.device_login_ok) {
        showDeviceLoginSuccess(slot, "✓ Вход выполнен (сохранённая сессия)");
        loadDeviceModels(slot, row);
      }
    } else {
      deviceBlock.hidden = true;
      resetDeviceLoginUI(slot);
      ...
      var currentProvider = slotCurrent(slot) || {};
      ...
      byId(slot.ids.base_url).value = baseUrlValue;
      ...
      // У своего эндпоинта адрес — обязательное поле, а не тонкая
      // настройка: подставлять нечего, каталог про этот сервер не знает
      // ничего. Прятать его за «изменить» значило бы спрятать половину
      // того, ради чего строку и выбрали.
      var baseUrlDetails = byId(slot.ids.base_url_details);
      if (baseUrlDetails) {
        baseUrlDetails.open = !!row.endpoint_from_client
          || !!(isCurrentProvider && currentProvider.base_url);
        var summary = baseUrlDetails.querySelector("summary");
        if (summary) {
          summary.textContent = row.endpoint_from_client
            ? "Адрес API вашего сервера"
            : "Адрес API — изменить";
        }
      }
      // Подсказка под полем: у обычного провайдера адрес подставляется из
      // каталога и менять его почти никогда не нужно; у своего сервера
      // подставлять НЕЧЕГО, и та же фраза была бы прямой ложью.
      var baseUrlHint = byId(slot.ids.base_url_hint);
      if (baseUrlHint) {
        baseUrlHint.textContent = row.endpoint_from_client
          ? "Адрес, по которому отвечает ваш сервер, вместе с версией API — например https://llm.example.org/v1."
          : "Обычно менять не нужно — подставляется автоматически для выбранного провайдера.";
      }
      var modelSelect = byId(slot.ids.model);
      // Genuinely switching catalogs here (a different provider's model
      // list) — this IS the one place a full reset is correct, unlike
      // the live-probe merge in fetchLiveModelsForRow(slot) below.
      ...
      var modelCustom = byId(slot.ids.model_custom);
  ...
  // the provider row above (renderProviderRow(slot)) — with the signup URL
  ...
  // scheme-checked posture as setVerificationLink(slot) below — our own catalog
  // only ever produces https:// signup_url values, this is defense in
  // depth, not a case expected to actually trigger).
  function renderSignupHint(slot, row) {
    var el = byId(slot.ids.signup_hint);
    clearContainer(el);
  ...
  // which is NOT a pass; see renderKeyVerdict(slot)'s own comment on why that
  ...
  // independently issuing a fresh live probe. slot.keyCheckSeqInFlight (set
  // while a probe is outstanding, cleared once it settles) plus
  // slot.keyCheckSettled (true once a response has actually rendered for the
  ...
  function renderKeyVerdict(slot, data, row) {
    ...
    if (data.reachable && data.ok) {
      var defaultModel = (row && row.fallback_models && row.fallback_models[0]) || "";
      setVerdict(slot.ids.key_verdict, "ok", "Ключ принят.", defaultModel ? " Модель по умолчанию: " + defaultModel : "");
      return;
    }
    if (data.message) {
      ...
      setVerdict(slot.ids.key_verdict, "bad", data.message, "");
      return;
    }
    ...
    //
    // У своего эндпоинта клиента причина ДРУГАЯ, и общий текст тут врёт:
    // проверка существует и была выполнена — по введённому адресу, — но
    // сервер не ответил. Сказать «не проверяется автоматически» значило
    // бы списать на нас чужую недоступность и увести клиента от
    // единственного, что он может проверить, — адреса.
    if (row && row.endpoint_from_client) {
      setVerdict(
        slot.ids.key_verdict,
        "wait",
        "Ваш сервер не ответил.",
        " Ключ сохранится, но проверить его не удалось — проверьте адрес и доступность сервера с этой машины."
      );
      return;
    }
    setVerdict(slot.ids.key_verdict, "wait", "Ключ сохранён — этот провайдер не проверяется автоматически.", "");
  }

  function runProviderKeyCheck(slot) {
    var row = slot.chosenRow;
    if (!row || row.kind !== "api_key") return;
    var value = (byId(slot.ids.api_key).value || "").trim();
    if (!value) {
    ...
    var proxyValue = (byId("proxy").value || "").trim();
    var seq = ++slot.keyCheckSeq;
    slot.keyCheckSeqInFlight = seq;
    var rowAtCallTime = row;
    // Свой эндпоинт проверяется по введённому адресу: переменной
    // окружения у него нет, и без адреса сервер ответил бы «ок, не
    // проверяли» — то есть клиент узнал бы об опечатке в ключе только
    // когда бот впервые не ответит.
    var endpointUrl = row.endpoint_from_client ? (byId(slot.ids.base_url).value || "").trim() : "";
    jsonFetch("/api/check/key", { method: "POST", body: JSON.stringify({ env_var: envVar, value: value, proxy: proxyValue, base_url: endpointUrl }), signal: checkTimeoutSignal() })
      .then(function (res) { return res.json(); })
      .then(function (data) {
        // Finding 7 (review 2026-08-26, owner-approved fix): only clear
        ...
        // check just set.
        if (seq === slot.keyCheckSeqInFlight) slot.keyCheckSeqInFlight = -1;
        if (seq !== slot.keyCheckSeq) return;
        slot.keyCheckSettled = true;
        renderKeyVerdict(slot, data, rowAtCallTime);
  ...
  function maybeRunProviderKeyCheck(slot) {
    var row = slot.chosenRow;
    if (!row || row.kind !== "api_key") return;
    if (slot.keyCheckSettled) return;
    if (slot.keyCheckSeqInFlight === slot.keyCheckSeq) return;
    var value = (byId(slot.ids.api_key).value || "").trim();
    if (!value) return;
    runProviderKeyCheck(slot);
  }
  ...
  // called from runProviderKeyCheck(slot) above once a typed key's check
  // round-trip answers, never from a click. slot.modelsFetchSeq discards a
  // stale response the same way slot.keyCheckSeq/telegramCheckSeq/
  // proxyCheckSeq already do elsewhere in this file (fast edits can fire
  // more than one of these before the first reply lands).

  function fetchLiveModelsForRow(slot, row) {
    if (!row || row.kind !== "api_key") return;
    var seq = ++slot.modelsFetchSeq;
    setHidden(slot.ids.models_error, true);
    jsonFetch("/api/models", {
      method: "POST",
      body: JSON.stringify({
        provider: row.name,
        api_key: byId(slot.ids.api_key).value,
        base_url: byId(slot.ids.base_url).value,
      ...
      .then(function (res) { return res.json(); })
      .then(function (data) {
        if (seq !== slot.modelsFetchSeq) return;
        ...
        addModelOptions(byId(slot.ids.model), data.models);
      })
      .catch(function (err) {
        if (isAuthError(err) || isRateLimitedError(err)) return;
        if (seq !== slot.modelsFetchSeq) return;
        ...
        setVerdict(slot.ids.models_error, "bad", "Не удалось получить список моделей у провайдера.", " Можно выбрать «Ввести вручную» в списке моделей.");
      });
  }
  ...
  function stopDevicePoll(slot) {
    if (slot.devicePollTimer) {
      clearInterval(slot.devicePollTimer);
      slot.devicePollTimer = null;
    }
  }

  function resetDeviceLoginUI(slot) {
    stopDevicePoll(slot);
    setHidden(slot.ids.device_info, true);
    setHidden(slot.ids.device_model_block, true);
    var btn = byId(slot.ids.device_start);
    btn.disabled = false;
    btn.textContent = "Войти по аккаунту";
    byId(slot.ids.device_status).textContent = "";
    var err = byId(slot.ids.err_device_login);
    err.hidden = true;
    err.textContent = "";
  }

  function showDeviceLoginSuccess(slot, message) {
    setHidden(slot.ids.device_info, false);
    byId(slot.ids.device_status).textContent = message;
    var btn = byId(slot.ids.device_start);
    btn.disabled = false;
    btn.textContent = "Войти повторно";
  }

  function loadDeviceModels(slot, row) {
    setHidden(slot.ids.device_model_block, false);
    jsonFetch("/api/models", {
      method: "POST",
      ...
      .then(function (res) { return res.json(); })
      .then(function (data) { addModelOptions(byId(slot.ids.model_device), data.models); })
      .catch(function () {});
  }
  ...
  function setVerificationLink(slot, url) {
    var link = byId(slot.ids.device_url);
    if (isHttpUrl(url)) {
      link.href = url;
  ...
  function startDevicePoll(slot, row, loginId) {
    stopDevicePoll(slot);
    var startedAt = Date.now();
    slot.devicePollTimer = setInterval(function () {
      if (Date.now() - startedAt > DEVICE_POLL_TIMEOUT_MS) {
        stopDevicePoll(slot);
        byId(slot.ids.device_status).textContent = "";
        var timeoutErr = byId(slot.ids.err_device_login);
        timeoutErr.textContent = "Время ожидания входа истекло. Нажмите «Войти по аккаунту» ещё раз.";
        timeoutErr.hidden = false;
        var timeoutBtn = byId(slot.ids.device_start);
      ...
      jsonFetch("/api/device/status")
        ...
        .then(function (data) {
          ...
          if (data.state === "ok") {
            stopDevicePoll(slot);
            showDeviceLoginSuccess(slot, "✓ Вход выполнен");
            loadDeviceModels(slot, row);
        ...
        .catch(function (err) {
          ...
          if (isAuthError(err) || isRateLimitedError(err)) stopDevicePoll(slot);
        });
    }, 3000);
  }


  // Слушатели одного слота. Раньше регистрировались один раз на
  // модуль — теперь их две пары, по одной на слот, и вешает их
  // renderProviderSlots() ниже сразу после загрузки формы.
  function wireProviderSlot(slot) {
    byId(slot.ids.api_key).addEventListener("input", function () {
      setHidden(slot.ids.key_verdict, true);
      slot.keyCheckSeq++;
      slot.keyCheckSettled = false;
    ...
    // Свой эндпоинт: ключ проверяется ПО АДРЕСУ, поэтому правка адреса
    // обязана перезапустить проверку. Иначе клиент, набравший ключ
    // раньше адреса (естественный порядок — поле ключа выше), получил бы
    // молчание навсегда: первая проверка ушла с пустым адресом и
    // вернулась «не проверяли», а второй никто не запускает.
    byId(slot.ids.base_url).addEventListener("change", function () {
      if (!(slot.chosenRow && slot.chosenRow.endpoint_from_client)) return;
      setHidden(slot.ids.key_verdict, true);
      slot.keyCheckSeq++;
      slot.keyCheckSettled = false;
      maybeRunProviderKeyCheck(slot);
    });
    byId(slot.ids.api_key).addEventListener("change", function () { maybeRunProviderKeyCheck(slot); });
    byId(slot.ids.api_key).addEventListener("blur", function () { maybeRunProviderKeyCheck(slot); });
    byId(slot.ids.device_start).addEventListener("click", function () {
      var row = slot.chosenRow;
      if (!row) return;
      var btn = byId(slot.ids.device_start);
      btn.disabled = true;
      ...
      jsonFetch("/api/device/start", {
        ...
        .then(function (data) {
          setHidden(slot.ids.device_info, false);
          setVerificationLink(slot, data.verification_url);
          byId(slot.ids.device_code).textContent = data.user_code;
          byId(slot.ids.device_status).textContent = "Ожидаем подтверждение…";
          btn.textContent = "Входим…";
          startDevicePoll(slot, row, data.login_id);
        })
  ...
  PROVIDER_SLOTS.forEach(wireProviderSlot);
  ...
  function renderPrefill() {
    ...
    // Оба слота провайдера — одинаково: сохранённый ключ никогда не
    // echo-ится в поле, вместо него нейтральная отметка «что-то сохранено».
    PROVIDER_SLOTS.forEach(function (slot) {
      applySecretPlaceholder(slot.ids.api_key, (slotCurrent(slot) || {}).api_key);
    });
  ...
  // ---- Идентичность строки веб-каталога vs имя бэкенда ----------------
  //
  // Значение <option> в блоках «Поиск в интернете» / «Чтение страниц» —
  // это ИДЕНТИЧНОСТЬ СТРОКИ каталога, а не имя бэкенда. Две разные строки
  // законно несут один бэкенд: «Firecrawl» (облако, FIRECRAWL_API_KEY) и
  // «Firecrawl Self-Hosted» (FIRECRAWL_API_URL) — оба `firecrawl`. Пока
  // значением было имя бэкенда, в списке получались два <option> с
  // ОДИНАКОВЫМ value: `select.value` отдаёт первый, и клиент, выбравший
  // облачный Firecrawl, получал поле «Адрес сервера» вместо поля ключа.
  // Прежнее правило «побеждает первая строка» лечило симптом (не давало
  // сабмиту уехать в чужую переменную), но выбор второй строки так и
  // оставался физически невозможным. Имя строки внутри блока уникально —
  // инвариант закреплён тестом на живом каталоге.
  function webRowChoiceValue(row) { return row.name; }

  // Обратный перевод для возврата клиента: в config.yaml лежит ИМЯ
  // БЭКЕНДА, а выбрать надо строку. Если строк с таким бэкендом
  // несколько, побеждает та, чья переменная окружения действительно
  // сохранена (адрес self-hosted против ключа облака) — то же правило,
  // что и на сервере в _current_web_capability_env(). Незнакомый бэкенд
  // возвращается как есть: pickPreselected() покажет «настроено вручную».
  function webRowValueForSavedBackend(rows, savedBackend, currentEnv) {
    if (!savedBackend) return "";
    var matches = rows.filter(function (r) { return r.web_backend === savedBackend; });
    if (!matches.length) return savedBackend;
    if (matches.length > 1 && currentEnv && currentEnv.env_var) {
      for (var i = 0; i < matches.length; i++) {
        var envs = matches[i].env_vars || [];
        if (envs.length && envs[0].key === currentEnv.env_var) return webRowChoiceValue(matches[i]);
      }
    }
    return webRowChoiceValue(matches[0]);
  }

  // Строка блока по значению её <option>. Общая для рендера и для сборки
  // payload, чтобы обе стороны не разъезжались в правиле сопоставления.
  function webRowByChoiceValue(category, value) {
    var block = toolBlockFor(category);
    var rows = (block && block.rows) || [];
    for (var i = 0; i < rows.length; i++) {
      if (webRowChoiceValue(rows[i]) === value) return rows[i];
    }
    return null;
  }
  ...
  function renderSearchBlock() {
    ...
    var options = rows.map(function (row) {
      return {
        value: webRowChoiceValue(row),
        label: rowOptionLabel(row),
        recommended: !!row.recommended,
      };
    });
    // Значение опции — идентичность строки (см. webRowChoiceValue), а не
    // имя бэкенда, поэтому коллизии «две строки, один value» здесь больше
    // нет и правило «побеждает первая» не нужно: ключ уникален по
    // построению.
    var rowByValue = {};
    rows.forEach(function (row) { rowByValue[webRowChoiceValue(row)] = row; });
    var picked = pickPreselected(options, webRowValueForSavedBackend(rows, current.search_backend, current.search_env));
  ...
  function renderExtractBlock() {
    ...
    // Тот же переход на идентичность строки, что и в renderSearchBlock():
    // «Чтение страниц» — это как раз блок, где ОБЕ строки Firecrawl живут
    // одновременно и законно (облачная и self-hosted), так что выбор
    // второй обязан работать.
    var rowByValue = {};
    rows.forEach(function (row) { rowByValue[webRowChoiceValue(row)] = row; });

    var savedValue = webRowValueForSavedBackend(rows, current.extract_backend || "", current.extract_env);
  ...
  function extractBackendChoiceValue() {
    ...
    if (value && value !== "off") {
      var row = webRowByChoiceValue("web_extract", value);
      return row ? row.web_backend : value;
    }
  ...
  function extractEnvPayload() {
    var select = byId("extract_choice");
    var value = select ? select.value : "";
    // Та же сборка по выбранной СТРОКЕ, что и в searchEnvPayload().
    // "off" строкой каталога не является — вернётся null ниже.
    var row = value && value !== "off" ? webRowByChoiceValue("web_extract", value) : null;
    var envs = (row && row.env_vars) || [];
  ...
  function modelFieldValue(slot) {
    ...
    var isDeviceCode = slot.chosenRow && slot.chosenRow.kind === "device_code";
    var customId = isDeviceCode ? slot.ids.model_device_custom : slot.ids.model_custom;
    var selectId = isDeviceCode ? slot.ids.model_device : slot.ids.model;
  ...
  function summaryModelValue() {
    var row = providerSlot.chosenRow;
    var providerLabel = "";
    var modelLabel = "";
    if (row) {
      providerLabel = row.display_name || row.name;
      modelLabel = modelFieldValue(providerSlot);
    } else {
  ...
  // Слот -> то, что уезжает на сервер. Одна функция на оба блока: поля
  // у них теперь одинаковые, и «запасной» отличается только тем, что
  // может быть пустым (строка «не использовать» в списке).
  //
  // Раньше запасной слот слал base_url и model ЖЁСТКО пустыми, хотя
  // сервер обе величины принимает и пишет в fallback_providers. Из-за
  // этого адрес провайдера с нестандартным endpoint (у GLM их два) было
  // физически негде задать.
  function slotPayload(slot) {
    var row = slot.chosenRow;
    if (!row) return null;
    var isDeviceCode = row.kind === "device_code";
    return {
      name: row.name,
      env_var: row.env_var || null,
      // У варианта со входом по аккаунту поля ключа и адреса лежат в
      // скрытом (и нетронутом) под-блоке api_key — читать их отсюда
      // значило бы утащить в payload остаток от ранее выбранного
      // варианта. Вход уже произошёл через /api/device/*, а настоящий
      // base_url сервер подставит сам, когда здесь пусто.
      api_key: isDeviceCode ? "" : (byId(slot.ids.api_key).value || "").trim(),
      base_url: isDeviceCode ? "" : byId(slot.ids.base_url).value,
      model: modelFieldValue(slot),
    };
  }

  function buildPayload() {
    var providerPayload = slotPayload(providerSlot);
    ...
    return {
      ...
      // Основной слот обязателен: если ничего не выбрано, уезжает пустое
      // имя — ровно как и раньше, ворота шага 4 такой отправки не пустят.
      provider: providerPayload || { name: "", env_var: null, api_key: "", base_url: "", model: "" },
      fallback: slotPayload(fallbackSlot),
  ...
  // Значение <select> — идентичность строки; наружу (в payload и в
  // config.yaml) уходит ИМЯ БЭКЕНДА этой строки.
  function searchBackendChoiceValue() {
    var select = byId("search_choice");
    var value = select ? select.value : "";
    if (!value) return "";
    var row = webRowByChoiceValue("web", value);
    // Строки нет в каталоге — значит это «настроено вручную» из
    // pickPreselected(): отдаём как есть, ровно как и раньше.
    return row ? row.web_backend : value;
  }
  ...
  function searchEnvPayload() {
    var select = byId("search_choice");
    // По ВЫБРАННОЙ СТРОКЕ, а не по имени бэкенда: иначе у двух строк с
    // одним бэкендом сабмит уезжал в переменную первой из них.
    var row = webRowByChoiceValue("web", select ? select.value : "");
    var envs = (row && row.env_vars) || [];
    if (!envs.length) return null;
  ...
  function pendingToolInstallNames() {
    ...
    // По выбранной СТРОКЕ, а не по имени бэкенда (см. webRowChoiceValue):
    // у двух строк с одним бэкендом установочный хук может быть только у
    // одной из них.
    var searchSelect = byId("search_choice");
    consider(webRowByChoiceValue("web", searchSelect ? searchSelect.value : ""));
    ...
    var extractSelect = byId("extract_choice");
    var extractValue = extractSelect ? extractSelect.value : "";
    consider(extractValue && extractValue !== "off" ? webRowByChoiceValue("web_extract", extractValue) : null);
    [
        ...
        return res
          ...
          .then(function (data) {
            ...
            if (data.ok) {
              ...
              // Spec 15: the same machine-health pass the /support page's
              // own button runs, executed once more at the end of "Готово"
              // (app.py's _run_submit -> _run_post_submit_support_pass).
              // A bad verdict never fails this response (ok is already
              // true here) -- it is just one more honest line on the
              // success screen, the same posture as tool_install_failures/
              // warnings above. support_check_message is one of
              // trix_support.build_client_report()'s three fixed Russian
              // sentences, or null when the pass itself could not run --
              // nothing is shown in that case, never a placeholder error.
              var supportCheckNotice = byId("support-check-notice");
              if (data.support_check_message) {
                supportCheckNotice.textContent = data.support_check_message;
                supportCheckNotice.hidden = false;
              } else {
                supportCheckNotice.textContent = "";
                supportCheckNotice.hidden = true;
              }
            } else {
...
def render_shell(*, title: str, rail_html: str, content_html: str, script: str = "") -> str:
    """The product's one page shell — ``wrap`` → ``canvas`` → (``rail`` +
    ``content``) — plus its one CSS constant (``_CSS``), factored out of
    ``render_page()`` so a second surface can render inside the exact
    same look without copying markup or styles into a second file.

    **Why a shared function instead of a shared constant.** ``_CSS`` alone
    would have been enough to make colors/fonts match, but the support
    page (spec 15) also needs the identical DOCTYPE/head/``.wrap``/
    ``.canvas``/rail-plus-content skeleton a client already associates
    with this product from the wizard — that structure, not just the
    palette, is what makes it read as "the same program" rather than a
    reskinned second one. Centralizing the *whole* shell here means an
    edit to this function or to ``_CSS`` automatically reaches every
    caller; a copy in ``support_view.py`` would silently drift the first
    time either file changed and nobody remembered the other one existed.

    ``rail_html`` and ``content_html`` are pre-rendered strings, not
    parameters this function builds from — callers own their own rail
    (``_rail_html`` for the wizard) and content (``_MAIN_FORM_HTML`` for
    the wizard, ``support_view.py``'s own markup for support) markup; this
    function only owns the parts that must never fork between the two:
    the document frame, ``_CSS``, and the wrap/canvas/rail/content
    structure. ``script`` is optional (support's own page has a much
    smaller inline script than the wizard's) but always wrapped in a
    ``<script>`` tag, even when empty, to keep this byte-for-byte
    equivalent to the wizard's own previous inline construction.
    """
    body = (
        '<div class="wrap">'
        '<div class="canvas">'
...
def _js_with_slot_ids() -> str:
    """Javascript страницы с подставленной картой имён элементов слота.

    Карта — ОДНА, питонова: та же, из которой собирается разметка обоих
    слотов. Вклеенная копия внутри javascript разъехалась бы с ней на
    первой же правке (и успела: добавление одного поля в шаблон уже
    потребовало бы правки в двух местах), а расхождение здесь означает
    молча неработающий элемент у одного из слотов.
    """
    ...
    return _JS.replace(
        "@@SLOT_IDS@@",
        _json.dumps(
            {"provider": _PRIMARY_SLOT_IDS, "fallback": _FALLBACK_SLOT_IDS},
            ensure_ascii=False,
            indent=2,
        ).replace("\n", "\n  "),
    )
...
def render_page(host: str | None = None, *, support_visible: bool = False) -> str:
    ...
    ``support_visible`` (spec 15) gates the rail's "Что-то не работает?"
    entry — see ``_rail_html``'s own docstring for the reasoning
    (``WizardState.is_completed()`` is the only real signal: before a
    first successful "Готово" there is nothing yet for a support pass to
    check). Defaults to ``False`` so this stays the exact page every
    existing pure/host-only caller already renders.
    """
    ...
    # Обновление показывается по тому же признаку, что и поддержка:
    # обе двери нужны вернувшемуся клиенту, а не тому, кто прямо сейчас
    # заполняет форму первый раз. Признак один и тот же — настройка уже
    # доведена до конца хотя бы однажды.
    rail_html = _rail_html(_rail_address(host), header_intro, support_visible=support_visible,
                           update_visible=support_visible)
    # #progress/#success live inside _MAIN_FORM_HTML's own string as
    # siblings AFTER </form> (never as descendants of <form id="main"> —
    # hiding the form via setHidden("main", true) must not also hide the
    # "Готово!" screen). No separate _PROGRESS_HTML/_SUCCESS_HTML
    # constants — they're part of the same triple-quoted block.
    return render_shell(
        title="Настройка Trix Agent",
        rail_html=rail_html,
        content_html=_MAIN_FORM_HTML,
        script=_js_with_slot_ids(),
    )
