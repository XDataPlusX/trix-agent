...
_MSG_INSTALLED = "Шлюз установлен и запущен"
...
def restart_gateway() -> dict:
    ...
    *before* either attempt runs — pass both to :func:`wait_bot_alive` so
    ...
    # Третья ступень: юнита нет вовсе. Модульный докстринг выше давно это
    # называет ("an uninstalled VM fails both ranks alike"), но вывод из
    # этого не был сделан, и на клиентской машине он смертелен.
    #
    # Установщик ставит юнит шлюза ТОЛЬКО интерактивно: он требует
    # открывающийся /dev/tty и задаёт вопрос "install the gateway as a
    # background service?" (scripts/install.sh). Рецепт исполняется без
    # терминала, поэтому установщик выходит там строкой "Gateway setup
    # skipped (no terminal available)" — и юнита на машине не появляется.
    #
    # Дальше клиент проходит мастер, мастер доходит сюда, обе ступени
    # падают, и он читает "Не удалось перезапустить или запустить шлюз".
    # Починить это ему нечем: SSH и консоли у него нет по устройству
    # продукта, мастер — единственная дверь. Проверено исполнением
    # 2026-09-04 на trix-testing6.ru: после ручного `gateway install` тот
    # же самый конфиг поднялся и бот подключился к Telegram.
    #
    # Ставим сами. Это не расширение области мастера, а завершение того,
    # что он и так делает: он уже пишет .env и config.yaml и уже отвечает
    # за то, чтобы после этого продукт работал.
    try:
        install_result = _run_gateway_cli("install")
    except (OSError, subprocess.SubprocessError):
        install_result = None
