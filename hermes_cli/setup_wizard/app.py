...
from hermes_cli.setup_wizard.providers_view import (
    CLIENT_SUPPLIED_ENDPOINT_PROVIDERS,
...
from hermes_cli.setup_wizard.update_view import register_update_routes
...
class _CheckKeyBody(BaseModel):
    ...
    value: str = ""
    proxy: str = ""
    # Адрес своего эндпоинта клиента. Нужен ровно для провайдера
    # ``custom``: переменной окружения у него нет, и без адреса проверять
    # ключ было бы не у кого. Для всех остальных строк поле игнорируется —
    # адрес берётся из курированной записи проверки.
    base_url: str = ""
...
def _current_web_capability_env(
    ...
    if backend:
        ...
        # Строк с одним и тем же ``web_backend`` может быть НЕСКОЛЬКО —
        # «Firecrawl» (облако, FIRECRAWL_API_KEY) и «Firecrawl Self-Hosted»
        # (FIRECRAWL_API_URL) обе несут ``firecrawl``. Брать первую
        # попавшуюся значит соврать возвращающемуся клиенту: мастер
        # показал бы поле адреса там, где на машине сохранён ключ облака.
        # Побеждает строка, чья переменная РЕАЛЬНО задана; если не задана
        # ни одна — первая, как и раньше. То же правило действует на
        # клиенте (``webRowValueForSavedBackend`` в page.py), и обе
        # стороны обязаны совпадать, иначе выбранная строка разъедется с
        # показанным полем.
        matches = [
            row
            for row in (block or {}).get("rows", [])
            if row.get("web_backend") == backend and (row.get("env_vars") or [])
        ]
        for row in matches:
            key = (row.get("env_vars") or [])[0].get("key")
            if key and get_env_value(key):
                env_var = key
                break
        else:
            if matches:
                env_var = (matches[0].get("env_vars") or [])[0].get("key")
...
def _current_provider_key(provider_name: str, env_var: str | None, model_cfg: dict) -> str | None:
    """Сохранённый ключ активного провайдера — из .env или из config.yaml.

    У всех провайдеров каталога ключ лежит в своей переменной окружения.
    Исключение одно — свой эндпоинт клиента (``custom``): у него
    переменной нет вовсе (``env_vars=()``), и рантайм читает ключ из
    ``model.api_key`` (источник ``model_config``,
    agent/credential_sources.py). Без этой ветки возвращающийся клиент
    видел бы «ключ не сохранён» при сохранённом ключе и, скорее всего,
    вводил бы его заново на каждом заходе.

    Возвращает сырое значение; маскирует уже вызывающий.
    """
    if provider_name in CLIENT_SUPPLIED_ENDPOINT_PROVIDERS:
        value = model_cfg.get("api_key") if isinstance(model_cfg, dict) else None
        return value or None
    return get_env_value(env_var) if env_var else None


def _current_fallback(config: dict) -> dict:
    """Сохранённый запасной провайдер — первый элемент цепочки
    ``fallback_providers`` в config.yaml (её же пишет apply_settings()).

    Форма ответа повторяет ``current.provider``: имя, адрес, модель,
    маскированный ключ и признак живого device-code входа. Клиент этим
    предзаполняет ровно тот же блок — см. page.py.

    Цепочка может быть длиннее одного элемента (её умеет писать CLI и
    руками правит администратор), но у мастера один слот, поэтому берётся
    ПЕРВЫЙ — тот же приём, что и ``_primary_backend`` для цепочки поиска:
    показать выбор клиента, а не потерять его.
    """
    chain = config.get("fallback_providers")
    entry = chain[0] if isinstance(chain, list) and chain and isinstance(chain[0], dict) else {}
    name = (entry.get("provider") or "").strip()
    env_var = _current_provider_env_var(name) if name else None
    device_login_ok = device_login_looks_active(name) if name in DEVICE_CODE_PROVIDERS else False
    return {
        "name": name,
        "base_url": (entry.get("base_url") or "") if name else "",
        "model": (entry.get("model") or "") if name else "",
        "api_key": _mask(get_env_value(env_var) if env_var else None),
        "device_login_ok": device_login_ok,
    }
...
def _current_state(config: dict, tools_rows: list[dict]) -> dict:
    ...
    return {
        ...
        "provider": {
            ...
            "api_key": _mask(_current_provider_key(provider_name, provider_env_var, model_cfg)),
            "device_login_ok": provider_device_login_ok,
        },
        # Запасной провайдер — та же форма, что и у основного, потому что
        # на странице это теперь ТОТ ЖЕ блок (см. page.py, слоты
        # provider/fallback). Пока блок был голым списком, читать его
        # состояние было незачем и /api/form его не отдавал вовсе: клиент,
        # вернувшийся в мастер, не видел ни выбранного запасного, ни его
        # модели, ни адреса — и любое повторное сохранение молча
        # превращало настроенный запасной в «не использовать».
        "fallback": _current_fallback(config),
...
_MSG_ENDPOINT_URL_REQUIRED = "Укажите адрес API вашего сервера."
_MSG_ENDPOINT_MODEL_REQUIRED = (
    "Укажите модель: у своего сервера каталог моделей нам неизвестен, "
    "подставить нечего."
)
...
def _run_submit(form: dict, app: FastAPI) -> tuple[int, dict]:
    ...
    if provider_name:
        ...
        if provider_name in CLIENT_SUPPLIED_ENDPOINT_PROVIDERS:
            # Свой эндпоинт клиента: каталог про него не знает НИЧЕГО —
            # ни адреса (``base_url`` в профиле пуст), ни списка моделей
            # (``fallback_models`` пуст), ни переменной для ключа. Значит
            # всё три величины обязан дать клиент, и проверить это может
            # только здесь: у остальных провайдеров адрес и модель
            # подставляются из каталога, поэтому общих ворот на них нет.
            #
            # Без ворот на модель агент уехал бы с пустым ``model.default``
            # и отказал бы на первом же сообщении — то есть отказ уехал бы
            # из мастера в чат, где клиенту с ним делать нечего.
            if not (provider.get("base_url") or "").strip():
                logger.info("submit validation failed: field=provider.base_url endpoint_required")
                return 422, {"errors": {"provider.base_url": _MSG_ENDPOINT_URL_REQUIRED}}
            if not (provider.get("model") or "").strip():
                logger.info("submit validation failed: field=provider.model endpoint_model_required")
                return 422, {"errors": {"provider.model": _MSG_ENDPOINT_MODEL_REQUIRED}}
            # Ключ: пустой законен ровно тогда, когда он уже сохранён —
            # тот же контракт, что у ключей в .env ниже, только источник
            # другой (config.yaml, см. _current_provider_key).
            if not (provider.get("api_key") or "").strip():
                saved = (load_config().get("model") or {}).get("api_key")
                if not saved:
                    logger.info("submit validation failed: field=provider.api_key endpoint_key_missing")
    ...
    if fallback_name:
        ...
        if fallback_name in DEVICE_CODE_PROVIDERS:
            # Зеркало проверки основного слота выше. С 2026-09-11 запасной
            # провайдер настраивается тем же блоком, а значит и вариант со
            # входом по аккаунту у него доступен — и ровно так же не имеет
            # ни env_var, ни api_key, которые поймали бы более поздние
            # проверки. Без этих ворот «запасной ChatGPT» сохранился бы в
            # fallback_providers, ни разу не подтвердив вход, и отказал бы
            # ровно в тот момент, ради которого его и заводили.
            if not device_login_is_valid(fallback_name):
                logger.info("submit validation failed: field=fallback.name device_login_missing")
                return 422, {"errors": {"fallback.name": _MSG_DEVICE_LOGIN_REQUIRED}}
    ...
    if provider_name:
        ...
        if provider_api_key:
            # Свой эндпоинт клиента проверяется по введённому адресу: у
            # него нет переменной окружения, и без адреса проверка была бы
            # коротким замыканием «ок, не проверяли».
            provider_key_result = check_provider_key(
                provider_env_var,
                provider_api_key,
                proxy,
...
def create_app() -> FastAPI:
    ...
    async def check_key(
        ...
        # Адрес принимается ТОЛЬКО в паре с отсутствующей переменной, то
        # есть только для своего эндпоинта клиента. Иначе форма могла бы
        # попросить сервер сходить с ключом известного провайдера на
        # произвольный адрес — то, что закрытый список переменных выше как
        # раз и не пускает.
        endpoint = body.base_url.strip() if body.env_var is None else ""
        return await asyncio.to_thread(
            check_provider_key, body.env_var, body.value, body.proxy or None, endpoint or None
        )
