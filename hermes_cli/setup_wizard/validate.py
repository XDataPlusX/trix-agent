...
def check_provider_key(
    ...
    wizard should treat it as passed without contacting anything. ЕДИНСТВЕННОЕ
    исключение — свой эндпоинт клиента (``custom``): у него переменной нет
    ВООБЩЕ (``env_vars=()``), но адрес клиент ввёл сам, а
    ``probe_provider_key`` умеет вывести проверку из ``base_url``
    (``trix_derived_probes.derived_probe_url`` — ``GET {base_url}/models``
    с Bearer). Без этой ветки ключ своего сервера не проверялся никак, и
    об опечатке клиент узнавал только когда бот впервые не отвечал.
    ...
    if env_var is None:
        ...
        # Имя переменной здесь чисто техническое: у probe_provider_key нет
        # курированной записи под него, и он уйдёт в ветку вывода адреса
        # из base_url — ровно то, что нужно.
        return {
            **probe_provider_key(
