...
EXCLUDED_ROWS: dict[tuple[str, str], str] = {
    # Своего поиска у self-hosted Firecrawl нет. Его /search — это
    # маршрутизатор к чужому движку: fire-engine (только облако Firecrawl,
    # FIRE_ENGINE_BETA_URL) → SearXNG, если задан SEARXNG_ENDPOINT →
    # иначе DuckDuckGo (apps/api/src/search/index.ts в firecrawl/main,
    # сверено 2026-09-10). То есть выбрать его «поиском» = в лучшем случае
    # получить тот же DuckDuckGo, который у нас и так первой строкой
    # списка и помечен рекомендованным, только через лишний сетевой прыжок
    # и чужой контейнер. Каталог апстрима этого различия не знает: обе
    # строки Firecrawl (облако и self-hosted) несут один и тот же
    # ``web_backend: "firecrawl"``, а капабилити мастер спрашивает у
    # РЕЕСТРА ПО ИМЕНИ БЭКЕНДА (``web_provider_capabilities``), где ответ
    # «умеет и поиск, и чтение» верен только для облака. Поэтому строка
    # режется здесь, по имени, а не структурным правилом.
    # В "web_extract" ("Чтение страниц") строка остаётся — это ровно то,
    # ради чего self-hosted Firecrawl и ставят: SearXNG ищет, Firecrawl
    # читает страницы.
    ("web", "Firecrawl Self-Hosted"): (
        "у self-hosted Firecrawl нет собственного поиска — его /search "
        "проксирует fire-engine (только облако), SearXNG или DuckDuckGo; "
        "как поисковик он не даёт ничего сверх строки DuckDuckGo, которая "
        "в списке уже есть. Остаётся в «Чтении страниц»"
    ),
}
...
RU_ENV_PROMPTS: dict[str, str] = {
    ...
    # Хвост «(оставьте пустым для self-hosted)» — дословный перевод
    # апстримной подписи. Клиенту он врёт: пустой ключ здесь = нерабочий
    # Firecrawl, а self-hosted — это ОТДЕЛЬНАЯ строка каталога со своим
    # полем адреса (FIRECRAWL_API_URL выше), и живёт она теперь только в
    # «Чтении страниц». Найдено владельцем 2026-09-10 в живом мастере.
    "FIRECRAWL_API_KEY": "Ключ Firecrawl API",
