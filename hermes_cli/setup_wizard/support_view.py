...
<div id="chat" hidden>
  ...
  noBtn.addEventListener('click', function () {{ sendFeedback(false); }});
    ...
    rail_html = _rail_html(
        _rail_address(host), _header_intro(host), subtitle="Поддержка",
        wizard_entry_visible=True, update_visible=True,
    )
...
def register_support_routes(app: FastAPI) -> None:
    ...
    async def support_run(request: Request) -> JSONResponse:
        ...
        try:
            ...
            # Доступность чата и исход проверки ключа — РАЗНЫЕ вопросы, и
            # смешивать их нельзя. Проверка отвечает «поломан ли настроенный
            # ключ» и по своему контракту возвращает «в порядке», когда не
            # настроено НИЧЕГО: для диагностики отсутствие настройки не
            # поломка. Доступность отвечает «можем ли мы сейчас позвать
            # модель», и на пустой машине ответ «нет».
            #
            # Первая версия выводила доступность из проверки, и на чистой
            # клиентской VM это дало ровно тот тупик, который весь раздел
            # убирает (проверено исполнением 2026-09-04): вердикт обещал
            # «давайте разберёмся вместе», клиент писал сообщение и получал
            # «чат не открывается, напишите в поддержку».
            #
            # Поэтому источник доступности один — is_chat_available(). А
            # результат прогона используется только чтобы НАЗВАТЬ ПРИЧИНУ,
            # когда ключ настроен и отвергнут.
            ...
            elif not chat_available and verdict is not None and not verdict.key_ok:
