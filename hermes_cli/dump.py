...
def _gateway_status() -> str:
    ...
    try:
        ...
        # The snapshot shells out to systemd/launchd and can raise on hosts
        # where the session bus or the service manager is unreachable from
        # this process — typically when ``/debug`` runs INSIDE the gateway
        # (observed in the 2026-09-12 debug report: gateway up, dump said
        # "unknown").  gateway/run.py sets ``_HERMES_GATEWAY=1`` at import,
        # so use that marker to report the process we are actually running
        # in instead of a meaningless "unknown".  The underlying failure is
        # kept at DEBUG so the cause stays diagnosable in verbose mode.
        logger.debug("gateway runtime snapshot failed; falling back to in-process marker", exc_info=True)
        if os.environ.get("_HERMES_GATEWAY") == "1":
            return f"running (in-process, pid {os.getpid()})"
