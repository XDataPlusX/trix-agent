...
from hermes_cli.release_source import (
    ...
    canonical_remote as _canonical_release_remote,
    resolve_local_release_tag,
)
...
_RELEASE_REPO_URL = RELEASE_REPO_HTTPS
_RELEASE_REPO_CANONICAL = _canonical_release_remote(RELEASE_REPO_HTTPS)
...
def _is_release_ssh_remote(url: str | None) -> bool:
    return _is_ssh_remote(url) and _canonical_github_remote(url) == _RELEASE_REPO_CANONICAL
...
def _check_via_rev(local_rev: str) -> Optional[int]:
    """Compare an embedded git revision to our release branch via ls-remote.
    ...
    try:
        result = subprocess.run(
            ["git", "ls-remote", _RELEASE_REPO_URL, f"refs/heads/{RELEASE_BRANCH}"],
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    """Count commits behind origin/<RELEASE_BRANCH> in a local checkout."""
    ...
    # `rev-list --count HEAD..origin/<RELEASE_BRANCH>` would report a huge
    # bogus "behind" number (e.g. "12492 commits behind"). Detect shallow up
    # front: fetch with --depth 1 to preserve the boundary and compare tip
    # SHAs instead of counting. Full clones (developers, Docker dev images)
    # keep the exact count path unchanged. Mirrors the desktop fix in
    ...
    try:
        ...
        # for the same reason. Modern git updates the ``origin/<RELEASE_BRANCH>``
        # tracking ref on a scoped fetch, so the
        # ``HEAD..origin/<RELEASE_BRANCH>`` count below is unaffected; the
        # shallow path compares against FETCH_HEAD, which a scoped fetch also
        # updates.
        fetch_args = ["git", "fetch", "origin", RELEASE_BRANCH]
        if is_shallow:
            fetch_args += ["--depth", "1"]
        ...
        if fetch_result.returncode != 0:
            # Deliberately not user-facing (this is a cosmetic startup-banner
            # check) and deliberately not raised -- a failed fetch here must
            # never block the CLI from starting. But the return code was
            # previously discarded entirely, with no signal anywhere: that
            # silence is exactly how a scoped fetch of a nonexistent branch
            # (e.g. a stale "main" literal after this product's branch
            # changed to RELEASE_BRANCH) went unnoticed. A debug-level log
            # line costs nothing at runtime and gives a developer grepping
            # agent.log a place to look.
            logger.debug(
                "banner: git fetch origin %s failed (rc=%s): %s",
                RELEASE_BRANCH,
                fetch_result.returncode,
                (fetch_result.stderr or b"").decode("utf-8", "replace").strip(),
            )
    except Exception:
        pass  # Offline or timeout — use stale refs, that's fine

    if is_shallow:
        # No history to count across the shallow boundary.
        # `origin/<RELEASE_BRANCH>` may not be a tracking ref in a
        # `clone --depth 1`, so prefer FETCH_HEAD (just updated by the fetch
        # above) and fall back to origin/<RELEASE_BRANCH>.
        head_rev = _git_stdout(["rev-parse", "HEAD"], cwd=repo_dir)
        target_rev = (
            _git_stdout(["rev-parse", "FETCH_HEAD"], cwd=repo_dir)
            or _git_stdout(["rev-parse", f"origin/{RELEASE_BRANCH}"], cwd=repo_dir)
        )
    ...
    try:
        result = subprocess.run(
            ["git", "rev-list", "--count", f"HEAD..origin/{RELEASE_BRANCH}"],
            capture_output=True, text=True, encoding="utf-8", errors="replace",
            timeout=5,
            cwd=str(repo_dir),
        )
...
def check_for_updates() -> Optional[int]:
    ...
    it to our release branch via ``git ls-remote``. Otherwise look for a
    local git checkout and count commits behind
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    upstream = _git_short_hash(repo_dir, f"origin/{RELEASE_BRANCH}")
    local = _git_short_hash(repo_dir, "HEAD")
    if not upstream or not local:
        # Live-git lookup failed (e.g. shallow clone without origin/<RELEASE_BRANCH>).
    ...
    try:
        result = subprocess.run(
            ["git", "rev-list", "--count", f"origin/{RELEASE_BRANCH}..HEAD"],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
_RELEASE_URL_BASE = RELEASE_TAG_URL_BASE
_latest_release_cache: Optional[tuple] = None  # (tag, url) once resolved


def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    """Return ``(tag, release_url)`` for the latest product release tag, or None.

    Local-only — runs ``git describe --tags --abbrev=0`` against the
    checkout, scoped to ``RELEASE_TAG_PREFIX*`` (e.g. ``trix-v1.2.3``) so an
    upstream-style tag (``v2026.8.3``) left over from the fork's shared
    history never gets reported as this product's own version. Cached
    per-process. Release URL always points at our own release repository
    (see ``hermes_cli.release_source``).
    """
    ...
    tag = resolve_local_release_tag(str(repo_dir))
    if not tag:
...
def _format_version_base() -> str:
    """Return just the product name + version, no git-maintenance tail.

    Prefers the product's own release tag (``trix-vX.Y.Z``, resolved
    locally via ``get_latest_release_tag()``) when one is reachable. A
    fresh checkout with no matching tag (e.g. installed from a branch, or
    ``git`` unavailable) falls back to an honest "no known release" label
    instead of fabricating a version or naming the upstream product.

    Shared by :func:`format_banner_version_label` (CLI startup banner,
    which appends the git tail) and :func:`format_version_reply_text`
    (the ``/version`` slash-command reply, which does not).
    """
    release_info = get_latest_release_tag()
    if release_info:
        tag, _url = release_info
        version = tag[len(RELEASE_TAG_PREFIX):] if tag.startswith(RELEASE_TAG_PREFIX) else tag
        return f"Trix Agent v{version}"
    from agent.i18n import t

    return f"Trix Agent {t('trix.version.unreleased_build')}"


def format_banner_version_label() -> str:
    """Return the version label shown in the startup banner title.

    Appends a ``· upstream <sha> [· local <sha> (+N carried commits)]``
    tail when git state is available: it names the tracked remote of OUR
    repo, which is useful in the CLI banner we read during development.
    See :func:`format_version_reply_text` for the customer-facing variant
    that omits this tail.
    """
    base = _format_version_base()
...
def format_version_reply_text() -> str:
    """Return the product name + version, with no git-maintenance tail.

    ``format_banner_version_label()``'s ``· upstream ... (+N carried
    commits)`` tail names the tracked remote of OUR repository — useful in
    the CLI startup banner, but pure maintenance jargon in a customer-facing
    ``/version`` reply: a fresh install is always 0 commits "ahead", and
    "upstream"/"carried commits" mean nothing to someone who never cloned
    this repo. Used by the shared ``/version`` slash-command executor
    (``hermes_cli/slash_exec.py``), reachable from the CLI, gateway, and TUI.
    """
    return _format_version_base()
