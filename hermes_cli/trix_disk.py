...
_REMOVABLE_DIRS: tuple[tuple[str, str], ...] = (
    # В contained этот каталог — ещё и дефолт ручного ``hermes backup``
    # (`backup.py::_default_backup_dir`), то есть место, где лежит артефакт
    # отката. Чистится он там НЕ как остальные: уходят только служебные
    # точки отката движка, см. ``_clear_candidate``.
    ("backups", "backups"),
    ...
    # В contained TMPDIR профиля лежит ВНУТРИ корня и durable: ни systemd,
    # ни перезагрузка его не чистят — только наша уборка, и та раз в сутки.
    # Пусть клиент и поддержка умеют освободить место руками прямо сейчас.
    # В shared-host каталога просто нет, и строка ничего не стоит.
    #
    # Чистится этот каталог НЕ как остальные: у него свой контракт возраста,
    # см. ``_clear_candidate``.
    ("tmp", "tmp"),
)
...
def _removable_bytes(home: Path, target: Path) -> int:
    """Сколько байт из каталога реально освободит уборка.

    Для почти всех каталогов это их размер целиком. Исключение —
    ``<root>/backups`` в contained: там уборка сносит только служебные
    точки отката движка, и обещать клиенту вес его собственных архивов
    значило бы обещать место, которое никто не освободит (правило 3 в
    шапке модуля: числа либо сходятся, либо об этом сказано словами).

    ``<root>/tmp`` сюда НЕ входит намеренно: там граница проходит по
    возрасту записей, то есть по состоянию на момент уборки, а не по
    именам. Замер, сделанный заранее, всё равно был бы не тем числом.
    Поэтому расхождение закрыто второй половиной того же правила 3 —
    словами, и словами клиенту, а не только здесь: строка «временные
    файлы» в отчёте идёт с оговоркой ``_tmp_caveat()``.
    """
    if _keeps_operator_backups(home, target):
        tally = _Tally()
        for entry in _auto_backup_entries(target):
            tally.merge(_measure(entry))
        return tally.total
    return _measure(target).total
...
def _scan_tree(home: Path) -> _TreeScan:
    ...
    for label, target in removable_candidates(home):
        size = _removable_bytes(home, target)
        if size > 0:
...
# Метка каталога, у которого обещание и уборка считаются по-разному, —
# берётся из той же таблицы, что и сама строка отчёта, чтобы оговорка не
# осталась висеть над переименованной меткой.
_TMP_LABEL = _REMOVABLE_LABELS["tmp"]


def _ru_plural(count: int, one: str, few: str, many: str) -> str:
    """Согласование числительного: 1 — one, 2–4 — few, 5+ и 11–14 — many.

    Своя копия, а не импорт из ``trix_session_notices``: это чистая
    функция на четыре строки, а модуль замера места сознательно не тянет
    за собой ничего из шлюза, кроме ``get_hermes_dir``.
    """
...
def _tmp_caveat() -> str:
    """Оговорка к строке «временные файлы» — единственная, где число выше
    больше того, что освободится.

    ``_removable_bytes`` считает ``<root>/tmp`` целиком, а уборка не
    трогает записи моложе ``MIN_AGE_SECONDS``: их почти наверняка держит
    работающая операция (см. ``_clear_candidate``). Посчитать обещание по
    тому же порогу нельзя — граница проходит по состоянию на момент
    уборки, а не на момент замера, и заранее сделанный замер всё равно был
    бы не тем числом. Остаётся вторая половина правила 3 из шапки модуля:
    раз числа не сходятся, об этом сказано словами — и сказано клиенту, а
    не только в докстринге.

    Порог берётся у той же уборки, что его и применяет: оговорка,
    разъехавшаяся с кодом на «десять минут», хуже отсутствующей.
    """
    from hermes_cli.trix_profile_tmp import MIN_AGE_SECONDS

    minutes = max(1, round(MIN_AGE_SECONDS / 60))
    word = _ru_plural(minutes, "минуты", "минут", "минут")
    return (
        f"Из временных файлов уйдут только те, что старше {minutes} {word}, — "
        "свежие держит операция, которая идёт прямо сейчас. Поэтому по этой "
        "строке освободится меньше, чем в ней написано."
    )
    ...
    *,
    ...
    if is_alarming(report.used_percent, warn_percent, report.free, min_free_bytes):
        ...
        if cleanup_helps:
            ...
            grouped = group_removable(report.removable)
            for label, size in grouped:
                lines.append(f"• {label} — {_size(size)}")
            if any(label == _TMP_LABEL for label, _ in grouped):
                lines.append(_tmp_caveat())
...
def _clear_candidate(home: Path, path: Path) -> list[str]:
    """Вычистить один каталог из списка уборки — по его собственным правилам.

    Правило одно на всех и простое: содержимое уходит целиком. Исключений
    ровно два, и оба — каталоги, у которых своя жизнь помимо уборки.

    **``<root>/tmp``.** У него уже есть контракт: записи моложе
    ``MIN_AGE_SECONDS`` неприкосновенны, потому что их почти наверняка
    держит работающая операция. Два контракта на один каталог — это не
    придирка к стилю. «Освободить место» клиент жмёт ровно тогда, когда
    диск забит, то есть когда в ``tmp`` качается или распаковывается
    что-то тяжёлое. ``rmtree`` в этот момент даёт не мегабайты, а
    невоспроизводимую ошибку посреди операции. Поэтому здесь зовётся та
    же ``sweep()``, что и суточная уборка, но с порогом возраста,
    опущенным до самого пола. Агрессивнее уже нельзя: ниже начинается
    живое.

    **``<root>/backups`` в contained.** Там это одновременно склад
    служебных точек отката движка и дефолт ручного ``hermes backup``
    (в shared-host ручной архив по умолчанию уходит в дом ОС, и каталог
    остаётся чисто служебным — там уборка не меняется ни на байт).
    Снести его целиком значит потерять ровно тот артефакт, ради которого
    клиент и нажал «сделать бэкап», причём тихо и по кнопке «освободить
    место». Поэтому в contained уходит только то, что движок написал сам
    и сам же прунит по поколениям, — см. ``_auto_backup_entries``.
    """
    if _is_profile_tmp(home, path):
        from hermes_cli.trix_profile_tmp import MIN_AGE_SECONDS, sweep

        result = sweep(
            home,
            max_age_days=MIN_AGE_SECONDS / 86400.0,
            # Квота не нужна: возраст уже снял всё, что законно снимать.
            max_size_mb=0,
        )
        return list(result.errors)
    if _keeps_operator_backups(home, path):
        problems: list[str] = []
        for entry in _auto_backup_entries(path):
            try:
                _remove_tree(entry)
            except OSError as exc:
                problem = _clean_problem(exc, entry)
                if problem:
                    problems.append(problem)
        return problems
    return _clear(path)
...
def _is_profile_tmp(home: Path, path: Path) -> bool:
    """``True``, если это ветвь ``tmp`` того самого корня.

    Сравниваются разрешённые пути: список уборки строится через
    ``get_hermes_dir``, который умеет отдать легаси-имя.
    """
    try:
        return (_resolve(path) or path) == (_resolve(home / "tmp") or home / "tmp")
    except OSError:  # pragma: no cover — недоступный путь
        return False


def _is_profile_backups(home: Path, path: Path) -> bool:
    """``True``, если это ветвь ``backups`` того самого корня."""
    try:
        return (_resolve(path) or path) == (
            _resolve(home / "backups") or home / "backups"
        )
    except OSError:  # pragma: no cover — недоступный путь
        return False


def _keeps_operator_backups(home: Path, path: Path) -> bool:
    """``True``, когда в этом каталоге могут лежать архивы оператора.

    Режим спрашивается про ТОТ корень, который убираем, а не про корень
    процесса: одна установка обслуживает несколько профилей, и ответ
    ``is_contained()`` без аргумента относился бы к чужому каталогу.

    Не смогли определить режим — считаем, что архивы оператора там есть.
    Цена ошибки несимметрична: в одну сторону это несколько неубранных
    мегабайт, в другую — потерянная точка отката.
    """
    if not _is_profile_backups(home, path):
        return False
    try:
        from hermes_constants import is_contained

        return is_contained({"HERMES_HOME": str(home)})
    except Exception:  # pragma: no cover — режим не прочитался
        return True


def _auto_backup_entries(path: Path) -> list[Path]:
    """Служебные точки отката движка внутри ``backups``.

    Имена — те же, по которым ``hermes_cli.backup`` сам прунит их по
    поколениям (``_prune_pre_update_backups``, ``_prune_pre_migration_
    backups``). Всё остальное — архив оператора, дашборда или положенный
    руками файл: незнакомое имя трактуется в пользу сохранения.
    """
    from hermes_cli.backup import _PRE_MIGRATION_PREFIX, _PRE_UPDATE_PREFIX

    prefixes = (_PRE_UPDATE_PREFIX, _PRE_MIGRATION_PREFIX)
    try:
        entries = list(os.scandir(path))
    except OSError:
        return []
    ...
    for entry in entries:
        ...
        try:
            if not entry.is_file(follow_symlinks=False):
                continue
        except OSError:  # pragma: no cover — недоступная запись
            continue
...
def _clean_once(home: Path, *, docker_prune=None) -> CleanResult:
    ...
    for label, path in removable_candidates(home):
        ...
        problems = _clear_candidate(home, path)
