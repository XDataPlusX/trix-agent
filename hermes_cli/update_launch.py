...
def start_detached_update(extra_args: list | None = None) -> list:
    ...
    # Обновление — скоуп УСТАНОВКИ, не профиля: его git/uv/npm обязаны
    # видеть настоящий дом ОС, иначе предзалитые рецептом кэши uv и npm
    # (сотни мегабайт) будут качаться заново на каждой машине при каждом
    # /update. Сегодня HOME шлюза и так настоящий — движок дом своего
    # процесса не меняет никогда. Но дорогу назад передаём явно, чтобы
    # гарантия не зависела от того, кто и как запустил шлюз.
    try:
        from hermes_constants import get_real_home

        env.setdefault("HERMES_REAL_HOME", get_real_home(env))
    except Exception:
        pass
