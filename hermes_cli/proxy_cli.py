...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    def yn(value: bool) -> str:
        return t("trix.egress.enabled_yes") if value else t("trix.egress.enabled_no")

    if status.pid:
        process_line = t("trix.egress.process", value=f"pid {status.pid}")
    else:
        process_line = t("trix.egress.process", value=t("trix.egress.stopped"))

    lines = [
        t("trix.egress.title"),
        "",
        t("trix.egress.enabled", value=yn(bool(proxy_cfg.get('enabled')))),
        t("trix.egress.binary", value=status.binary_path or t("trix.egress.missing")),
        t("trix.egress.binary_version", value=status.binary_version or t("trix.egress.unknown")),
        t("trix.egress.config", value=status.config_path or t("trix.egress.not_generated")),
        t("trix.egress.ca_cert", value=status.ca_cert_path or t("trix.egress.not_generated")),
        t("trix.egress.tunnel_port", value=status.tunnel_port),
        process_line,
        t("trix.egress.listening", value=yn(status.listening)),
        t("trix.egress.credential_src", value=proxy_cfg.get('credential_source', 'env')),
        t("trix.egress.docker_enforce", value=yn(bool(proxy_cfg.get('enforce_on_docker', True)))),
        t("trix.egress.scope"),
    ]

    mappings = ip.load_mappings()
    if mappings:
        lines.extend(["", t("trix.egress.token_mappings_header")])
        for m in mappings:
            tok = m.proxy_token if show_tokens else _redact_token(m.proxy_token)
            lines.append(
                t(
                    "trix.egress.token_mapping_entry",
                    name=m.real_env_name,
                    token=tok,
                    hosts=', '.join(m.upstream_hosts),
                )
            )
    ...
    if uncovered:
        lines.extend([
            "",
            t("trix.egress.uncovered_header"),
        ])
        for name in uncovered:
            lines.append(t("trix.egress.uncovered_entry", name=name))

    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", t("trix.egress.next_setup")])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", t("trix.egress.next_start")])
