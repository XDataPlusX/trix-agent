...
_CATEGORY_MERGE: Dict[str, str] = {
    ...
    "mcp": "agent",
    # `session_search.allow_cross_profile` is the only schema-surfaced
    # session_search field — it is an isolation boundary (may this profile's
    # agent read ANOTHER profile's history), so it belongs on the security
    # tab rather than spawning a one-field orphan category.
    "session_search": "security",
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    """Collect a redacted debug report + full logs and return them directly.

    Trix Agent never uploads debug data anywhere — customer logs stay on
    their own VM (see
    docs/product/specs/2026-08-13-trix-agent-persona-design.md). This used
    to upload the bundle to a public paste service and return shareable
    URLs; it now collects the same redacted bundle and returns its content
    directly instead, so the dashboard (the customer's own browser talking
    to their own server) can render or download it without anything ever
    leaving the VM.
    """
    ...
    try:
        ...
        _log.exception("debug share collection failed")
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")

    return {
        "ok": True,
        "files": bundle,
        "redacted": bool(req.redact),
        # Legacy keys from the pre-privacy-fix paste-upload response, kept
        # with neutral values ONLY so an un-updated dashboard frontend
        # (which still reads shareResult.urls / .auto_delete_seconds) reads
        # "nothing to show" instead of throwing a TypeError. They carry no
        # meaning and nothing here uploads anywhere — see
        # docs/product/specs/2026-08-13-trix-agent-persona-design.md.
        "urls": {},
        "failures": [],
...
# Live credential probes — shared with the setup wizard. Table lives in the
# leaf module hermes_cli/credential_probes.py; keep additions there.
from hermes_cli.credential_probes import CREDENTIAL_PROBES as _CREDENTIAL_PROBES
