...
def share_to_nous(report_bundle: bytes) -> dict:
    ...
    This used to orchestrate the Nous-S3 upload of a gzipped *report_bundle*
    (mint a presigned PUT URL, then PUT the bundle). ``run_debug_share`` no
    longer calls this path -- it forces ``local_only``/disables ``--nous``
    before any network I/O can happen -- but this function stayed fully
    functional underneath that one guard. This repo absorbs a steady stream
    of upstream merges, and a merge conflict resolved the wrong way in
    ``run_debug_share`` would silently re-enable Nous-internal upload of
    conversation logs with no other line of defense. Raising unconditionally
    here, at the actual network boundary, means that can't happen no matter
    how ``run_debug_share`` is merged.
    """
    raise RuntimeError(
