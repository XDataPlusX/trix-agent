...
def omo_runtime_binary() -> Optional[Path]:
    ...
    # SHARED_IMMUTABLE: the sg binary is a machine asset, not profile data —
    # omo installs one copy per OS user and we only ever execute it. Under
    # contained isolation HOME is <root>/home, so the line above stops seeing
    # that copy and ast-grep silently degrades to the slower fallbacks. Probe
    # the real OS home too; HERMES_REAL_HOME is always exported to contained
    # subprocesses. This is a read-only lookup and adds no write outside root.
    real_home = os.environ.get("HERMES_REAL_HOME", "").strip()
    if real_home:
        candidates.append(
            Path(real_home) / ".omo" / "runtime" / "ast-grep" / slug / binary_name
        )
