...
metadata:
  hermes:
    tags: [skills, authoring, trix-agent, conventions, skill-md]
    related_skills: [plan, requesting-code-review]
---

# Authoring Trix Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.trix/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the Trix Agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with Trix Agent
...
- Don't use for: personal skills in `~/.trix/skills/` (just use `skill_manage`)
...
author: Real Name (github-handle), Trix Agent
license: MIT
...
- Credit the **human first**, then "Trix Agent" as secondary collaborator: `Ben Barclay (benbarclay), Trix Agent`.
- Never `author: Trix Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Trix Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.trix/skills/`.
...
|---|---|
| Trix tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.trix/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Trix Agent` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through Trix tools
