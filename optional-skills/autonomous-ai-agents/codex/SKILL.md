...
metadata:
  hermes:
    ...
    related_skills: [claude-code, trix-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Trix terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Trix itself, `model.provider: openai-codex` uses Trix-managed Codex
...
When invoking the Codex CLI from a Trix gateway/service context (for example,
