...
version: 2.0.0
author: Francesco Bonacci (f-trycua), Trix Agent
license: MIT
...
Trix drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The Trix-side `computer_use` tool exposed
in this skill is a higher-level Trix vocabulary; the raw cua-driver
...
Normal Trix sessions use `standard`, which requires a certified protected
host and fails closed when Trix has none. Explicit Trix YOLO (`--yolo`,
...
Trix intentionally keeps THIS skill focused on the Trix-side
...
When `cua-driver skills install` autodetects Trix (planned follow-up
