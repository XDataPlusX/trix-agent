...
metadata:
  hermes:
    ...
    related_skills: [trix-agent, mcporter, fastmcp]
---
...
2. **Read the source, not just the skill docs.** `tools/mcp_oauth.py` and `hermes_cli/mcp_catalog.py` are the authoritative references. Grep the tree before telling the user a feature "doesn't exist."
