...
hermes mcp serve                      Run Trix Agent as an MCP server
```
...
```
hermes portal               Quick setup / sign in via Portal
hermes kanban <verb>        Multi-agent work-queue board
hermes project              Named multi-folder workspaces
hermes skin list|use|set    Switch/tweak skins (terminal only)
...
|---|---|
| Config options | `hermes config edit` · this skill's `configuration.md` reference |
| Tools / toolsets | `hermes tools list` |
| Skills catalog | `hermes skills browse` |
| Provider setup | `hermes model` · this skill's `providers-and-models.md` reference |
