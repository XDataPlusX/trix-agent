...
Trix Agent runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`AF_INET` socket. Root cause is usually Trix Agent's env scrubber dropping
...
Trix Agent-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Trix Agent tool and most Windows APIs. Prefer forward slashes in code
