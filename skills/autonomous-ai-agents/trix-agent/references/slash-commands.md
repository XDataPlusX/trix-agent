...
/snapshot [sub]          Create/restore Trix Agent config+state snapshots (CLI)
...
/subscription (/upgrade) Portal plan info (CLI)
/topup                   Portal balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Trix Agent to latest
