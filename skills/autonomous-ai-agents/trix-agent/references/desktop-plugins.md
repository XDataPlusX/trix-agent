...
Write plugins for the Trix Agent desktop app: statusbar items, layout panes,
...
- The Trix Agent desktop app (it loads plugins; the CLI/gateway alone does not).
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Trix Agent (use `host.notify` for the
