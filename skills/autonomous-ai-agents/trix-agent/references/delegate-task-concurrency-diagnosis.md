...
**three** code paths in Trix Agent that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Trix Agent — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Trix Agent
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Trix Agent is
   refusing my value."
...
# 2. Did Trix Agent's truncator or rejector actually fire?
...
appears, **the cap is the model**, not Trix Agent.
...
- **`max_concurrent_children` is a per-parent cap, not a global cap.**
  Two different parents can each spawn `max_children` workers
  concurrently.
