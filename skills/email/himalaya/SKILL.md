...
This skill is separate from the Trix Email gateway adapter. The gateway
adapter lets people email the agent and uses Trix's built-in IMAP/SMTP
...
To reply non-interactively from Trix, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Trix)** — pipe the message via stdin:
