...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            # NOT the exa/firecrawl/parallel self-heal pattern, despite an
            # earlier version of this comment claiming it was. Those
            # providers' is_available() checks only for an API key
            # (agent/web_search_provider.get_provider_env), so search() is
            # genuinely the first place that imports the SDK, and a lazy
            # install there runs on the very first real call. ddgs has no
            # key: is_available() above checks THIS SAME IMPORT, so if it
            # were missing, is_available() would already have returned
            # False and _get_capability_backend() (tools/web_tools.py)
            # would never have selected this provider in the first place —
            # a lazy-install call here could never run. It silently fell
            # through to firecrawl instead and advertised a paid key /
            # Nous Portal login to the client, which is what this comment
            # used to hide (dead code, found by execution, not by reading).
            #
            # The package is actually guaranteed to land at install time
            # (scripts/install.sh's install_ddgs_search_backend() + the
            # check_ddgs_preflight() report at the tail of print_success())
            # and can be (re)installed on demand via `hermes tools` (its
            # "ddgs" post_setup step, hermes_cli/tools_config.py) — both
            # routes go through the same tools.lazy_deps.ensure("search.ddgs")
            # pin as this error message. This branch is a defensive guard
            # for a caller that bypasses _get_capability_backend() and
            # selects this provider directly, not a self-heal.
            return {
                "success": False,
                "error": (
                    "ddgs package is not installed — run `pip install ddgs` "
                    "or `uv pip install -e '.[ddgs]'`, or run `hermes tools` "
                    "and re-select DuckDuckGo (ddgs)."
                ),
            }
