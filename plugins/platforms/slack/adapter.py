...
class SlackAdapter(BasePlatformAdapter):
    ...
    def _is_interactive_user_authorized(
        ...
        *,
        ...
        ``thread_ts`` is the source's ``thread_id`` everywhere else in this
        adapter, and it reaches the runner for the same reason as the Telegram
        mirror: under ``gateway.profile_routes`` a thread can route to a
        different profile than its channel, and the profile picks the
        allowlist. Without it a thread-scoped route never matches, and the
        button is judged by the CHANNEL's (wider) list.
        """
        ...
        normalized_thread_ts = str(thread_ts).strip() if thread_ts else None

        runner = getattr(getattr(self, "_message_handler", None), "__self__", None)
        # Prefer the scope-resolving entry point (see the Telegram mirror in
        # ``_is_callback_user_authorized``): it installs the event's own
        # profile secret scope before reading any allowlist, and off
        # multiplexing it is a straight call to ``_is_user_authorized``.
        ...
        if callable(auth_fn):
            try:
                ...
                source = SessionSource(
                    ...
                    thread_id=normalized_thread_ts,
                    scope_id=str(team_id) if team_id else None,
                )
        ...
        # Multiplex: ``_message_handler`` is a closure
        # (``GatewayRunner._make_profile_message_handler``), so it has no
        # ``__self__`` and the runner lookup above always misses — every Block
        # Kit button in a multiplexed gateway silently drops to the env-only
        # fallback below, while ordinary messages keep the full auth chain.
        # Same defect as the Telegram inline buttons in RAF-176, and the same
        # seam fixes it: ``set_authorization_check`` is the runner's own
        # profile-bound delegate to ``_is_user_authorized``, installed for
        # primary and secondary adapters alike. ``None`` means no check was
        # registered, which leaves the fallback reachable.
