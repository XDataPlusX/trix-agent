...
# Mask ``scheme://user:password@host`` credentials for ANY scheme (http,
# https, socks5, ...).  The shared ``redact_sensitive_text`` deliberately
# passes web-URL userinfo through unmasked (OAuth callbacks and pre-signed
# URLs the agent is meant to follow), so a *known-credential* URL like the
# resolved proxy needs its own deterministic mask at the logging boundary.
_PROXY_USERINFO_RE = re.compile(r"://([^/@\s]+:[^/@\s]+|[^:@/\s]+)@")


def _mask_proxy_userinfo(text: str) -> str:
    """Replace proxy userinfo credentials with ``user:***`` / ``***``."""
    if "@" not in text:
        return text
    ...
    return _PROXY_USERINFO_RE.sub(_sub, text)


def _redact_proxy_url_for_log(url: object) -> str:
    """Mask proxy credentials in a URL before it reaches any log line.

    ``connect()`` logs the resolved proxy at INFO ("Proxy detected; passing
    explicitly to HTTPXRequest: …").  Proxy URLs routinely carry userinfo
    credentials (``http://user:pass@host:port``); the raw form must never be
    written to logs — the gateway's secret-redaction pipeline scrubs outbound
    *chat/tool* text, not logger output, and ``/debug`` reports bundle raw
    log tails (leak observed in the 2026-09-12 debug report).  Scheme, host
    and port stay visible so connectivity debugging still works.
    """
    text = "" if url is None else str(url)
    if not text:
        return text
    return _mask_proxy_userinfo(text)


def _log_polling_network_error(name: str, error: BaseException) -> None:
    """Log a polling-path network error with the exception text redacted.

    Extracted from the PTB ``error_callback`` closure so the redaction is
    unit-testable without standing up a full polling application.
    (Regression: a04fcbf779 pasted the redactor call into the format string
    instead of calling it, so raw transport errors hit gateway logs and the
    literal ``_redact_telegram_error_text(error)`` text hit the message.)
    The proxy-userinfo mask is layered on top because transport exception
    reprs can embed the proxy URL with credentials.
    """
    logger.warning(
        "[%s] Telegram network error, scheduling reconnect: %s",
        name, _mask_proxy_userinfo(_redact_telegram_error_text(error)),
    )


def _log_polling_error(name: str, error: BaseException) -> None:
    """Log an unrecognized polling error, redacted (see _log_polling_network_error)."""
    logger.error(
...
def _clarify_text(key: str, **kwargs) -> str:
    """Catalog lookup for the clarify window's own strings.

    Same deferred-import shape as ``_approval_outcome_text`` below; kept
    separate only so the call sites read as what they are. ``send_clarify``
    has no local ``t`` import of its own.
    """
...
def _approval_outcome_text(key: str, **kwargs) -> str:
    """Catalog lookup for the approval flow's outcome / toast strings.

    The prompt itself was localized first (header, reason, buttons, timeout
    note); the OUTCOME was not, so the client tapped a Russian "Разрешить"
    and got "✅ Approved once by Иван" back -- the last thing they see in
    the whole scenario. The English catalog repeats every historical
    literal byte-for-byte, so the operator path is unchanged.

    Deferred import (like every other i18n call here) so the active
    language is read at render time, not at module import.
    """
    from agent.i18n import t
...
async def _answer_callback_denied(query, text: str) -> None:
    """Answer a button tap that was refused on RIGHTS, as a modal alert.

    ``answerCallbackQuery`` without ``show_alert`` renders as a toast that
    fades after a couple of seconds near the top of the chat — easy to miss
    entirely on mobile. Every other outcome here (resolved, expired, already
    answered) is also visible in the EDITED message, so a missed toast still
    leaves a trace; a rights refusal edits nothing, so the toast is the only
    signal there is. When it goes unnoticed the button reads as simply dead,
    which is exactly how RAF-176 was reported ("верхняя кнопка не делает
    ничего") — the gateway had answered "no rights" and the owner never saw
    it. A refusal must be a modal the user has to dismiss.
    """
    await query.answer(text=text, show_alert=True)


def _approval_button_labels(lang: str | None = None) -> dict[str, str]:
    """Labels for the exec-approval prompt's inline keyboard buttons.

    ``callback_data`` (``ea:once:...`` etc.) is a protocol the callback
    handler parses -- it is untouched by language. Only the visible label
    text comes from the catalog.
    """
    from agent.i18n import t

    return {
        "once": t("trix.approval.button_once", lang=lang),
        "session": t("trix.approval.button_session", lang=lang),
        "always": t("trix.approval.button_always", lang=lang),
        "deny": t("trix.approval.button_deny", lang=lang),
    }


def _slash_confirm_button_labels(lang: str | None = None) -> dict[str, str]:
    """Labels for the slash-command confirmation prompt (``sc:`` prefix).

    Same pattern as ``_approval_button_labels`` -- only the visible text is
    language-dependent, ``callback_data`` (``sc:once:...`` etc.) is a fixed
    protocol string the callback handler parses.
    """
    from agent.i18n import t

    return {
        "once": t("trix.cmd.confirm.button_once", lang=lang),
        "always": t("trix.cmd.confirm.button_always", lang=lang),
        "cancel": t("trix.cmd.confirm.button_cancel", lang=lang),
    }


def _update_prompt_button_labels(lang: str | None = None) -> dict[str, str]:
    """Labels for the mid-update Yes/No prompt (``update_prompt:`` prefix)."""
    from agent.i18n import t

    return {
        "yes": t("trix.update.button_yes", lang=lang),
        "no": t("trix.update.button_no", lang=lang),
    }


def _model_picker_button_labels(lang: str | None = None) -> dict[str, str]:
    """Labels for the ``/model`` picker's inline keyboard buttons.

    Covers both the provider-selection screen (``_build_provider_keyboard``)
    ...
    return {
        "prev": t("trix.cmd.model.picker.button_prev", lang=lang),
        "next": t("trix.cmd.model.picker.button_next", lang=lang),
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _is_callback_user_authorized(
        ...
        *,
        ...
        # Prefer the scope-resolving entry point: it installs the event's own
        # profile secret scope before reading any allowlist. Off multiplexing
        # it is a straight call to ``_is_user_authorized``, so the
        # single-profile path this branch serves is unchanged. ``getattr``
        # order, not a version check — a runner stub in a test may expose only
        # the older method.
        auth_fn = getattr(runner, "_authorize_in_source_profile_scope", None) or getattr(
            runner, "_is_user_authorized", None
        )
        if callable(auth_fn):
            try:
        ...
        # Multiplex: ``_message_handler`` is a CLOSURE, not a bound method.
        # ``GatewayRunner._make_profile_message_handler`` (and its
        # default-profile twin) wrap ``_handle_message`` to stamp the owning
        # profile and install that profile's runtime scope, so the
        # ``__self__`` lookup above finds nothing and the full auth chain
        # silently drops out — for EVERY inline button in EVERY multiplexed
        # gateway, while ordinary messages keep going through
        # ``_is_user_authorized``. That is HALF of RAF-176: it explains why the
        # owner's ``/update`` buttons were judged by a narrow env-only read
        # while his messages in the same chat went through the full chain.
        #
        # It does not explain why that read came back EMPTY — his chat is the
        # ``default`` profile, whose allowlist was filled. The other half is
        # that the callback was reading another profile's secret scope
        # entirely; see ``GatewayRunner._authorize_in_source_profile_scope``.
        # Routing through the delegate below is what makes that fix reachable
        # from here at all.
        #
        # ``set_authorization_check`` is the runner's own profile-bound
        # delegate to ``_is_user_authorized`` (installed for primary AND
        # secondary adapters), and its ``SessionSource`` carries the owning
        # ``profile`` — which the closure-less path above cannot know. Consult
        # it before degrading to the env-only fallback. Returns ``None`` when
        # no check is registered (bare adapters, tests), which keeps the
        # fallback reachable.
        # ``thread_id`` is forwarded because the runner resolves the event's
        # profile from the source, and under ``gateway.profile_routes`` a chat
        # and one of its topics can belong to DIFFERENT profiles — the owner's
        # DM chat routes to ``default`` while thread 570710 inside it routes to
        # ``system_admin``. The profile selects the allowlist, so dropping the
        # thread here would judge a topic's button by the other profile's list.
        delegated = self._is_sender_authorized(
            normalized_user_id,
    ...
    def _note_link_incident(self, kind: str) -> None:
        """Отметить разрыв связи в счётчике (см. link_health.py).

        Повторы и осушение пула переживаются молча, и у клиента нет ни
        одного способа узнать, что связь рвалась: сказать ему об этом
        через Телеграм в тот момент невозможно. Счётчик — способ ответить
        на это потом, числом.

        Ничего не поднимает наружу: телеметрия разрывов не имеет права
        сорвать отправку, ради устойчивости которой она и заведена.
        """
        try:
            link_health.record(kind)
        except Exception:  # pragma: no cover — счётчик и сам всё ловит
            logger.debug("[%s] link health counter unavailable", self.name, exc_info=True)
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # during reconnect/bootstrap. The safer numbers, and the rules for
            # overriding them from config.yaml, live in telegram_network.py --
            # see resolve_http_request_kwargs() and the comment above it.
            request_kwargs = resolve_http_request_kwargs(self.config.extra)
            logger.info(
                "[%s] Telegram HTTP client: pool=%s, pool_timeout=%ss, connect=%ss, "
                "read=%ss, write=%ss, media_write=%ss",
                self.name,
                request_kwargs["connection_pool_size"],
                request_kwargs["pool_timeout"],
                request_kwargs["connect_timeout"],
                request_kwargs["read_timeout"],
                request_kwargs["write_timeout"],
                request_kwargs["media_write_timeout"],
            )
            ...
            if fallback_ips and not proxy_url and not disable_fallback:
                ...
                logger.info("[%s] Proxy detected; passing explicitly to HTTPXRequest: %s", self.name, _redact_proxy_url_for_log(proxy_url))
                request = HTTPXRequest(
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or lower telegram.network.connect_timeout in "
                            f"config.yaml (and HERMES_TELEGRAM_INIT_TIMEOUT)."
                        )
                    logger.warning(
                    ...
                    rebuild_app = True
                    self._note_link_incident(link_health.KIND_INIT_RETRY)
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or lower telegram.network.connect_timeout in config.yaml."
                        )
            ...
            if webhook_url:
                ...
                if not webhook_secret:
                    raise RuntimeError(
                        ...
                        "anyone who can reach it: without the shared secret "
                        "Telegram sends, any request to your webhook URL is "
                ...
                def _polling_error_callback(error: Exception) -> None:
                    ...
                    if self._looks_like_polling_conflict(error):
                        ...
                        _log_polling_network_error(self.name, error)
                        self._polling_error_task = loop.create_task(self._handle_polling_network_error(error))
                        self._background_tasks.add(self._polling_error_task)
                        self._polling_error_task.add_done_callback(self._background_tasks.discard)
                    else:
                        _log_polling_error(self.name, error)
                ...
                for _send_attempt in range(3):
                    try:
                        ...
                        if is_pool_timeout:
                            ...
                            self._note_link_incident(link_health.KIND_POOL_TIMEOUT)
                        elif self._looks_like_connect_timeout(send_err):
                            self._note_link_incident(link_health.KIND_CONNECT_TIMEOUT)
                        if _send_attempt < 2:
                            self._note_link_incident(link_health.KIND_SEND_RETRY)
                            ...
                            self._note_link_incident(link_health.KIND_SEND_FAILED)
                            raise
                    except Exception as send_err:
    ...
    async def send_update_prompt(
        ...
        try:
            ...
            default_hint = (
                t("trix.update.default_hint", default=" (default: {value})", value=default)
                if default else ""
            )
            header = t("trix.update.needs_input_header", default="Update needs your input:")
            text = self.format_message(f"⚕ *{header}*\n\n{prompt}{default_hint}")
            button_labels = _update_prompt_button_labels()
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(button_labels["yes"], callback_data="update_prompt:y"),
                    InlineKeyboardButton(button_labels["no"], callback_data="update_prompt:n"),
                ]
            ])
            thread_id = self._metadata_thread_id(metadata)
    ...
    # _EA_HEADER / _EA_REASON_LABEL are properties (not class-level strings)
    # so the catalog is read at render time -- the active language can
    # change between messages (config edit, /language switch) and every
    # prompt must reflect it, not whatever was resolved at import time.
    @property
    def _EA_HEADER(self) -> str:
        from agent.i18n import t
        return t("trix.approval.header")

    @property
    def _EA_REASON_LABEL(self) -> str:
        from agent.i18n import t
        return t("trix.approval.reason_label")

    @property
    def _EA_TIMEOUT_NOTE(self) -> str:
        """Tells the client up front that silence means the action will NOT
        run (the approval wait times out to a deny, not to "keep going").
        Without this line the client only finds out after the fact and reads
        it as the product breaking, not as expected behavior. Empty in the
        English catalog -- the operator already knows the timeout semantics
        -- so this stays a no-op there and the historical English wording is
        unchanged.
        """
        from agent.i18n import t
        return t("trix.approval.timeout_note")
    ...
    def _ea_reason_text(self, description: str) -> str:
        """Russian wording of the reason, if we have one; otherwise the upstream text.

        Sandbox-delete pattern descriptions are NOT the only thing that
        reaches the client: an ``execute_code`` script that deletes, a
        ``python -c "...rmtree..."`` one-liner, and an ``rm -rf`` long enough
        to defeat the command parser all arrive with descriptions that live
        outside ``DANGEROUS_PATTERNS``. All of them are covered by
        ``client_reason_ru``; the coverage invariant lives in
        ``tests/gateway/test_approval_prompt_russian.py`` and derives the
        reachable set by executing the real guards. Anything still unknown
        is the operator path, where upstream's English beats an approximate
        translation.
        """
        from hermes_cli.trix_sandbox_guard import client_reason_ru
        return client_reason_ru(description)
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            button_labels = _approval_button_labels()
            buttons = [
                InlineKeyboardButton(button_labels["once"], callback_data=f"ea:once:{approval_id}")
            ]
            if not smart_denied and allow_session:
                buttons.append(
                    InlineKeyboardButton(button_labels["session"], callback_data=f"ea:session:{approval_id}")
                )
                if allow_permanent:
                    buttons.append(
                        InlineKeyboardButton(button_labels["always"], callback_data=f"ea:always:{approval_id}")
                    )
            buttons.append(InlineKeyboardButton(button_labels["deny"], callback_data=f"ea:deny:{approval_id}"))
    ...
    async def send_slash_confirm(
        ...
        Three buttons (once / always / cancel) by default; ``allow_always=
        False`` (Ruling 8, ``/update``) drops the middle "always" button so
        the persistent opt-out can never be reached from this specific
        ...
        try:
            ...
            button_labels = _slash_confirm_button_labels()
            once_row = [
                InlineKeyboardButton(button_labels["once"], callback_data=f"sc:once:{confirm_id}"),
            ]
            if allow_always:
                once_row.append(
                    InlineKeyboardButton(button_labels["always"], callback_data=f"sc:always:{confirm_id}")
                )
            keyboard = InlineKeyboardMarkup([
                once_row,
                [
                    InlineKeyboardButton(button_labels["cancel"], callback_data=f"sc:cancel:{confirm_id}"),
                ],
            ])
    ...
    async def send_clarify(
        ...
        try:
            ...
            if choices:
                ...
                rows.append([
                    InlineKeyboardButton(
                        _clarify_text("trix.clarify.button_other"),
    ...
    async def _handle_choice_picker_callback(
        ...
        if not state:
            await query.answer(text=t("trix.cmd.choice.expired_run_again"))
            return
        ...
        try:
            ...
            await query.answer(text=t("trix.cmd.choice.invalid_selection"))
            return

        callback = state.get("on_choice_selected")
        if not callback:
            await query.answer(text=t("trix.cmd.choice.expired"))
            return
        ...
        try:
            ...
            result_text = t("trix.cmd.choice.apply_error", error=exc)
    ...
    def _model_picker_provider_text(
        ...
        Shared by every entry point that redraws this screen (initial send,
    ...
    def _model_picker_more_available(self, total: int, shown: int) -> str:
        ...
        return "\n_" + t("trix.cmd.model.picker.more_available", count=total - shown) + "_"

    def _model_picker_model_list_text(self, provider_name: str, page_info: str, extra: str) -> str:
        """Localized text for the /model picker's model-selection screen."""
        from agent.i18n import t

        return "\n".join((
            f"⚙ *{t('trix.cmd.model.picker.header')}*",
            "",
            f"{t('trix.cmd.model.picker.provider')}: *{provider_name}*{page_info}",
            f"{t('trix.cmd.model.picker.select_model')}{extra}",
        ))

    def _model_picker_group_text(self, family_label: str) -> str:
        """Localized text for the /model picker's provider-family screen."""
        from agent.i18n import t

        return "\n".join((
            f"⚙ *{t('trix.cmd.model.picker.header')}*",
            "",
            f"{t('trix.cmd.model.picker.provider_family')}: *{family_label}*",
            "",
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        if total_pages > 1:
            nav: list = []
            if page > 0:
                nav.append(InlineKeyboardButton(button_labels["prev"], callback_data=f"mpv:{page - 1}"))
    ...
    def _build_model_keyboard(self, models: list, page: int) -> tuple:
        ...
        if total_pages > 1:
            nav: list = []
            if page > 0:
                nav.append(InlineKeyboardButton(button_labels["prev"], callback_data=f"mg:{page - 1}"))
            nav.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="mx:noop"))
            if page < total_pages - 1:
                nav.append(InlineKeyboardButton(button_labels["next"], callback_data=f"mg:{page + 1}"))
            rows.append(nav)

        rows.append([
            InlineKeyboardButton(button_labels["back"], callback_data="mb"),
            InlineKeyboardButton(button_labels["cancel"], callback_data="mx"),
        ])

        return InlineKeyboardMarkup(rows), page_meta["page_info"]

    def _model_picker_switch_authorized(self, query) -> bool:
        """Gate the /model picker's state-mutating callbacks.

        Same authorization gate as the choice picker (cp:) and every other
        inline-window callback: unauthorized users in a shared group must
        not flip session/config state via someone else's picker message.
        That covers both branches that actually invoke ``on_model_selected``
        (mm:/mc:, the active model and its billing) AND the browsing
        branches that rewrite ``_model_picker_state`` for the chat
        (mp:/mg:/mpv:/mpg:/mb -- ``model_list``, ``selected_provider``,
        ``model_page``, ``provider_page``). Browsing is not read-only: an
        unauthorized tap there can still steer what the *owner's* next tap
        resolves against, by swapping ``model_list`` out from under a
        keyboard the owner already has on screen. Only mx:/mx:noop (cancel)
        are exempt -- discarding the picker window touches neither billing
        nor a list a later authorized tap depends on.
        """
        query_message = getattr(query, "message", None)
        query_chat = getattr(query_message, "chat", None)
    ...
    async def _handle_model_picker_callback(
        ...
        if not state:
            await query.answer(text=t("trix.cmd.model.picker.expired_use_model"))
            return

        # Browsing branches rewrite _model_picker_state (model_list,
        # selected_provider, model_page, provider_page) -- gate them like
        # the switch branches (see _model_picker_switch_authorized). Only
        # mx:/mx:noop (cancel) are exempt, handled below without this check.
        if data.startswith(("mp:", "mg:", "mpv:", "mpg:")) or data == "mb":
        ...
        if data.startswith("mp:"):
            ...
            if not provider:
                await query.answer(text=t("trix.cmd.model.picker.provider_not_found"))
                return
            ...
            shown = len(models)
            extra = self._model_picker_more_available(total, shown)

            await query.edit_message_text(
                text=self.format_message(
                    self._model_picker_model_list_text(pname, page_info, extra)
                ),
                parse_mode=ParseMode.MARKDOWN_V2,
                reply_markup=keyboard,
            )
            ...
            try:
                ...
                await query.answer(text=t("trix.cmd.model.picker.invalid_page"))
                return
            ...
            try:
                ...
                await query.answer(text=t("trix.cmd.model.picker.invalid_selection"))
                return

            model_list = state.get("model_list", [])
            if idx < 0 or idx >= len(model_list):
                await query.answer(text=t("trix.cmd.model.picker.invalid_model_index"))
                return
            ...
            # Same authorization gate as the choice picker (cp:): unauthorized
            # users in a shared group must not flip session/config state --
            # here, switch the model and its billing -- via someone else's
            # picker message.
            if not self._model_picker_switch_authorized(query):
                await _answer_callback_denied(query, t("trix.cmd.shared.not_authorized_setting"))
                return
            ...
            await query.answer(
                ...
                if switch_failed
                else t("trix.cmd.model.picker.switched")
            )
            ...
            # users in a shared group must not flip session/config state --
            # here, switch the model and its billing -- via someone else's
            # picker message. Gate before the expensive-model lookup below:
            # that lookup does a network call AND (on a hit) rewrites the
            # message with a fresh "switch anyway" button carrying the
            # tapper's own idx, which is itself a state-mutating redraw of
            ...
            if warning is not None:
                button_labels = _model_picker_button_labels()
                keyboard = InlineKeyboardMarkup([
                    [InlineKeyboardButton(button_labels["switch_anyway"], callback_data=f"mc:{idx}")],
                    [
                        InlineKeyboardButton(button_labels["back"], callback_data="mb"),
                        InlineKeyboardButton(button_labels["cancel"], callback_data="mx"),
                    ],
                ...
                await query.edit_message_text(
                    text=self.format_message(
                        f"⚠ *{t('trix.cmd.model.cost_warning_title')}*\n\n{warning.message}"
                    ),
                    parse_mode=ParseMode.MARKDOWN_V2,
                    reply_markup=keyboard,
                )
                await query.answer(text=t("trix.cmd.model.picker.confirm_expensive"))
                return
            ...
            if not members:
                await query.answer(text=t("trix.cmd.model.picker.group_not_found"))
                return
            ...
            await query.edit_message_text(
                text=self.format_message(
                    self._model_picker_group_text(_label or group_id)
                ),
                parse_mode=ParseMode.MARKDOWN_V2,
                reply_markup=keyboard,
            )
    ...
    async def _notify_clarify_expired(self, query, user_display: str) -> None:
        ...
        try:
            await query.answer(text=t("trix.clarify.expired_tooltip"))
        except Exception:
            pass
        try:
            await query.edit_message_text(
    ...
    async def _handle_callback_query(
        ...
        if data.startswith("ea:"):
            ...
            if len(parts) == 3:
                ...
                try:
                    ...
                    await query.answer(
                        text=_approval_outcome_text("trix.approval.callback_invalid_data")
                    )
                    return
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await _answer_callback_denied(
                        query,
                        _approval_outcome_text("trix.approval.callback_unauthorized"),
                    )
                    return
                ...
                if not session_key:
                    await query.answer(
                        text=_approval_outcome_text("trix.approval.callback_already_resolved")
                    )
                    return

                user_display = getattr(query.from_user, "first_name", None) or \
                    _approval_outcome_text("trix.approval.default_user")
                ...
                if count:
                    # Map choice to human-readable label (localized -- see
                    # _approval_outcome_text).
                    label_map = {
                        "once": _approval_outcome_text("trix.approval.resolved_once"),
                        "session": _approval_outcome_text("trix.approval.resolved_session"),
                        "always": _approval_outcome_text("trix.approval.resolved_always"),
                        "deny": _approval_outcome_text("trix.approval.resolved_deny"),
                    }
                    label = label_map.get(
                        choice, _approval_outcome_text("trix.approval.resolved_fallback")
                    )
                    edit_text = _approval_outcome_text(
                        "trix.approval.resolved_by", label=label, user=user_display
                    )
                else:
                    label = _approval_outcome_text("trix.approval.expired_label")
                    edit_text = _approval_outcome_text(
                        "trix.approval.expired_text", label=label
                    )
        ...
        if data.startswith("sc:"):
            ...
            if len(parts) == 3:
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await _answer_callback_denied(
                        query,
                        _approval_outcome_text("trix.approval.confirm_unauthorized"),
                    )
                    return
                ...
                if not session_key:
                    await query.answer(
                        text=_approval_outcome_text("trix.approval.confirm_already_resolved")
                    )
                    return

                label_map = {
                    "once": _approval_outcome_text("trix.approval.confirm_once"),
                    "always": _approval_outcome_text("trix.approval.confirm_always"),
                    "cancel": _approval_outcome_text("trix.approval.confirm_cancel"),
                }
                user_display = getattr(query.from_user, "first_name", None) or \
                    _approval_outcome_text("trix.approval.default_user")
                label = label_map.get(
        ...
        if data.startswith("cl:"):
            ...
            if len(parts) == 3:
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await _answer_callback_denied(query, t("trix.approval.confirm_unauthorized"))
                    return

                session_key = self._clarify_state.get(clarify_id)
                if not session_key:
                    await query.answer(text=t("trix.approval.confirm_already_resolved"))
                    return
                ...
                if choice_token == "other":
                    ...
                    await query.answer(text=t("trix.clarify.awaiting_text_tooltip"))
                    try:
                        await query.edit_message_text(
                            text=(
                                f"❓ {query.message.text or ''}\n\n"
                                + t(
                                    "trix.clarify.awaiting_text_body",
                ...
                try:
                    ...
                    await query.answer(text=t("trix.clarify.invalid_choice"))
                    return
                ...
                if resolved_text is None:
                    ...
                    # rather than nothing. This string lands in the chat as
                    # the CUSTOMER'S ANSWER (not a system message), so it
                    # must speak the customer's language.
                    resolved_text = t("trix.clarify.choice_fallback", n=idx + 1)
        ...
        if not self._is_callback_user_authorized(
            ...
            await _answer_callback_denied(query, t("trix.update.not_authorized"))
            return
        # Same label the button itself shows (_update_prompt_button_labels)
        # -- so what the client tapped and what the reply says match.
        button_labels = _update_prompt_button_labels()
        label = button_labels["yes"] if answer == "y" else button_labels["no"]
        await query.answer(text=t("trix.update.answer_sent_tooltip", label=label))
        # Edit the message to show the choice and remove buttons
        try:
            await query.edit_message_text(
                text=self.format_message(t("trix.update.answer_recorded", label=label)),
                parse_mode=ParseMode.MARKDOWN_V2,
                reply_markup=None,
            )
        except Exception:
    ...
    def _missing_media_path_error(self, label: str, path: str) -> str:
        ...
        The gateway process runs on the host, so a container path is uploadable
        only through a mount. ``/workspace`` and ``/root`` are bound to host
        directories in a persistent sandbox and translate on their own; a
        path that is missing *anyway* means either the file is gone or the
        sandbox is ephemeral. ``/output`` is not mounted by anything and never
        has been — a file written there exists only in the container's own
        layer, which is why it is called out separately.
        """
        error = f"{label} file not found: {path}"
        if path.startswith(("/output/", "/outputs/")):
            error += (
                " (nothing is mounted at /output — the file exists only inside "
                "the container. Write to the working directory instead.)"
            )
        elif path.startswith(("/workspace/", "/root/")):
            error += (
                " (this path is host-visible only while the sandbox is "
                "persistent — check terminal.container_persistent, or that the "
    ...
    def _enqueue_text_event(self, event: MessageEvent) -> None:
        ...
        if existing is None:
            ...
            # Spec 19, Ruling 5 — track every message id this batch will ever
            # merge, not just the first (event.message_id). A naive
            # delete_message(event.message_id) on the flushed event would only
            # ever delete the FIRST message of the batch; when a client types
            # "here's the key" then the key as a second message, Telegram
            # merges them into one event that keeps the first id, and the key
            # itself would survive in the chat history.
            if event.message_id:
                event.metadata = dict(event.metadata or {})
                event.metadata["telegram_merged_message_ids"] = [event.message_id]
            ...
            if event.message_id:
                ...
                merged_ids = existing.metadata.setdefault(
                    "telegram_merged_message_ids",
                    [existing.message_id] if existing.message_id else [],
                )
                if event.message_id not in merged_ids:
...
def interactive_setup() -> None:
    ...
    Delegates to the existing CLI setup helpers (@BotFather token prompt,
...
def _apply_yaml_config(yaml_cfg: dict, telegram_cfg: dict) -> dict | None:
    ...
    # HTTP pool size + timeouts. No env bridge on purpose: the adapter reads
    # these straight from PlatformConfig.extra at the point it builds the
    # HTTPXRequest, so a round trip through os.environ would only add a way
    # for a stale value to win. See resolve_http_request_kwargs().
    if "network" in telegram_cfg:
        extras.setdefault("network", telegram_cfg["network"])
