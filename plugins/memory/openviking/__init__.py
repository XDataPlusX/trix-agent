...
def _default_ovcli_config_path() -> Path:
    """``ovcli.conf`` живёт в доме ПРОФИЛЯ, а не в доме ОС.

    Это конфиг внешнего CLI с его собственным входом — приватные данные
    одного профиля. В shared-host ``user_home_path()`` отдаёт ровно
    ``Path.home() / ...``, поэтому существующие установки ничего не
    замечают; в contained два профиля на машине перестают видеть чужой
    вход.
    """
    from hermes_constants import user_home_path

    return user_home_path(_OVCLI_DEFAULT_RELATIVE_PATH)
