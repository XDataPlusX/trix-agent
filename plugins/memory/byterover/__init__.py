...
def _resolve_brv_path() -> Optional[str]:
    ...
    if not found:
        # Дом профиля идёт ПЕРВЫМ, дом ОС остаётся вторым. Под containment
        # HOME подпроцессов переехал, значит npm-установка, сделанная
        # агентом, лежит под корнем профиля — искать её надо там, куда её
        # кладут. Настоящий дом при этом из поиска не выбрасывается: brv —
        # исполняемый файл, общий машинный ассет (SHARED_IMMUTABLE), и
        # установка, сделанная оператором руками до переключения профиля,
        # не должна перестать находиться. В shared-host оба пути совпадают,
        # и список кандидатов остаётся прежним.
        from hermes_constants import user_home_path

        profile_home = user_home_path()
        real_home = Path.home()
        candidates = [
            profile_home / ".brv-cli" / "bin" / "brv",
            profile_home / ".npm-global" / "bin" / "brv",
            real_home / ".brv-cli" / "bin" / "brv",
            Path("/usr/local/bin/brv"),
            real_home / ".npm-global" / "bin" / "brv",
        ]
        for c in candidates:
            if c.exists():
                found = str(c)
