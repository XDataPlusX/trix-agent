...
def verify_password(password: str, encoded: str) -> bool:
    ...
    Public so other in-tree callers that store credentials in this same
    ``scrypt$n$r$p$<salt_b64>$<dk_b64>`` format (see the module docstring)
    can verify against it without reimplementing scrypt parsing — e.g. the
    setup wizard's ``hermes_cli/setup_wizard/state.py`` (spec 8, §13.1).
    """
    try:
...
def _verify_password(password: str, encoded: str) -> bool:
    """Deprecated alias for :func:`verify_password` — kept for callers
    (and tests) that still reference the old private name."""
