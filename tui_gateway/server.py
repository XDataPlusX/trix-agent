...
def _configured_cwd_from_cfg(cfg: dict | None) -> str | None:
    ...
    # Плейсхолдер (``.``/``auto``/``cwd``) не называет каталог — шлюз
    # разрешает его в рантайме, и рабочей папкой оператора он не является.
    # Правило одно на весь движок: см. hermes_constants.is_cwd_placeholder.
    from hermes_constants import is_cwd_placeholder

    if not raw or is_cwd_placeholder(raw):
        return None
