...
class SessionSearchMixin:
    ...
    def _run_trigram_search(
        ...
        *,
        ...
        self._append_session_scope_clauses(
            tri_where, tri_params, session_key, chat_id, thread_id
        )
        tri_sql = f"""
            SELECT
                m.id,
    ...
    def search_messages(
        ...
        ``session_key`` / ``chat_id`` / ``thread_id`` restrict matches to
        sessions carrying that exact conversation identity (equality on the
        already-stored ``sessions`` columns — see
        ``idx_sessions_gateway_peer``). ``None`` means "do not filter on
        this column".
        """
        started = time.time()
        rows = None
    ...
    @staticmethod
    def _append_session_scope_clauses(
        where: List[str],
        params: List[Any],
        ...
        """Append exact conversation-identity filters over the sessions join.

        Every search path JOINs ``sessions s`` already; these clauses narrow
        matches to rows carrying the requested identity. ``None`` leaves the
        column unfiltered. Values are matched by exact equality — callers
        pass the stable technical ids stored at insert time (``session_key``,
        ``chat_id``, ``thread_id``), never display names.
        """
    ...
    def _search_messages_like_fallback(
        ...
        *,
        ...
        self._append_session_scope_clauses(
            where, params, session_key, chat_id, thread_id
        )
    ...
    def _search_messages_impl(
        ...
        ``session_key`` / ``chat_id`` / ``thread_id`` restrict the search to
        sessions with that exact conversation identity (equality filters on
        the sessions join — the conversation-scoped session_search ladder
        builds on these; display names are never identity).
        ...
        self._append_session_scope_clauses(
            where_clauses, params, session_key, chat_id, thread_id
        )
        ...
        if is_cjk:
            ...
            if (
                ...
                self._append_session_scope_clauses(
                    cjk_where, cjk_params, session_key, chat_id, thread_id
                )
                cjk_sql = f"""
                    SELECT
                        m.id,
            ...
            if not _trigram_succeeded:
                ...
                self._append_session_scope_clauses(
                    like_where, like_params, session_key, chat_id, thread_id
                )
                like_sql = f"""
                    SELECT m.id, m.session_id, m.role,
                           substr(m.content,
