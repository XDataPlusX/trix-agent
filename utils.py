...
def atomic_write_text(
    ...
    *,
    ...
    Args:
        newline: Forwarded to the underlying text-mode open() exactly like
            the builtin ``open()``'s own ``newline`` parameter. Default
            ``None`` keeps the historical behavior (platform line-ending
            translation on write). Pass ``""`` when *content* already
            contains the exact line endings you want on disk (e.g. a
            caller that detected the target's own dominant separator and
            joined its lines with it) — otherwise a text-mode write on
            Windows re-translates every embedded ``"\\n"`` to
            ``os.linesep``, corrupting any ``"\\r\\n"`` the caller already
            placed into a doubled ``"\\r\\r\\n"``.
...
def atomic_write_bytes(
    ...
    *,
    ...
    Byte-for-byte counterpart to ``atomic_write_text`` for callers that
    already hold the exact bytes to place on disk (e.g. restoring a file
    verbatim from a backup archive) and must not risk leaving a truncated
    or partially-written target if the write is interrupted (crash, disk
    full, killed process) partway through.

    Args:
        preserve_mode: carry an existing target's permission bits and
            (POSIX, best-effort) owner across the replace. See
            ``atomic_write_text`` for the rationale.
        create_mode: permission bits to apply when the target does not yet
            exist. Ignored when the target already exists (its mode is
            preserved instead, if ``preserve_mode`` is set) and ignored
            entirely when ``force_mode`` is given.
        force_mode: permission bits to apply unconditionally, regardless of
            whether the target already existed or what mode it had --
            overrides both ``preserve_mode`` and ``create_mode``. Applied to
            the temp file *before* the atomic replace so the target is never
            briefly visible at the wrong permissions. Use for files that must
            always end up locked down (credentials, tokens) no matter what
            mode a previous copy had on disk.

    Returns the resolved real path used for the replace (see
    ``atomic_replace``).
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if force_mode is not None:
        ...
        effective_mode = _preserve_file_mode(path) if preserve_mode else None
        if effective_mode is None and create_mode is not None and not path.exists():
            effective_mode = create_mode
    ...
    try:
        with os.fdopen(fd, "wb") as handle:
            if effective_mode is not None and hasattr(os, "fchmod"):
                # fchmod the temp fd BEFORE the replace so the target never
                # transits through mkstemp's 0600 (or an unrelated inherited
...
def atomic_roundtrip_yaml_save(
    ...
    new_state: dict,
    *,
    extra_content: "str | list[str] | None" = None,
) -> None:
    ...
    * Non-dict values (lists, scalars) that are UNCHANGED from the on-disk
      value are left alone — ``dst[key]`` is never reassigned for a value
      that already equals ``new_state``'s, so the ruamel CommentedSeq/scalar
      object (and every comment attached to it — inline per-list-item
      comments, a trailing block comment after the last item, ...) survives
      untouched. Only a value that actually CHANGED is overwritten wholesale
      — list element comments are not individually preserved across a real
      change, matching ruamel's semantics.
    ...
    ``extra_content``, when given, is one or more commented-out example
    sections (e.g. ``config.py``'s ``_SECURITY_COMMENT``/
    ``_FALLBACK_COMMENT``) appended after the YAML dump inside the same
    atomic write — mirrors ``atomic_yaml_write``'s parameter of the same
    name. Pass a ``list[str]`` when there's more than one independent block
    (the common case); a bare ``str`` is treated as a single block.
    **Idempotent by construction**: a block already present in the freshly
    dumped document text (its first non-blank line is matched verbatim) is
    NOT re-appended. This matters because ruamel's round-trip loader treats
    a trailing comment block appended after the document as part of the
    loaded structure — it survives the *next* load and gets re-emitted by
    ``yaml_rt.dump`` on its own, so an unconditional append would duplicate
    the block on every single subsequent save (observed: 3 saves -> the
    block emitted 3 times, file growing by a full copy on every save).
    ...
    def _merge(dst: CommentedMap, src: dict) -> None:
        ...
        for key, value in src.items():
            if isinstance(value, dict):
                ...
                # A non-dict value (list or scalar) that's already identical
                # to what's on disk must NOT be reassigned — dst[key] = ...
                # replaces the ruamel CommentedSeq/scalar object wholesale,
                # which silently drops every comment attached to it even
                # when nothing about the value actually changed. Comparing
                # against the pre-quoting value is fine: ruamel's loaded
                # scalar/seq types subclass the plain python types, so
                # equality is by value regardless of the wrapper.
    ...
    # Render the body first so each extra_content block can be checked for
    # idempotency against what's actually about to be written — including
    # any earlier block that survived the reload above as a preserved
    # trailing comment on `existing` (see docstring: without this check the
    # block duplicates itself on every single save).
    _dump_buf = io.StringIO()
    yaml_rt.dump(existing, _dump_buf)
    dumped_text = _dump_buf.getvalue()

    if extra_content is None:
        _blocks = []
    elif isinstance(extra_content, list):
        _blocks = extra_content
    else:
        _blocks = [extra_content]
    ...
    for _block in _blocks:
        if not _block:
            continue
        _marker_line = next((line for line in _block.splitlines() if line.strip()), None)
        if _marker_line is not None and _marker_line in dumped_text:
            continue
        blocks_to_append.append(_block)
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            f.write(dumped_text)
            for _block in blocks_to_append:
                f.write(_block)
            f.flush()
            os.fsync(f.fileno())
        real_path = atomic_replace(tmp_path, path)
